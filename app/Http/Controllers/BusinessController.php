    ...
    */
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            // Campos fiscais NFe/CTe
            $fiscal_fields = $request->only([
                'ambiente', 'regime',
                'numero_serie_nfe', 'numero_serie_nfce', 'numero_serie_cte',
                'cst_csosn_padrao', 'cst_pis_padrao', 'cst_cofins_padrao', 'cst_ipi_padrao',
                'perc_icms_padrao', 'perc_pis_padrao', 'perc_cofins_padrao', 'perc_ipi_padrao',
                'ncm_padrao', 'cfop_saida_estadual_padrao', 'cfop_saida_inter_estadual_padrao',
                'csc', 'csc_id',
            ]);
            $business_details = array_merge($business_details, $fiscal_fields);

            // Senha do certificado (só atualiza se enviada)
            if ($request->filled('senha_certificado')) {
                $business_details['senha_certificado'] = $request->input('senha_certificado');
            }

            // Upload certificado A1 (.pfx) — armazena como base64
            if ($request->hasFile('certificado_pfx') && $request->file('certificado_pfx')->isValid()) {
                $pfx_content = file_get_contents($request->file('certificado_pfx')->getRealPath());
                $business_details['certificado'] = base64_encode($pfx_content);
            }
