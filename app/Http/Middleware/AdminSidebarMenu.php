...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            if (auth()->user()->can('supplier.view') || auth()->user()->can('customer.view') || auth()->user()->can('supplier.view_own') || auth()->user()->can('customer.view_own')) {
                $menu->dropdown(
                    ...
                    function ($sub) {
						...
						$sub->url('/transportadoras', 'Transportadoras', [
						    'icon' => '<svg xmlns="http://www.w3.org/2000/svg" class="tw-size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M3 17l3-3h11l3 3M4 17v3h16v-3"/></svg>',
            ...
            if (in_array('purchases', $enabled_modules) && (auth()->user()->can('purchase.view') || auth()->user()->can('purchase.create') || auth()->user()->can('purchase.update'))) {
                $menu->dropdown(
                    ...
                    function ($sub) use ($common_settings) {
						...
						$sub->url('/devolucao/lista', 'Devolução de NF-e', [
						    'icon' => '<svg xmlns="http://www.w3.org/2000/svg" class="tw-size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M4 4v7h7"/><path d="M20 20v-7h-7"/><path d="M5 19a9 9 0 0 1 14 -14"/></svg>',
						    'active' => request()->is('devolucao/lista'),
						]);

						$sub->url('/purchase-xml', 'Importar XML - Arquivo', [
						    'icon' => '<svg xmlns="http://www.w3.org/2000/svg" class="tw-size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M14 3v4a1 1 0 0 0 1 1h4"/><path d="M17 21H7a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h7l5 5v11a2 2 0 0 1-2 2z"/></svg>',
						    'active' => request()->is('purchase-xml'),
						]);

						$sub->url('/manifesto', 'Manifestação do destinatário', [
						    'icon' => '<svg xmlns="http://www.w3.org/2000/svg" class="tw-size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M5 4h14a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2z"/></svg>',
						    'active' => request()->is('manifesto'),
						]);
                    },
                    ['icon' => '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
            ...
            if ($is_admin || auth()->user()->hasAnyPermission(['sell.view', 'sell.create', 'direct_sell.access', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping', 'access_sell_return', 'direct_sell.view', 'direct_sell.update', 'access_own_sell_return'])) {
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin, $pos_settings) {
						...
						$sub->url('/nfelista', 'Lista de NF-e', [
						    'icon' => '<svg xmlns="http://www.w3.org/2000/svg" class="tw-size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M3 6h18M3 12h18M3 18h18"/></svg>',
						    'active' => request()->is('nfelista'),
						]);
			...
			$menu->dropdown('CT-e', function ($sub) {
			    $sub->url('/cte', 'Lista de CT-e', [
			        'icon' => '<svg xmlns="http://www.w3.org/2000/svg" class="tw-size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M3 12h18M3 6h18M3 18h18"/></svg>',
			        'active' => request()->is('cte'),
			    ]);
			    $sub->url('/cte/new', 'Novo CT-e', [
			        'icon' => '<svg xmlns="http://www.w3.org/2000/svg" class="tw-size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M12 5v14M5 12h14"/></svg>',
			        'active' => request()->is('cte/new'),
			    ]);
			    $sub->url('/veiculos', 'Veículos', [
			        'icon' => '<svg xmlns="http://www.w3.org/2000/svg" class="tw-size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M4 17l1-2h14l1 2v3H4zM6 10h12l1 5H5z"/></svg>',
			        'active' => request()->is('veiculos'),
			    ]);
			    $sub->url('/cte/xmls', 'Lista de XML', [
			        'icon' => '<svg xmlns="http://www.w3.org/2000/svg" class="tw-size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M6 4v16h12V8l-6-4z"/></svg>',
			        'active' => request()->is('cte/xmls'),
			    ]);
			    $sub->url('/naturezas', 'Naturezas de Operação', [
			        'icon' => '<svg xmlns="http://www.w3.org/2000/svg" class="tw-size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>',
			        'active' => request()->is('naturezas'),
			    ]);
			}, [
			    'icon' => '<svg xmlns="http://www.w3.org/2000/svg" class="tw-size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M4 17l1-2h14l1 2v3H4zM6 10h12l1 5H5z"/></svg>',
			])->order(33);
