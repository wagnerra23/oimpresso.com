...
class SellsCockpitAggregator
{
    ...
    public function buildInsightsAggregates(int $businessId): array
    {
        ...
        //
        // Nao reusa `$base` (Eloquent) de proposito: `DB::table` devolve stdClass e
        // evita os `property.notFound` que o selectRaw gera no PHPStan — mesma forma
        // do `methodsRaw` acima. Em troca, o recorte de tenant e REPETIDO aqui em vez
        // de herdado, e por isso ele aparece explicito nas 4 primeiras linhas.
        //
        ...
        $churnRaw = DB::table('transactions')
            ->where('transactions.business_id', $businessId)
            ->where('transactions.type', 'sell')
            ->where('transactions.status', 'final')
            ->whereNull('transactions.sub_type')
