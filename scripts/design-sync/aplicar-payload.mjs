 ...
 *
 * O payload é build-only. Documentos, charters, casos e contratos são recusados antes de
 * qualquer escrita: conhecimento é reconciliado no canon por seu processo próprio, nunca
 * espelhado em uma árvore paralela.
 */
...
for (const f of files) {
  ...
  if (!rel.startsWith('_ds/') && !BUILD_SOURCE_RE.test(rel)) {
    forade.push(`${rel} (fora do contrato build-only; documento/canon deve ser reconciliado na fonte única)`);
    continue;
  }
...
// Mesmos bytes em dois paths ativos são duas fontes concorrentes com nomes diferentes.
// O cache `_ds` fica fora desta regra porque é derivado e pode repetir assets por peso/nome.
const fontePorSha = new Map();
for (const p of preparados.filter((item) => item.destinoBase === DESTINO)) {
  const digestSha = sha256(p.conteudo);
  const original = fontePorSha.get(digestSha);
  if (original) forade.push(`${p.rel} (conteúdo duplicado de ${original})`);
  else fontePorSha.set(digestSha, p.rel);
}
