...
| `memory/requisitos/*/topicos/*.md` | `topico.schema.json` | Unidade temática por módulo (🟠 forward-only, **advisory-que-morde** — STRICT vermelho, NÃO required) |
...
>
> 🟠 **`topico`:** nasceu grace forward-only em 2026-07-21 (ADR 0345) e foi promovido a **advisory-que-morde** no mesmo dia ([W] "promova") — o `grace: true` foi **removido**, então STRICT segue a var global: tópico malformado deixa a perna do gate **VERMELHA**, mas **NÃO é required** (fora do branch protection) → não bloqueia merge. Só valida tópico NOVO/tocado (diff-aware + forward-only); não há backfill em massa. **grace→required segue ADIADO** — a medição do piloto (0 dias de soak, gate nunca mordeu, 3 tópicos só do seed) é o anti-padrão foundation-ratchet; required exigiria janela advisory + FP=0 sobre autoria independente + emenda estilo ADR 0327 (ADR 0314: required = só Tier-0). Relatório: [`memory/sessions/2026-07-21-medicao-piloto-topico-flip-grace-required.md`](../../memory/sessions/2026-07-21-medicao-piloto-topico-flip-grace-required.md).
>
...
- **2026-07-21** — `topico.schema.json` entrou em grace/forward-only (ADR 0345), com âncoras código↔tela↔teste, afirmações atômicas, críticas independentes preservadas e síntese central separada da aprovação humana. `BRIEFING-TEMPLATE.md` passou a ser índice, sem migração em massa do legado. **No mesmo dia**, após a medição do piloto (relatório em `memory/sessions/2026-07-21-medicao-piloto-topico-flip-grace-required.md`), `topico` foi promovido de grace → **advisory-que-morde** ([W] "promova"): removido o `grace: true`, STRICT vermelho em malformado, **não** required. Flip a required segue adiado (0 dias soak / 0 mordidas = foundation-ratchet).
