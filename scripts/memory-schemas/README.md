...
| `memory/requisitos/*/topicos/*.md` | `topico.schema.json` | Unidade temática por módulo (🔴 forward-only, **REQUIRED** — tópico malformado BLOQUEIA merge; emenda ADR 0346) |
| `memory/reference/*.md` | `reference.schema.json` | Doc canônico/gerado (🟡 fiado em GRACE — warn-only, diff-aware) |

> 🟡 **`briefing` e `reference`:** ficam na matriz do [memory-schema-gate.yml](../../.github/workflows/memory-schema-gate.yml) com `grace: true` (warn-only). **NÃO bloqueiam merge** nesta fase; promoção grace→required só depois de backfill oportunístico zerar o FP por família (ADR 0314).
>
> 🔴 **`topico`:** nasceu grace forward-only em 2026-07-21 (ADR 0345), passou por advisory e chegou a **REQUIRED** no mesmo dia ([W] "flip", emenda [ADR 0346](../../memory/decisions/0346-promove-topico-gate-required-override-soberano-emenda-0314.md)). Sem `grace: true` → STRICT: tópico malformado **BLOQUEIA merge** (context `Tópico (memory/requisitos/*/topicos/*.md)` no branch protection). Só valida tópico NOVO/tocado (diff-aware + forward-only + always-run); não há backfill em massa. ⚠️ **HONESTIDADE (registrada na ADR 0346):** os critérios objetivos de promoção **NÃO foram batidos** — a medição do piloto (0 dias de soak, gate nunca mordeu, 3 tópicos só do seed) recomendou ESPERAR; é **override soberano [W]** sobre a recomendação técnica, i.e. o anti-padrão foundation-ratchet aceito de olhos abertos. Dano limitado por greenfield + schema determinístico + reversibilidade. Relatório: [`memory/sessions/2026-07-21-medicao-piloto-topico-flip-grace-required.md`](../../memory/sessions/2026-07-21-medicao-piloto-topico-flip-grace-required.md).
>
...
- **2026-07-21** — `topico.schema.json` entrou em grace/forward-only (ADR 0345), com âncoras código↔tela↔teste, afirmações atômicas, críticas independentes preservadas e síntese central separada da aprovação humana. `BRIEFING-TEMPLATE.md` passou a ser índice, sem migração em massa do legado. **No mesmo dia**, após a medição do piloto (relatório em `memory/sessions/2026-07-21-medicao-piloto-topico-flip-grace-required.md`), `topico` foi promovido de grace → **advisory-que-morde** ([W] "promova": removido o `grace: true`, STRICT vermelho) → e em seguida a **required** ([W] "flip", emenda [ADR 0346](../../memory/decisions/0346-promove-topico-gate-required-override-soberano-emenda-0314.md), label renomeado sem `[advisory]`, baseline 31→32). ⚠️ Override soberano com os critérios objetivos **NÃO batidos** (0 dias soak / 0 mordidas / 3 tópicos só do seed = foundation-ratchet consciente — a ADR 0346 registra o desvio, não o maquia).
