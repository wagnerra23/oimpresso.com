...
WRITE_OFF_AGE_DAYS = 365

STATUS_FILTERS = {
    "ATIVO*": "skip_saldo_virtual",
    "INATIVO AGRUPADO": "skip_filha_agrupada",
    "INATIVO EXCLUIDO": "skip_venda_cancelada",
    "INATIVO EXCLUIDA": "skip_venda_cancelada",
    "INATIVO EXCULIDO": "skip_venda_cancelada",
    "INATIVO EXC.AGRUPADO": "skip_lixo",
    "INATIVO PREVISÃO": "skip_previsao_cancelada",
    "INATIVO PREVISAO": "skip_previsao_cancelada",
    "ATIVO": "real",
    "ATIVO EM ESPERA": "real_em_espera",
    "ATIVO AGRUPADO": "real_agrupador",
    "ATIVO PREVISAO": "real_previsao",
    "INATIVO CANCELADA": "real_cancelado",
}


def classify_status(raw_status):
    s = (raw_status or "ATIVO").upper().strip()
    kind = STATUS_FILTERS.get(s)
    if kind is None:
        return ("import", f"unknown:{raw_status}")
    if kind.startswith("skip_"):
        return ("skip", kind)
    return ("import", kind)
...
def map_financeiro_to_titulo(
    ...
    decision, kind = classify_status(fin.get("STATUS"))
    status = map_status(fin.get("STATUS"), datapagto, kind)
    ...
    data = {
        ...
        "metadata": json.dumps({
            ...
            "delphi_status_kind": kind,
            "delphi_codfinanceiro_grupo": fin.get("CODFINANCEIRO_GRUPO"),
            "delphi_agrupador": normalize_str(fin.get("AGRUPADOR")),
            "is_agrupador_pai": kind == "real_agrupador",
            "is_em_espera": kind == "real_em_espera",
            "is_previsao_cheque": kind == "real_previsao",
    ...
    audit = {
        ...
        "transaction_id_resolved": transaction_id_resolved,
        "delphi_status_raw": fin.get("STATUS"),
        "delphi_codfinanceiro_grupo": fin.get("CODFINANCEIRO_GRUPO"),
...
def main() -> int:
    ...
    stats = {
        ...
        "is_agrupador_pai": 0, "is_em_espera": 0, "is_previsao_cheque": 0,
        "skip_by_status": {}, "skip_valor_zero": 0, "skip_ativo_n": 0,
    }
    ...
    with firebird_connect(args.alias, password_override=args.firebird_password) as fb_con:
        ...
        try:
            ...
            for row_tuple in cur:
                ...
                # Filter inactive (ATIVO col é vestigial em Martinho v1404)
                ativo = (fin.get("ATIVO") or "S").upper()
                if ativo == "N":
                    stats["skipped"] += 1
                    stats["skip_ativo_n"] += 1
                    continue

                # Filter valor=0
                if normalize_decimal(fin.get("VALOR")) == 0:
                    stats["skipped"] += 1
                    stats["skip_valor_zero"] += 1
                    continue

                # Filter by STATUS — descobertas Wagner 2026-05-14:
                # ATIVO*, AGRUPADO filhos, EXCLUIDO (venda cancelada) etc = lixo/duplicação
                status_decision, status_kind = classify_status(fin.get("STATUS"))
                if status_decision == "skip":
                    stats["skipped"] += 1
                    stats["skip_by_status"][status_kind] = stats["skip_by_status"].get(status_kind, 0) + 1
                    continue
                ...
                if audit["kind"] == "real_agrupador": stats["is_agrupador_pai"] += 1
                if audit["kind"] == "real_em_espera": stats["is_em_espera"] += 1
                if audit["kind"] == "real_previsao": stats["is_previsao_cheque"] += 1
    ...
    print("\n== Relatório ==")
    print(f"  Lidos              : {stats['lidos']}")
    print(f"  Skipped TOTAL      : {stats['skipped']}")
    for k, v in sorted(stats["skip_by_status"].items(), key=lambda x: -x[1]):
        print(f"    {k:30s} : {v}")
    if stats["skip_valor_zero"]: print(f"    skip_valor_zero               : {stats['skip_valor_zero']}")
    if stats["skip_ativo_n"]: print(f"    skip_ativo_n                  : {stats['skip_ativo_n']}")
    print(f"  Agrupadores pais   : {stats['is_agrupador_pai']}")
    print(f"  Em espera          : {stats['is_em_espera']}")
    print(f"  Cheques previsão   : {stats['is_previsao_cheque']}")
