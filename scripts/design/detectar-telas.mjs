...
import { pageNamespacePath, raizesDePages } from '../qa/page-path.mjs';
...
const ALIAS = [
  ...
  // suporte-page.jsx é declarado pelo charter de Suporte/Visao. Não duplicar aqui: o charter
  // é a âncora canônica e um ALIAS concorrente deslocava o mesmo arquivo para Empresas.
];
...
async function selftest() {
  const fx = resolve(HERE, '../../tests/Design/fixtures/detectar-telas');
  if (!existsSync(join(fx, 'staging')) || !existsSync(join(fx, 'repo'))) {
    console.error('[selftest] fixture ausente em tests/Design/fixtures/detectar-telas/'); process.exit(2);
  }
