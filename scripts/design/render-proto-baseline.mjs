...
// ("prototipo-ui/cowork/Wagner/financeiro-page.jsx (design real; corrigido...)") — a âncora é o 1º token,
...
// FURO fechado fail-closed: o sha é carimbado do proto no REPO (SSOT prototipo-ui/cowork/Wagner/),
...
export function conferirIdentidadeProto(srcRepo, srcStaging) {
  ...
  if (srcRepo == null) return { ok: false, motivo: 'âncora NÃO existe no SSOT do repo (prototipo-ui/cowork/Wagner/) — sync do espelho pendente (importar-bundle --sync-cowork).' };
...
function cmdNudge(args) {
  ...
  for (const b of afetados) {
    ...
    console.log(`\n  · **${tela}** — baseline \`${b}\``);
    console.log(`      node scripts/design/render-proto-baseline.mjs --gerar ${tela}   # fonte importada atual + preview DS atualizado`);
...
function selftest() {
  ...
  const ANC = 'prototipo-ui/cowork/Wagner/financeiro-page.jsx';
  ...
  const ancDiv = verificarBaseline(bom, { ancoraAtual: 'prototipo-ui/cowork/Wagner/outra-page.jsx', shaAtual: 'abc1234' });
  ...
  t('rota: financeiro-page.jsx → financeiro', rotaDoAnchor('prototipo-ui/cowork/Wagner/financeiro-page.jsx') === 'financeiro');
  ...
  t('primeiroToken corta a prosa do charter', primeiroToken('prototipo-ui/cowork/Wagner/financeiro-page.jsx (design real; corrigido)') === 'prototipo-ui/cowork/Wagner/financeiro-page.jsx');
  ...
  t('nudge: fora de Pages/ silencia (proto/backend não é dever-de-casa de compare)',
    telasAfetadas(['prototipo-ui/cowork/Wagner/vendas-page.jsx', 'Modules/Sells/Http/X.php'], BLS).length === 0);
