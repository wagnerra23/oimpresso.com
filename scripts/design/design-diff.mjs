 ...
 *   node scripts/design/design-diff.mjs --compare prod.json design.json --contrato <c.json>
 *                                                                   # D0: prova que os 2 lados sao a MESMA tela
 *   node scripts/design/design-diff.mjs --shell-roles                 # mapa de papéis do SHELL, pra colar nos 2 lados
 *   node scripts/design/design-diff.mjs --compare p.json d.json --check-shell            # exit 1 se o SHELL divergir
 *   node scripts/design/design-diff.mjs --compare p.json d.json --declarado decl.json    # allowlist {campo,motivo}
...
function dimShell(prod, design) {
  ...
  if (!P || !D) {
    return [SEM_DADO_SHELL('shell', P ? 'ok' : 'não medido', D ? 'ok' : 'não medido',
      'declare window.__SB_ROLES nos DOIS lados — node scripts/design/design-diff.mjs --shell-roles imprime o mapa')];
  }
  const rows = [];
  ...
  if (argv.includes('--check-shell')) {
    ...
    if (semDadoShell.length) {
      console.error(
        ...
        `\n       node scripts/design/design-diff.mjs --shell-roles\n`);
      process.exit(2);
    }
