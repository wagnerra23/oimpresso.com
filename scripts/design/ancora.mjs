...
import { isAuxiliaryPagePath, raizesDePages } from '../qa/page-path.mjs';
import { ultimaVerificacaoDe, KIND_LIVE_ONLY, liveOnlyVerdict } from '../governance/cowork-mirror-freshness.mjs';
...
const LUGAR_FIXO = 'prototipo-ui/cowork/Wagner';
...
export async function resolveAncora(query, { repoRoot = REPO_DEFAULT, stagingDir = null } = {}) {
  ...
  // pra um `-page.jsx` que hoje está VERSIONADO em `prototipo-ui/cowork/Wagner/`. Sem a flag, esta
...
async function classeExiste(nome, repoRoot) {
  ...
  const safeDirectory = String(repoRoot).replace(/\\/g, '/');
  const r = spawnSync('git', ['-c', `safe.directory=${safeDirectory}`, 'grep', '-q', '-E', `(class|interface) ${nome}\\b`, '--', 'Modules/', 'app/'],
...
async function selftest() {
  ...
  t('CONTROLE n/a: caminho real NÃO é declaração', ehDeclaracaoNa('prototipo-ui/cowork/Wagner/jana-merge.jsx') === false);
  t('CONTROLE n/a: nome que só CONTÉM "na" não casa', ehDeclaracaoNa('prototipo-ui/cowork/Wagner/nao-a-toa.jsx') === false);
  ...
  t('CONTROLE precedência: protótipo bespoke GANHA do bundle (design aprovado vem 1º)',
    fonteDoCharter({ related_prototype: 'prototipo-ui/cowork/Wagner/jana-merge.jsx', bundle_source: 'produtos-page.jsx' }).via === 'related_prototype');
  ...
  // Reusa o `cli()` do bloco acima (PR #7131) — mesmo CLI, um caminho so. `status !== 0`
  // vira lista vazia de proposito: os asserts caem VERMELHOS e visiveis, em vez de o
  // selftest morrer com excecao de parse (ausencia de medicao nao vira estado do medido).
  {
    const cLista = cli('--list', '--json');
    const rowsReais = cLista.status === 0 ? JSON.parse(cLista.stdout) : [];
  ...
  const FX_CHARTER = [
    ...
    'mwart_pattern_reuse:',
    '  blueprint_cowork: prototipo-ui/cowork/Wagner/outra-page.jsx',
    '  divergence_from_blueprint: "wizard substitui o modal do blueprint"',
    '---',
    '# corpo',
    'blueprint_cowork: prototipo-ui/cowork/Wagner/ISTO-E-PROSA.jsx',
  ].join(String.fromCharCode(10));
  t('BITE fm: chaveAninhada lê chave INDENTADA (o frontmatter() compartilhado não lê)',
    chaveAninhada(FX_CHARTER, 'blueprint_cowork') === 'prototipo-ui/cowork/Wagner/outra-page.jsx');
  t('CONTROLE fm: chave depois do frontmatter é PROSA, não declaração',
    chaveAninhada(FX_CHARTER, 'blueprint_cowork') !== 'prototipo-ui/cowork/Wagner/ISTO-E-PROSA.jsx');
  ...
  t('CONTROLE porte reverso: -page.jsx do espelho NÃO é código do repo',
    ehCodigoDoRepo('prototipo-ui/cowork/Wagner/produtos-page.jsx') === false);
  ...
  const dj = await defeitosDaAncora('prototipo-ui/cowork/Wagner/jana-merge.jsx');
  ...
  const dc = await defeitosDaAncora('prototipo-ui/cowork/Wagner/chat-jana.jsx');
  ...
  t('CONTROLE real: caminho inexistente não explode nem inventa fantasma',
    (await defeitosDaAncora('prototipo-ui/cowork/Wagner/__nao-existe__.jsx')).lido === false);
  ...
  const dSufixo = await defeitosDaAncora('prototipo-ui/cowork/Wagner/jana-merge.jsx (PT-04 Dashboard)');
  ...
  t('CONTROLE printer: o MESMO arquivo sem o sufixo É lido (o defeito era o path, não o arquivo)',
    (await defeitosDaAncora('prototipo-ui/cowork/Wagner/jana-merge.jsx')).lido === true);
  ...
  t('CONTROLE aspas: caminho real entre aspas NÃO vira declaração',
    ehDeclaracaoNa('"prototipo-ui/cowork/Wagner/jana-merge.jsx"') === false);
  ...
  t('BITE formato 4: diretório também não é arquivo',
    caminhoDaAncora('prototipo-ui/cowork/Wagner/venda-menu/') === null);
  t('BITE formato 1: caminho limpo resolve como está (os 55 que já funcionam)',
    caminhoDaAncora('prototipo-ui/cowork/Wagner/jana-merge.jsx') === 'prototipo-ui/cowork/Wagner/jana-merge.jsx');
  t('BITE formato 2: caminho + prosa entre parênteses — o parêntese sai do path',
    caminhoDaAncora('prototipo-ui/cowork/Wagner/jana-merge.jsx (PT-04 Dashboard)') === 'prototipo-ui/cowork/Wagner/jana-merge.jsx');
  t('BITE formato 3: prosa ANTES, nome solto — resolve no LUGAR_FIXO',
    caminhoDaAncora('"F1 Cowork — fiscal-page.jsx §FxNotasPage"') === 'prototipo-ui/cowork/Wagner/fiscal-page.jsx');
  ...
  await mkdir(join(fxList, 'prototipo-ui', 'cowork', 'Wagner'), { recursive: true });
  await mkdir(fxListPages, { recursive: true });
  await writeFile(join(fxList, 'prototipo-ui', 'cowork', 'Wagner', 'fx-real.jsx'), '// existe de verdade\n', 'utf8');
  const charterFx = (page, rp) => ['---', `page: ${page}`, `related_prototype: ${rp}`, '---', '# fx'].join('\n');
  await writeFile(join(fxListPages, 'Quebrado.charter.md'), charterFx('/fx/quebrado', 'prototipo-ui/cowork/Wagner/fx-nao-existe.jsx'), 'utf8');
  ...
  t('BITE list: fonte que nao abre → existe:false, e `hasSource` segue TRUE (a guarda)',
    !!lQuebrado && lQuebrado.existe === false && lQuebrado.hasSource === true
      && lQuebrado.caminho === 'prototipo-ui/cowork/Wagner/fx-nao-existe.jsx');
  t('CONTROLE list: caminho real da o existe true',
    !!lReal && lReal.existe === true && lReal.caminho === 'prototipo-ui/cowork/Wagner/fx-real.jsx');
  ...
  await mkdir(join(fxBRepo, 'prototipo-ui', 'cowork', 'Wagner'), { recursive: true });
  ...
  await writeFile(join(fxBRepo, 'prototipo-ui', 'cowork', 'Wagner', 'bundle-page.jsx'), '// no lugar fixo do repo\n', 'utf8');
  ...
  await writeFile(join(pages('Dupla'), 'Index.charter.md'),
    chB('/fxb/dupla', ['related_prototype: prototipo-ui/cowork/Wagner/bundle-page.jsx', 'bundle_source: bundle-page.jsx']), 'utf8');
  ...
  t('BITE bundle sem staging: `bundle_source` resolve no LUGAR_FIXO, sem a flag',
    soBundle(rFixo).length === 1 && soBundle(rFixo)[0].valor === 'prototipo-ui/cowork/Wagner/bundle-page.jsx');
  ...
  await writeFile(join(pages('Diverge'), 'Index.charter.md'),
    chB('/fxb/diverge', ['related_prototype: prototipo-ui/cowork/Wagner/especifico-page.jsx', 'bundle_source: bundle-page.jsx']), 'utf8');
