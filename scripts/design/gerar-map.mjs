...
import { contentHash } from '../governance/cowork-mirror-freshness.mjs';

const HERE = dirname(fileURLToPath(import.meta.url));
const REPO = resolve(HERE, '../..');

// extrai os caminhos de arquivo do campo frontmatter `prototipo:` — formato real observado:
// "prototipo-ui/cowork/Wagner/financeiro-page.jsx + financeiro-ops.jsx (NOVO) @ 4e3aacfc0f (import ...)"
...
function selftest() {
  ...
  const fx = resolve(HERE, '../../tests/Design/fixtures/gerar-map');

  t('resolverArquivosPrototipo: path completo + basename compartilha dir', (() => {
    const arqs = resolverArquivosPrototipo('prototipo-ui/cowork/Wagner/financeiro-page.jsx + financeiro-ops.jsx (NOVO) @ 4e3aacfc0f');
    return arqs.length === 2 && arqs[0] === 'prototipo-ui/cowork/Wagner/financeiro-page.jsx' && arqs[1] === 'prototipo-ui/cowork/Wagner/financeiro-ops.jsx';
  })());
  ...
  t('computeGitSha: arquivo inexistente / sem repo git → sem-historico, não lança', computeGitSha(['tests/Design/fixtures/gerar-map/__nao-existe.jsx'], '/tmp') === 'sem-historico');
  ...
  if (existsSync(join(fx, 'boa-gap.md'))) {
    ...
    t('gap_fonte relativo ao root, POSIX', g.mapa.gap_fonte === 'tests/Design/fixtures/gerar-map/boa-gap.md');
    ...
    const h1 = computeProtoHash(['boa-gap.md'], fx);
    t('computeProtoHash: formato sha256:<12hex> e determinístico', /^sha256:[0-9a-f]{12}$/.test(h1) && h1 === computeProtoHash(['boa-gap.md'], fx));
    t('computeProtoHash: conteúdo diferente → hash diferente (morde re-export sem commit)', h1 !== computeProtoHash(['sem-tabela-gap.md'], fx));
    t('computeProtoHash: nenhum arquivo legível → sem-arquivo (nunca lança)', computeProtoHash(['__nao-existe.jsx'], HERE) === 'sem-arquivo');
    t('shaAtualPara: roteia sha256: → contentHash e legado → git-sha', shaAtualPara('sha256:abc', ['boa-gap.md'], fx) === h1 && /^sha256:/.test(shaAtualPara('sha256:abc', ['boa-gap.md'], fx)) === true && shaAtualPara('4e3aacfc0f', ['__nao-existe.jsx'], '/tmp') === 'sem-historico');
