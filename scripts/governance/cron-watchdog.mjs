 ...
 * quebrado, curadoria envelhecida ou mecanismo morto.
 *
...
function reportarEntrega(r) {
  ...
  console.error(`\n✗ ${r.parados.length} artefato(s) de estado parado(s) além do limite. Este eixo mede IDADE, não autoria — ele não sabe se o artefato tem escritor automático. Ao investigar, o achado cai num dos 3 casos: (a) cron que rodava e parou de ENTREGAR → conserta a entrega; (b) artefato CURADO À MÃO cuja revisão envelheceu → re-cura, ou aposenta quem o consome; (c) mecanismo inteiro parado → aposenta com lápide. Como distinguir: varra os escritores do path (quem faz write nele) — sem escritor, não é (a). Precedente medido em 2026-07-26: os 5 scorecards do Governance v4 são (c), zero escritores; os crons de 06:05/07:00 apenas LEEM.`);
  return 1;
}
