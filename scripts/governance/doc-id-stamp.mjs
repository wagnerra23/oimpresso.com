...
const SAFE_PREFIXES = ['memory/reference/', 'memory/governance/', 'memory/research/', 'memory/audits/', 'memory/dominios/'];
const TOXIC_PREFIXES = ['memory/requisitos/', 'resources/js/Pages/'];
...
export function collectTargets(root, { includeToxic = false } = {}) {
  ...
  return all.filter((p) => prefixes.some((pre) => p.startsWith(pre)) && !PROTECTED.has(p));
}
...
export function plan(root, opts = {}) {
  ...
  const toStamp = []; const skipped = { hasId: 0, generated: 0, deferredNoFrontmatter: 0 }; const byId = new Map();
...
function runSelftest() {
  ...
  try {
    ...
    mkdirSync(join(fx, 'memory/research'), { recursive: true });
    ...
    writeFileSync(join(fx, 'memory/research/livre.md'), '# Research sem fm\n');             // família não-mapeada → carimba
    ...
    ok('plan: carimba família NÃO-mapeada sem frontmatter (research)', p.toStamp.some((t) => t.rel === 'memory/research/livre.md' && t.id === 'research-livre'));
    ...
    ok('plan: NÃO toca gate-toxic (requisitos) sem --include-toxic', !p.toStamp.some((t) => t.rel.includes('requisitos')));
...
function main() {
  ...
  console.log(`[doc-id-stamp] alvos: ${p.total} · a carimbar: ${p.toStamp.length} · já com id: ${p.skipped.hasId} · gerados (pulados): ${p.skipped.generated}`);
