...
function parseImportsCruzados(linhas, { modulosVivos, incluirTestes = false }) {
  ...
  for (const linha of linhas) {
    ...
    // FQCN é a IDENTIDADE do símbolo. Agrupar por nome CURTO funde classes homônimas de
    // módulos diferentes e infla o limiar cross-cutting — foi o que elegeu `Subscription`
    // errado: `Superadmin\Entities\Subscription` (4 importadores) + `RecurringBilling\
    // Models\Subscription` (1) somaram 5 e cruzaram o corte, sem nenhuma qualificar sozinha.
 ...
 * símbolos DIFERENTES e somá-las infla o limiar (defeito medido — ver `identidade`).
 */
