...
//   MORDE quando há qualquer drift e SOLTA quando está em dia. Usa fixtures em dir temporário
//   + o motor real ds-token-diff via --snapshot/--tokens. Não há baseline que absorva dívida.
...
function run(snapPath, extra = []) {
  const args = [SCRIPT, '--snapshot', snapPath, '--tokens', TOK, ...extra];
  try { execFileSync('node', args, { encoding: 'utf8' }); return 0; }
  catch (e) { return e.status ?? 1; }
}

console.log('ds-mirror-drift.test — bite / release sem baseline');

const match = snap('match.css', 'oklch(0.5 0 0)');   // == git → diverge 0
const drift = snap('drift.css', 'oklch(0.9 0 0)');   // valor difere → diverge 1

// RELEASE — em dia (diverge 0), --enforce → passa
ok(run(match, ['--enforce']) === 0, 'RELEASE: snapshot == git, --enforce → exit 0');

// BITE — drift acima do baseline, --enforce → morde (exit 1)
ok(run(drift, ['--enforce']) === 1, 'BITE: qualquer drift, --enforce → exit 1');

// ADVISORY — mesmo drift SEM --enforce → nunca bloqueia (exit 0)
ok(run(drift) === 0, 'ADVISORY: drift sem --enforce → exit 0 (só ::warning::)');
...
{
  const args = [SCRIPT, '--snapshot', match, '--tokens', join(ROOT, 'tokens-nao-existe'), '--enforce'];
  ...
  const args0 = [SCRIPT, '--snapshot', match, '--tokens', join(ROOT, 'tokens-nao-existe')];
...
// Controle anti-anistia: flags antigas de baseline são recusadas explicitamente.
ok(run(drift, ['--update-baseline']) === 2, '--update-baseline é recusado; drift não pode ser incorporado');
ok(run(drift, ['--baseline', join(ROOT, 'qualquer.json')]) === 2, '--baseline é recusado; não há piso alternativo');
