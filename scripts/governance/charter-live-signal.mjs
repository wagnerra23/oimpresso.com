...
// ou um campo `smoke:` no frontmatter (ref a um smoke datado). Sem nenhum → `live_sem_sinal`.
// `live = evidência, não palavra`. fs-puro (1 JSON + walk de charters). Sem deps/DB/PHP.
//
...
for (const f of files.sort()) {
  ...
  const signal = biz.length ? `prod-flags:biz=${biz.join(',')}` : (smoke ? `smoke:${smoke}` : null);
...
if (JSON_OUT) {
  process.stdout.write(JSON.stringify({
    _meta: { lint: 'charter status:live precisa de sinal de prod (prod-flags.json) ou smoke:', generator: 'scripts/governance/charter-live-signal.mjs', regra: 'live_ok = component em governance/prod-flags.json `live` (>=1 biz) OU campo smoke:. live_sem_sinal = nenhum. fs-puro, sem timestamp/sha (re-run sem mudança = diff vazio).', escopo: args.length ? 'diff-aware (args)' : 'full-tree' },
...
console.log(`\n  CHARTER LIVE SIGNAL — \`status: live\` precisa de sinal de prod (governance/prod-flags.json) ou \`smoke:\` · escopo: ${args.length ? 'diff-aware' : 'full-tree'}`);
console.log(`  live: ${rows.length} · com sinal: ${rows.length - semSinal.length} · SEM sinal: ${semSinal.length}\n`);
for (const r of semSinal) console.log(`  ⚠️ ${r.rel} (${r.key}): \`status: live\` SEM sinal de prod → adicione \`${r.key}\` em governance/prod-flags.json \`live\` (ou campo \`smoke:\` datado no charter).`);
if (!semSinal.length) console.log('  ✓ todo charter `status: live` carrega sinal de prod (prod-flags.json ou smoke).');
