...
2. **Contador inflado.** Desde 2026-09-15 o contador é **DERIVADO** (`base:<N>` congelada + 1 por linha `- **rec**`), então o **vetor mudou** — inflar continua possível, só que agora se faz **mexendo na `base`**, e é isso que você audita. E **se o PR edita a linha `- **Ocorrências:**`, isso por si é o achado**: registrar é ADICIONAR um `- **rec**`, nunca tocar o campo. O que sobra pra atacar: **(a)** o recibo adicionado não carrega ocorrência real (linha sem caso datado — conte se o `- **rec**` novo prova alguma coisa); **(b)** a `base` foi mexida (ela é congelada — qualquer alteração dela precisa de justificativa explícita); **(c)** o inverso de sempre, que o derivado NÃO resolve: reincidência declarada em prosa e sem `- **rec**` correspondente. Em LC legada (sem `base:`) o ataque antigo continua valendo: conte os recibos contra o inteiro escrito.
