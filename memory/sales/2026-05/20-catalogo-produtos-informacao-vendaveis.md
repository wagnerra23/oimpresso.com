...
purpose: catalogar produtos vendáveis a partir do acesso autorizado a 37 bancos OfficeImpresso (132k clientes finais, GMV R$ [redacted Tier 0]M agregado, 12 UFs diretas + 27 indiretas)
relates:
...
|---|---|---|---|---|---|---|---|---|
| 1 | **Financial Snapshot Tier 1-3** | Cliente OfficeImpresso (38 bases) | R$ [redacted Tier 0]/299/599 mês | Dashboard web + PDF email + push alerts | `FINANCEIRO`, `MENSALIDADE_FINANCEIRO`, `BOLETOS` por tenant | 🟢 Verde (dado próprio do cliente) | M (110h IA-pair, já especificado) | R$ [redacted Tier 0]k–145k |
| 2 | **Benchmark Setorial Anônimo** | Cliente OfficeImpresso (38 + futuros oimpresso.com) | R$ [redacted Tier 0]/m add-on | Dashboard "você vs média gráficas SP/região/porte" | Agregado k-anon ≥5 de FINANCEIRO + VENDA todos tenants | 🟡 Amarelo (precisa opt-in DPA) | M (60h — engine k-anon + UI) | R$ [redacted Tier 0]k–60k |
| 3 | **Score de Crédito Gráfica B2B** | Fintechs (Asaas, Iugu, Cora, BV+) e seguradoras | R$ [redacted Tier 0]–15 por consulta API + R$ [redacted Tier 0]k/m fee fixo | API REST `POST /credit-score {cnpj}` | Histórico pagamento agregado de PESSOAS×FINANCEIRO×BOLETOS cross-tenant | 🔴 Vermelho (revenda dado pra terceiro — exige DPA forte + LGPD Art. 7º base legal "legítimo interesse" + opt-out cliente origem) | G (200h — modelo + API + compliance) | R$ [redacted Tier 0]k–240k |
| 4 | **Lead Intelligence pra Fornecedores** | HP, Avery, 3M, Mimaki, Roland, distribuidores tinta/vinil | R$ [redacted Tier 0]k–20k mês por fornecedor (whitelabel) | Dashboard + Excel mensal: "gráficas SP que faturam R$ [redacted Tier 0]-100k/mês comprando vinil" | VENDA + FINANCEIRO agregado anônimo + segmentação porte/UF | 🟡 Amarelo (perfil agregado, sem PII) | M (80h — query engine + UI white-label) | R$ [redacted Tier 0]k–480k |
| 5 | **Relatório de Mercado Anual Com. Visual BR** | ABRIGRAF, ABRACOM, SEBRAE, FGV, investidores PE/VC | R$ [redacted Tier 0]/cópia (PDF) ou R$ [redacted Tier 0]k/ano licença ilimitada | PDF 60 pgs + 1 webinar por release | Agregado nacional + 5 anos histórico | 🟢 Verde (k-anon ≥10, agregado UF) | P (40h primeira edição, depois 20h/ano) | R$ [redacted Tier 0]k–90k |
| 6 | **Newsletter Setorial "ImpressaInsights"** | Lead magnet B2B — atrai prospects oimpresso + monetiza ads em 12m | R$ [redacted Tier 0] (free) → ads R$ [redacted Tier 0]–2.500 por edição em 12m | Email semanal Mailchimp + RSS | Indicadores macro do banco (volume, ticket médio anonimizado) | 🟢 Verde (só dado agregado) | P (16h setup + 2h/sem operação) | R$ [redacted Tier 0]k–40k em ads (recompensa real é pipeline indireto) |
| 7 | **API White-Label "Insights Gráfica"** | Software houses concorrentes que queiram embed (Bling, Conta Azul) | R$ [redacted Tier 0]–2 por chamada API + R$ [redacted Tier 0]k/m mínimo | API REST + SDK PHP/JS | Camada de agregados k-anon + score | 🔴 Vermelho (concorrente direto pode usar contra) | G (160h) | R$ [redacted Tier 0]k–180k (alto risco competitivo) |
| 8 | **Forecast Receita 90d (preditivo)** | Cliente OfficeImpresso e oimpresso.com | R$ [redacted Tier 0]/m add-on Tier 2+ | Card no dashboard + alerta WhatsApp | ML simples (Prophet/ARIMA) sobre 24m FINANCEIRO próprio | 🟢 Verde (dado próprio) | M (60h — modelo + integração) | R$ [redacted Tier 0]k–90k |
| 9 | **Churn Risk Score Cliente Final** | Cliente OfficeImpresso (saber qual cliente DELE vai parar de comprar) | R$ [redacted Tier 0]/m add-on | Lista priorizada "top 20 clientes em risco" + razão | ML sobre VENDA + FINANCEIRO próprio (sazonalidade + pagamento + engagement) | 🟢 Verde (dado próprio) | M (80h — modelo + UI) | R$ [redacted Tier 0]k–72k |
| 10 | **Mapa de Expansão Geográfica** | Cliente quer abrir filial nova / fornecedor quer entrar UF | R$ [redacted Tier 0] por consulta (Cliente) / R$ [redacted Tier 0]k/m subscription (Fornecedor) | PDF 12 pgs + mapa interativo: densidade gráficas, ticket médio UF, gap competitivo | Agregado VENDA × CIDADE/UF (PESSOAS) + IBGE | 🟡 Amarelo (precisa k-anon ≥5 por cidade) | M (50h) | R$ [redacted Tier 0]k–120k |
| 11 | **Compliance Fiscal Snapshot** | Cliente OfficeImpresso (alerta NFe vs receita) | R$ [redacted Tier 0]/m add-on | Dashboard: "receita declarada × NFe emitida — gap" + alerta | NOTA_FISCAL × FINANCEIRO próprio | 🟢 Verde (dado próprio, ajuda compliance) | P (30h) | R$ [redacted Tier 0]k–70k |
| 12 | **Webinar Trimestral "Saúde do Setor"** | Lead magnet → conversão oimpresso + sponsor fornecedor | R$ [redacted Tier 0] + sponsor R$ [redacted Tier 0]k–15k por evento | Webinar Zoom 60min + replay + slide deck | Mesmo dado do Relatório Anual mas vivo | 🟢 Verde | P (20h por edição) | R$ [redacted Tier 0]k–120k em sponsor |
| 13 | **Score de Saúde Operacional** | Cliente próprio (autoavaliação) + investidor/banco (due diligence) | R$ [redacted Tier 0]/m cliente / R$ [redacted Tier 0] por relatório due diligence | Score 0-100 + 5 dimensões (caixa, AR, churn, growth, fiscal) | Multi-tabela próprio cliente | 🟢 Verde (próprio) → 🟡 Amarelo (DD: cliente autoriza release pro investidor) | M (60h) | R$ [redacted Tier 0]k–80k |
| 14 | **Histórico de Adimplência B2B (consulta CNPJ→CNPJ)** | Cliente OfficeImpresso quer saber se PROSPECT dele paga em dia | R$ [redacted Tier 0] por consulta ou R$ [redacted Tier 0]/m (50 consultas) | Web app + API: "CNPJ X paga em dia em N gráficas (anônimas) — score Y" | PESSOAS.CNPJCPF × FINANCEIRO histórico cross-tenant | 🔴 Vermelho (revenda dado de comportamento de pagamento de PJ — exige DPA + base legal forte; PJ tem proteção menor que PF na LGPD mas ainda exige cuidado) | G (140h — engine + LGPD + UI) | R$ [redacted Tier 0]k–180k |
| 15 | **Dataset Anônimo Pesquisa Acadêmica** | USP, FGV, FIA, PUC — research grants | R$ [redacted Tier 0]k–25k licença por dataset × pesquisa | CSV + dicionário de dados via Google Drive licenciado | Dump anonimizado k-anon ≥10 + DP noise | 🟢 Verde (totalmente desidentificado) | P (24h por release) | R$ [redacted Tier 0]k–50k (low volume, high margin, branding acadêmico) |
...
**Caso de uso real**: piloto ServidorWR2 (2026-05-09) revelou déficit -R$ [redacted Tier 0]k 12m + R$ [redacted Tier 0]k em atrasos que Wagner não tinha visibilidade clara. Cliente não sabe estado real → paga pra saber.
...
📊 12 meses (jun/25 → mai/26)
   Receita recebida    R$ [redacted Tier 0]  (+8,2% vs ano anterior)
   Despesa paga        R$ [redacted Tier 0]
   ⚠️ Déficit operacional   -R$ [redacted Tier 0]

🚨 Atenção imediata
   • R$ [redacted Tier 0]k em atraso > 90d (5 clientes — ver pág 4)
   • Top atraso: CLIENTE-007 R$ [redacted Tier 0]k (vencido 127d)
   • MRR contratos vivos: R$ [redacted Tier 0] — 12 contratos sem valor
     definido precisam reconciliar

📈 Tendência 6m: receita estável (-2%), despesa subindo (+11%)
                 → projeção 90d: déficit acumula ~R$ [redacted Tier 0]k
```
...
```
🏆 BENCHMARK — Você vs Gráficas similares (SP, R$ [redacted Tier 0]-150k/m, n=12)

Ticket médio venda    Você R$ [redacted Tier 0]    Média R$ [redacted Tier 0]    🔴 -23%
% atrasos > 30d       Você 18%       Média 11%       🔴 +7pp
MRR contratos         Você R$ [redacted Tier 0]k    Média R$ [redacted Tier 0]k    🟡 -36%
...
API que recebe `{cnpj_grafica}` e retorna score 0-1000 baseado em histórico de pagamento agregado nos 37+ bancos. Asaas/Iugu/Cora pagam **R$ [redacted Tier 0]-15 por consulta** porque hoje gastam mais com Serasa/Boa Vista que cobrem mal o nicho gráfico (Serasa não tem visibilidade em fornecedor de tinta/vinil que vende parcelado pra gráfica).
...
Fornecedor de tinta/vinil/máquinas paga R$ [redacted Tier 0]-20k/mês por dashboard que mostra: "232 gráficas em SP+RJ faturando R$ [redacted Tier 0]-100k/mês, comprando categoria X (proxy via VENDA + descrição produto), com tendência crescimento Y". Permite o fornecedor priorizar onde gastar SDR/visita comercial.

**Caso de uso**: HP quer lançar plotter novo R$ [redacted Tier 0]k. Compra dashboard WR. Filtra: SP+RJ+MG, gráficas R$ [redacted Tier 0]-200k/m faturamento (porte que cabe no plotter), atualmente sem plotter dessa faixa (proxy: ticket médio venda < threshold). Saída: lista de 87 gráficas qualificadas → SDR HP liga já sabendo perfil.
...
**Mockup**:
```
🎯 SEGMENTO: Gráficas SP+RJ — Médio porte (R$ [redacted Tier 0]-150k/m)
                Categoria: alta conversão sublimação têxtil

   Total identificadas:        232
   Crescimento 12m:            +14%
   Ticket médio compra:        R$ [redacted Tier 0]
   ...
   📞 Quer abordar? Contrate SDR-as-a-Service (R$ [redacted Tier 0]/lead qualif.)
```
...
**Caso de uso**: PE fund avaliando comprar grupo de gráficas. Quer entender setor sem custar R$ [redacted Tier 0]k em consultoria McKinsey. Compra relatório WR R$ [redacted Tier 0]k licença ilimitada → distribui pro investment committee.
...
📊 Esta semana, 232 gráficas em SP movimentaram R$ [redacted Tier 0]M
   (-3% vs semana anterior — Dia das Mães caindo num domingo)

💡 DICA Operacional
   Faturou R$ [redacted Tier 0]k de NFC-e mas só emitiu R$ [redacted Tier 0]k? Risco fiscal.
   [Ver checklist 5min]
...
Bling/Conta Azul/concorrente embeda widget "score de saúde do seu cliente gráfica" no dashboard deles. WR cobra R$ [redacted Tier 0]-2 por chamada + mínimo R$ [redacted Tier 0]k/m. **RISCO ALTO**: concorrente direto pode usar pra benchmarking competitivo. Recomendado SÓ pra integradores que NÃO competem (Conta Azul OK, Bling OK; Calcgraf NÃO).

---

### Produto 8: Forecast Receita 90d

ML simples (Prophet/ARIMA) sobre histórico próprio do cliente → previsão receita próximos 90d com intervalo de confiança. Card no dashboard. R$ [redacted Tier 0]/m add-on Tier 2+.

**Mockup**:
```
📈 FORECAST 90d — Junho a Agosto 2026

   Junho       R$ [redacted Tier 0]k ± 18k    🟢 Confiança alta (sazonalidade clara)
   Julho       R$ [redacted Tier 0]k ± 24k    🟡 Confiança média
   Agosto      R$ [redacted Tier 0]k ± 31k    🟡 Confiança média

   ⚠️ Alerta: julho costuma ter pico de devolução +12% — provisione
              R$ [redacted Tier 0]k margem extra pra pagar fornecedor cedo
```
...
Dashboard interativo + PDF: densidade de gráficas por município, ticket médio, gap competitivo (cidades com poucas gráficas vs poder de compra IBGE). Vendido como consulta pontual (R$ [redacted Tier 0]) pra cliente decidindo abrir filial, ou subscription (R$ [redacted Tier 0]k/m) pra fornecedor entrar em região.

---

### Produto 11: Compliance Fiscal Snapshot

Compara `FINANCEIRO.RECEBIDA` vs `NOTA_FISCAL` emitida → calcula gap. Alerta vermelho quando gap > X%. R$ [redacted Tier 0]/m add-on. Argumento de venda: "evita autuação Receita Federal". Dado próprio do cliente — LGPD verde.

---

### Produto 12: Webinar Trimestral

Mesmo dado do Relatório Anual, mas vivo, 1h Zoom, com Q&A. Sponsor (HP, Mimaki) paga R$ [redacted Tier 0]-15k por evento pra apresentar 5min + ter logo. Replay vira lead magnet.

---

### Produto 13: Score de Saúde Operacional

Score 0-100 + 5 dimensões (caixa, AR, churn, growth, fiscal). Cliente vê na própria. Versão "due diligence" entrega PDF assinado digitalmente que o cliente leva pro banco/investidor — R$ [redacted Tier 0] por relatório.

---

### Produto 14: Histórico de Adimplência B2B (CNPJ→CNPJ)

Gráfica X quer saber se PROSPECT Gráfica Y (CNPJ Z) paga em dia. Consulta API WR: "Gráfica Z paga em dia em 4 fornecedores rastreados, score 680 — atrasou 1 vez 12d". R$ [redacted Tier 0]/consulta. **LGPD vermelho** — exige DPA forte + base legal + opt-out. PJ tem proteção menor que PF mas ainda demanda cuidado.

---

### Produto 15: Dataset Anônimo Pesquisa Acadêmica

USP/FGV/PUC pagam R$ [redacted Tier 0]-25k por dataset anonimizado pra pesquisa (artigo, dissertação, mestrado). Branding acadêmico vale mais que receita direta — paper publicado mencionando "WR Sistemas dataset" vira moat reputacional.
...
1. **Lead Intelligence Fornecedores (#4)** — R$ [redacted Tier 0]k–480k. Tickets altos (R$ [redacted Tier 0]-20k/m) com poucos clientes (5-20 fornecedores). Esforço médio.
2. **Score de Crédito Gráfica B2B (#3)** — R$ [redacted Tier 0]k–240k. Volume alto de consultas paga bem; mas LGPD exige investimento upfront sério.
3. **Histórico de Adimplência B2B (#14)** — R$ [redacted Tier 0]k–180k. Volume médio + ticket médio + retenção alta.
...
- **Cenário pior**: ANPD recebe reclamação de cliente final dizendo "minha gráfica forneceu meu dado de pagamento pra terceiro sem consentimento" → multa até 2% do faturamento (R$ [redacted Tier 0]M cap). Mitigação: cláusula no contrato OfficeImpresso autorizando uso agregado anônimo + comunicação ativa pré-lançamento.
...
1. Financial Snapshot Tier 1-3 (#1) — R$ [redacted Tier 0]k–145k
2. Compliance Fiscal (#11) — R$ [redacted Tier 0]k–70k
3. Benchmark Setorial Anônimo (#2) — R$ [redacted Tier 0]k–60k
4. Forecast Receita 90d (#8) — R$ [redacted Tier 0]k–90k
5. Lead Intelligence Fornecedores (#4) — R$ [redacted Tier 0]k–480k

**Cenário conservador agregado 12m**: R$ [redacted Tier 0]k
**Cenário realista agregado 12m**: R$ [redacted Tier 0]k
**% da meta R$ [redacted Tier 0]M/ano**: 7% conservador / 17% realista

Combinado com receita de assinatura oimpresso.com tradicional (Starter/Pro/Enterprise pricing tiers já definidos), atinge meta R$ [redacted Tier 0]M ano com **diversificação de risco** — não dependendo só de SaaS subscription puro. Data products têm margem 80%+ (custo marginal de servir cliente extra é desprezível) — são o caminho natural pra escalar sem virar consultoria humana intensiva.
