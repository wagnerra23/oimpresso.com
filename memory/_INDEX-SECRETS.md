...
| **Token MCP pessoal (`mcp.oimpresso.com`)** | Bearer `mcp_*` — **per-dev**, um por pessoa | `.claude/settings.local.json` em `mcpServers.oimpresso.headers.Authorization` (gitignored, **sem backup no git**). Gerado em `/copiloto/admin/team`. Template: [`.claude/settings.local.json.example`](../.claude/settings.local.json.example) · cofre de restauração opcional: `.claude/oimpresso-local/mcp-settings.local.json` | **não é acessível ao agente** — é credencial pessoal do dev, gerada na UI. Sem ela o hook `brief-fetch-curl` cai em fallback (fail-open: sessão abre, mas sem brief e sem tool MCP). Formato aceito pelo `readAuthHeader`: prefixo `Bearer mcp_` **e** sem a marca `COLE_SEU` | por pessoa / ao revogar acesso | 🔒 humano-only (cada dev gera o seu) |
