...
| **SMTP transacional — KingHost** (⚠️ **NÃO é Mailgun**) | usuário + senha SMTP | Hostinger `.env`: `MAIL_HOST=smtpi.kinghost.net`, `MAIL_PORT=465`, `MAIL_ENCRYPTION=ssl`, `MAIL_USERNAME=no-reply@wr2.com.br`, `MAIL_PASSWORD` | `ssh ... 'grep ^MAIL .env'` — **mascare a senha ao imprimir** | humano-only: conta de e-mail do domínio `wr2.com.br` no painel KingHost | 🔴 **auth rejeitada** — medido 2026-09-22: `535` em **694** ocorrências desde **2026-06-21**, todas às 01h/09h (só `backup:run` e `backup:monitor`; nenhum e-mail de usuário no corpus). Esta linha dizia "Mailgun API key ✅ active" e estava **errada**: não existe nenhuma chave `MAILGUN_*`/`SES_*`/`POSTMARK_*` no `.env` (verificado). Nota: `.env` define o legado `MAIL_DRIVER`; quem vale é `MAIL_MAILER` (`config/mail.php:16`), ausente → cai no default `smtp` — não é a causa do 535, mas confunde quem for diagnosticar |
