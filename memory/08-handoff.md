...
- [2026-05-14 03:00 — Whatsapp history sync ASYNC queue final fix + pesquisa especialista (root cause QUEUE_CONNECTION=sync)](handoffs/2026-05-14-0300-whatsapp-async-queue-final-fix.md) (PRs #825/#827/#828 mergeados · root cause descoberto madrugada: Hostinger `QUEUE_CONNECTION=sync` + handler inline saturava PHP-FPM → 404 Apache gateway timeout → daemon retry esgotava → 10k msgs históricas perdidas no Suporte canal id=6 · fix `PersistHistorySyncBatchJob` via `dispatchAfterResponse()` Laravel built-in — não precisa queue:worker daemon · pesquisa especialista Baileys: WhatsApp manda full 90d SEMPRE no pareamento fresco (não há cursor protocol-level — Wagner perguntou "ele guarda... ou é retardado?"), dedup obrigatório app-side via `provider_message_id` UNIQUE · 3 PRs em ~2h · Suporte reset aguardando Wagner re-parear pra validar)
- [2026-05-13 23:30 — Whatsapp daemon saga 11 PRs + rebuild CT 100 + safeguards (incident manhã→noite resolvido + auto-fix permanente)](handoffs/2026-05-13-2330-whatsapp-daemon-saga-11prs-rebuild-safeguards.md) (PRs #813-#824 mergeados + #825 aberto · agent canônico `whatsapp-doctor` (SRE) + `capterra-senior` (Capterra módulo-agnóstico) · 2 fixes raiz "QR não abre" (auto-purge banned antes /connect) + "msgs não vêm" (`syncFullHistory: true` + Desktop browser + handler `messaging-history.set`) · automação reconciler cron 5min + reset 1-comando + drift sentinel weekly · daemon CT 100 rebuilt manual `:v823` · RUNBOOK `daemon-ct100-rebuild.md` codifica processo · CI `daemon-docker-build.yml` pega clash antes prod · mismatch DB descoberto purgado · 11 PRs em ~10h)
