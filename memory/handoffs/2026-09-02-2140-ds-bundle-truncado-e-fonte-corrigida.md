...
| # | o quê | linhas |
|---|---|---:|
| 1 | `SINTAXE` (1) + `IMPORTANT` (2) + `FONTRAMP` (99 em degrau) na fonte, **com o ledger de frescor no mesmo commit** | 102 no CSS |
| 2 | rodapés nos 2 CODE_NOTES + session log + este handoff | — |

> **Por que não é um PR por família.** Comecei com 3 PRs empilhados e o gate **required**
> `espelho — mexeu depois de verificar` reprovou o primeiro (run 100388265578): ele compara o
> hash gravado no ledger com o hash do arquivo no disco, e eu tinha posto o CSS num PR e a prova
> no seguinte. O ledger prova **um** estado de bytes — separar cria um intermediário sem prova
> válida. Reconstruir a prova do intermediário exigiria reescrever a fonte viva pra trás **ou**
> ordenar à mão um ledger append-only; os dois são piores. As famílias seguem separadas e
> revertíveis isoladamente dentro do diff. Erro meu de empilhamento, não do gate.
