...
tldr: "A F3 de /roles (Blade -> Inertia) está bloqueada pelo MESMO teto de transporte do Backup, remedido aqui. Achado novo: a esteira cowork-inbox, que seria a rota alternativa sem transcrição, foi APOSENTADA em #3878 e desemboca no mesmo DesignSync — não é saída. O backend da área já está no main (6 PRs)."
decided_by: [W]
...
## Estado do código da área (medido, não suposto)

Os 6 PRs citados no pedido estão **mergeados** (`gh pr view`, 19–20/ago):

| PR | Entrega |
|---|---|
| #5962 | Tier 0 — papel de outro negócio deixou de ser editável via `/roles/{id}` |
| #5964 | `app/Utils/PermissionCatalog.php` — catálogo fechado: **151** permissões CORE + padrões dinâmicos |
| #5971 | excluir papel em uso passou a ser bloqueado, com a contagem |
| #5959 · #5960 · #5970 | fuso horário (default e vazamento entre jobs) + guarda de comissionado |

Estado da tela: `/roles` segue em Blade (`resources/views/role/{index,create,edit}.blade.php` +
`partials/module_permissions.blade.php`). `resources/js/Pages/Roles/` não existe.
`node prototipo-ui/ancora.mjs Roles/Index` responde *"sem charter — NÃO invente âncora"*.
`--preview-ds` saiu **completo** (exit 0): o DS runtime está reposto; falta só o protótipo de `acessos/`.
