...
- **Embedder**: Ollama `qwen3-embedding:0.6b` local (PT-BR validado) — R$ 0/mês
- **Reranker**: BGE-v2-m3 self-host CT 100 — R$ 0/mês (CPU CT 100 incluído)
- **Search engine**: Meilisearch self-host CT 100 — R$ 0/mês
...
| **AWS Bedrock KB + S3 Vectors** | -90% vs OS | Titan $0.0002/M | Amazon-rerank $1/1k queries | 100-400ms | **~R$ 1.000-1.500/mês** (OpenSearch min $701) ou **~R$ 250-400** (S3 Vectors) |
| **AWS Bedrock KB + OpenSearch** | $701/mês mín | igual | igual | igual | **~R$ 4.000/mês** (USD 5.50 câmbio) |
...
| **Self-hosted Meilisearch + Ollama + BGE (oimpresso atual)** | ~R$ 0 (CT 100 já operacional) | Ollama free local | BGE CT 100 free | 50-300ms hybrid + 100-300ms BGE | **~R$ 0/mês** (custo afundado CT 100 ~R$ 200/mês total) |

**Vencedor custo absoluto:** oimpresso self-host (~R$ 0 marginal).
...
2. Custo IA mensal Jana >R$ 500/mês justificar dropar manutenção do stack
...
| **Meilisearch + Ollama qwen3 + BGE-v2-m3** | hybrid + reranker self-host CT 100; PT-BR validado (RETRIEVAL-GOTCHAS §1); R$ 0/mês marginal | [RETRIEVAL-GOTCHAS.md](../requisitos/Jana/RETRIEVAL-GOTCHAS.md) |
...
1. **Não trocar Meilisearch por Pinecone/Qdrant cloud** — custo afundado CT 100 + LGPD BR + R$ 0 marginal. Reavaliar só se CT 100 RAM ≥80% sustentado.
...
- **Custo IA mensal Jana > R$ 500/mês** ([ADR 0132](../decisions/0132-langfuse-self-host-ct100.md) review_trigger) → reavaliar prompt caching + cloud
