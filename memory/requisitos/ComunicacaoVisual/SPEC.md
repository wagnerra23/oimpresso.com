...
- [ ] Cert A1 lido de **`Modules/NfeBrasil`** — tabela `nfe_certificados` via `CertificadoService`. O local do `.pfx` **não é repetido aqui**: o dono é o disk `nfe_certs` declarado em `config/filesystems.php`, que é quem o `CertificadoService` usa. Antes dizia o módulo `SRS`, que nunca guardou credencial e foi removido (ADR 0357)
