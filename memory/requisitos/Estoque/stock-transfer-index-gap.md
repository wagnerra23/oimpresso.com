...
| Coluna "Mexeu no saldo?" | **Presente** (`Index.tsx:215`, `:236`), com o predicado `mexeuNoSaldo` derivado de leitura medida nos 3 sites do `StockTransferController` — não do nome do status (`:118-134`) | Nada — vivo à frente. O protótipo tem a mesma coluna (`estoque-page.jsx:311` — `:310` é a coluna Status; célula em `:348`), mas decide por `D.STATUS_TRF[t.status].move` sobre mock. O vivo cita R-XFER-003 e UC-INV-02 e registra a medição no código. |
