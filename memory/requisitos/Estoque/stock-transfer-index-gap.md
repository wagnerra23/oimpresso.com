...
>
> **Porte reverso** do vivo lido em 2026-08-22 (`estoque-page.jsx:1-3`) → expectativa-base é paridade. Frescor: verificação estrutural **SYNC** (nota completa no [gap do StockAdjustment/Index](stock-adjustment-index-gap.md)).
>
...
| Abas por status | Ausente como aba; existe como select (`Index.tsx:173-177`) | **Decidir.** O protótipo usa `window.CliTabs` com uma aba por status e contador em cada (`estoque-page.jsx:368-373`). É a mesma capacidade do select do vivo, com outra forma e o ganho do contador. Construir ou rejeitar por escrito. |
| Coluna "Mexeu no saldo?" | **Presente** (`Index.tsx:215`, `:236`), com o predicado `mexeuNoSaldo` derivado de leitura medida nos 3 sites do `StockTransferController` — não do nome do status (`:118-134`) | Nada — vivo à frente. O protótipo tem a mesma coluna (`estoque-page.jsx:310`, célula em `:355`), mas decide por `D.STATUS_TRF[t.status].move` sobre mock. O vivo cita R-XFER-003 e UC-INV-02 e registra a medição no código. |
...
| Sub-linha com os SKUs da rota | Ausente | **Decidir.** O protótipo põe os SKUs da carga como sub-linha da coluna rota (`estoque-page.jsx:353`), o que deixa ver o conteúdo sem abrir nada. Exige o backend enviar os itens na listagem. Construir ou rejeitar por escrito. |
