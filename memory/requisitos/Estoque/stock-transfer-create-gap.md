...
| Origem → Destino (R-XFER-004) | Bloco em destaque com selos **DE**/**PARA**, o destino **já filtrando a origem da lista** (`Create.tsx:225-228`), mensagem de erro explícita e submit bloqueado (`Create.tsx:190-240`, predicado em `:113-114`) | Nada — vivo à frente. O protótipo valida `mesmoLocal` (`estoque-forms.jsx:268`) mas mantém a origem selecionável no destino; o vivo remove a opção impossível antes do erro acontecer. É também a divergência declarada no charter (*"Origem→Destino destacado no topo"*). |
...
| Busca e adição de produto | `Input` de texto livre + botão "Adicionar linha"; `product_name` é digitado e `product_id`/`variation_id` nascem `null` (`Create.tsx:86-101`, `:252-262`) | **Decidir.** No protótipo `BuscaProduto` (`estoque-forms.jsx:12-66`) é autocomplete contra o catálogo, bloqueado até escolher o local e ciente do que já está na lista. ⚠️ **Toca ESTOQUE**. Antes de construir, medir o que o `StockTransferController@store` faz com `product_name` sem id. Construir ou rejeitar por escrito. |
