...
|---|---|---|
| Header / identidade | Gap: o mockup mostra counter numérico dentro de cada tab (`cli-moduletopnav-n`). A tela viva computa `tab_counts` server-side mas removeu o badge da tab (decisão Wagner 2026-05-25: "duplicava o KPI strip"). → não é gap, é decisão explícita. Anti-regressão:… | Nada — decisão explícita já registrada na seção (anti-regressão: não re-adicionar) |
