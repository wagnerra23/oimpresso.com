...
- `2` = erro de uso (type inválido) — validado **na entrada** desde 2026-09-15, então tipo torto reprova mesmo **sem arquivos**; antes caía no "[OK] nada a validar" e saía `0`
...
### "'--selftest' não é um modo deste script"
Causa: você chamou `validate-memory-schema.sh --selftest` (ou `--help`, ou um tipo com typo). Ele **não tem** modo próprio de selftest, de propósito — duplicaria o dono do tema.
Fix: o bite-test dele é **`node scripts/tests/memory-schema-detect.test.mjs`**, que o CI roda no job `selftest` do `memory-schema-gate.yml`. Até 2026-09-15 esse comando saía **`exit 0`** com `[OK] nada a validar` — verde de graça para quem achava ter rodado um selftest.
...
| 2026-09-15 | C | TYPE validado **na entrada**: tipo inválido sem arquivos parou de sair `exit 0`. `--selftest` devolvia verde sem medir nada; agora reprova com `exit 2` e aponta o bite-test real (`memory-schema-detect.test.mjs`). O `case` dentro do laço virou guarda de invariante. |
