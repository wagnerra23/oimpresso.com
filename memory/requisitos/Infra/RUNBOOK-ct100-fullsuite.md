...
| **shard morre 0-byte com `PlaywrightNotInstalledException`** (fix "todos" 2026-07-12) | o shard herdou um dir `tests/Browser/*` (Pest Browser exige Playwright, ausente na imagem). Já podado pelo `--exclude` (`SHARD_EXCLUDE`). Se reaparecer com outro dir não-rodável, some ao `FULLSUITE_SHARD_EXCLUDE`. |
| **shard morre 0-byte SEM loader-blocker, `run.log` diz "killer-test em voo detectado via events"** | um teste trava o processo mid-run (segfault/exit, sem fatal). O harness deriva o arquivo do `pest-events-shard-<i>.txt` e o quarentena, re-tentando (até `FULLSUITE_SHARD_ATTEMPTS`). Ver `loader-blockers.txt` (lista os quarentenados). Se um mesmo arquivo reaparece → "sem progresso, desiste" (provável OOM externo genuíno, não teste-culpado). Consertar o teste é burn-down; a quarentena mantém a noite viva. |
