...
| **`Base table or view not found` em massa NO MySQL** (`business`/`users`/`activity_log` somem mid-run) + `mysql: not found` no `pest-out.txt` | **US-GOV-018 Frente A.1 (2026-06-13):** testes com `RefreshDatabase`/`migrate:fresh` rodam "Loading stored database schemas", que chama o CLI `mysql` pra recarregar `database/schema/mysql-schema.sql` — e a imagem `oimpresso/mcp` **não tinha** esse client (só `pdo_mysql`). 72× `mysql: not found` no run pós-C1 → o fresh dropa as 364 tabelas do baseline e não recarrega → cascata. Fix duplo: (a) `mariadb-client` no `Dockerfile` da imagem (durável — exige rebuild+deploy via `bootstrap-ct100.sh`); (b) `apk add mariadb-client` no `docker run` do pest (passo 6, fallback imediato). Se reaparecer: conferir que a imagem tem `mysql` (`docker run --rm --entrypoint sh oimpresso/mcp:latest -c 'command -v mysql'`). |
