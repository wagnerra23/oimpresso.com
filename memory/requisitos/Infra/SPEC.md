...
- [ ] 🟡 matriz inicial `máquina → invocador → owner → documento → evidência` revisada — **4 de 5 eixos derivados** (era 1 em 32,7% das máquinas antes desta sessão). Cobertura medida sobre **450 das 474 linhas** do índice (as 24 restantes não casaram o parser da medição, então os percentuais abaixo são **piso**, não exato): `Documento` **92,2%** · `Invocador` **66,7%** (subiu de 155 para 300 linhas, agora incluindo workflows pelo gatilho `on:` e hooks) · `Evidência` **21,1%** (só onde há prova de mordida real). O eixo **`owner` foi MEDIDO E RECUSADO**, não esquecido: `.github/CODEOWNERS` tem 20 regras e **todas resolvem para o mesmo handle**, e não cobre `.claude/**` nem `scripts/**` — derivar dali daria coluna constante, e escrevê-la à mão em 474 linhas é a forma que o §D.2 deste plano proíbe (*"inventários: sempre derivados; nunca editar à mão"*). A condição de reabertura está registrada no próprio gerador. Mesma disciplina em `Evidência` para workflows: **0 de 123** deriváveis, declarado no índice em vez de preenchido com ruído;
