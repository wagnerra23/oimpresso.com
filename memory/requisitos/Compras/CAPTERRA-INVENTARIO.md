...
| C07 | Cálculo de estoque na entrada (baixa/movimentação) | P1 | 🟡 | `/purchases/store` legacy UltimatePOS movimenta estoque; `/compras` só lê | Teste de invariante de estoque no fluxo de entrada (pareia com C04) |
...
| C10 | Grade tam×cor (entrada matricial vestuário) | P1 | 🟡 | `GradeMatrixInput.tsx` em `Pages/Purchase/Create.tsx` (auto-detect 2D) — **fora do `/compras`** (convergência C1) | Smoke + canary + validação Larissa (US-COM-005 aguarda) |
