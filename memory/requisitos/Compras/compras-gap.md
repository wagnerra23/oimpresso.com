...
| Ações por linha (dropdown) | Gap: mockup declara dropdown com 9 ações (paridade com Blade /purchases); vivo usa AcoesDropdown em component separado, possivelmente subset · Vivo à frente: condiciona por status/paymentStatus | **Decidir.** `_pendente_` — §Veredito adotar #5: ler `AcoesDropdown.tsx` antes (não lido nesta fase) para saber se o gap 5 é real ou já coberto. ⚠️ "Reembolso de compra" e "Ver pagamentos" tocam valor: qualquer fluxo novo é Tier 0 (Regra Mestre + permissões) — Parte 5 Esforço/risco. |
| Drawer / Sheet — maior gap | Gap: mockup tem drawer 5 tabs (Resumo · Itens · Documentos · Pagamentos · Histórico) com trilho FSM e footer de ações; vivo tem 2 tabs (Waves 6+ abertas) · Vivo à frente: compra_detalhe real via Deferred | **Decidir.** `_pendente_` — §Veredito adotar #6 só depois de ler `Drawer.tsx` (define se o gap 6 é real ou já coberto). ⚠️ toca valor FORTE — Tier 0: Itens mostra custo/total/margem, Pagamentos registra pagamento, footer muda estado FSM; qualquer escrita passa por `ExecuteStageActionService` + Regra Mestre, sessão limpa por tab (Parte 6 Esforço/risco, G). |
