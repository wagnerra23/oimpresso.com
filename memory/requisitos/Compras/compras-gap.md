...
| Drawer / Sheet — maior gap | Gap: mockup tem drawer 5 tabs (Resumo · Itens · Documentos · Pagamentos · Histórico) com trilho FSM e footer de ações; vivo tem 2 tabs (Waves 6+ abertas) · Vivo à frente: compra_detalhe real via Deferred | Nada — o `_pendente_` da Parte 6/§Veredito #6 foi resolvido lendo o código: `Pages/Compras/components/Drawer.tsx` declara as 5 abas (l.22–26: resumo · itens · documentos · pagamentos · historico) e `initialTab` (l.95/154). A prosa de 2026-06-30 assumia "2 tabs" sem ler o componente; o gap 6 estava coberto. O que ficar de diferença por REGIÃO dentro de cada aba é trabalho de âncora do map (linhas TODO), não gap de tela. ⚠️ Qualquer escrita nas abas Itens/Pagamentos/footer segue Tier 0 (`ExecuteStageActionService` + Regra Mestre). |
