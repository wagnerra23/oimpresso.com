...
| **P** | **ligar `--omission` no CI** — a catraca que pega omissão sem declarar item a item | não | ✅ **LIGADA 2026-09-22** (advisory, no `contrato-de-tela.yml`). FP medido ANTES em 231 merges de PR: como estava, acusaria 22 merges (16,3%) com 152 acusações, ~68% ruído — 34,2% símbolo apenas **movido** de arquivo e 34,9% **descrição de `it()`** tratada como símbolo. As duas viraram correção no script (C1/C2, docblock de `SYMBOL_RES`), com bite-test + controle negativo + prova por mutação; depois delas, 11 merges (8,1%) e 48 acusações. ⚠️ Escopo é a **árvore de telas/módulos**, não os `alvo[]` dos contratos: ali o modo é **cego** (172 linhas removidas na janela, **zero** casando — o que some numa tela é JSX/copy, não símbolo exportado), e ligá-lo naquele escopo seria gate-carimbo |
