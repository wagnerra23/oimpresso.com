...
| **P** | **ligar `--omission` no CI** — a catraca que pega omissão sem declarar item a item | não | ✅ **LIGADA 2026-09-22** (advisory, no `contrato-de-tela.yml`). FP medido ANTES de ligar. As duas causas de ruído viraram correção no script (C1 símbolo apenas **movido** de arquivo; C2 **descrição de `it()`** tratada como símbolo), com bite-test + controle negativo + prova por mutação — e essas seguem certas pelo mérito. ⚠️ **ERRATA no mesmo dia:** os números que este PR publicou (*"231 merges, 22 acusados (16,3%) → 11 (8,1%)"*) descreviam **outro universo** — `git log --merges` morreu em 2026-06-08 (main é squash-only desde então, 4.905 commits invisíveis), os 35 contratos nasceram em 2026-09-11 (23 dos 40 alvos nem existiam no corpus) e a taxa saiu sobre o escopo do `--alvo` em vez da população que o `detect` realmente dispara. **Número válido — corpus vivo 2026-09-15..09-22, 300 commits de squash:** 21 disparam o `detect`, **2 acusados = 9,5%**. Detalhe das 3 causas + receita de reprodução no docblock de `SYMBOL_RES` e no [RUNBOOK-contrato-de-tela](../_DesignSystem/RUNBOOK-contrato-de-tela.md) §2.6 |
...
2. **O hue 90 dos textos tem fonte, não é lapso — mas a mensagem do commit e o diff discordam.**
   ...
   exatamente o que o [`prototipo-ui/Design System v4.html:1771-1779`](../../../prototipo-ui/Design%20System%20v4.html) _(path removido em 2026-09-11, 878e6069d1a — o CONTEÚDO vive em `prototipo-ui/design-system/prototipo-ui/Design System v4.html`, blob idêntico)_
