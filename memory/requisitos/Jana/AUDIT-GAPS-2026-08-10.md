...
| # | Arquivo | Problema |
|---|---|---|
| 22 | `memory/governance/scorecards/screens/jana-painel.yaml` | ✅ **RESOLVIDO** ([#5536](https://github.com/wagnerra23/oimpresso.com/pull/5536), 98 checks pass). Scorecard de tela **que não existe** — apontava pra `Pages/Jana/Painel.tsx`, removida em 06/08 pelo [#5357](https://github.com/wagnerra23/oimpresso.com/pull/5357), que tirou `.tsx` + charter + casos e **esqueceu o scorecard**: era dívida daquele PR, não achado novo. Era **nota 74 congelada num piso que nada podia graduar**. ⚠️ **Duas afirmações desta linha estavam ERRADAS e ficam registradas, não apagadas** (append-only): **(a)** *"Zero referenciadores (`rc=1`)"* — medido contra `origin/main` dá `rc=0`, e o referenciador **é este próprio documento**; a união de `jana-painel` + `Jana/Painel` + `Painel.tsx` + `Pages/Jana/Painel` dá **16 arquivos**, dos quais só 3 não são prosa (e os 3 caem: um é `Financeiro/Painel.tsx`, outro é comentário, outro é lido só por `file_exists()`). **(b)** *"193 scorecards = as 193 telas"* — **nunca foram iguais**: o contador de telas é outro sistema. Não restateio o número aqui (§5 2026-07-17) — o dono é `npm run screen-coverage:report`, e o recibo de 2026-08-10 diz `206 telas · SCORECARD 191/206 (92,7%)`. **Achado colateral, NÃO tratado:** sobra 1 de delta (192 `.yaml` no disco × 191 telas cobertas) e não é órfão — é **slug**: `kb-index-v2.yaml` deriva `kb-index.v2` do próprio `path:` (`kb/Index.v2.tsx`), então o mapa não o casa. Outro módulo, decisão [W] |
