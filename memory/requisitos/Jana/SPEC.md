...
- [x] **Rota B — judge local zero-egress IMPLEMENTADA (2026-07-17, decisão [W]):** `RagasJudgeService` ganhou backend `ollama` (`httpOllama` → POST `{ollama_url}/api/chat`, self-hosted CT 100); o Job roteia `judge='local'` pra ele via `useBackend('ollama')`. PII redigida antes mesmo indo pra nossa infra. `judge` desconhecido → SKIP (fail-safe). Substitui o `local NÃO-implementado → SKIP` antigo.
- [ ] **DoD ([W]) — gates + pré-reqs de infra da rota B:** (1) instalar modelo de CHAT no Ollama do CT 100 (`ollama-embedder` só tem embeddings — `ollama pull qwen2.5:3b`); (2) expor esse Ollama ao worker (ADR 0062 Hostinger≠CT 100 — subdomínio Traefik + IP-whitelist, como langfuse.oimpresso.com) e apontar `RAGAS_OLLAMA_URL`; (3) `enabled=true` + `judge=local` + `RAGAS_JUDGE_BACKEND=ollama`. Só então o eval flui zero-egress no tráfego real.
- [ ] Só faithfulness no v1 (sem gt no tráfego real → recall/relevancy ficam offline/follow-up).

**Ressalva do adversário:** trocar RAGAS tautológico por RAGAS não-calibrado pode ser trocar teatro por teatro com casas decimais — a literatura 2026 mediu falha de validade discriminante do `faithfulness` (19,3pp retrieval vs 15,9pp geração). Por isso o mecanismo **nasce sem gate** — só publica número no trace. Calibrar antes de confiar na média. Um juiz local 3B (rota B) é **menos preciso** que gpt-4o-mini — a calibração importa em dobro; começar comparando o score local vs openai numa amostra antes de confiar.

**DoD:** existe score `ragas_faithfulness_online` em ao menos 1 trace real (biz≠1) com PII redigida — só possível após [W] ligar os gates + (rota B) os pré-reqs de infra.

**Testado em:** `Modules/Jana/Tests/Feature/Telemetry/JudgeTraceOnlineJobTest.php` (shouldSample 0/1/determinismo/~5% · judge=local roteia backend Ollama + PII redigida · judge desconhecido → SKIP · judge=openai roda PiiRedactor antes · mock não pontua) + `tests/Feature/Ragas/RagasJudgeServiceTest.php` (backend ollama POST /api/chat + parse + guarda anti-OpenAI · url-vazia → 0.0 sem HTTP · 500 fail-open · backend openai regressão). Ambos ANCORADOS na lane sqlite do CI — eram teste-fantasma (Jana fora do modules-pest matrix, nunca rodavam em CI).

**Refs:** ADR 0318 · ADR 0093 · ADR 0062 (Hostinger≠CT 100) · `config/ragas.php` (backend/ollama_url/ollama_model + pré-reqs) · `Modules/Jana/Services/Ragas/RagasJudgeService.php` · `Modules/Jana/Services/Telemetry/LangfuseClient.php` · `Modules/Jana/Services/Privacy/PiiRedactor.php`
