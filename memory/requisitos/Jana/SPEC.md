...
**O que é fato (com o comando ao lado, reproduzível):** o agente gera as propostas (structured output, 7 campos × 3 cenários) e o driver valida o shape campo-a-campo, e o método `sugerir()` apenas delega ao driver e devolve o array sem gravar. _(Até 2026-08-27 o **docblock da CLASSE** `SuggestionEngine` declarava *"STUB spec-ready"*, contradizendo esta própria medição de 2026-08-10; o rótulo foi removido naquele dia e a ref de linha `:9-15` saiu com ele, por apodrecer no primeiro refactor — §5 2026-07-26.)_ O `ChatController` **injeta** `SuggestionEngine $suggestions` no construtor e **nunca o chama** (`rg -F '$this->suggestions'` → só `.md`).
...
**Protótipo F1 V2 navegável** existia em `prototipo-ui/prototipos/chat/cowork-app-v2.jsx` (removido em 2026-05-20, 1070e3759b7):
...
**Implementado em:** `Modules/Jana/Mcp/Tools/CharterFetchTool.php` · `.claude/skills/charter-first/SKILL.md` (`tier: B` auto-trigger por design — ADR 0225) · `.claude/hooks/charter-validate.mjs` (advisory) · verificado@5873313 (2026-07-28) — tool + hook + skill presentes; tiering resolvido como Tier B (ADR 0225 supersede a meta "Tier A" original). **Âncora reconciliada em 2026-07-28:** citava `charter-validate.ps1` + `.sh`, ambos DELETADOS no porte para Node cross-plataforma (`.ps1` em [#4592](https://github.com/wagnerra23/oimpresso.com/pull/4592) 2026-07-20 · `.sh` em [#4738](https://github.com/wagnerra23/oimpresso.com/pull/4738) 2026-07-23, "superseded por .mjs") — a US seguia `status: done` apontando para path inexistente (`anchored_dead`). O comportamento não mudou; só o path. Detectado pelo `doneness-lint` ao tocar este SPEC: a dívida existia desde 07-20 e ficava dormente enquanto ninguém tocava o arquivo
...
- [`AcaoHitlService`](../../../Modules/Jana/Services/AcaoHitlService.php) (novo em 2026-08-18) — prévia + aprovação **humana** das ações sugeridas no Painel `/ia`, gravada em `jana_acao_aprovacoes` (charter `Index.charter.md` v10 · `UC-JPAIN-12`).
...
**~~⏳ Onda 2 — abas.~~** (texto original, preservado) Estender `resources/js/Pages/Jana/_components/JanaAreaHeader.tsx` + `resources/js/Pages/Jana/_shared/JanaSubNav.tsx` pro vocabulário `Painel | Conversa | Memória`. Os dois **já existem** e já servem as 4 telas (`Chat.tsx:268` · `Cockpit.tsx:963` · `Dashboard.tsx:271` · `Memoria.tsx:150`). ⛔ **Não criar `JanaTabs.tsx`** como o pedido propõe: seria máquina paralela a tema que já tem dono (classe LC-19, lápide §5 2026-08-03). Precedente de como fazer: a Governança saiu deste mesmo header pra strip própria em 2026-08-05.
