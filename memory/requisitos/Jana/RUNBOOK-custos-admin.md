...
function GastoDiarioChart({ dados }: { dados: DiaRow[] }) {
  ...
  const max = Math.max(0.01, ...valores);   // evita div/0; piso é R$ [redacted Tier 0]
...
| `KpiCard tone="success"` | Card "Esse período (R$)" — sempre verde, mesmo se R$ [redacted Tier 0] | shadcn KpiCard |
...
- ❌ **`Math.max(0.01, ...valores)` no chart** — piso de R$ [redacted Tier 0] evita div/0 mas distorce visualmente quando o gasto real é R$ [redacted Tier 0] (cria área falsa de 1 cent). Pegadinha original: dia sem nenhum uso de IA mostra um "barzinho" enganoso. Fix: se `max === 0.01 && valores.every(v => v === 0)`, render empty state em vez de chart.
