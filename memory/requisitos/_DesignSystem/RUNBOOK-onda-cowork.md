...
- [ ] Identificar classes novas necessárias (`.vd-*`, `.os-*`, etc) lidas verbatim de `prototipo-ui/cowork/Wagner/legado/<modulo>/styles.css` _(alvo não resolve no repo — proveniência não determinada)_
...
Aplicável a TODO módulo com prototype Cowork em `prototipo-ui/cowork/Wagner/legado/<modulo>/`: _(alvo não resolve no repo — proveniência não determinada)_
