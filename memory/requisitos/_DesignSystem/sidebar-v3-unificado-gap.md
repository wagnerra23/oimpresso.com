...
| Atalhos kbd (`G I`, `G V`…) | A prosa media "NÃO renderizado". Hoje: `Sidebar.tsx:490-501` `ItemEnd` renderiza `.sb-kbd` com o atalho; `:523-527` só pra atalho que o listener realmente liga (`atalhosUsaveis` — "não prometemos atalho que não funciona"); listener em `AppShellV2.tsx:400`. | Nada — fechado no código depois da prosa (§Ordem 4 era "hints kbd (M)"): hint existe e só aparece pra atalho ligado. Quantos itens têm sequência `G X` ligada não foi medido aqui. |
