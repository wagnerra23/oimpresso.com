...
function SidebarMenu({ activeRoute, onSelectRoute }) {
  // Persiste quais grupos estão expandidos
  const [expanded, setExpanded] = useState(() => {
    try {
      const saved = localStorage.getItem("oimpresso.menu.expanded");
      if (saved) return JSON.parse(saved);
    } catch (e) {}
    return { OFFICEIMPRESSO: true, "PRODUÇÃO": false, PESSOAS: false, FINANCEIRO: false, OUTROS: false, "CONFIGURAÇÕES": false };
  });
  useEffect(() => {
    localStorage.setItem("oimpresso.menu.expanded", JSON.stringify(expanded));
  }, [expanded]);
  ...
  return (
    <div className="sb-menu">
      {MOCK.MENU.map((entry, i) => {
        ...
        return (
          <div key={entry.group} className="sb-group">
...
function Sidebar({ company, onCompany, tab, onTab, activeConvId, onSelectConv, activeRoute, onSelectRoute }) {
  return (
    <aside className="sb">
      ...
      <SidebarTabs tab={tab} onTab={onTab}/>
      <div className="sb-body">
        {tab === "chat"
          ? <SidebarChat activeConvId={activeConvId} onSelectConv={onSelectConv} onSelectRoute={onSelectRoute}/>
