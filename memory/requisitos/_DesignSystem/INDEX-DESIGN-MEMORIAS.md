...
- **Protótipo Cowork** — `prototipo-ui/cowork/Wagner/legado/<tela>/` (export do Cowork; **read-only no repo** — regra de ouro §"Regra de ouro" do `memory/reference/prototipo-ui/README.md`). _(removido em 2026-09-15, 2bed3bbb214)_
...
**Código produtivo** (editável na F3 do MWART) = `resources/js/Pages/<Mod>/<Tela>.tsx`. O `page.tsx`/`*.jsx` em `prototipo-ui/cowork/Wagner/legado/` **não** se edita no repo (re-exporta do Cowork).
...
1. **[PRE-FLIGHT-TELA.md](../../../memory/reference/prototipo-ui/PRE-FLIGHT-TELA.md)** — monta o pacote de pré-requisitos DAQUELA tela (identidade/não-inventar/não-repetir/validar).
2. **[GOLDEN-REFERENCE.md](../../../memory/reference/prototipo-ui/GOLDEN-REFERENCE.md)** — a tela-ouro do arquétipo + 10 regras binárias. "Faça idêntico, mude só X."
...
3. **[REGISTRY_DS_COMPONENTES.md](../../../memory/reference/prototipo-ui/REGISTRY_DS_COMPONENTES.md)** — componentes `@/Components/ui` disponíveis. Se está aqui, **não hand-rola**. ➕ **Layout: usar primitivos `@/Components/layout` ([ADR 0253](../../decisions/0253-primitivos-layout.md)) — `Stack/Inline/Grid/Box/Container/Text`.** ⚠️ Onda F (`Segmented`/`FormSection`/`InputGroup`/`FieldState`) **ainda NÃO existe** — não assuma que existe.
4. **[LICOES_F3_FINANCEIRO_REJEITADO.md](../../../memory/reference/prototipo-ui/LICOES_F3_FINANCEIRO_REJEITADO.md)** — os 21 anti-padrões (exemplo errado). Leitura obrigatória antes de F3.
...
| [memory/reference/prototipo-ui/GLOSSARY.md](../../../memory/reference/prototipo-ui/GLOSSARY.md) | Siglas, fases, comparáveis externos por arquétipo |
...
| **[memory/reference/prototipo-ui/METODO-KB-9.75.html](../../../memory/reference/prototipo-ui/METODO-KB-9.75.html)** ⭐ | **evoluir um módulo existente** (o "9.75" que RUNBOOK-onda-cowork e GOLDEN-REFERENCE citam): 5 princípios · 7 etapas · **22 features-tipo em 5 categorias** · 2 pré-reqs arquiteturais + o formato de diagnóstico ("reuse a forma, mude o conteúdo"). Histórico **v1 2026-05-15** — ler a lápide no topo: §1.4-A2/§3 (multi-tenant) descrevem o protótipo Cowork, **não** o repo ([ADR 0093](../../decisions/0093-multi-tenant-isolation-tier-0.md) vale) |
...
| **ui_kits/_BACKUP-NAO-USAR-cowork-2026-04-27** | **removido em 2026-08-28** | snapshot abril, mantido como histórico negativo desde a decisão Wagner original. Apagado por decisão [W] 2026-08-28 (*"só deve ter uma só origem do protótipo"*): medido antes — **0 arquivos de código** (`.js/.ts/.tsx/.jsx/.php/.html/.css/.mjs`) e **0 de 279 charters** ancorando nele. Dois índices de **máquina** citavam o caminho (`governance/deadlink-baseline.json` e `governance/doc-id-index.json`) — ambos tratados neste mesmo PR, apontados pela refutação GT-G5. A origem única do protótipo é `prototipo-ui/cowork/Wagner/`. Histórico recuperável no git; o link morto que a [UI-0018 §Não-decidido](adr/ui/0018-canon-visual-vivo-ds-v6-manual-identidade.md) registrava na UI-0010 continua morto — agora por ausência, não por rename. |
...
**Fundações** (tokens DS v3 + camada semântica **DS v6**/ADR 0249, accent **roxo 295** via ADR 0235) → **Shell** (AppShellV2/Cockpit, sidebar **preta / dark-fixo** single-pane — [UI-0023](adr/ui/0023-sidebar-dark-fixo-preto-definitivo-supersede-0019.md); dizia "light" até 2026-08-28) → **Padrão de Tela** (PT-01 Lista / Cockpit Pattern V2 / `os-page.jsx`) → **Módulo**. Owner da UI = **Claude Design plugin** (ADR 0235 §3), dentro do loop MWART (0104→0114) + persona (UI-0016).
...
| `BRIEFING_PROXIMA_SESSAO.md` ✅ reconciliado 2026-06-04 | briefing de sessão antiga (azul 220 + sidebar dark) — errata no topo para roxo 295 + sidebar light. **A metade "sidebar" dessa reconciliação estava ERRADA** (UI-0023, 2026-07-16): o dark original estava certo. Reabrir. | parcialmente errado |
...
| [ADR 0243](../../decisions/0243-processo-memoria-evolucao-design-cowork.md) | Processo de memória/evolução de design do Cowork — loop medido/auto-corretivo (3 planos · anéis · DS-GUARD · bateria · benchmark · gatilho); método em `memory/reference/prototipo-ui/PROCESSO_MEMORIA_CC.md` |
