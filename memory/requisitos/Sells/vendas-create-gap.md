---
tela: Sells/Create (/sells/create)
prototipo: prototipo-ui/cowork/vendas-create-page.jsx
...
|---|---|---|
| Header | Gap: Affordance "voltar com esc" explícito (vivo tem Cancelar no rodapé + Esc=blur, não um botão-voltar no topo). P · risco baixo. Toggle de vertical = ver Parte "Conceito transversal". · Vivo à frente: KPI 4-cards, scroll-spy, draft-recover — vivo tem,… | **Decidir.** Affordance "voltar com esc" explícito (vivo tem Cancelar no rodapé + Esc=blur, não um botão-voltar no topo). P · risco baixo. Toggle de vertical = ver Parte "Conceito transversal". Construir ou rejeitar por escrito. |
| Seleção de cliente | Gap: Cadastro rápido de cliente _inline_ (sem abrir outra aba) — o mockup resolve num mini-form na própria tela. Affordance melhor pra balcão. M · risco baixo (UI only; o backend de criar contato já existe). · Vivo à frente: Auto-pull de… | **Decidir.** Cadastro rápido de cliente _inline_ (sem abrir outra aba) — o mockup resolve num mini-form na própria tela. Affordance melhor pra balcão. M · risco baixo (UI only; o backend de criar contato já… Construir ou rejeitar por escrito. |
| Busca / adição de produtos | Gap: (1) Navegação ↑/↓/Enter no popover de produto — _pendente_ confirmar se `ProductSearchAutocomplete` já tem (não inspecionado aqui; provável que sim). (2) Leitura de código de barras/EAN — não evidente no vivo; é affordance forte pra balcão/PDV. M ·… | **Decidir.** (1) Navegação ↑/↓/Enter no popover de produto — _pendente_ confirmar se `ProductSearchAutocomplete` já tem (não inspecionado aqui; provável que sim). (2) Leitura de código de barras/EAN — não… Construir ou rejeitar por escrito. |
| Grade de itens | Gap: (1) m² por linha (largura×altura) — central pra ComVis (banner/lona/fachada). ⚠️ toca cálculo de valor — regra mestre exige dupla confirmação: muda como `lineSubtotal` é computado (área × preço × qty). Só DESCREVER, não implementar lógica aqui. G ·… | **Decidir.** (1) m² por linha (largura×altura) — central pra ComVis (banner/lona/fachada). ⚠️ toca cálculo de valor — regra mestre exige dupla confirmação: muda como `lineSubtotal` é computado (área × preço ×… Construir ou rejeitar por escrito. |
| Totais / descontos / impostos ⚠️ (zona Tier 0) | Gap: Conceito "cobrar só o aprovado" (Oficina) — muda a base do total. ⚠️ toca cálculo de valor — regra mestre exige dupla confirmação. Vivo é mais completo em todo o resto; não há gap de desconto/imposto a adotar do mockup. Apenas descrever, não mexer em… | **Decidir.** Conceito "cobrar só o aprovado" (Oficina) — muda a base do total. ⚠️ toca cálculo de valor — regra mestre exige dupla confirmação. Vivo é mais completo em todo o resto; não há gap de… Construir ou rejeitar por escrito. |
| Pagamento | Gap: (1) Cards de método com "clearing"/prazo — UX mais visual que o select do `PaymentRow`. M · risco baixo. (2) Parcelamento explícito (Nx) — _pendente_ confirmar se `PaymentRow` cobre; não evidente. M · risco médio. (3) Aviso "gera cobrança/boleto ao… | **Decidir.** (1) Cards de método com "clearing"/prazo — UX mais visual que o select do `PaymentRow`. M · risco baixo. (2) Parcelamento explícito (Nx) — _pendente_ confirmar se `PaymentRow` cobre; não evidente. M… Construir ou rejeitar por escrito. |
| Fiscal (bloco NOVO do mockup — não existe no vivo) | Gap: Maior gap conceitual do mockup. Painel "que documento vou emitir" inferido pela composição é uma affordance forte (reduz erro fiscal). ⚠️ NÃO toca valor, mas integra com NfeBrasil/FSM — G · risco médio-alto (integração multi-módulo, não é só UI). Adoção… | **Decidir.** Maior gap conceitual do mockup. Painel "que documento vou emitir" inferido pela composição é uma affordance forte (reduz erro fiscal). ⚠️ NÃO toca valor, mas integra com NfeBrasil/FSM — G · risco… Construir ou rejeitar por escrito. |
| Frete / entrega | Gap: (1) Picker de múltiplos endereços do cliente (vivo só tem 1 campo de texto livre) — M · risco baixo. (2) Radio retirada/entrega como decisão de topo (vivo deixa frete sempre disponível colapsado) — P · risco baixo. (3) Detecção de município → MDF-e… | **Decidir.** (1) Picker de múltiplos endereços do cliente (vivo só tem 1 campo de texto livre) — M · risco baixo. (2) Radio retirada/entrega como decisão de topo (vivo deixa frete sempre disponível colapsado) —… Construir ou rejeitar por escrito. |
| Ações / rodapé | Gap: (1) Desconto editável no próprio rodapé (mockup) vs card Resumo (vivo) — preferência de layout; ⚠️ toca valor. P · risco baixo-médio. (2) Label de ação contextual por vertical ("Salvar e gerar OS") — depende do fluxo OS. P · risco baixo. (3) Atalho F2… | **Decidir.** (1) Desconto editável no próprio rodapé (mockup) vs card Resumo (vivo) — preferência de layout; ⚠️ toca valor. P · risco baixo-médio. (2) Label de ação contextual por vertical ("Salvar e gerar OS")… Construir ou rejeitar por escrito. |
