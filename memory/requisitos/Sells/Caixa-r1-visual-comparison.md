...
| 2 | Header h1+subtitle+CTAs | `<h1>Caixa do dia</h1>` + `<p>Conferência por forma de pagamento, sangrias e fechamento</p>` + date picker + ghost "Imprimir Z" + primary "Fechar caixa" | replicado verbatim com `os-head/os-head-l/os-head-r` · CTA "Fechar caixa" navega `/cash-register/close-register/{id}` legacy preserved · "Imprimir Z" ghost (futuro Onda 6+1) | ⚠️ divergiu — ver §Reconciliação 2026-09-23 |
| 3 | 4 KPI hero | Faturado dia · Esperado em caixa · Conferido · Diferença (alert quando ≠0) | replicado · `os-kpi` + `os-kpi-alert` quando `Math.abs(diff)>0.01` · cor verde/vermelha/amarela via inline style oklch (canon Cowork) | ⚠️ divergiu — ver §Reconciliação 2026-09-23 |
...
| 9 | **Refs OS clicáveis** | `g.id === "oficina" && g.items.some(v => v.osRef) && <small>` com até 3 links `<a onClick={...}>↗ #{v.osRef}</a>` + `+N` overflow | replicado · onClick dispara `window.dispatchEvent(new CustomEvent('oimpresso:open-venda', { detail: { venda_id: v.id } }))` · Sells/Index listener já existe (Onda 4 commit `e40289010`) | ⚠️ divergiu — ver §Reconciliação 2026-09-23 |
...
## Reconciliação 2026-09-23 (decisão [W])

[W], textual: *"o caixa segue o código atual"*. O charter foi corrigido no mesmo PR (regra de precedência de `memory/proibicoes.md`) e os casos UC-SCAIXA-01..09 descrevem o que a tela faz hoje. As três linhas abaixo tinham ✅ paridade, mas o `.tsx` já não fazia o que elas descrevem:

| # | O que esta ficha dizia | O que a tela faz (vale) |
|---|---|---|
| 2 | "Fechar caixa" sempre visível; "Imprimir Z" ghost futuro | "Fechar caixa" só com `close_cash_register` e vira "Abrir caixa" sem caixa aberto; "Imprimir Z" abre `/cash-register/register-details` |
| 3 | KPIs Faturado · Esperado em caixa · Conferido · Diferença | KPIs Faturado · Vendas em dinheiro · Caixa aberto/fechado · Origens hoje (Esperado/Conferido/Diferença ficam no `[BACKLOG]` do casos.md) |
| 9 | refs de OS disparam `CustomEvent('oimpresso:open-venda')` | refs de OS navegam para `/sells?open=<id>` |

⚠️ Consequência declarada: nesses três pontos a tela agora diverge do protótipo (`vendas-extras.jsx`, `VendasCaixaPage`). É divergência escolhida por [W], não pendência desta ficha. Quando o protótipo for reaplicado à tela, a decisão precisa ser revista junto.
...
- ✅ Sem Modal/Dialog — link OS dispara `oimpresso:open-venda` custom event (Sells/Index drawer SaleSheet abre cross-módulo) · _superado 2026-09-23: o link navega para `/sells?open=<id>` (§Reconciliação)_
...
- **Listener cross-módulo** `oimpresso:open-venda` já registrado em Sells/Index (commit `e40289010` linha 928-929) · _o Caixa não usa mais esse evento desde a reconciliação 2026-09-23; o listener segue existindo para o Repair_
