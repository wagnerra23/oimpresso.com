...
|---|---|---|
| Header / PageHeader | Gap: sub-nav em página via `window.VdModNav here="lista"` logo abaixo do header (refino [W] 2026-06-11). VIVO usa `vd-visoes-menu` (dropdown "Visões") em vez de sub-nav inline. · Vivo à frente: subtitle métrica live vindo de `rows` reais (`X vendas · Y… | **Decidir.** sub-nav em página via `window.VdModNav here="lista"` logo abaixo do header (refino [W] 2026-06-11). VIVO usa `vd-visoes-menu` (dropdown "Visões") em vez de sub-nav inline. Construir ou rejeitar por escrito. |
