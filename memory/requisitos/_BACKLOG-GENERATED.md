...
> **893 tarefas abertas** em **51 módulos**. Regenera com `node scripts/governance/tasks-index-generate.mjs --write`.
...
| [`Cms`](#cms) | 10 | 0 | 0 | 0 | 10 |
| [`Officeimpresso`](#officeimpresso) | 10 | 0 | 0 | 0 | 10 |
...
- **US-COPI-140** — Os 2 evals de staging da Jana nunca rodam sozinhos (schedule fantasma no CT 100) _(`p0`)_
- **US-COPI-137** — Eval online em 5% dos traces reais (hoje: zero avaliação no tráfego do cliente) _(`p1`)_

### todo

- **US-COPI-132** — Langfuse traces sem tag business_id (isolamento multi-tenant observability) _(`p1`)_
- **US-COPI-135** — Desbloquear modelo frontier + fallback na Jana (gpt-4o-mini é o mais fraco do mercado) _(`p1`)_
...
- **US-SELL-054** — Offline-first no PDV — fila IndexedDB + reemissão ao reconectar _(`p1`)_
- **US-SELL-055** — Pix QR no PDV + webhook auto-reconcile ligado à venda/caixa _(`p1`)_
- **US-SELL-056** — Keyboard-first coeso no Create — hotkeys configuráveis + Enter-avança + F-key pagar _(`p2`)_
- **US-SELL-057** — Skeleton de carregamento no Create + INP < 200ms _(`p2`)_
...
- **US-RB-056** — Unificar as 3 implementações de "próximo vencimento" (NoOverflow/EN vs Overflow/PT) — REGRA MESTRE _(`p1`)_
...
- **US-PG-010** — Refund uniforme nos 6 drivers (full+partial + Pix devolução generalizada) _(`p0`)_
- **US-PG-011** — Boleto híbrido (boleto + QR Pix no mesmo documento) _(`p1`)_
- **US-PG-012** — Extrato/saldo bancário unificado + conciliação contábil _(`p2`)_
- **US-PG-013** — Split de pagamento (múltiplos recebedores) — feature-wish até sinal de cliente _(`p2`)_
...
- **US-NFSE-016** — Geração própria do DANFSe conforme NT 008/2026 _(`p0` · @eliana)_
- **US-NFSE-017** — Validar DPS contra leiaute RTC v2.00 / integrar nfse-nacional/nfse-php _(`p0` · @eliana)_
- **US-NFSE-012** — Deploy sandbox _(`p1` · @eliana · sprint D)_
- **US-NFSE-018** — Substituição de NFSe (evento, mantém vínculo original↔nova) _(`p1` · @eliana)_
- **US-NFSE-019** — Webhook/callback assíncrono de eventos SEFIN _(`p1` · @eliana)_
...
- **US-NFSE-020** — Alerta proativo de certificado A1 vencendo (cron D-30/D-7/D-1) _(`p2` · @eliana)_
- **US-NFSE-021** — Dashboard de métricas NFSe (volume/erro/ISS pago) _(`p2` · @eliana)_
- **US-NFSE-022** — Carta de correção (campo discriminação dos serviços) _(`p2` · @eliana)_
