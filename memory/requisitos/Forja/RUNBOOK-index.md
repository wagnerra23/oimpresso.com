...
title: "RUNBOOK — Forja Triage + Inbox (`/project-mgmt/triage` · `/project-mgmt/inbox`)"
module: Forja
tela: Forja/Triage/Index + Forja/Inbox/Index
owner: W
status: ativo
last_validated: "2026-05-29"
preconditions:
  - "Usuário autenticado com permission `jana.mcp.usage.all` (Spatie UPOS canon — mesmo gate do Board/MyWork)"
  - "Modules/Forja + Modules/Jana ativos (tabelas mcp_tasks / mcp_inbox_notifications)"
  - "Projeto default `COPI` resolvível (config `projectmgmt.default_project_key`) — Triage"
preconditions_short: jana.mcp.usage.all, Forja+Jana ativos, projeto COPI (Triage)
steps:
...
Duas superfícies humanas das tools MCP de governança de tasks (ADR 0070), parte do redesign Forja (ADR 0100):
...
- **Modules/Forja/Board** — destino do deep-link (`?task=ID` abre DetailSheet); reuso de `badges` (PRIORITY_BADGE)
- **Modules/Forja/MyWork** — fonte do padrão J/K + inbox payload espelhado (`MyWorkController::buildInboxPayload`)
...
- [ADR 0100 — Forja UI Redesign 4 fases](../../decisions/0100-projectmgmt-ui-redesign.md)
