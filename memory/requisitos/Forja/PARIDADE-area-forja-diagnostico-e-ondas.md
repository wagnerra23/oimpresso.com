...
| 4 | Trabalho · lista **+ chrome** | compare 0 bug — **✅ [#6582](https://github.com/wagnerra23/oimpresso.com/pull/6582)** (merge `ae7689d8d8`, 2026-09-02 23:44Z): as 3 barras de filtro, a `fj-row` densa, o KPI que FILTRA (`BUTTON`, valor 17px) e o `--accent` dark 0,70. ⚠️ **Errata de crédito (2026-09-03):** esta linha citava o [#6577](https://github.com/wagnerra23/oimpresso.com/pull/6577) *"aguardando merge"* — ele está **`CLOSED` sem merge**; quem levou o código ao `main` foi o #6582, achado por `git log -S "fj-frentebar"`, não por proximidade de data. ALVO medido no protótipo antes de codar (dark, `__oiLazyDone` + 2 leituras iguais — recibo em [forja-cockpit-visual-comparison.md §2026-09-02 noite](../TeamMcp/forja-cockpit-visual-comparison.md)): filtro **3** linhas · KPI **4/BUTTON/17px/left** · `.fj-row` **13** filhos. A réplica entrega **11 de 13** — os 2 ausentes são DECLARADOS (`fj-rowcheck`, que exige mutação em massa sem endpoint; `fj-fresco`, campo que `mcp_tasks` não tem). **Esta linha também é a dona do `chrome`** (= Onda **2** do export, §3.2): fechada por declaração em 2026-09-03 com **18 de 20** — `frentebar` 2/2 · `kpirow` 5/5 · `filterbar2` 9/9 · `toolbar` **2 de 4**, ausentes os 2 `.fj-ia-btn` (`Papéis`/`Perguntar ✦`), que abrem painéis **sem receptor no `main`** (§1 do export) — renderizá-los seria afordância falsa (LC-15). Achado do fecho: o `padding 11px 18px` do alvo é o valor do `@media(max-width:1100px)` (a medição do export rodou a 924px), **não** divergência. Recibo em [§2026-09-03 (Onda 2 do export · fecho)](../TeamMcp/forja-cockpit-visual-comparison.md). **A sonda pareada ainda NÃO rodou** — ela exige o deploy, e nada foi declarado "0 bug" antes dela |
