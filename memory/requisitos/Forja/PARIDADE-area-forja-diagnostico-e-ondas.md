...
- **Âncora:** `prototipo-ui/cowork/Wagner/forja-page.jsx` (1.253 ln) + os irmãos que o shell carrega — `forja-data.jsx` · `forja-aprova.jsx` · `forja-mcp.jsx` · `forja-integra.jsx` · `forja-page.css`. Resolvida por `node scripts/design/ancora.mjs <Mod/Tela>`, **não** escolhida no olho.
...
3. O "como" sai do painel — `node scripts/design/protocolo.config.mjs` — lido **no momento** da onda, incluindo o portão fail-closed que proíbe editar produto com grafo/preview incompleto.
...
- **Não autoriza editar nada sob `prototipo-ui/cowork/Wagner/`.** É espelho **read-only** ([ADR 0374](../../decisions/0374-emenda-0315-espelho-cowork-e-rota-prevista.md)); o durável nasce no Cowork vivo e desce pela máquina. Remendo à mão aqui some no próximo `--export-from`.
...
| 3 | Aprovações (view `hoje`) | compare 0 bug — **código aplicado, medição pendente**: a tela virou o markup do [`forja-aprova.jsx`](../../../prototipo-ui/cowork/Wagner/forja-aprova.jsx) (herói + faixa "Ao vivo no MCP" + mesa + placar), com backend novo pros 2 itens que o [W] pediu em 2026-08-08 e estavam sem fonte. **Achado:** a âncora do charter apontava o `forja-page.jsx`, que só MONTA a view — o markup mora no `forja-aprova.jsx`; corrigido. 3 divergências declaradas (verbos do FSM, dono da caixa de nota, os 4 tipos) e 2 colunas do placar sem fonte, que mostram "—" em vez de número inventado. Gates locais verdes; a11y da fila **melhorou** vs o protótipo. Falta deploy + sonda nos dois lados — recibo em [forja-cockpit-visual-comparison.md §2026-09-02 noite](../TeamMcp/forja-cockpit-visual-comparison.md) |
| 4 | Trabalho · lista **+ chrome** | **✅ ENTREGUE — fechada em 2026-09-03** (é a **Onda 3** na numeração do `COLAR-NO-CODE-EXPORT-FORJA-MODULO.md` §1; esta tabela numera por conta própria). Réplica em [#6582](https://github.com/wagnerra23/oimpresso.com/pull/6582) (merge `ae7689d8d8`, 2026-09-02 23:44Z) + papéis ARIA e região viva em [#6669](https://github.com/wagnerra23/oimpresso.com/pull/6669) (merge `63b9fecba1`, 2026-09-03 18:09Z). ⚠️ **Errata desta linha:** ela dizia *"#6577 aberto, aguardando merge [W]"* — o [#6577](https://github.com/wagnerra23/oimpresso.com/pull/6577) foi **fechado SEM merge** em 2026-09-02 19:05Z, e o `git log` do `TrabalhoLista.tsx` no `main` (clone completo, não raso) traz só os dois commits acima — nenhum dele. ALVO medido no protótipo antes de codar: filtro **3** linhas · KPI **4/BUTTON/17px/left** · `.fj-row` **13** filhos. **PLACAR: 11 de 13** na linha e **4 de 6** na `.fj-totalbar`; as 5 ausências são ~~DECLARADAS~~ **REGISTRADAS** no docblock do `TrabalhoLista.tsx`, sem placeholder — e desde a revogação de [W] em **2026-09-18** (*"agora é o Protótipo quem manda, e a paridade deve ser o objetivo"*) registrar **não** é absolver: as 5 são **DÍVIDA A FECHAR**, e o docblock diz por que ainda não fecharam, não que possam ficar. A **ordem** dos slots foi re-medida nos dois lados em 2026-09-03 (`forja-page.jsx:414-441` × `TrabalhoLista.tsx`) e é **idêntica posição a posição** depois de removidos os ausêntes. **T7 segue PENDENTE** (prod pede auth: `302 → /login`) — nada aqui é "0 bug". **Esta linha também é a dona do `chrome`** (= Onda **2** do export, §3.2), fechada por declaração em 2026-09-03 com **18 de 20** — `frentebar` 2/2 · `kpirow` 5/5 · `filterbar2` 9/9 · `toolbar` **2 de 4**, ausentes os 2 `.fj-ia-btn` (`Papéis`/`Perguntar ✦`), que abrem painéis **sem receptor no `main`** — o de `Papéis` ganhou receptor no [#6691](https://github.com/wagnerra23/oimpresso.com/pull/6691) (merge `fd83e6db06`) — mas o placar segue **2 de 4**: o botão foi montado DENTRO do `.fj-groupby` com `.fj-gb-btn`, não como filho direto da toolbar com `.fj-ia-btn`; virou divergência de posição, não mais ausência por falta de receptor. Achado do fecho: o `padding 11px 18px` do alvo é o valor do `@media(max-width:1100px)` (a medição do export rodou a 924px), **não** divergência; recibo em [§2026-09-03 (Onda 2 do export · fecho)](../TeamMcp/forja-cockpit-visual-comparison.md). Recibo em [§2026-09-03 (Onda 3 do export · fecho da lista)](../TeamMcp/forja-cockpit-visual-comparison.md) |
| 5 | Trabalho · quadro (2 eixos) | compare 0 bug — **✅ ENTREGUE, fechado declarando** (Onda **4** do export; o offset de 1 entre as duas numerações é antigo): [`TrabalhoQuadro.tsx`](../../../Modules/Forja/Resources/js/Pages/Forja/Trabalho/_components/TrabalhoQuadro.tsx) já é a réplica do `KanbanView`, e o alvo §3.4 fecha **6 de 7** elementos — o 7º é a altura de 111px do card, que é derivada do conteúdo e exige render. Tipografia/gap medida folha × folha: **15 de 20** seletores byte-idênticos ao `forja-page.css`, **5** divergindo só por token inlinado (o MESMO px — `--fs-1/2/3` valem 10.5/11.5/12.5 nos **dois** lados, incluindo `_generated-foundations-light.css`; mesma classe da Onda 10, dona é a Onda 1), **0** ausente e **0** bug de valor. ⚠️ **A armadilha do §3.4 NÃO se conserta:** o alvo diz 6 `.fj-kcol` e produção tem **7**, porque [W] decidiu em **2026-08-11** que *"F4 Merge É coluna"* — a fonte de design (`FORJA_PHASES`) **tem** F4 nas 7, e quem filtra é a *view* do protótipo; backend e front têm 7 (contado com `grep -o`), e a `DIVERGENCIA_DECLARADA` do `PipelineParidadeTest` está vazia por causa dessa decisão. Copiar o filtro **reverteria** [W] em silêncio. Idem o eixo Execução `blocked` × `done` (regra do charter `:71`). ⚠️ **Gap de defesa medido e declarado:** as 7 colunas são defendidas por prosa em 3 lugares e por **nenhuma máquina** — o `UC-TRAB-07` lê o bloco `const FASES` por regex, então um filtro no `map` do render passaria verde (specs de render `.tsx` da Forja: **0**; e2e: **0**). Não armei gate: intent diferente, e a forma sintática é família morta no §5. Recibo em [forja-cockpit-visual-comparison §2026-09-03 (Onda 4 · fecho)](../TeamMcp/forja-cockpit-visual-comparison.md). **Nada é "0 bug"**: prod pede auth (`302 → /login`) |
| 6 | Trabalho · gantt | **✅ PARCIAL [#6624](https://github.com/wagnerra23/oimpresso.com/pull/6624)** (merge `20875e152`, deploy `cb38ae2af` 13:27Z) — os 2 elementos FORA do motor entregues e medidos em prod autenticado dark: `.fj-quadro-ancora` (1 · 12px · `oklch(0.58 0.005 90)` = `--text-mute`, batendo o bundle) + `.fj-totalbar.fj-g-foot` (1 · `display:flex` · 3 legendas) · recibo em [forja-cockpit-visual-comparison §2026-09-03](../TeamMcp/forja-cockpit-visual-comparison.md). ⚠️ O CORPO do gantt **não** virou réplica, e é decisão [W] em aberto: o protótipo desenha `.fj-g-*` à mão e a tela usa `@svar-ui/react-gantt`; trocar o motor custaria as **163** dependências que hoje viram setas (Goal do charter) pra melhorar uma linha do tempo com **7** prazos reais em 1186 tasks (medido em prod 2026-09-03). O smoke também derrubou uma previsão minha: o contador de vencidas mostra **5**, não 0 **↳ Onda 5 do EXPORT (mesma tela, outra régua) FECHA DECLARANDO em 2026-09-03:** a moldura está entregue (2 de 4 filhos do `.fj-gantt`; a `.fj-totalbar` bate **6 de 6**); o corpo **não entra** — e o motivo é **único e medido**: decisão [W] em aberto. Varredura contada no `main` (excluindo o espelho): as **7** classes do §3.5 (`fj-g-scale`·`fj-g-body`·`fj-g-row`·`fj-g-lbl`·`fj-g-track`·`fj-g-bar`·`fj-g-fds`) têm regra no `cowork-forja-bundle.css` e **0 markup** em `.tsx` — de **17** classes `fj-g-*` no bundle, **2** têm consumidor (`fj-g-foot`/`fj-g-leg`, as da Onda 6) e **15 estão inertes**. Logo o CSS **já está pago** e trocar o motor não custaria folha de estilo: custaria as **163** dependências que o SVAR desenha hoje (`^2.6.1` MIT, **1** consumidor no repo). Alvos sem contraparte enquanto o motor for o SVAR: **32** `.fj-g-row` · **33** `fj-g-lbl` · **192** `fj-g-fds` · **32** `fj-g-bar` · o wrapper `.fj-gantt`. `compare --check` **não roda** aqui e não seria honesto: prod pede auth (`302 → /login`) e os dois lados têm **motores diferentes**. Recibo + PLACAR em [§2026-09-03 (Onda 5 do export · fecho)](../TeamMcp/forja-cockpit-visual-comparison.md). |
...
| 9 | Changelog | compare 0 bug — **réplica aplicada** (PR desta onda, 2026-09-02): a linha virou a do `ChangelogFeed` (**2** células, dot + corpo em 3 blocos) contra as **5** colunas achatadas que a medição da manhã pegou, e a parede de `"Sessão Claude Code"` acabou (o título cai em `summary_auto` → 1º prompt de `mcp_cc_messages` → **vazio honesto**). `flags`/`modules` passam a vir de coluna real (`tags` ∩ {`tier-0`,`breaking`} · `module`). Zero CSS novo. **O `compare --check` segue PENDENTE** — precisa do deploy, e o merge de `.tsx` é humano ([ADR 0283](../../decisions/0283-handoff-loop-zero-paste.md)); o alvo do protótipo e a estrutura da réplica já estão medidos em [forja-cockpit-visual-comparison.md §2026-09-02 (Onda 9)](../TeamMcp/forja-cockpit-visual-comparison.md), com o comando do pós-deploy escrito lá. **Tipografia/gap medida em 2026-09-03** (o eixo que o §7 do export declarava NÃO medido): **14 de 17** seletores da seção com corpo idêntico ao `forja-page.css`, **3** divergindo só por token inlinado (`var(--fs-N)` → o MESMO px) e **0** ausente; e o alvo re-conferido contra o `forja-changelog.jsx` do build de 03/09 — arquivo NOVO, ainda fora do espelho — com sequência de classes idêntica à réplica. Recibo em [§2026-09-03 (Onda 9 · fecho)](../TeamMcp/forja-cockpit-visual-comparison.md) |
