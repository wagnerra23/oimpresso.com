...
# Forja — SPEC complementar (US-PROJ-001..008)
...
Estado vivo de cada US: tools MCP `tasks-list module:Forja`.
...
- [x] `Modules/Forja/Tests/Feature/ScaffoldForjaTest.php` — Module::find + Route::has (8 rotas) + ServiceProvider carregado (9 cenários)
- [x] `Modules/Forja/Tests/Feature/MultiTenantProjectTest.php` — Project biz=1 vs biz=99, Task biz=1 não vaza, bulk anti-vazamento, cross-tenant coexistência (5 cenários)
- [x] `Modules/Forja/Tests/Feature/SmokeRoutesForjaTest.php` — 7 rotas GET <500 + middleware auth bloqueia anônimo (8 cenários)
...
- [ ] Migration `mcp_saved_views` (id, user_id, business_id, module='Forja', name, filters JSON, is_default bool, created_at)
...
- [ ] `Modules/Forja/Http/Controllers/CycleController.php` — index + create + close
...
Validação: `php artisan module:grade Forja --detail` (skill `avaliar-modulo`).
