...
| **Edição em massa › o caminho inteiro** | — | **`BulkEdit`** | 🔶 **decisão [W]** — o botão está atrás de `enable_product_bulk_edit = false` (upstream: *"Will be depreciated in future"*) **e** a tela React submete pra rota inexistente `/products/mass-update`. Antes de fechar paridade, [W] decide se a feature vive, morre ou vira `/unificado` |
