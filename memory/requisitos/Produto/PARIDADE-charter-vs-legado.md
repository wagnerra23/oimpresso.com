...
| Estoque › Geral + saldo por local | AR-PROD-050..057, 144..145 | `StockHistory` / `Unificado` / **`Show`** | 🟡 parcial — na ficha o **local do rack** e o **saldo** nunca chegam (shape, `UC-PSHOW-03`) |
| **Cabeçalho › gate de permissão de custo** | **AR-PROD-015** | **`Show` (trio fechado 2026-07-26)** | 🔴 **regressão** — o Blade gateia custo por `@can('view_purchase_price')` e o Delphi faz o campo **sumir**; a ficha React entrega a qualquer um com `product.view` (`UC-PSHOW-01`) |
| Cabeçalho › quantidades padrões (alerta mín./máx.) | AR-PROD-053 | `Show` | 🟡 `alertQuantity` é enviado e **nunca renderizado** (prop morta — `UC-PSHOW-07`) |
| **Consultar › lista/busca do catálogo** | **AR-PROD-023** + `AR-PROD-003/022` (inativo volta por filtro) | **`Index` (trio fechado 2026-07-26)** | 🔴 **regressão dupla** — (a) `limit(200)` sem paginação com o KPI anunciando o total = perda silenciosa (`UC-PIDX-01`); (b) busca roda no cliente e **não** enxerga `sub_sku` da variação, que o Blade procura explicitamente (`UC-PIDX-02`) |
| **Lista › gate de permissão de preço/custo** | **AR-PROD-015** | **`Index`** | 🔴 **regressão, agora em LOTE** — a **própria lista Blade** esconde as colunas com `@can('view_purchase_price')`/`@can('access_default_selling_price')` (`index.blade.php:287,294`); o branch Inertia manda `price`/`cost`/`margin` do catálogo inteiro sem consultar permissão (`UC-PIDX-03`). Mesma família do `UC-PSHOW-01` — o furo é do **módulo**, não da tela |
| **Lista › ações por linha** | AR-PROD-022 (excluir/reativar) · `CU-PROD-03/07/09/11` | `Index` | 🔶 **decisão [W]** — Blade tem **10 ações** no dropdown (`ProductController@index:202-258`), React tem **1** (card→ficha). O charter declara Non-Goal pra 4 famílias (CRUD inline · bulk · estoque · preço inline), mas **não** pra etiqueta, histórico, tabela de preço, duplicar e reativar. É a classe de regressão que motivou a [ADR 0351](../../decisions/0351-sdd-from-source.md) (o menu de Ações da lista de vendas que sumiu no #1032) |
| Lista › relatório de estoque (custo · valor de venda · lucro potencial) | AR-PROD-051/061 | `Index` (2ª aba do Blade) / `Unificado` | ❌ sem cobertura no React — pareia com `CU-PROD-12` `[V0]` (agregação de valor em estoque, G-03) |
| **Edição em massa › formação de preço da linha** | **AR-PROD-007/008/093** (`Margem% = (Valor/Custo − 1) × 100`, binding Valor↔Margem pivotando no Custo) | **`BulkEdit` (trio fechado 2026-07-26)** | 🔴 **regressão de 5 campos → 2** — a Blade edita `default_purchase_price` · `dpp_inc_tax` · **`profit_percent`** · `default_sell_price` · `sell_price_inc_tax` com **recálculo encadeado** (o "desmonta peça por peça" do legado); o React edita só custo e venda, sem margem e sem base com-imposto. E o writer lê os 5 **sem `??`** → o lote inteiro reverte (`UC-PBULK-05`) |
| **Edição em massa › preço por tabela na própria linha** | **AR-PROD-090/109** | **`BulkEdit`** | 🔴 **ausente no React** — a Blade traz um campo por `selling_price_group` dentro de cada variação (`group_prices[$k]`); o React nem envia a chave. E o writer, que aceita, grava **sem guard de tenant** (`UC-PBULK-03` — mesmo defeito do `UC-PTAB-04`/[#4300](https://github.com/wagnerra23/oimpresso.com/pull/4300)) |
| **Edição em massa › adicionar produto à matriz sem sair** | — (ergonomia do lote) | **`BulkEdit`** | 🔴 **ausente** — a Blade tem autocomplete `#search_product` → `/products/get-product-to-edit/{id}` que injeta a linha; no React só dá pra editar o que veio da seleção da lista |
| **Edição em massa › o caminho inteiro** | — | **`BulkEdit`** | 🔶 **decisão [W]** — o botão segue atrás de `enable_product_bulk_edit = false` (upstream: *"Will be depreciated in future"*). Sobre a rota inexistente `/products/mass-update`, [W] decidiu em 2026-07-27 **repontar** pro `/products/bulk-update` (doc corrigido; a linha do `.tsx` **foi aplicada em 2026-09-21**, depois de o [#7523](https://github.com/wagnerra23/oimpresso.com/pull/7523) entregar o `UC-PBULK-05` — era a condição, pois sozinha não fazia a tela salvar) — mas isso **não** decide o destino da feature: [W] segue decidindo se ela vive, morre ou vira `/unificado` |
