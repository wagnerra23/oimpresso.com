...
- Submit POST `/products/bulk-update` (writer real — `ProductController@bulkUpdate`, `routes/web.php:443`). A rota `mass-update` que este RUNBOOK declarava **nunca existiu**; [W] decidiu repontar a tela em 2026-07-27; charter e este RUNBOOK já declaravam o writer real, e a **linha do `.tsx` foi aplicada em 2026-09-21**, depois de o [#7523](https://github.com/wagnerra23/oimpresso.com/pull/7523) entregar a correção do payload (`UC-PBULK-05`) — era essa a condição, porque sozinha ela não fazia a tela salvar.
