...
| Consulta de produtos (rota unificada) | `ProdutoUnificadoController` | — | `Produto/Unificado/Index` | 🟡 Em 2026-08-18 a tela recebeu o layout do handoff **"Consulta de Produtos"**: índice em paridade com a `/contacts` (golden master do padrão de índice) — abas por tipo do item, seis KPI-filtros contados sobre a aba, busca e filtros resolvidos no servidor num único `where`, colunas montadas por autorização, disponibilidade com quatro rótulos e drawer de detalhe. O backend passou a servir saldo real (`SUM(vld.qty_available)`), mínimo (`alert_quantity`), tipo derivado e última venda, tudo de uma subconsulta única — mesma fonte pras linhas, pros KPIs e pras contagens. As quatro visões anteriores (categorias · insumos·BOM · tabelas de preço · histórico) continuam na mesma rota — no menu de ações **até 2026-08-24**, quando o grupo saiu e elas passaram a ser alcançadas só por `?tela=` (ver o retrato de 24/08 no fim desta célula). Em 2026-08-19 o pacote **V2** foi aplicado por cima, em duas ondas: a moldura da referência de produção (abas com fundo e badge de contagem próprios, gatilhos de filtro sem moldura em repouso, chips do recorte ativo removíveis, avatar do item, grid sem raio com cabeçalho sticky opaco) e a **paginação server-side** — rodapé 10/25/50/100 com «‹ N/M ›», `page`/`per_page` e `ORDER BY` no servidor com lista branca de colunas, total autoritativo, e o teto de 500 linhas + rolagem interna aposentados. Ordenar por custo ou margem é ignorado pra quem não tem `view_purchase_price`: a posição na lista denunciaria o número invisível. As cinco cores cruas que o pacote pedia entraram como token com par claro/escuro, então o claro não mudou de pixel e o escuro deixou de quebrar. A onda 3 fechou a **revelação progressiva**: passar o mouse no estoque abre o **saldo por local** (com alerta quando há zero num local e saldo em outro), o ícone de recado ao lado do nome abre a **observação do produto** (sem HTML — o campo é WYSIWYG e a tag morre no servidor), a célula Produto ganhou a **terceira linha de variações**, e o painel lateral ganhou três seções (Por local, Variações, Observações). Os três popovers usam um componente só, com hover como atalho e clique fixando — o que torna a informação alcançável por toque e por teclado, não só por mouse. Continua **🟡** e não 🟢 por duas frentes em aberto: comportamento abaixo do desktop declarado, e o **“a partir de” (preço por faixa de quantidade)**, que o pacote desenha e o UltimatePOS não tem onde guardar — ele tem preço por **grupo de cliente** (`variation_group_prices`), não por quantidade comprada, e o próprio handoff proíbe derivar a faixa do preço em produção. A decisão de schema está aberta em [proposta 2026-08-19](../../decisions/proposals/2026-08-19-faixa-de-preco-por-quantidade.md). **Em 2026-08-24 entrou o pacote V6 + um patch de cor**, e o efeito de produto é triplo. **(1)** O menu `⋯` virou lista fechada de apresentação + dados (2 grupos, 7 itens): as quatro visões secundárias **perderam entrada na tela** e só abrem por `?tela=` — a sidebar cobre Categorias e Grupo de preços de venda nas telas legadas, mas **não** cobre Insumos·BOM nem Histórico de uso, e dar-lhes acesso é decisão de sidebar, fora do handoff. **(2)** A faixa de abas deixou de copiar o ciano da Consulta de Contatos e passou ao **roxo canônico do DS** (hue 295, ADR 0190), com contadores das abas fechadas em claro; os seis literais crus saíram de `cockpit.css` num lugar só e o bloco `[data-theme="dark"]` deles morreu, porque os tokens de origem já trazem par de tema. Ao contrário do que a onda V2 registrou, **aqui o claro MUDOU de pixel** — de propósito, e é o que o gate visual acusou como zona cinza (0,17%). **(3)** O selo da coluna Disponível ganhou a receita medida no componente do DS (`StatusBadge`: fundo 16%, borda 30%, `--fs-2` 11,5px, pílula redonda), que **desmente** os 6%/22%/12px que o patch pedia alegando ser a receita do DS; a placa do KPI, que não é um `StatusBadge` e não tem equivalente no DS, ficou nos 6%/22% do patch. **Em 2026-08-25 (rev. 9, revisão do drawer)** mais 4 ajustes de polimento: o cartão da tabela ganhou a moldura canônica do PT-01 (cantos arredondados + sombra de 1px — até aqui era reto por um workaround de `backdrop-filter` que já não existe); a densidade compacta passou a encolher também o padding lateral da página, não só a altura da linha; e no painel de detalhe o rótulo do par e o subtítulo "Por local" foram pro tamanho/tracking do template (12px e 10,5px/.05em, respectivamente — eram 12,5px e 10px/.1em, um nível tipográfico inventado na tela). "+N reservado" no marcador da 2ª linha segue fora — dependência de outra tela (natureza do local no cadastro), confirmado por [F]. _(Retrato anterior, datado: até 2026-08-07 a rota respondia 500 em toda carga porque `categorias()` chamava `withCount('products')` numa relação que `App\Category` não declara — corrigido pelo helper com `leftJoin` + `COUNT`.)_ |
