...
| **C02 (P0)** | Preço por tabela / price group (+ **multiplicador/markup por tabela**) | 4 | Conta Azul (cálculo automático/markup) / VTEX (price table por trade policy) / Linx (tabela por loja) | 🟡 preço **por variação×grupo** existe (`group_prices`), mas **`SellingPriceGroup.mult` hardcoded 1.00** com TODO ([ADR ARQ-0001](adr/arq/0001-selling-price-multiplier.md)) — multiplicador/markup por tabela **não funciona**; bloqueia F3 do `/produto/unificado` | **5** |
...
| **G-02** | **Multiplicador/markup por tabela de preço** — resolver `SellingPriceGroup.mult` (ADR ARQ-0001): coluna `multiplier` OU cálculo via VariationGroupPrice. Fecha C02, desbloqueia F3 `/unificado` | C02 | M (~10-16h) ⚠️ **Tier 0 valor** | **alto** — preço por tabela é core; hoje aparenta funcionar mas é 1:1 | ✅ execute (dor de catálogo real) | Conta Azul (markup auto), Linx |
...
2. **Multiplicador de preço por tabela é oco.** `ProdutoUnificadoController::tabelas()` retorna `'mult' => 1.00` **hardcoded com TODO** (`l.183`), e o modelo `SellingPriceGroup` não tem coluna de multiplicador ([ADR ARQ-0001 proposed](adr/arq/0001-selling-price-multiplier.md)). "Preço por grupo/tabela" **aparenta funcionar** na UI mas é **1:1** — não há markup/desconto por tabela como Conta Azul (cálculo automático) ou Linx (tabela por loja). Isso **bloqueia o F3** do `/produto/unificado`.
