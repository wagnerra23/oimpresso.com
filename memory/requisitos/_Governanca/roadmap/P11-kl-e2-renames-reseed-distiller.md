...
## Estado 2026-07-01 — E2c: fila humana dos ghosts FORA do escopo do detector (executada)

A `fila_humana_fora_de_escopo` do [`reseed-meilisearch-manifest.json`](../../../../governance/reseed-meilisearch-manifest.json) (docs vivos em `memory/` raiz + `reference/` + `governance/` citando nomes Classe A) foi resolvida por **fix manual pontual** — sessão [2026-07-01-p11-e2c](../../sessions/2026-07-01-p11-e2c-fila-humana-ghosts-fora-escopo.md):

- **Corrigidos (referência VIVA):** `what-oimpresso.md` (diagrama — @import do CLAUDE.md), `why-oimpresso.md`, `00-user-profile` / `02-technical-stack` / `04-conventions` / `05-preferences`, `reference/mcp-endpoints`, `reference/ideia-chat-ia-contextual`, `reference/trigger-guarde-no-cofre`, `reference/feedback-claude-aprova-merge-verde-criticos-nao`, `onboarding/team/maiara-suporte`, `legacy-delphi/MAPEAMENTO-DELPHI-LARAVEL`.
- **Restaurada corrupção histórica** em `governance/ARCHITECTURE.md` + `governance/MODULE-DRIFT-MIGRATION-PLAN.md`: um replace antigo tinha virado "renames Jana→Jana" — coluna "De" restaurada pra `Modules/Copiloto` (tombstone intencional) + nota de status "renames executados PHP-only".
- **NÃO tocados (citação histórica datada):** `CHANGELOG.md` entries (ganhou nota de época no header), `reference/post-mortem-v4-go-live.md`, audits/comparativos/sessions/handoffs/ADRs, `memory_backup/**`.

**Decisão do escopo do detector (a pergunta "estender knowledge-drift.mjs pra memory/ raiz + reference + governance?"): NÃO estender agora.** Razões:
1. **Métrica armada é série, não número:** `ghost_count` está `armed:true value:8 direction:down` medindo `memory/requisitos/**`. Mudar o escopo muda a definição → série incomparável. Precedente da própria baseline (`anchor_coverage`): série antiga APOSENTADA + re-armar com 3 medições novas (ADR 0275 §3). Estender "por dentro" do valor 8 seria regressão fabricada ou grandfather — os dois vetores que o avaliador adversarial caça.
2. **Simetria detector×corretor é conquista recente do P11:** estender só o detector recria o descasamento reconciliado em #3155 (`ghost-fix.mjs` continua `memory/requisitos/**`). Extensão exigiria detector+corretor+`knowledge-ghosts-baseline/` por área, com skip-list de zonas append-only/datadas (`decisions/`, `sessions/`, `handoffs/`, `memory_backup/`, `CHANGELOG.md`, audits) — custo alto.
3. **Valor marginal baixo pós-E2c:** com este fix, referências vivas Classe A fora de `requisitos/` = 0. O risco residual é rename FUTURO — coberto por processo, não por gate: **todo rename Classe A novo no `ghost-rename-map.json` DEVE incluir grep token-boundary `Modules/<Nome>` em `memory/` INTEIRO (não só requisitos) como passo do PR de rename.**
- **Reabrir se:** um rename Classe A novo entrar no map E o grep manual falhar 2× (two-strikes) → aí vale pagar a extensão simétrica como série NOVA de métrica (ex.: `ghost_count_full`), armada do zero com 3 medições (nunca dentro da série atual).
- **Fila residual (nomes puros, fora da regex do detector):** `reference/ultimatepos-integracao.md` (ex.: `Module::find('PontoWr2')`), `reference/financeiro-integracao.md`, `reference/feedback-topnav-i18n-pattern.md`, `memory/modulos/PontoWr2.md` (doc-módulo legado — candidato a tombstone junto com ghost-dirs `requisitos/{MemCofre,PontoWr2,Copiloto}`), `.github/CODEOWNERS` linhas 32-33 (`Modules/Copiloto/` morto = Jana SEM code-owner — task spawnada).
