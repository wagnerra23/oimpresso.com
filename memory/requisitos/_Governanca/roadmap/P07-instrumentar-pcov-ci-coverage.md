...
> **🔴→🛠️ INCIDENTE 2026-07-02 — 1º run instrumentado MATOU o diagnóstico; kill-criteria aplicado (arquitetura revista):**
> O 1º run com pcov (`20260702-073601`) morreu **silencioso aos 5.933/11.144 testes (53%)** — sem fatal impresso, última linha íntegra, shim containerd morto 09:03:23, swap CT100 a 2,4G (padrão de kill externo por pressão de memória; OOM de LXC só aparece no dmesg do host Proxmox). Levou o **junit junto (0 bytes)** = noite de floor perdida — o "coverage é aditivo, fail não derruba o diagnóstico" era promessa no comentário, não arquitetura. Mecânica provada OK em fatia: `tests/Unit` (138 testes) gerou clover válido de 14MB na mesma imagem.
> **Correção estrutural (mesma data):** coverage roda numa **2ª invocação separada** (`[P07 coverage]`, container `oimpresso-fullsuite-cov`, `memory_limit=6G`) DEPOIS do junit salvo — falha de coverage agora só deixa `coverage_pct` honesto em `not_yet_measured`; o floor nunca mais é refém. Contrato travado no `tests/fullsuiteHarness.spec.ts` (FV-F1 revisto: pcov NUNCA volta pro processo do diagnóstico; run de coverage NUNCA passa `--log-junit`). Custo: nightly ~2× mais longa (dois passes na suíte). **Se o run de coverage morrer de novo mesmo com 6G** → degradar pra cadência semanal ou sharding por módulo com merge de clovers (desenho já esboçado em [T1](T1-mapa-teste-arquivo-per-test.md) §riscos) — o relógio das 3 medições (ADR 0275 §3) só conta medição VÁLIDA.
