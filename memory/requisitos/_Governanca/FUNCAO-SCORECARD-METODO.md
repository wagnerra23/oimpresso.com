...
**2026-07-21 (rodada 4 — desdobramento C7 → C7a–C7d + de-comment) — refutação adversarial absorvida; 3 juízes cegos passaram no pack DE-COMMENTADO.**
- **Motivo:** o C7 monolítico flipou no T1 (`getProductDiscount`: 2 discordo × 1 concordo) por misturar 3 perguntas num único veredito (docblock? polimórfico? nullabilidade/falha?). Regra §5 aplicada ("critério que flipa ⇒ endurecer pra binário") → C7 desdobrado em 4 sub-critérios **ortogonais + binários**: C7a docblock/tipo · C7b polimórfico não-declarado · C7c nullabilidade tipada · C7d falha observável. Total 11 vereditos (§1 v1.1).
- **Loop de crítica (sessão fresca REFUTA):** um crítico adversarial achou 4 buracos reais, 2 blockers — **todos absorvidos**:
  - **[blocker] comentário vazando (Q5):** o `--pack` emitia os `//` dos twins verbatim, e vários **nomeavam o mecanismo/veredito** → κ media "transcrever comentário", não "discriminar defeito". Fix: `pack()` roda `stripTells()` (remove `//` e `/* */`, **preserva** `/** */` de contrato); fatos de schema ("X é tenant-owned") migraram pra docblock. **Consequência honesta: o κ das rodadas 2-3 acima está INFLADO** por prosa vazando; a medição válida é ESTA (de-commentada). Rodadas 2-3 ficam como fóssil datado, não reescritas.
  - **[blocker] t12 impuro:** `value()` de 0-row devolvia null → t12 tinha C7c além de C7a (2 juízes flagraram). Fix: t12 virou cálculo `int` puro (sem DB, sem null) = C7a isolado.
  - **[Q2 C3↔C7c dupla-contagem]:** o texto da linha C3 ainda reivindicava "null" (dono é C7c) → null cru contava 2×. Fix: C3 vira **SENTINELA não-null** (`''`/`false`/`0`); **partição por caminho de ausência** (sentinela→C3, null→C7c, nunca os dois).
  - **[Q1/Q3/Q4 C7a vago · C7d subjetivo · C7b assimétrico]:** C7a ganhou cláusula (ii) — pseudo-tipo genérico (`obj`/`mixed`/`object`) = discordo (fecha o defeito REAL do `getProductDiscount`, antes órfão); C7d restrito à supressão **mecânica** (catch vazio/`@`/catch-sem-Log); C7b simétrico ao C7c (union **declarado** = concordo, +twin `t17`). Residual honesto catalogado na Fronteira (iterable dentro de supertipo declarado).
- **Resultado (pack de-commentado · 17 twins · 10 famílias · rubric v1.1):** **3/3 juízes cegos CALIBRADO** — famílias **10/10** (min 8) · **κ = 1,0** (≥0,6) · 0 over-flag no controle (t07) · **incerto certo no t08** (SEM o comentário que dizia "incerto") · **0 falso-discordo nos bons** (t11 `?T`→C7c com C3=`n/a`; t16 collection→C3; t17 union declarado→C7b). **T1 = 100%** (0 flips/17 salientes). **κ=1,0 SOBREVIVE ao de-comment** ⇒ a discriminação vem da ESTRUTURA + contrato, não de prosa — refuta o blocker Q5 na raiz.
- **Fronteira honesta (mantida):** calibra o **INSTRUMENTO** (juiz discrimina defeito mecânico não-circularmente), NÃO re-valida vereditos de função REAL. C7b/C7d são **completude proativa** (`n/a` no flip real), não flip-provados. Evidência reprodutível: `calibracao-2026-07-21/judge-v11-decomment-{a,b,c}.json`; runner `--selftest` 5/5. É mudança de **canon** → merge [W] = ratificação.
