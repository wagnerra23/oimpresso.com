...
**Given** Vargas Recapagem (`business_id=N`) vende serviço "recapagem cavalo placa ABC-1234" R$ 500 + 4 bandas R$ 200 em 2026-03-01
...
- `warranty_resolutions.id=701` com `decisao=aprovada`, `valor_envolvido=R$ 200`
- Transaction-10001 NÃO sofre estorno; lojista absorve R$ 200 custo
- KPI mensal: "% custo-garantia sobre faturamento" sobe

### Cenário B — Autopecas peça Bosch defeituosa (Vargas balcão)

**Given** Vargas Autopecas vende peça Bosch BB-318 cubo Gol em 2026-01-15 por R$ 200
...
- Side-effect `CriarTrocaImediataLoja` (NÃO cria OS-filha, é venda balcão):
  ...
  - `warranty_resolutions.decisao=aprovada_troca_imediata`, `valor_envolvido=R$ 200`
- Pipeline ramifica → `ressarcimento_solicitado`:
  - `warranty_reimbursements.id=702` criado com `fornecedor_id=Bosch`, `valor_solicitado=R$ 200`
  - Status `pendente`
...
- 45 dias depois Bosch deposita R$ 200 → financeiro marca `ressarcimento_recebido` (T) → ajuste contábil

### Cenário C — ComVis banner descolou (Gold-like)

**Given** Gold Comvis vende e instala banner 3x2m em 2026-04-01 por R$ 800
...
**When** cliente retorna alegando defeito, mas mostra histórico (cliente fiel há 8 anos, R$ 50k+ comprou)
...
- `warranty_resolutions.observacao_gerente="Cliente fiel R$50k+ histórico, decidi honrar fora prazo, ver SPI fidelização"`
