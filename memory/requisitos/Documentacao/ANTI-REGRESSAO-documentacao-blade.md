...
| AR-DOC-068 ❌ | **DEFEITO CONFIRMADO NA TELA VIVA — não é divergência de design.** A tela renderiza os KPIs `onda`/`posicao`/`task` a partir de `execucaoDaTrilha()`, que acha a onda por regex (`/\bD(\d+)\b\s+em execução/`) numa **linha de tabela escrita à mão** no plano. E o rodapé da mesma tela afirma ao leitor: *"Em que onda a trilha está e o que fechou é estado vivo das tasks MCP (`parent_plan=programa-ondas`), **nunca desta página**"*. As duas coisas não podem ser verdade. Medido em 2026-08-23: a linha que alimenta o KPI foi escrita em **2026-08-05** (`14d9414060`) e não muda desde então — 18 dias de "estado vivo" que é prosa datada. [ADR 0070](../../decisions/0070-jira-style-task-management-current-md-removed.md) põe as tasks MCP como *"source-of-truth absoluto pra status"* e existe justamente porque status "viveram em linhas de tabela markdown". `UC-PROGRA-01` estava certo | `programa()` · `execucaoDaTrilha()` × rodapé de `resources/views/documentacao/programa.blade.php` × ADR 0070 | ver `AR-DOC-069` — o conserto não é o literal do UC |
| AR-DOC-069 ⚠️ | **O literal do `UC-PROGRA-01` NÃO é implementável hoje — falta a chave.** Ele pede *"task `done` com `parent_plan=programa-ondas` → **aquela onda** aparece `done`"*. Medido: a tabela § D.3 tem as colunas `Onda \| Escopo \| Saída \| Gate de saída` — **nenhuma coluna de task**; e o contrato de `parent_plan` ([PlanDriftCommand](../../../Modules/Jana/Console/Commands/PlanDriftCommand.php)) é por **slug de plano**, não por onda. Não existe junção task→onda. Recomendação medida: mover o estado para o nível que o dado sustenta — contagem de tasks por `parent_plan=programa-ondas` via `jana:plan-drift --json`, que **já existe**, já lê `mcp_tasks` por Eloquent e já tem o caminho `skipped` + `reason` honesto quando o MCP não responde (que é exatamente o `UC-PROGRA-03`). O por-onda vira backlog explícito, nomeando a chave que falta (`parent_wave`, ou convenção de slug por onda) — decisão de [W], porque muda convenção de task | § D.3 do PLANO-MESTRE · `PlanDriftCommand::classifyDrift` · `PlanDriftChecker::check` | a F3 troca a fonte do KPI pro oráculo existente e reescreve `UC-PROGRA-01` no nível do plano; o por-onda só depois que a chave existir |
