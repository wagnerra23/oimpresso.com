...
**Implementado em:** `resources/js/Pages/Financeiro/Unificado/Index.tsx` · `Modules/Financeiro/Http/Controllers/UnificadoController.php` (método `bulk`) · `Modules/Financeiro/Routes/web.php` (`POST /unificado/bulk`) · `Modules/Financeiro/Tests/Feature/UnificadoBulkGuardTest.php` · verificado@ec17185 (2026-07-06) — checkbox + select-all + footer (Onda 12/15) + endpoint bulk genérico com as 5 ações (baixar/categoria/plano_conta/cancelar/exportar_csv), Sheet destrutivo com total no cancelar, ownership Tier 0 fail-closed de todos os ids, limite 500, audit trail `bulk_*`, 6 GUARDs UC-F04 na lane financeiro-pest. Nota: select-all cobre a página carregada (limit 200 do controller); "TODOS via filtro" além disso fica pra quando a paginação explícita existir (Non-Goal atual do charter).
