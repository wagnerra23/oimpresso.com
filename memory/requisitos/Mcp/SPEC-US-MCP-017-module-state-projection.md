...
- **NÃO chamar LLM por default** — princípio 2 Constituição v2 (tiered cost). Todas agregações são SQL+FS+git+tools MCP existentes. Síntese é rule-based markdown. Em fase 2 (backlog) pode ganhar `--narrative=true` opcional via gpt-4o-mini, mas default = R$ [redacted Tier 0]/call.
...
6. **Sintetizar via LLM por default** — princípio 2 Constituição v2 (tiered cost). Markdown rule-based primeiro. LLM opcional fase 2 (`--narrative=true`, custo ~R$ [redacted Tier 0]).
