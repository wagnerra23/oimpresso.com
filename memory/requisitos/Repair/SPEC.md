...
**Testado em:** `Modules/Repair/Tests/Feature/RepairJobSheetIndexContratoTest.php` (declara `@covers-us US-REPA-004`) — lane **Verticais · Pest (MySQL)**, allowlist. 6 UCs: gate 403, flag OFF→Blade, flag ON→Inertia com as 3 props, `datatable_url` apontando ao endpoint compartilhado, ramo `ajax` preservado com a flag ON, e isolamento cross-tenant. **Veredito medido em 2026-09-05** (run 33997371312, disparo manual — ver ressalva abaixo): `tests=6 · assertions=4 · failures=0 · skipped=3`. **Passam:** UC-JSIDX-01 (403 sem permissão), UC-JSIDX-02 (flag OFF entrega Blade) e **UC-JSIDX-05** — o mais importante, que prova que o ramo `ajax` segue devolvendo o envelope DataTables com a flag ON. **Pulam:** UC-JSIDX-03 e -04 (dependem de o render Inertia devolver 200 no ambiente da lane, e ele não devolve) e UC-JSIDX-06 (isolamento cross-tenant, que depende do insert mínimo em `repair_job_sheets`). O JUnit não grava a razão do skip, então a causa exata dos três **não está medida** — só sei que o mesmo request por `ajax` (UC-05) devolve 200, logo o problema é específico do caminho Inertia. **Os 3 skips são ausência de medição, nunca aprovação** (LC-13) — em especial o UC-JSIDX-06, que deixa o isolamento Tier 0 desta tela sem prova executada.

⚠️ **A lane não dispara sozinha neste PR.** `verticais-pest.yml` declara `pull_request: types: [opened, reopened, ready_for_review]` — **sem `synchronize`** —, então ela roda na abertura do PR e não a cada push. O teste entrou num commit posterior à abertura, e o veredito acima só existe porque a run foi disparada à mão (`workflow_dispatch`). Registrar na allowlist e casar o `paths` não bastou: enquanto o `types` não incluir `synchronize`, este contrato é verde que não prova nada em PR (§5 2026-08-02). Corrigir o gatilho muda o disparo de todos os PRs que tocam Repair/ComVis/Vestuario/Superadmin — decisão de [W], não deste PR.
