...
Meilisearch é motor de busca full-text + semântica (discutido previamente — ADRs de observação do ChatGPT consolidados em `memory/requisitos/MemCofre/CHANGELOG.md`). Ainda não está instalado.
...
### Piloto: MemCofre Inbox (esta sessão)
...
3. **Validação:**
   ...
   - `docvault:audit-module MemCofre` ≥ baseline
