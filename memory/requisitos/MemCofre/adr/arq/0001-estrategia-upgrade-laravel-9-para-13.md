...
- MemCofre: `RequirementsFileReader`, `MemoryReader`, `ChatAssistant`, `ModuleAuditor` — PHP puro, sem magia framework.
...
**Upgrade progressivo em 4 sessões dedicadas, DEPOIS de consolidar MemCofre e migrar 80%+ dos módulos ativos pra formato pasta.**

### Ordem recomendada

**Fase 0 — Consolidação (atual + próximas 2-3 sessões)**
- ✅ MemCofre funcional (19 commits, score 94/100)
...
- MemCofre tem scheduler registrado aqui — atualizar
...
- Broadcasting refactor se usar WebSockets (provavelmente não no caso do MemCofre)
...
2. **MemCofre é o medidor.** Se subirmos Laravel sem baseline de qualidade documentada, não sabemos o que quebrou.
...
- MemCofre vira **instrumento de medição** do upgrade (baseline → diff).
...
## Roadmap consolidado no MemCofre

Esse ADR referencia e alinha com:
- `memory/requisitos/MemCofre/adr/0006-analise-de-melhorias-e-roadmap-docvault.md` (backlog geral)
- `memory/08-handoff.md` (estado atual projeto)
- `memory/decisions/` (ADRs históricos do projeto pré-MemCofre)
