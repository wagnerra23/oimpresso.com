...
prototipo: prototipo-ui/cowork/Wagner/arquivos-page.jsx
...
| col0 sem `mono` | **NÃO é defeito — é o refino A6, aplicado de propósito.** O protótipo ainda tem `<code>nfe-xml</code>` (slug técnico, herda mono); o vivo trocou por `CONTEXTO_PT` em `<span>` porque `<code>` é para valor técnico, não para prosa (pedido `ARQUIVOS-REFINOS-PRODUCAO-2026-08-26.md` §A6, entregue no [#6339](https://github.com/wagnerra23/oimpresso.com/pull/6339)). **Prod à frente; o protótipo é que está atrás** → **PROTÓTIPO ATRASADO** (vocabulário de 2026-09-18). | **Re-exportar o protótipo** com o `CONTEXTO_PT` em `<span>`. O lado a mover é o protótipo — enquanto ele não descer, a próxima leitura volta a ver isto como bug. |
| col6 herda a cor / 0 blocos | ~~**NÃO é defeito — é escopo declarado.**~~ **DÍVIDA A FECHAR na onda 2+** ([W] 2026-09-18). Protótipo tem 3 ações com texto (Baixar · Classificar · Excluir); o vivo tem 1 botão só-ícone com `aria-label` ([#6345](https://github.com/wagnerra23/oimpresso.com/pull/6345)). A onda 1 é leitura pura e o `ArquivosAdminControllerTest` reprova mutação no controller. | **Fechar na onda 2+.** O escopo de onda diz *quando*; não dispensa a paridade. |
