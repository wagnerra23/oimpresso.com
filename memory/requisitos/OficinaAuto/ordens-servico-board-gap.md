...
| KPIs | `:977-1005` — 6 cartões, 5 clicáveis (`kpiClick` em `:978-998`, `kpiFilter` `:753`, filtro `:822`, incl. Urgentes); "Valor em curso" (`:1003`) sem `onClick`. | `:477-484` `kpiCards` (6, com sublinha; "Valor em curso" = `formatBRL(kpis.valor_em_curso)` do backend, `filterKey: null`) · `:354` `kpiClick` · `:486-492` predicado client-side · `:708` render · chip "limpar filtro" (canon D-05, cabeçalho `:38`). | Nada — paridade (charter §Goals "KPIs com sublinha … 5 clicáveis como filtro"). ⚠️ valor: o KPI só **exibe** o agregado computado no backend (`kpis.valor_em_curso`), não soma no front — é o que o gap antigo (parte 2) exigia. |
| Filtros — boxes/elevadores | `:1010-1025` `.prod-equip-filters` — "Todos os boxes" + 1 botão por recurso com contagem (`RECURSOS` `:33-39`, hardcoded). | `:316-319` estado da aba · `:501-510` contagem por box sobre `columns` · `:720` render das abas (paridade `.prod-equip-filters`); boxes vêm de `filterOptions.boxes` (`:545`), não hardcoded. | Nada — paridade (charter §Goals "Abas de box/elevador (filtro client-side com contador)"). Data-driven onde o mockup era `RECURSOS` fixo. |
