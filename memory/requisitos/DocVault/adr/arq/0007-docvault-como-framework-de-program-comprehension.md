# ADR ARQ-0007 (DocVault) · DocVault como framework de program comprehension (software archaeology)
...
Wagner também deu o framework em 5 fases (estrutural → comportamental → funcional → intencional → histórica). Este ADR **posiciona o DocVault** nesse mapa conceitual e identifica o que já fazemos × o que falta.

## Decisão

**DocVault é um framework de program comprehension aplicado ao OI Impresso**. Não é gerador de documentação — é **grafo de afirmações verificáveis sobre o sistema, com evidência e confiança, mantido vivo por automação + curadoria humana**.

### Posicionamento nas 5 fases

#### Fase 1 — Mapeamento estrutural (determinístico)

| Item do framework | Status no DocVault |
|---|---|
...
| Item | Status |
|---|---|
| Git blame + histórico | ❌ não integrado — DocVault não lê `git log` ainda |
...
| Glossário de domínio | ✅ `GLOSSARY.md` por módulo (DocVault, PontoWr2, Essentials, Accounting, Crm, Manufacturing, Project, Repair, _DesignSystem = 9 glossários) |
...
### Score global do DocVault vs framework ideal
...
10. **Self-healing model** — DocVault detecta drift entre código e doc, sugere PR
...
Este é o **norte arquitetural** do DocVault daqui pra frente. Toda nova feature deve responder: "isso torna o modelo mental **mais navegável**, **mais verificável**, ou **mais vivo**?". Se não responde sim pra nenhum, não entra.
...
| Ferramenta | Cobre o quê | DocVault diferencial |
|---|---|---|
| **Sourcegraph Cody** | Busca+chat sobre código | DocVault tem **ADRs + validação** que Sourcegraph não tem |
| **Cursor "codebase"** | Chat contextual | Cursor é IDE-bound; DocVault é versionado no repo (sobrevive change de IDE) |
| **Greptile** | Doc inference automática | Greptile é SaaS; DocVault é self-hosted + editável |
| **GitHub Copilot Workspace** | Planejamento IA | Copilot não tem estrutura persistente de doc |
| **Sweep** | PR automation | DocVault mantém *estado* entre PRs |
| **Swimm** | Docs inline no código | Swimm é doc linear; DocVault é grafo de afirmações |

Nenhuma ferramenta atual cobre as 5 fases **E** é file-based versionada **E** permite validação contínua. **DocVault ocupa esse nicho.**
...
- [ ] 3 módulos completando ciclo das 5 fases (DocVault, PontoWr2, + 1 ativo)
