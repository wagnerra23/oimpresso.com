...
positioning: "Whatsapp transacional dentro do ERP, no canal que o cliente lê. Status de OS, boleto, NFe, lembrete de pagamento, dunning multicanal — via Meta Cloud API oficial OU Z-API (5 min de onboarding), com fallback automático entre eles em caso de ban."
...
revenue_pricing:
  ...
  pro: "R$ 99/mês — até 5.000 conversas/mês, multi-número, bot Jana com handoff humano, métricas custo/deflection"
  enterprise: "R$ 299/mês — ilimitado, multi-driver (Take Blip/Twilio), SLA, white-label templates"
...
> **Pitch para o tenant:** _Whatsapp transacional no ERP, custo de selo._ Status de OS, boleto pago, NFe, lembrete e dunning entram no canal que o cliente abre 95% das vezes. Liga em **5 minutos via Z-API** (R$ 99/mês) ou **R$ 0 via Meta Cloud oficial** quando o número for verificado. Fallback automático entre os dois protege o operacional.
...
Padrão de mercado BR: Take Blip (enterprise, R$ 1.500+/mês) e Zenvia (mid-market, R$ 500+/mês). Inspiração de UX sem copiar custo — Meta Cloud API direto cobre 99% dos casos a fração do preço.
...
| **Pro** | R$ 99 | até 5.000 | multi-número, bot Jana com HITL, métricas |
| **Enterprise** | R$ 299 | ilimitado | multi-driver, SLA, white-label |
...
1. **Core** — Driver abstraction (3 drivers oficiais: `MetaCloudDriver` + `ZapiDriver` + `EvolutionDriver` Sprint 2 + `NullDriver` dev), models, jobs, eventos, factory
2. **Driver Health Check** — job 6h em 6h envia mensagem-piloto pra detectar ban (Sprint 2); fallback automático
3. **Inbox** — UI Cockpit conversas + chat real-time Centrifugo (driver-agnóstico)
4. **Templates** — gerenciador HSM (Meta Cloud) ou templates locais (Z-API/Evolution)
5. **Settings** — config driver (radio Meta Cloud / Z-API / Evolution) + credenciais cifradas + fallback driver opcional
6. **Webhook** — 3 receivers (`/webhook/meta/{uuid}`, `/webhook/zapi/{uuid}`, `/webhook/evolution/{uuid}`) com auth específica
...
### Sprint 1 (S2.6 → S2.7) — Plumbing + 2 drivers + Outbound

- Scaffold módulo (8 peças `criar-modulo`)
- `Services/Drivers/{DriverInterface, MetaCloudDriver, ZapiDriver, NullDriver, DriverFactory}`
- Migrations: `whatsapp_business_configs` (com colunas Meta + Z-API + Evolution), `whatsapp_messages`, `whatsapp_conversations`, `whatsapp_templates`
- CRUD `WhatsappBusinessConfig` com seletor radio driver (Meta Cloud / Z-API / Evolution)
- `SendWhatsappMessageJob` (retry exponencial, `$businessId` constructor, resolve Driver via Factory em runtime)
- Listener Repair: status `ready` dispara mensagem (cumpre ADR Repair tech/0001)
- Pest: `MultiTenantIsolationTest`, `MetaWebhookSignatureTest`, `ZapiWebhookTest`, `MetaCloudDriverTest`, `ZapiDriverTest`, `DriverFactoryTest`

### Sprint 2 — Inbox + Templates + Health Check + Evolution

- `MetaWebhookController` + `ZapiWebhookController` com auth específica
- `ProcessIncomingWebhookJob` (CT 100 Horizon, driver-agnóstico)
- `WhatsappDriverHealthCheckJob` + fallback automático
- `EvolutionDriver` (self-host CT 100 Docker compose-managed)
...
- `Pages/Whatsapp/Templates/Index.tsx` — sync HSM Meta + templates locais Z-API/Evolution
