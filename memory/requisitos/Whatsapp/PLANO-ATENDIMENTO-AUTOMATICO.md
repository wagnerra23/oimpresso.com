...
> **Escopo:** atendimento automático no inbox (Caixa Unificada V4) — **triagem** (analisa/prioriza pro operador) e **resposta** (bot fala com o cliente) — + escalação humana ("abrir suporte"). Atendimento vive em `Modules/Whatsapp` (fusão KL-E2 2026-06-15); o cérebro de IA vive em `Modules/Copiloto`/`Modules/Jana`.
...
|---|---|---|---|---|---|
| **E1. JANA Pro MVP** — brief diário + triagem pro operador (JANA-A) | ~16h | baixo (L1) | **sim** (pricing+Asaas prontos) | forte (ROTA LIVRE dogfood, ADR aceito) | 🟢 altíssimo |
| **E2. Triagem no inbox** — `ConversationOpened` → score + tag P1-P4 pro operador | ~10-12h | baixo (analisa, não responde) | sim (feature JANA Pro) | forte | 🟢 alto |
| **E3. Cobrança Asaas + 5 beta** (JANA-B) | ~16h | baixo | **é a receita** | forte | 🟢 alto |
...
**3 razões de E1/E2 ganharem:** (1) **bate a meta do cycle** — projeção JANA Pro = MRR ~R$2.287 no mês 3, margem ~92-94%; (2) **risco baixo + reuso** (Vizra + Baileys + Asaas + skill `ticket-triage`); (3) **sinal qualificado** — gated por conversão real (mês 2: 3/5 pagam ou congela).
...
**✅ Em prod (suíte humano-assistida — COMPARATIVO ~91% do top):** SLA policies + escalation, macros com ações, CSAT, métricas/`deflection_pct` schema, auto-link Contact CRM, mídia outbound, anti-ban, customer-memory, multi-número, tags (`ensureDefaultTags`), eval RAGAS interno, ERP nativo.
...
**❌ Outros gaps:** guardrail conversacional próprio · eval contínua do bot · ingestão de KB · ações agênticas via ERP · entidade "ticket" com lifecycle (hoje a conversa É a unidade — `status: open|awaiting_human|resolved|archived`).
...
|---|---|---|---|---|
| **E1** | JANA Pro MVP — brief diário operador (JANA-A: US-COPI-201..205) | Copiloto/Jana | ~16h | — |
| **E2** | **Triagem no inbox** (US-WA-TRIAGE-001 — §5) | Whatsapp + Copiloto | ~10-12h | reusa cérebro do E1 |
| **E3** | Cobrança Asaas + 5 beta pagantes (JANA-B: US-COPI-211..215) | Copiloto/RB | ~16h | E1 |
| **E4** | Bot responde cliente (US-WA-BOT-001 — §6) + ReplyPolicy + eval gate | Whatsapp + ADS | ~18-28h | E3 + eval gate |
| **E5** | "Abrir suporte" rico / entidade-ticket (Opção B) | Whatsapp | alto | sinal de cliente |

E1/E2/E3 fecham a meta de receita do cycle com baixo risco. E4 é o teto técnico — só quando houver sinal + segurança.
...
**Escopo (in):**
- `Modules/Copiloto/Services/Triage/TriageService.php` — porta runtime do que a skill `ticket-triage` descreve: 3 fontes (financeiro UPOS + histórico WhatsApp + giro) + 5 regras determinísticas + 1 chamada LLM barata (Brain A) pra sentimento/categoria → `{score 0-100, prioridade P1-P4, risco_churn, sugestao_acao, sla_sugerido_horas}`. **PII redigida antes do LLM.** (Esta é a base do `TicketsPriorizadosTool` do US-COPI-201 — construir aqui, reusar lá.)
...
Regra: o cérebro de IA mora em `Copiloto/Jana`; o inbox (listener, badge, fila) mora em `Whatsapp`.
