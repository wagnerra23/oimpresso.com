...
|---|---|---|
| Forma da tela: grade do mês × lista | **Diverge por inteiro.** Protótipo: grade de calendário `ess-cal-g` com `role="grid"` e `aria-label` do mês (`:499`), células por dia e eventos dentro do dia (`:509`). Vivo: lista simples — `Reminders/Index.tsx:155` (`reminders.map(...)`) dentro de um `<Card>` (`:146-147`). Varredura contada no vivo: os termos `grid`, `calend`, `cal-g` e `dia` = **0 ocorrência de cada um** — nenhuma estrutura de calendário. | **Decidir.** É a **forma** da tela, e o eixo FORMA tem cadeia própria — protótipo soberano sobre charter/ADR-UI ([UI-0029](../_DesignSystem/adr/ui/0029-prototipo-soberano-sobre-adr-ui.md), citada em [proibicoes.md](../../proibicoes.md) §Precedência). E aqui **não há artefato do lado do vivo defendendo a lista**: medido, o charter do Reminders não declara `related_prototype` nem Padrão de Tela nenhum — só `bundle_source: essenciais-page.jsx` (frontmatter completo: `page`, `component`, `bundle_source`, `owner`, `status`, `last_validated`, `parent_module`, `related_adrs`, `tier`, `charter_version`). O `ancora.mjs --list` resolve `/essentials/reminder → essenciais-page.jsx`. Ou seja: a única forma **declarada** é a do protótipo, e a lista do vivo não tem lastro escrito. Decisão de [W] sobre qual é a tela; o perdedor se corrige no MESMO PR. |
