...
> Os contratos e charter históricos de tarefas foram removidos com a árvore duplicada.
...
|---|---|---|
| Toolbar de filtros | **Paridade de capacidade.** Vivo `Todo/Index.tsx:219-224` (cabeçalho "Filtros" + botão "Limpar") e `:228-283` com 5 controles em grid — **3 `<Select>`** (Status `:233`, Prioridade `:245`, Atribuído a `:258`) **+ 2 `<Input type="date">`** (De `:267-275`, Até `:276-283`). Protótipo: `hrm-toolbar` em `:165`, com Situação (`:176-177`) e o botão "Adicionar" (`:180`). | Nada — paridade. |
