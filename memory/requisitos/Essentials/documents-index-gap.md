...
| Título do card | **Paridade literal nas duas abas.** Protótipo `:372`: `memo ? "Todas as notas" : "Todos os documentos"` — os dois `copy.titulo` dos contratos. Vivo: `:250` alterna `Memos`/`Documentos` no cabeçalho e `:286` alterna a descrição. | **Decidir.** O contrato trava *"Todos os documentos"* / *"Todas as notas"*; o vivo grafa *"Documentos"* / *"Memos"*. Divergência é **só de copy**, e copy de contrato é soberania de [W] — não se troca de passagem. |
