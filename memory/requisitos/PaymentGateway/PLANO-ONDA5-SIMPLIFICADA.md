...
| 5 | Credencial BCB Pix Automático cadastrada em `payment_gateway_credentials` (biz=1, driver='bcb_pix', ambiente='production') | ✅ Wagner via UI /paymentgateway/credenciais (PR #1135 UI 3 telas) | 15min Wagner + 1d homologação BCB | `SELECT * FROM payment_gateway_credentials WHERE business_id=1 AND driver='bcb_pix' AND ativo=true` |
| 6 | `fin_contas_bancarias.rb_gateway_credential_id` apontando pra credencial Onda 5 (FK already exists — migration `2026_05_06_000001`) | ✅ Wagner via UI Financeiro | 2min | `SELECT id FROM fin_contas_bancarias WHERE rb_gateway_credential_id IS NOT NULL` |
