...
- Receita Bruta: R$ 187k (somatório `transactions.final_total` venda finalizada do mês)
- (-) Impostos sobre venda: R$ 9.350 (5% Simples Nacional anexo I — `tax_rates`) + R$ 0 DIFAL (não SP→SC interestadual tributado nesse anexo)
- (=) Receita Líquida: R$ 177.650
- (-) CMV: R$ 67.200 (somatório `transaction_sell_lines.product.purchase_price` × qty — bridge `Inventory`)
- (=) Lucro Bruto: R$ 110.450 (62%)
- (-) Despesas operacionais: R$ 45k (`fin_titulos tipo=pagar status=quitado` + classificado em plano contas `4.x.x Despesas`)
- (-) Comissões: R$ 9k (bridge `Modules/Comissao` — futuro; hoje manual em plano contas)
- (=) EBITDA: R$ 56.450 (30%)
- (-) D&A: R$ 0 (Larissa não usa Accounting formal)
- (-) IRPJ/CSLL: R$ 0 (Simples Nacional — incluso na DAS já deduzida)
- (=) **Lucro Líquido: R$ 56.450 (30%)**
...
- Saldo atual consolidado (todas contas bancárias do business): R$ 23.450
- Dia D+15 (27/05): entradas previstas R$ 12k (3 boletos AR Asaas) − saídas previstas R$ 20k (folha + aluguel + fornecedor SP) = **saldo final D+15: R$ -8k 🔴 DESCOBERTO**
- Card alerta: "💡 Antecipar boleto #1234 (R$ 15k venc 30/05) via Asaas antecipa = +R$ 14.250 líquido. Tap pra simular cenário."
- Wagner tap → cenário what-if mostra novo saldo D+15: R$ 6.250 (resolvido)
...
Sync diário 07h BRT puxa extrato Inter biz=4. Vê entrada R$ 1.234,00 em 12/05 sem `extrato_lancamento.conciliacao_titulo_id`:
- IA Jana cruza: valor exato R$ 1.234 + cliente "MARIA JOSE OLIVEIRA" + tolerância 3d → 1 match `fin_titulos.numero=2030` (venda OS00125 ROTA LIVRE) com score 97%.
...
**D — Margem real per venda (banner R$ 500):**
Cliente comprou banner lona 2m². Wagner abre `/financeiro-avancado/margem/venda/{transaction_id}`:
- Receita: R$ 500
- Custos diretos: R$ 80 (lona — `Inventory.batch.purchase_price` 2m² × R$ 40/m²) + R$ 30 (tinta — sub-product) + R$ 50 (mão-obra instalação — `Modules/Repair` linkado) = R$ 160
- Comissão vendedor: R$ 25 (5% de R$ 500)
- Impostos venda: R$ 25 (5% Simples)
- Frete: R$ 0 (cliente retirou)
- Custo total: R$ 210
- **Margem bruta: R$ 290 (58%)**
...
- [_ANALISE-FINANCEIRA-CROSS-CLIENTE.md](../../research/clientes-legacy-officeimpresso/_ANALISE-FINANCEIRA-CROSS-CLIENTE.md) — 4 candidatos OfficeImpresso pricing (Martinho R$ 4,8M inadimplência → P0 dunning)
