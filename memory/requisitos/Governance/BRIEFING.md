...
- ~~Module grade v3 (rubrica ADR 0155) e a tela que a lê~~ — **capacidade aposentada** (ADR 0399). A tela, o controller e as rotas saíram na Onda 2. O CLI `module:grade` (`ModuleGradeCommand`) e o alias `composer module-grades-check` saíram na Onda 3 — **não há mais como pedir a nota de um módulo**. O `ModuleGradeService` ficou de pé porque o `ModuleGradeSnapshotCommand` o injeta (`handle(ModuleGradeService)` → `gradeAllModules()`, única definição do método no repo) pra alimentar o cron `module:grade-snapshot` das 06:05, cujo frescor o `checkModuleGradesSnapshotRecent()` vigia; Service, Snapshot, cron, health-check e tabela caem juntos na Onda 4. `MultiTenantGovernanceTest` (cenário 5) também asserta que o Service existe e sai com ele.
