...
Service `KbMemoryOpsService` que ao indexar novo session log: (a) extrai fatos via LLM (ADD/UPDATE/DELETE/NOOP); (b) RAG privilegia `updated_at DESC` em queries com palavras temporais ("agora", "atual", "última versão"). Mem0g paper mostra 58% vs 21% em temporal. Esforço: 3d. Custo: ~R$0,005 por session indexada (Gemini Flash).
