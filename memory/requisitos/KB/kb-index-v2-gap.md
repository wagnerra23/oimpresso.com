...
| Faixa de indicadores | Não existe como faixa na tela: os quatro cartões do mockup (`kb-page.jsx:737-758` — mais lido, fixados, recém-atualizados, a revisar) foram movidos para dentro do modal "Dashboard" (`_components/HealthPanel.tsx`, 223 ln, aberto em `Index.v2.tsx:591-599`). | Nada — decisão registrada. O `Index.v2.tsx:451-455` grava a razão e a data: Wagner, 2026-05-17, os cartões ocupavam cerca de 150px de topo sem retorno suficiente, e os quatro quadrantes do painel de saúde já os cobrem. Reabrir seria desfazer decisão tomada. |
...
| Navegação por categorias | `_components/CategorySidebar.tsx` (333 ln), montado em `Index.v2.tsx:487-509`, ocupa a primeira coluna do tri-pane, como a barra lateral do mockup (`kb-page.jsx:777-903`), que reúne categorias com subcategorias, favoritos, recentes, nuvem de etiquetas e a lista de atalhos. | **Decidir.** Região do mockup: `kb-page.jsx:777-903`, seção a seção; ponto no vivo: `_components/CategorySidebar.tsx`. **Pré-condição medida:** as subcategorias do mockup vêm de bibliotecas externas que não existem no repo (`window.KB_SUBCATS` e `window.kbDeriveSub`, chamados em `kb-page.jsx:784`, `:810` e `:555`), e a nuvem de etiquetas depende de etiquetas no schema. Antes de construir, é preciso decidir se a taxonomia do mockup, que é de temas de gráfica, é a da v2 — o `kb-gap.md` já registrou que ela não é a da tela irmã. Construir ou rejeitar por escrito, seção a seção. |
| Lista de SOPs | `_components/NodeList.tsx` (262 ln), montado em `Index.v2.tsx:510-527`, é a coluna do meio, como a seção do mockup (`kb-page.jsx:906-984`), que traz o contador de itens e a ordenação segmentada por recentes, mais lidos, mais úteis e a revisar. | **Decidir.** Região do mockup: `kb-page.jsx:907-935` (cabeçalho da lista com contador e ordenação); ponto no vivo: `_components/NodeList.tsx`. **Pré-condição:** ordenar por mais lido ou mais útil exige que essas métricas existam por SOP; o subtítulo da v2 já conta leituras no agregado (`Index.v2.tsx:365-372`), então a métrica existe em algum nível — falta confirmar se é por nó. Construir ou rejeitar por escrito. |
