...
| **lote A** (#3630-3632) | Ponto · Marketplaces · Infra | 79 | 0 | 0% |
| **wave 3 LOTE C** (#3642) | Cms · Superadmin · Woocommerce · Comissao · Garantia · ADS · Admin · Dashboard · Connector · Mwart · Mcp · TeamMcp · Spreadsheet | 88 | 0 | 0% |

**Universo 29 módulos reais · taxa agregada <1% ≪ gatilho de 20-25%** (kill-criteria §103 NÃO disparou em nenhuma wave). A fila de US ambíguas está **vazia** — o DoD 3 do P10 é satisfeito por esta prova, não por itens. _Nota de qualidade (≠ ambiguidade): a refutação Fable reprovou 9 lotes na rodada 1 (Financeiro 7,5% · OficinaAuto 3,7% · Compras 8,3% · Crm 4,5% · NfeBrasil 4,8% · RecurringBilling 30,9% · ComunicacaoVisual 11,1% · **Infra 4,5%** — 023 dead-code falso-ok + 032 justificativa-pendente falsa · LOTE C: C3 Connector 4% · C4 Mcp/TeamMcp 25%→6,25%) — todos corrigidos e re-aprovados a 0%; reprovados registrados no ledger (§6 do protocolo). Ponto e Marketplaces aprovados na 1ª rodada. O LOTE C confirmou o valor do tier-superior: os mesmos lotes C1-C4 haviam passado 0% num refutador Opus (tier-igual), e o Fable pegou 5 erros semânticos reais que o Opus aprovou como "honestos"._

> **Inventory — parking honesto `_pendente_`-bloqueado (2026-07-02):** o SPEC de Inventory continua **FUNDIR** na [`_TRIAGEM-IDENTIDADE-2026-06.md`](_TRIAGEM-IDENTIDADE-2026-06.md) (repartir os 29 docs → Produto/Compras/Estoque, cluster P6/P7 **ADIADO** por custo/risco — decisão Wagner mantida). Backfillar âncora de código num SPEC que vai ser repartido = retrabalho garantido — o motor `sdd-fase-2.js` proíbe tocar pastas FUNDIR/MATAR (aguardam trilha E gated no Wagner). **Correção de estado (não resolução):** as **25 US** (US-INV-001..025) foram convertidas de `sem_campo` → `**Implementado em:** _pendente_` com justificativa de bloqueio (ADR 0273 §1/§2 — `_pendente_` é estado coberto legítimo de 1ª classe pra tela/feature que **não deve** ser construída/ancorada agora). Isso tira Inventory da contagem `sem_campo` (deixa de contar como "mentira" no `anchor-lint`) **sem** ancorar código. **NÃO desadia** o cluster Estoque (P7 segue ADIADO); é parking até a decisão de repartição aterrissar. Lint pós-conversão: `Inventory 🟢 sem_campo 0 · pendente 25 · cov 100%` (`--check` exit 0). _(A nota histórica falava em "0% coverage" pré-conversão; hoje 100% via sentinela — cobertura de anchor ≠ feature construída.)_
...
| `Pages/Forja/Board/Index.charter.md` | `[US-TR-307]` (TR-308 é topicamente da tela mas está `todo`/gap — só listar quando entregue) | smoke ou prod-flags `ProjectMgmt/Board/Index` |
...
**Evolução**
- 2026-07-02 — **Inventory parking `_pendente_`-bloqueado:** as 25 US do SPEC do Inventory convertidas de `sem_campo` → `**Implementado em:** _pendente_` com justificativa de bloqueio P7 (não ancoradas — SPEC vai ser repartido, cluster Estoque ADIADO). Tira Inventory da contagem `sem_campo` sem tocar código; §Inventory (callout §1) atualizado. Lint: `Inventory 🟢 pendente 25 · cov 100% · --check exit 0`. NÃO desadia P7. [CC]
