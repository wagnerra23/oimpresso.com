...
| Anular marcação (append-only) | **Ausente na UI e SEM ROTA — a capacidade existe pela metade no backend.** A regra existe no serviço, mas falta decisão para superfície e rota. | **Decidir.** A âncora da frente histórica foi removida com a árvore duplicada; a invariante de competência continua a exigir decisão antes da implementação. |
