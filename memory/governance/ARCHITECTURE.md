...
>
> **Navegação técnica por módulo:** [modulos/INDEX.md](../modulos/INDEX.md), derivado dos
> `Modules/*/module.json` atuais por `php artisan module:specs --index-only`. O inventário
> completo de cada raiz está em `memory/requisitos/<M>/SUPERFICIE.md`; contrato e estado
> estão no `SPEC.md` e `BRIEFING.md` vizinhos. Snapshots antigos em `memory/modulos/*.md`
> permaneceram históricos e não foram usados como censo.
