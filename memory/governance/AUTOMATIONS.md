...
| `Bash` | `post-merge-ui-smoke-required` | Após `gh pr merge --admin` de PR com arquivos UI (.tsx/.css/.blade.php, inclusive `Modules/<X>/Resources/js/**`), marca flag pendente e bloqueia Claude de declarar "pronto"/"deployed" sem screenshot real. Quando o PR tocou tela cujo charter declara `related_prototype` resolvível **e** o diff mexeu em marcação/estilo, o screenshot sozinho não libera: cobra também a comparação MEDIDA com a âncora (`design-diff.mjs --compare`), conforme PROTOCOLO-COMPARACAO-RUNTIME §Regra 0. Enforcement Tier 0 smoke visual pós-merge. Também ativo em PreToolUse `mcp__computer-use__screenshot\|mcp__Claude_in_Chrome__.*`. Escapes: `<!-- no-ui-smoke: -->`, `<!-- no-ancora-compare: -->`, `OIMPRESSO_UI_SMOKE_OVERRIDE=1`, `OIMPRESSO_ANCORA_COMPARE_OVERRIDE=1`. | `.claude/hooks/post-merge-ui-smoke-required.mjs` |
...
| Matcher | Hook | O que faz | Arquivo |
|---------|------|-----------|---------|
| `Bash` | `post-merge-ui-smoke-required` | PostToolUse Bash: detecta `gh pr merge --admin` em PR que tocou arquivos UI e marca timestamp na flag `<tmpdir>/oimpresso-ui-merge-pending.flag`, junto das telas com âncora de design que o PR tocou. No mesmo evento, um `design-diff.mjs --compare` carimba a comparação como medida. Trabalha em conjunto com a verificação PreToolUse do mesmo hook. | `.claude/hooks/post-merge-ui-smoke-required.mjs` |
...
| `mcp:tasks:unassigned` | `dailyAt('06:45')` BRT | Flagga US `todo` sem `cycle_id` e/ou sem `owner` — ticket órfão, invisível no roadmap. Eixo de ATRIBUIÇÃO (o `health-check` das 06:20 mede staleness). Advisory (sem `--strict`): a série no log é o que permite decidir a promoção a ratchet com mordida provada (ADR 0336). **2026-08-04: `->onOneServer()` removido** — a série tinha 2 buracos em 8 dias (07-29, 08-04) e ele era a única diferença estrutural vs o `health-check`, que fez 12/12; **causa NÃO estabelecida**, é mitigação que discrimina (ver `app/Console/Kernel.php`, bloco do comando). O brief **não** depende deste cron: `TasksSemDonoBriefLineService` chama `detectarNaoAtribuidas()` e consulta o banco ao vivo, 6×/dia. US-INFRA-043. |
