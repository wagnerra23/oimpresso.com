...
Cada sprint **mensurável** + **com gatilho mensurável de pivot** + **R$ [redacted Tier 0]/mês recorrente** até validar.
...
**Custo:** R$ [redacted Tier 0] (rodar tokens em CI)
...
**Custo:** R$ [redacted Tier 0] (usa Redis/Cache atual)
...
**Custo:** R$ [redacted Tier 0]
**Risco:** baixo — feature já no Meilisearch, só estamos tuning

### Sprint 10 — HyDE / Query Expansion

**Por quê:** estado-da-arte 2026 — gera pergunta/documento hipotético com LLM antes da busca. **Bridge "phrasing gap"** — Larissa pergunta "como tá o caixa?" mas fato salvo é "meta de R$ [redacted Tier 0]k/mês". Hybrid search puro pode falhar; HyDE expande query pra "qual o status financeiro do business em relação à meta de faturamento?".
...
**Sem trigger:** continua MeilisearchDriver (custo R$ [redacted Tier 0]/mês).
...
| 8 | 0.5 sprint | -68.8% tokens LLM (~R$ [redacted Tier 0]-2000/mês em escala) | **muito alto** |
...
**Total se executar tudo:** ~3.5-6.5 sprints (~5-9 semanas Wagner solo). Custo recorrente continua R$ [redacted Tier 0]/mês até trigger ativar Mem0.
...
✅ Custo recorrente continua R$ [redacted Tier 0] (Meilisearch self-hosted + cache local).
