...
review_triggers:
  ...
  - Quando custo browser MCP > R$ [redacted Tier 0]/mês (telemetria mcp_observability_spans) → otimizar frequency / amostragem
...
5. Custo browser MCP estimado < R$ [redacted Tier 0]/mês (browser MCP `mcp__claude-in-chrome__*` é gratuito; LLM tokens ~$0.02 por smoke × ~50 telas/mês × ~2 rounds = ~$2/mês)
...
| **Total estimado** | **<R$ [redacted Tier 0]/mês** | aceitável |
