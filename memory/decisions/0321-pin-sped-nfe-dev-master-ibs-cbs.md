...
- **Pin reproduzível:** `composer.json` → `"nfephp-org/sped-nfe": "dev-master"` (constraint de branch) com o **SHA fixo travado no `composer.lock`** (`reference`). Reprodutível no `install`/deploy (que usam o lock); o branch-constraint evita o warning `commit-ref` do `composer validate --strict` que o `#sha` no composer.json dispara. Re-pin = regenerar o lock (mudança de config, não de decisão). `minimum-stability: dev` + `prefer-stable: true` já existem no projeto.
