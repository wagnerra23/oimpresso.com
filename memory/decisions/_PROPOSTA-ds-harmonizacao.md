> ⚰️ **LÁPIDE 2026-06-04 · [CC] — D-02 SUPERADO (não deletar — append-only L-07/L-08).**
> A **D-02 "identidade via `--accent` escopado" com o registry `Vendas 155 verde · Compras navy · Clientes 262 indigo · CRM 220 azul`** foi **SUPERADA pelo ROXO `--accent` UNIVERSAL** (ds-v6 · ADR 0190/0235 · D-02 reconciliado = roxo no STATUS/MEMORY_INDEX 2026-06-04). Identidade-por-hue-de-módulo **não é mais a norma**; accent escopado sobrevive só como **exceção declarada** (ex.: Oficina âmbar), nunca como registry de cor-por-tela. O resto (D-01 DS-é-piso · D-03 PT-03 · D-04 v4.2 · D-05/06/07) segue como **contexto histórico**.
> **Fonte única viva = `DESIGN_FONTE_UNICA.md` + `ds-v6/*`.** _(Título "ADR 0200" é rótulo Cowork — no git 0200 = Contacts; nunca cunhar número · L-09.)_

# ADR 0200 — Harmonização DS sem perder qualidade + caminho v4.2
