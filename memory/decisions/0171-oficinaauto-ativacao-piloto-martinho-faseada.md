...
review_triggers:
  ...
  - "Job mensal apuração WhatsApp por business_id NÃO implementado em Modules/RecurringBilling antes da 1ª fatura pós-beta 30d Martinho (gate operacional — pricing R$ 99/instância cravado 2026-05-20)"
  - "≥3 clientes WhatsApp com 5+ instâncias ativas — sinal pra criar tier volume (5+ = R$ 79/inst; 10+ = R$ 59/inst) e avaliar drift pricing per-instance"
...
3. **Pricing: pacote atual mantido + add-on apenas pra módulos NOVOS (não inclusos)**
   - **Base atual Martinho = R$ 850,00/mês** já cobre: núcleo oimpresso + **vertical mecânica (Modules/OficinaAuto)** + **NFe + NFSe** — **NÃO aumentar** (Wagner 2026-05-20: *"o que ela paga hoje não vou aumentar"*)
   - **OficinaAuto NÃO é add-on extra pro Martinho** — já está incluso no pacote R$ 850 (diferente do princípio P5 [ADR 0121](0121-oimpresso-modular-especializado-por-vertical.md) que prevê add-on separado pra novos clientes; Martinho é grandfathered como Cenário A modificado).
   - **Add-on extra cobrado adicional apenas pra módulos NOVOS** (não inclusos no pacote atual). 1º candidato: **Modules/WhatsApp** — Martinho hoje paga outro sistema externo; ao migrar pra inbox unificado oimpresso vira add-on faturável separado. **Pricing cravado Wagner 2026-05-20: R$ 99,00/mês por instância (= por número de telefone WhatsApp Business conectado), atendentes ilimitados dentro de cada instância** (paridade ZapBoss; diferencial oimpresso = multi-tenant Tier 0 + auto-link CRM + macros + SLA + Jana IA sem cobrança per-seat). Apuração via job mensal: `SELECT business_id, COUNT(*) FROM whatsapp_instances WHERE status='connected' AND deleted_at IS NULL GROUP BY business_id` × R$ 99,00.
   - **Beta primeiros 30 dias = R$ 0** sobre o add-on novo (validação — alinhado opção A do [charter-1pager §"Próximo passo"](../requisitos/OficinaAuto/demo-martinho-2026-05-13/charter-1pager.md)). Base R$ 850 segue normal.
...
- ⚠️ **Job mensal apuração WhatsApp instâncias ainda não implementado em Modules/RecurringBilling** — pricing R$ 99/instância cravado mas falta lógica que conta `whatsapp_instances WHERE status='connected'` por business e gera linha de fatura mensal. *Mitigação:* US criada no SPEC RecurringBilling pré-cobrança 1ª fatura pós-beta 30d Martinho.
- ⚠️ **Modelo per-instância pode ficar caro se cliente abrir múltiplos números** — franquia 10 unidades = R$ 990/m. *Mitigação:* review_trigger formal — quando ≥3 clientes com 5+ instâncias, criar tier volume (ex: 5+ instâncias = R$ 79 cada; 10+ = R$ 59 cada).
- ⚠️ **76.7% inadimplência legacy Martinho** (R$ 6.28M receita 12m apurada em [03-financeiro Martinho](../research/clientes-legacy-officeimpresso/05-martinho-cacambas/03-financeiro-2026-05-11.md)) exigirá **write-off batch** durante cleanup. *Mitigação:* US-OFICINA-005 cleanup tools com dry-run obrigatório + Wagner aprova batch-a-batch ([publication-policy](../../.claude/skills/publication-policy/SKILL.md))
...
4. **D — Faseada + pacote atual mantido + add-on apenas pra módulos NOVOS** ✅ **escolhida**. Combina:
   ...
   - Sem risco financeiro Martinho — R$ 850 atual mantém (resposta P4 Wagner; OficinaAuto + NFe/NFSe já estão inclusos nesse valor)
...
- [x] **Base atual Martinho R$ 850/mês confirmada** — mantém inalterada (cobre núcleo + OficinaAuto + NFe + NFSe pelo pacote grandfathered)
- [x] Wagner cravou pricing **WhatsApp add-on** 2026-05-20: **R$ 99,00/mês por instância (número WhatsApp Business)** · atendentes ilimitados · benchmark ZapBoss paridade
...
- [ ] Fatura template discrimina linha "WhatsApp add-on (N instâncias × R$ 99,00) = R$ 99N,00" — UI Inertia + PDF
...
|---|---|---|---|
| ARR módulo OficinaAuto | R$ 4,8k/ano (1 piloto) | R$ 14k (3 pilotos) | R$ 24-72k (5-10 pilotos) |
...
- [Financeiro Martinho legacy](../research/clientes-legacy-officeimpresso/05-martinho-cacambas/03-financeiro-2026-05-11.md) — R$ 6.28M receita 12m · 76.7% inadimplência
