...
> oferecer alternativa orçamento (Z-API ~R$ [redacted Tier 0]/mês vs Meta token+verificação).
...
- `Services/Drivers/ZapiDriver.php` — **alternativa orçamento PME**, SaaS BR Z-API (`api.z-api.io`). Mensagens freeform sem janela 24h restritiva. R$ [redacted Tier 0]-299/mês fixo. **Risco ban Meta** (não-oficial, baseado em Whatsapp Web).
...
|---|---|---|---|
| `meta_cloud` | Meta Business Manager + verificação número (1-3 dias) + HSM templates pendentes (1-3 dias cada) | R$ [redacted Tier 0] (free tier) | tenant + Wagner ajuda |
| `zapi` | Cadastro Z-API + scan QR Code Whatsapp (~5 min) | R$ [redacted Tier 0]/mês fixo | tenant sozinho |
| `evolution` | Subir container Docker CT 100 + scan QR | R$ [redacted Tier 0] (CT 100 dele) | Wagner |
...
1. **Custo 30× menor** que BSP no perfil real (50-200 conv/mês ROTA LIVRE). Take Blip R$ [redacted Tier 0]/mês fixo justifica em ~5k conv/mês ou compliance enterprise — não é nosso caso hoje.
...
4. **Custo previsível por business** — R$ [redacted Tier 0]/mês Z-API ou R$ [redacted Tier 0] Evolution self-host CT 100 (Wagner já tem CT 100).
...
5. **Pricing Pro R$ [redacted Tier 0]/mês inclui suporte Wagner-mediado** se ban acontecer (ele ajuda re-onboarding).
...
- Repair finalmente cumpre ADR tech/0001 sem custo de SMS (R$ [redacted Tier 0] Meta / freeform Z-API vs R$ [redacted Tier 0]+ SMS).
