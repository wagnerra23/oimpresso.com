...
- [ADR 0036 — Meilisearch first](0036-replanejamento-meilisearch-first.md) — alinhamento R$ [redacted Tier 0]/mês recorrente
...
3. ❌ **Sem PII redactor** pré-LLM — CPF/CNPJ do cliente final do ROTA LIVRE vai pro OpenAI sem DPA = **violação LGPD ativa** (multa até R$ [redacted Tier 0]M).
...
**Restrição financeira (mantida do ADR 0036):** Copiloto tem **0 cliente pagante** hoje. Pagar Braintrust/LangSmith mensal (US$$ recorrente) **antes** de validar tese de retenção viola a filosofia "R$ [redacted Tier 0]/mês recorrente até comprovar valor" estabelecida no replanejamento Meilisearch-first.

---

## Decisão

**Adotar Caminho B — self-host pragmático.** 3 sprints sequenciais (7, 8, 9 do roadmap consolidado), R$ [redacted Tier 0]/mês recorrente, PHP-native quando possível, observabilidade nativa Laravel já existente (Pail+Telescope+Horizon de sessão 18).
...
4. `.github/workflows/eval.yml` — GitHub Action triggera em PR que toca `Modules/Copiloto/`. Custa ~R$ [redacted Tier 0]-10/PR (LLM-judge calls).
...
❌ **Rejeitado por:**
- US$$ recorrente em ERP que cobra R$ [redacted Tier 0]/cliente — math não fecha pré-validação
- Lock-in cloud + per-seat pricing
- Viola filosofia ADR 0036 (R$ [redacted Tier 0]/mês recorrente)
- DPA + LGPD complicado em vendor americano pra PII
- **Quando reconsiderar:** MRR >R$ [redacted Tier 0]k/mês OU cliente enterprise (>R$ [redacted Tier 0]k/mês ticket) pedindo explicitamente.
...
1. **R$ [redacted Tier 0]/mês recorrente** mantido até primeira validação comercial — alinha ADR 0036.
...
3. **Custo LLM-judge** em sampling 5% online: estimado R$ [redacted Tier 0]-500/mês quando Copiloto tiver 1k conversas/dia. Não-zero, mas << SaaS players.
...
- [ ] Custo LLM-judge ainda < R$ [redacted Tier 0]/mês? Se >R$ [redacted Tier 0]k/mês, considerar self-hosted Claude/Llama judge
