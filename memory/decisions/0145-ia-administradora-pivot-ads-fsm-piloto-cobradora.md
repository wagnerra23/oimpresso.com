...
ANPD Nota Técnica 12/2025 (publicada abril 2026) regulamenta LGPD Art. 20: toda decisão exclusivamente automatizada que afete interesse do titular exige (a) informar "decidido por IA" e (b) canal de revisão humana. Multa até 2% receita ou R$ 50M. Backend oimpresso (`mcp_dual_brain_decisions` audit) está pronto; **UI de cliente final visível não existe**.
...
4. **HITL adjustável por confidence + blast radius.** Cobrança WhatsApp <R$ 50 → L0 (auto). >R$ 500 OU cliente VIP OU primeira cobrança do mês → L2 (Wagner aprova 1-click). Bloqueio cliente → L3 (Wagner + Eliana aprovam). Limiares per-agent declarados em config + auditáveis.
...
**PolicyEngine config "cobranca_fatura"** — adicionar action_type em `config/ads.php` com matriz HITL: valor <R$ 50 = L0; valor R$ 50-R$ 500 = L1; valor >R$ 500 OR cliente VIP OR primeira cobrança mês = L2; bloqueio Asaas/cancelamento = L3. Thresholds documentados + auditáveis. Pest matriz completa.
...
4. **Custo Brain B mensal** <R$ 150 pro biz=4 (controle financeiro — se exceder, reroute pra Brain A/Haiku)
...
- Custo Brain B >R$ 500/mês biz=4 (3x orçado)
...
- ❌ Não é "ERP autônomo" agora — é piloto unitário. ERP autônomo R$ 10M/24m segue norte estratégico, não escopo deste ADR.
...
2. **Custo Brain B operacional** — orçamento R$ 150/mês biz=4 (controlável, mas é novo OPEX).
