...
> **Data-as-a-Service como linha de negócio formal.** Monetizar — com consentimento explícito + LGPD-first — os dados anonimizados e agregados dos 37 clientes OfficeImpresso (132k clientes finais, R$ 45M GMV agregado).
>
...
| Eixo | Decisão |
|---|---|
| **Produto** | `oimpresso.com/insights` — DaaS com 5 tiers (Free, Snapshot R$ 99, Pro R$ 299, Insights Pro R$ 599, Data Partner R$ 1.999) + one-shot reports R$ 2.5k-15k |
| **Wedge** | OfficeImpresso (37 clientes) → leitor legacy de Firebird vira lead-magnet → migração pro oimpresso.com novo |
| **ARR projetado 24m** | ~R$ 200k recorrente + R$ 50k one-shot = R$ 250k total (~5% da meta R$ 5M; combinado com migração ~10%) |
| **Sequência** | Tier 2 (Snapshot R$ 99) primeiro — já tem ETL provado + 5 pilotos saudáveis. Tiers 3-5 só após 5 pagantes validarem |
| **Risco #1** | LGPD — penalidade ANPD até 2% faturamento ou R$ 50M (Lei 13.709 Art. 52). Mitigação: opt-in explícito + DPO + auditoria |
...
### One-shot reports (R$ 2.500-15.000)

**LGPD**: agregados k≥10 + differential privacy ε=1.0. Buyer assina NDA + uso restrito.

| Tipo | Preço | Audiência |
|---|---:|---|
| Snapshot setorial 1 página | R$ 2.500 | leads, fornecedores curiosos |
| Relatório anual gráficas SP | R$ 7.500 | fornecedores médios (Avery BR, 3M BR) |
| Relatório setorial nacional + entrevistas | R$ 15.000 | grandes fornecedores, gov, investidores |
...
|---|---:|---|---|---|---|
| Free | R$ 0 | leads | 1 KPI mensal anônimo | 1 banco demo | sem dado real |
| Snapshot | R$ 99 | cliente atual | dashboard básico próprio | 1 banco | Art. 7º V (contrato) |
| Pro | R$ 299 | cliente médio | + benchmark + alerts | 1 banco | Art. 7º V + IX |
| Insights Pro | R$ 599 | cliente grande | + forecast + multi-banco | 3 bancos | Art. 7º IX + opt-in |
| Data Partner | R$ 1.999 | parceiro/fornecedor | API + white-label | unlimited (rate limit) | DPA específica |
...
- [ ] 5 pilotos viram **pagantes Tier 2** (R$ 99/m × 5 = R$ 495/m base)
- [ ] Pricing-lock anual oferecido (R$ 990/ano)
...
Oferta: 30d grátis Tier 2, depois R$ 99/m com pricing-lock anual R$ 990. Sem cartão upfront.
...
Pitch: *"Quer saber qual % das gráficas SP/MG/RJ usa lona impermeável? Relatório anual setorial R$ 7.500."*
...
- **Q4/2026**: 1º "Estado da Comunicação Visual BR — gráficas rápidas pequeno/médio porte" (R$ 7.500 unit)
- Vendido a 5-10 fornecedores + assoc. setoriais (ABIGRAF, ABICOM)
- Receita esperada: 5 vendas × R$ 7.500 = R$ 37.500/relatório (~R$ 50k/ano se 4 relatórios)
...
Tier 5 Data Partner R$ 1.999/m + revenue share opcional em volume alto.
...
|---|---:|---|---|---|---|
| Free | R$ 0 | leads | 1 KPI mensal genérico anônimo | 1 banco demo | sem dado real |
| Snapshot | R$ 99 | cliente atual OfficeImpresso/oimpresso | dashboard básico próprio | 1 banco | Art. 7º V |
| Pro | R$ 299 | cliente médio | + benchmark anônimo + alerts WhatsApp | 1 banco | Art. 7º V + IX |
| Insights Pro | R$ 599 | cliente grande/multi-unidade | + forecast + recomendações + multi-banco (3) | 3 bancos | Art. 7º IX + opt-in escrito |
| Data Partner | R$ 1.999 | parceiro estratégico | API REST + white-label + multi-cliente portfolio | 1000 req API/dia | DPA específica + audit anual |

**One-shot reports**: R$ 2.5k (snapshot 1pg) → R$ 7.5k (anual setorial regional) → R$ 15k (nacional + entrevistas).

**Setup**: zero default. R$ 999 só pra integração customizada (banco em local incomum, charset diferente, etc).

**Anual paga 10**: Tier 2 R$ 990/ano · Tier 3 R$ 2.990/ano · Tier 4 R$ 5.990/ano · Tier 5 R$ 19.990/ano.
...
|---|---:|---:|---:|
| Snapshot R$ 99/m | 30 clientes | R$ 99 | R$ 35.640 |
| Pro R$ 299/m | 10 clientes | R$ 299 | R$ 35.880 |
| Insights Pro R$ 599/m | 5 clientes | R$ 599 | R$ 35.940 |
| Data Partner R$ 1.999/m | 2 parceiros | R$ 1.999 | R$ 47.976 |
| **Subtotal recorrente** |  |  | **R$ 155.436** |
| Relatórios setoriais | 4 relatórios/ano × R$ 12.500 médio | — | R$ 50.000 |
| **Total ARR DaaS 24m** |  |  | **~R$ 205.000** |

### Bonus implícito — conversão pra oimpresso.com novo

- Cada cliente Tier 3+ que migrar pro oimpresso.com = +R$ 599-1.499/m receita ERP completo
- Estimativa: 5 migrações em 24m × R$ 899 médio × 12m = **R$ 53.940 adicional**

**Total combinado realista 24m**: ~R$ 260k ARR (~5% da meta R$ 5M; combinado migração ~R$ 260k = ~5%)

### Cenário otimista (sinal forte fase 3-5 vinga)

- 50 Snapshot + 25 Pro + 12 Insights Pro + 6 Data Partner + 8 relatórios/ano
- ~R$ 450k ARR (~9% da meta)
...
**Penalidade**: até **2% faturamento ou R$ 50M** por infração (Art. 52 LGPD).
**Mitigação**:
...
- Seguro responsabilidade civil cyber (ex: Chubb, Tokio) — R$ 5-10k/ano por R$ 1M cobertura
...
- Começar SaaS (1 codebase) → escalar custo marginal (~R$ 10/cliente/m em infra)
...
### Pivot
- Se Tier 2 vinga mas Tier 4-5 não: ficar em SaaS B2C-de-cliente (sem agregação cross-cliente). Ainda viável ~R$ 60k/ano + lead-magnet pra migração.
...
- [ ] Aprovar contratar counsel LGPD externo (orçamento R$ 5-15k one-shot + retainer)?
