...
5 tiers usage-based + 1 free + 1 Power. ARPU médio mês 12 projetado **~R$ [redacted Tier 0]**. Margem global **76%** sustentável. Principal risco: **complexidade percebida vs preço fixo simples** (cliente Larissa-tipo decora preço, não tabela). Mitigação: dashboard de uso em tempo real + alerta proativo aos 80% + opção "modo conforto" (fixo Premium).
...
| **Queries Brain A** (gpt-4o-mini wrapper) | Conversação simples, recall, sumarização | Incluso até X, depois R$ [redacted Tier 0]/extra |
| **Queries Brain B** (Sonnet/Opus) | Análise complexa, decisão crítica, planejamento | Incluso até X, depois R$ [redacted Tier 0]/extra |
| **Push notifications** (Centrifugo) | Alertas proativos Jana → cliente | Incluso até X, depois R$ [redacted Tier 0]/push extra |
| **API calls** (REST/MCP exposed) | Integração externa cliente → Jana | Incluso até X, depois R$ [redacted Tier 0]/call extra |
| **Storage histórico** | Memória persistente (facts + sessions + embeddings) | 1GB grátis, depois R$ [redacted Tier 0]/GB-month |
| **Voice queries** (premium add-on) | STT+TTS (Whisper + ElevenLabs) | R$ [redacted Tier 0]/query, só Premium+ |

### Custo unitário (ranges públicos OpenAI/Anthropic Q1-2026)

| Item | Custo oimpresso | Cobrado | Markup |
|---|---|---|---|
| Brain A query (gpt-4o-mini, ~2k tokens in + 500 out) | ~R$ [redacted Tier 0] | R$ [redacted Tier 0] | 4x |
| Brain B query (Sonnet, ~4k tokens in + 1k out) | ~R$ [redacted Tier 0] | R$ [redacted Tier 0] | 2.6x |
| Brain B query (Opus, raro) | ~R$ [redacted Tier 0] | R$ [redacted Tier 0] | 2x |
| Push notification (Centrifugo CT 100) | ~R$ [redacted Tier 0] | R$ [redacted Tier 0] | 50x (margem alta — barato pra nós) |
| API call (Hostinger CPU) | ~R$ [redacted Tier 0] | R$ [redacted Tier 0] | 20x |
| Storage GB-month (Hostinger + Meilisearch) | ~R$ [redacted Tier 0] | R$ [redacted Tier 0] | 20x |
| Voice query (Whisper + ElevenLabs) | ~R$ [redacted Tier 0] | R$ [redacted Tier 0] | 2.5x |
...
### Pro — R$ [redacted Tier 0]/m ⭐ (sweet spot)
...
- 50.000 API calls
- 100GB storage
- Voice queries habilitado (R$ [redacted Tier 0]/each)
- Suporte: chat 4h SLA
- **Target:** gráfica 5-15 funcionários, multi-loja

### Enterprise — R$ [redacted Tier 0]/m
...
**Quando faz sentido:**
- Cliente Pro com GMV R$ [redacted Tier 0]k/m → 0,5% = R$ [redacted Tier 0] → vale pagar R$ [redacted Tier 0] + R$ [redacted Tier 0] = R$ [redacted Tier 0] (em vez de Premium R$ [redacted Tier 0] com caps)
- Não vale: cliente GMV R$ [redacted Tier 0]k → 1% = R$ [redacted Tier 0] melhor Pro fixo
...
| Item | Overage |
|---|---|
| Brain A | R$ [redacted Tier 0]/query extra |
| Brain B | R$ [redacted Tier 0]/query extra |
| Push | R$ [redacted Tier 0]/push extra |
| API call | R$ [redacted Tier 0]/call extra |
| Storage | R$ [redacted Tier 0]/GB-month extra |
| Voice | R$ [redacted Tier 0]/query (sempre cobrado em Premium) |
...
**Cap de proteção:** overage máximo = 50% do tier base. Cliente Pro (R$ [redacted Tier 0]) overage cap R$ [redacted Tier 0] → upgrade automático sugerido com 1-clique pra Premium (R$ [redacted Tier 0] saldando overage acumulado).
...
- Custo oimpresso: 150 × R$ [redacted Tier 0] = **R$ [redacted Tier 0]/m**
- Receita: **R$ [redacted Tier 0]/m**
- **Margem: 99%** (cliente subutiliza tier — bom problema, lucra)
- Valor extraído cliente: 5h/m economizadas em consulta de pedido = R$ [redacted Tier 0]/m em tempo de operador → **ROI 7,5x**

### Cliente Pro típico (perfil ROTA LIVRE)
- 800 queries Brain A, 30 Brain B, 200 push, 2k API
- Custo: 800×0,005 + 30×0,30 + 200×0,001 + 2k×0,0005 = R$ [redacted Tier 0] + R$ [redacted Tier 0] + R$ [redacted Tier 0] + R$ [redacted Tier 0] = **R$ [redacted Tier 0]/m**
- Receita: **R$ [redacted Tier 0]/m**
- **Margem: 95%**
- Valor extraído: 15h/m operador + 3 decisões críticas/m apoiadas = R$ [redacted Tier 0]/m → **ROI 8,3x**

### Cliente Premium típico
- 3.000 Brain A, 150 Brain B, 800 push, 20k API, 10GB
- Custo: 3000×0,005 + 150×0,30 + 800×0,001 + 20k×0,0005 + 10×0,005 = R$ [redacted Tier 0] + R$ [redacted Tier 0] + R$ [redacted Tier 0] + R$ [redacted Tier 0] + R$ [redacted Tier 0] = **R$ [redacted Tier 0]/m**
- Receita: **R$ [redacted Tier 0]/m**
- **Margem: 88%**
- Valor extraído: 30h/m + analytics + multi-loja sync = R$ [redacted Tier 0]/m → **ROI 9,2x**

### Cliente Enterprise (rede)
- 10k Brain A, 800 Brain B, 5k push, 100k API, 200GB, 100 voice
- Custo: 50 + 240 + 5 + 50 + 1 + 6 = **R$ [redacted Tier 0]/m**
- Receita: **R$ [redacted Tier 0]/m**
- **Margem: 76%**
- Valor extraído: R$ [redacted Tier 0]k/m → **ROI 12x**

### Cliente Power (white-label)
- 30k Brain A, 2k Brain B (incluindo Opus 5%), 20k push, 500k API
- Custo: 150 + 700 + 20 + 250 = **R$ [redacted Tier 0]/m**
- Receita: **R$ [redacted Tier 0]/m**
- **Margem: 63%** (mais baixa por incluir Opus + SLA + onboarding)
- Valor extraído: R$ [redacted Tier 0]k+/m → **ROI 16x**

---

## 7. Custo unitário detalhado (referências públicas)

- **OpenAI gpt-4o-mini** (jan 2026): $0.15/M input, $0.60/M output → ~$0.001/query típica → **R$ [redacted Tier 0]**
- **Anthropic Sonnet 4.5** (2026): $3/M input, $15/M output → ~$0.06/query → **R$ [redacted Tier 0]**
- **Anthropic Opus 4.7** (2026): $15/M input, $75/M output → ~$0.30/query → **R$ [redacted Tier 0]**
- **Storage Hostinger**: ~R$ [redacted Tier 0]/GB-month (shared)
- **Meilisearch CT 100**: R$ [redacted Tier 0]/GB-month adicional embedding storage
- **Centrifugo push** (CT 100 self-hosted): custo marginal zero, computa CPU/RAM ~R$ [redacted Tier 0]/push
- **ElevenLabs Brazilian PT** (voice): $0.30/1k chars → ~R$ [redacted Tier 0]/query 30s
...
- ❌ **Tier "Custom" sem preço público** (perde transparência) → Power tem preço público R$ [redacted Tier 0] custom só Power+
...
|---|---|---|---|---|
| Free | — | (não conta ARPU) | R$ [redacted Tier 0] | R$ [redacted Tier 0] |
| Starter | 35% | 25 | R$ [redacted Tier 0] | R$ [redacted Tier 0] |
| Pro | 38% | 27 | R$ [redacted Tier 0] | R$ [redacted Tier 0] |
| Premium | 17% | 12 | R$ [redacted Tier 0] | R$ [redacted Tier 0] |
| Enterprise | 8% | 6 | R$ [redacted Tier 0] | R$ [redacted Tier 0] |
| Power | 2% | 1 | R$ [redacted Tier 0] | R$ [redacted Tier 0] |
| **Total** | 100% | **71** | **ARPU R$ [redacted Tier 0]** | **R$ [redacted Tier 0] MRR** |

**ARPU médio mês 12: ~R$ [redacted Tier 0]** (acima da banda alvo R$ [redacted Tier 0]-450, indica que mix Pro/Premium puxou pra cima — saudável).

**ARR projetado mês 12:** R$ [redacted Tier 0]k (vs meta R$ [redacted Tier 0]M/ano = 8% — precisa scale customer base, não pricing).

### LTV

Churn assumido 3%/m (SaaS B2B BR vertical):
- LTV Starter: R$ [redacted Tier 0] × (1/0,03) × 0,99 margin = **R$ [redacted Tier 0]**
- LTV Pro: R$ [redacted Tier 0] × 33 × 0,95 = **R$ [redacted Tier 0]**
- LTV Premium: R$ [redacted Tier 0] × 33 × 0,88 = **R$ [redacted Tier 0]**
- LTV Enterprise: R$ [redacted Tier 0] × 33 × 0,76 = **R$ [redacted Tier 0]**
- LTV Power: R$ [redacted Tier 0] × 33 × 0,63 = **R$ [redacted Tier 0]**

**LTV médio ponderado:** R$ [redacted Tier 0] CAC alvo (1/3 LTV): R$ [redacted Tier 0] — viável com afiliados + organic.

### Curva ARPU mês 1 → 12

- M1 (lançamento): R$ [redacted Tier 0] (early adopters Starter/Pro)
- M3: R$ [redacted Tier 0] (overage começa, Pro→Premium upgrades)
- M6: R$ [redacted Tier 0] (Enterprise pipeline matura)
- M9: R$ [redacted Tier 0] (Power pilots)
- M12: R$ [redacted Tier 0] (mix estável)
...
2. **"Modo previsível"** — opt-in pra pagar Premium fixo R$ [redacted Tier 0] sem overage tracking visível
...
5. **Garantia "menor preço"** — primeiros 3 meses, se Pro fixo R$ [redacted Tier 0] sairia mais barato que tier escolhido + overage, oimpresso refunda diferença
6. **A/B test pricing display** — testar "R$ [redacted Tier 0]/m + uso" vs "R$ [redacted Tier 0]/m flat" em landing — métrica conversão
...
- **Voice queries explodirem custo** (cliente faz 1000/m) → cap fair-use 1k/m em Premium, depois R$ [redacted Tier 0]/each transparente
