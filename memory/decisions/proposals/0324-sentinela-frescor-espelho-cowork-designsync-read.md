...
> **Reconciliação 2026-07-06 (sessão paralela já em `origin/main`):** o registro canônico da fonte Cowork (2 projetos + integração viva) é o **[INDEX-DESIGN-MEMORIAS §0.2](../../requisitos/_DesignSystem/INDEX-DESIGN-MEMORIAS.md)** (PR #3875). Ele fixa: o mirror **está em sincronia e não apodrece sozinho**, Cowork **sempre foi a fonte (NÃO supersede)**, e manda **"diffar antes de concluir"** — que é exatamente o que este sentinela AUTOMATIZA. A ideia de **migrar a fonte pro vivo / git virar cache** (que eu havia rascunhado numa ADR 0325) foi **RETIRADA**: contradiz o §0.2 e depende de uma decisão que o Wagner ainda **não cravou** (§0.2 pendência: *"antigo = arquivo defasado [refutado] vs direção de design a redesenhar [aberto]"*). ADR registra decisão, não pergunta aberta — enquanto a pendência não fecha, a migração fica parada no §0.2, sem ADR.
