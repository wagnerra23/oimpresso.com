...
2. **Inventário COMPLETO dos consumidores** (a v2 dizia "≥12" — incompleto). Os REQUIRED omitidos, achados pelo adversário:
   ...
   - advisory: `charter-refs`, `charter-us-lint`, `charter-promote-signal`, `reconcile-triplet`, `design-code-map-check`, subsistema `scripts/design/ancora.mjs` + `_lib-charter.mjs` + `block-ancora-no-olho`, `charter-da-tela-que-o-controller-serve`
