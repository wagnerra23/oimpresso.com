...
> **AVISO DE TENSÃO COM R25 + R26.** Este roadmap propõe transformar Jana em produto habit-forming multi-vertical com benchmark setorial e usage-based pricing. Ele **assume premissas mais agressivas** que `roadmap-tecnico-12m-2026-2027.md` (que prevê 5-7 clientes ERP em 12m, ARR ~R$ 100k) e que `auto-vertical-strategy.md` (que recomendou STAY-FOCUSED em com.visual). Se o roadmap ERP-base (R25) entregar 5 clientes em mai/2027, **DAU/MAU >50% e R$ 1.5-2M ARR não são plausíveis**: math abaixo mostra teto realista ~R$ 250-400k ARR. Wagner precisa reconciliar: aceitar Jana como camada premium SOBRE o roadmap base R25 (recomendado) OU substituir R25 por este (não recomendado — quebra ADR 0105).
...
**Risco crítico M2:** Meta WhatsApp Business onboarding pode levar >14d (verificação CNPJ, número, política). **Mitigação:** começar onboarding em cycle J1 (não J3). Fallback: usar ZAPI/Twilio temporário (custo ~R$ 0.05/msg).
...
- **Goal primary:** Goal-setting mensal — cliente define meta ("faturar R$ 50k em novembro") via UI Jana, sistema tracka diário
- **Goal secondary:** Daily check-in 8h BRT — Jana envia "Boa terça Larissa, você está em 67% da meta novembro (R$ 33.5k de R$ 50k)"
...
- **Esforço:** ~40h codáveis (4h IA-pair) + ADR voice + custo R$ 1-3 por query Brain B (vs R$ 0.01 Brain A)
...
- Custo Brain B sob controle: ≤R$ 5/cliente/mês em voice (alvo R$ 2)
...
2. **Custo Brain B explode** (R$ 50+/cliente/mês) — mitigação: gate `decide()` ALLOW_BRAIN_A em 80% queries simples
...
- **Goal primary:** Benchmark Premium pago — R$ 99-299/m extra pra ver ranking detalhado dentro do vertical (top 10% / mediana / bottom 10% em 8 métricas)
...
- ≥R$ 50k ARR DaaS combinado (Benchmark Premium + Snapshot — compromisso template R$ 100k é stretch)
...
| **ARPU médio** | ~R$ 0 (Larissa não paga formal) | R$ 400+ | R$ 600-900 (R25 prevê tier R$ 599 dominante) |
...
| **ARR total combinado** (ERP + DaaS) | ~R$ 0 | R$ 1.5-2M (template) | **R$ 250-400k realista** ⚠️ |

> **Discrepância ARR.** Template projeta R$ 1.5-2M mas R25 base prevê R$ 100k ARR ERP em mai/2027 (5-7 clientes × R$ 600-1.499). Pra bater R$ 1.5M precisaria 100+ clientes ERP OU 50 clientes Snapshot DaaS no Tier 3 (R$ 599) — ambos violam ADR 0105 (sem sinal hoje pra esse volume). Math realista 12m fim mai/2027:
>
> - ERP base R25: 5-7 clientes × R$ 600-1.499 = **R$ 36-126k ARR**
> - Snapshot DaaS M6: 3-5 clientes × R$ 99-299 = **R$ 4-18k ARR**
> - Benchmark Premium M6: 2-5 clientes × R$ 99-299 = **R$ 2-18k ARR**
> - API parceiros: 1-3 deals × R$ 5-50k = **R$ 5-150k ARR (alto risco wallclock)**
> - **Total realista: R$ 47-312k ARR.** Pra bater R$ 1.5M = **pivot massivo Snapshot multi-cliente como produto principal** (precisa ADR canon nova + revisitar ADR 0105 com sinais novos).
...
**Risco se não aprovar:** ARPU plana em R$ 600 médio, ARR teto baixo.
**Bloqueia:** R$ 250k+ ARR (sem usage-based, math não fecha mesmo no realista).
...
**Risco se aprovar sem sinal:** voice queima R$ 5-50k em compute Brain B + 6h IA-pair sem retorno.
...
|---|---|---|---|---|
| **WhatsApp Business API oficial Meta** | [E] | 14d onboarding (≥30d realista) | R$ 0.05-0.20/msg após verificação | Twilio/ZAPI temporário |
...
| **Brain B Sonnet/Opus** (voice + reasoning complexo) | [F] | imediato | R$ 1-3 por query | Brain A (gpt-4o-mini) com prompt mais elaborado |
| **OpenAI Whisper + TTS** (voice) | [F] | 1d setup | ~R$ 0.50/min audio | Google Speech-to-Text + ElevenLabs |
...
- **Tensão a reconciliar com Wagner:** este roadmap propõe ~30% capacidade Jana paralela a R25. R25 hoje é 100% (sem fatia Jana explícita). Wagner precisa decidir:
  - **Opção A (recomendada):** aceitar este roadmap como **adendo a R25** — 30% Jana / 70% ERP base. ARR realista 12m: R$ 250-400k combinado.
  ...
  - **Opção C:** rejeitar este roadmap inteiramente — manter R25 como única verdade, Jana fica como feature incremental sem milestone próprio. ARR R$ 100k.
