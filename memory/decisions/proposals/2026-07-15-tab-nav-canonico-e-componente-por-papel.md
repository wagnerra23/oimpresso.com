...
- **Ondas do detector — estado:**
  ...
  - ✅ **`status-badge`** (pill de status) — **ABERTA** (PR desta Onda). Achado que muda o tripé: **o papel JÁ está no DS** — o canon existe em duas camadas (primitivo `<Badge variant="success|warning|danger|info|neutral">` tokenizado `-soft/-fg` + wrapper de domínio `<StatusBadge kind value>` em `@/Components/shared`). Logo a peça (a) do tripé é **consumir**, não criar. Entregue: regra `ds/no-handrolled-status-pill` (component-substitute, ratchet 0209) + assinatura `status-badge` em `ROLE_SIGNATURES` (canon=StatusBadge, `canonImport` vira lista pra aceitar Badge OU StatusBadge) + seção no REGISTRY + fidelidade `tests/statusBadgeFidelity.spec.tsx` (+ gate path-scoped advisory). Reconciliação do "11": o número era a população inline+componente; a Onda a divide pela fronteira natural — o detector rastreia **3 componentes** independentes (`Pills`, `ServiceOrderStatusBadge`, `VehicleStatusBadge`; fiscal é exceção R-DS-002), e a regra `ds/*` + ratchet absorve os **usos inline** (o restante). `FiscalStatusBadge` fica de fora por decisão (canon fiscal próprio).
  - ✅ **`sub-navegacao-contextual`** (switch in-page controlado — abas dentro da página sem mudar URL) — **ABERTA** [W] 2026-07-15. Como no status-badge, **o canon JÁ existe** (`SubNav` em `@/Components/shared`) — a peça (a) do tripé é **consumir**, não criar. Entregue: assinatura `sub-navegacao-contextual` no `--roles` com discriminador `role="tablist"` + `value` + `onChange` **sem** marca de topo (`moduletopnav`/`subtabs`/`subnav`) nem consumo do `PageHeaderTabs` (não double-claima o `Financeiro/Unificado`) + controle-negativo no self-test (morde o hand-roll in-page, não reivindica barra-de-topo) + entrada no REGISTRY §"Sub-navegação contextual". **Sem regra `ds/` nova:** reusa a `ds/no-inline-tablist` (mesmo selector `role="tablist"`), cuja **mensagem** passou a nomear OS DOIS alvos (topo→`PageHeaderTabs`, in-page→`SubNav`) — antes misdirigia todo tablist pro `PageHeaderTabs`. **Sem spec de fidelidade:** `SubNav` é componente DS genérico (underline/segmented), não fiel a um `.cli-*` de protótipo específico como o tab-nav — não há 4 propriedades-âncora a travar. Detector cataloga **5 independentes** (`SavedViewsChips`, `Jana/Chat`, `DrawerGateway`, `team-mcp/Tasks/Index`, `ConversationList`); migração = incremental por tela (fora do escopo desta onda).
