...
| **E3** | `bundle → alvo no repo` (import-time) | `scripts/design/detectar-telas.mjs` | mockup do bundle Cowork sem alvo no repo (ORFAO) / >1 (AMBIGUO) | **script + `--selftest` advisory** (`design-memory-gates.yml`). É o passo ANTES dos outros — não duplica |
