...
| 2026-06-09 | [CL] | **G-4 Salto #3 — domínio ALÉM do enum.** `dominio:check` passou a varrer o CÓDIGO de aplicação (não só migrations) nos módulos com dicionário: valor-literal usado contra coluna governada via `where`/`whereIn`/comparação-de-campo/validação `in:` tem que ser canônico. Nova chave `dominio:undeclared-code-value` (ratchet por valor). Pegou 11 ramificações `order_type === 'locacao'` em código morto que sobreviveram à erradicação do enum (ADR 0265) — débito caçável absorvido no baseline. Meta-testes físicos + anti-FP (operador `like`, `self::CONST`, `Database/`/`Tests/` excluídos). |
