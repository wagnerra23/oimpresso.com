---
slug: 0328-ds-transicao-congelado-para-vivo-git-ssot
number: 328
title: "DS v6 congelado → projeto vivo no claude.ai/design (git permanece SSOT) — loop bidirecional + sentinela de drift; emenda 0315 (pull design→git controlado)"
type: adr
status: aceito
authority: canonical
lifecycle: ativo
...
module: governance
tags: [governance, design-system, design-sync, tokens, git-ssot, drift, ci, dark]
supersedes: []
---

# ADR 0328 — DS v6 congelado → projeto vivo no claude.ai/design (git permanece SSOT)

> **Status:** aceito ([W] 2026-07-09). Append-only — Constituição v2 [ADR 0094](0094-constituicao-v2-7-camadas-8-principios.md); soberania [ADR 0238](0238-soberania-constituicao-wagner.md). ([CL] rascunhou como proposta 2026-07-09; [W] aceitou.)
> **Irmã de:** [`proposals/2026-07-08-profissionalizar-ds-sync-git-espelho.md`](proposals/2026-07-08-profissionalizar-ds-sync-git-espelho.md) (FASE 1, canvas) + [`proposals/2026-07-08-ds-direcao-design-git-e-sidebar-dark-fixa.md`](proposals/2026-07-08-ds-direcao-design-git-e-sidebar-dark-fixa.md) (FASE 2, D-1 direção).
...
**O problema que forçou esta decisão** (investigação 2026-07-08): o canvas dark do cockpit divergiu em **3 cópias** (git `0.165/hue 282`, snapshot jun `0.205/282`, espelho `0.26/hue 240`). Causa-raiz não foi "cor errada" — foi **ausência de loop de sincronização** entre git (SSOT) e espelho vivo. Alguém editou o canvas no claude.ai/design e o git seguiu no valor dele: **drift silencioso**, o vetor que a 0239 existe pra prevenir. A FASE 1 (reconciliar por imagem) foi resolvida em #3981/#3982/#3983; o primeiro push de reconciliação git→espelho (19 tokens dark → 0) foi executado + verificado ao vivo em 2026-07-09.
...
3. **Loop bidirecional determinístico e incremental**, em dois runbooks companheiros:
   ...
   - **PUSH (git → design)** — [`design-sync-push.md`](../../.claude/runbooks/design-sync-push.md): re-espelha o token já canonizado no git, incremental (`finalize_plan`/`write_files`, nunca replace atacado), via o montador determinístico `ds-mirror-build.mjs` (troca só valores de tokens compartilhados, preserva scaffold), fechando o loop.

4. **Sentinela de drift** ([`ds-mirror-drift.mjs`](../../scripts/governance/ds-mirror-drift.mjs), sobre o motor `ds-token-diff.mjs`) compara git × espelho e **alerta** quando separam. **Advisory primeiro**; promoção a `required` só se/quando a política [ADR 0314](proposals/0314-poda-gates-onda-2-lei-fusoes.md) permitir (required = só Tier-0). O CI do GH Actions não loga no claude.ai → compara contra um **snapshot commitado** do espelho; o diff contra o espelho vivo roda local/cron.
...
- **D-2 (sidebar preto-fixa, supersede UI-0009/UI-0014)** e **D-3 (valores dark reconciliados)** da proposta-irmã `proposals/2026-07-08-ds-direcao` são **UI-ADR/PR à parte** — não entram aqui.

## Implementação (landeada)

- Runbooks: [`design-sync-pull.md`](../../.claude/runbooks/design-sync-pull.md) (P1 #3982) + [`design-sync-push.md`](../../.claude/runbooks/design-sync-push.md) (P2 #3990).
- Sentinela: [`ds-mirror-drift.mjs`](../../scripts/governance/ds-mirror-drift.mjs) + workflow `ds-mirror-drift.yml` advisory (P3 #3991).
- Montador: `scripts/design-sync/ds-mirror-build.mjs` + snapshot `mirror-snapshot/colors_and_type.css` (PR conclusão #3997).
- Pipeline: [`PIPELINE-TOKENS.md`](../requisitos/_DesignSystem/PIPELINE-TOKENS.md).

---

**Rodapé de evolução**
- 2026-07-09 — [CL] rascunho da transição (P4, proposta). Consolida a emenda 0315 (D-1) num ADR único de loop.
- 2026-07-09 — [W] **aceito**; proposta promovida a ADR 0328 (`proposals/2026-07-09-ds-transicao-congelado-para-vivo-git-ssot.md` → `decisions/0328-…`). Primeiro push git→espelho executado + verificado (19 dark → 0); sentinela vivo drift 0.
