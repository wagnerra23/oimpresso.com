...
number: 140
title: "JANA Pro — Produto comercial SaaS de IA pra PMEs BR (upsell sobre oimpresso, R$ 149-499/mês)"
type: adr
status: aceito
authority: canonical
lifecycle: ativo
...
review_triggers:
  ...
  - "Concorrente BR lançar produto equivalente <R$ 99/mês → reavaliar diferenciação"
  - "≥3 clientes pedirem feature fora do escopo (CRM completo, helpdesk full) → reavaliar product boundaries"
  - "Volume LLM tokens >R$ 5k/mês sem revenue cobrindo → freeze growth + buscar Anthropic SDK partner pricing"
---
...
- WhatsApp Baileys próprio em CT 100 ([ADR 0096](0096-modulo-whatsapp-meta-cloud-api-direto.md))
  — custo marginal zero vs Z-API R$ 300/chip/mês
...
**R$ 9.945 MRR + R$ 119k ARR** em 12 meses de produto isolado (sem contar
upsell do core ERP).
...
|---|---|---|---|---|
| **JANA Free** | R$ 0 | 50 triages/mês ad-hoc | Clientes oimpresso curiosos | -R$ 1,50 (loss leader) |
| **JANA Pro** | **R$ 149** | Brief diário + 500 triages | Operador ativo (ROTA LIVRE-like) | **~94%** |
| **JANA Enterprise** | **R$ 499** | Autonomous events + Wagner consultoria 1h/mês | Cliente top 10% LTV | **~92%** |

Modelo de custo LLM Claude Sonnet 4.5 via [`laravel/ai`](https://laravel-ai.dev/):
- Triage ad-hoc: ~3k tokens = R$ 0,02
- Brief diário 10 tools: ~25k tokens = R$ 0,15
- Brief semanal executivo: ~80k tokens = R$ 0,50
- Event-driven (Enterprise): ~150k tokens/dia = R$ 0,90/dia = R$ 27/mês
...
- ARR isolado +R$ 119k em 12 meses (50 Pro + 5 Enterprise)
...
- ADR 0022 (Meta R$ 5M/ano financeira) ganha vetor adicional — produto
...
**Goal:** 5 clientes pagando R$ 149/mês (Beta).
...
- CAC orgânico (clientes oimpresso): R$ 0
- CAC marketing (Fase 4+): R$ 80/cliente
...
**Margem operacional 12m:** ~93%. LLM tokens ~R$ 600/mês, hospedagem
alocada CT 100 ~R$ 100/mês.

**Lucro operacional 12m:** ~R$ 110k. Realista pra Wagner solo + 4 time
...
- **Mês 6:** MRR R$ 5k + churn <5% + 1 case study publicado
- **Mês 12:** MRR R$ 10k + ARR R$ 100k + margem >90%
