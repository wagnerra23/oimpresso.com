...
**Smoke real validou:** 928 tokens → 227 tokens (**4.09× compressão**), preservando fatos numéricos críticos: *"R$ [redacted Tier 0] abril 2026"*, *"R$ [redacted Tier 0] março maior venda"*.

**Trade-off:** 1× LLM call cheap por compressão (R$ [redacted Tier 0]) vs amortização em 5+ turnos seguintes. Vale.
...
**Trade-off:** R$ [redacted Tier 0]/dia/biz vs -30% tokens system prompt em CADA request. Em 100 requests/dia: economia 30k tokens × R$ [redacted Tier 0] = R$ [redacted Tier 0]/dia. Paga em 1 dia se biz tem >50 requests.
...
User: *"como tá o caixa?"* — query CURTA, ambígua. Embedding desse texto tem POUCO sinal pra match com facts armazenados como *"faturamento líquido R$ [redacted Tier 0] abril 2026"*.
...
**Literatura RAG 2026:** +15% Recall@10. **Custo:** R$ [redacted Tier 0]/expansão + cache 1h.
...
**Solução:** LLM-as-Reranker — gpt-4o-mini ranqueia top-K candidatos por relevância à query. Custo: R$ [redacted Tier 0]/rerank vs +5pp recall@5 esperado. Cache 5min pro mesmo query+candidatos.
...
| **Semanal (sex 23:00)** | `copiloto:eval --resposta` | + RAGAS (Faithfulness, Answer Relevancy) — custo R$ [redacted Tier 0]/run |
...
| Sinal | Decisão |
|---|---|
| Recall@3 plateau em 0.50-0.60 com tudo ativo | Avalia upgrade Mem0 (R$ [redacted Tier 0]k/ano) ou Letta (free) — ADR 0036 trigger |
...
Custo/turno:     R$ [redacted Tier 0] (-82%)
Cache hit rate:  ≥ 30% (gate Cycle 01)
```

**Impacto comercial:** Margem por cliente Tier 1A (R$ [redacted Tier 0]/mês plano):
- Antes: ~R$ [redacted Tier 0]/dia × 30 = R$ [redacted Tier 0]/mês custo IA = **8% do plano**
- Depois: ~R$ [redacted Tier 0]/dia × 30 = R$ [redacted Tier 0]/mês = **1,5% do plano** → **+6,5pp margem por cliente**

Em 50 clientes: economia R$ [redacted Tier 0]/mês × 12 = **R$ [redacted Tier 0]/ano** só de IA.
