...
| **PII em commit/log/ticket** | CPF/CNPJ cliente em ticket Crm vaza pro git via commit message ou log estruturado | CI `pii-scan` bloqueia merge; use `[REDACTED]` ou `PiiRedactor` helper. Em ticket: substituir `123.456.789-00` por `[CPF-REDACTED]` antes de copiar pra commit/PR. |
