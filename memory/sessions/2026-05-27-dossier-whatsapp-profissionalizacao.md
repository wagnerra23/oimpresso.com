...
- **Service messages unlimited grátis desde nov/2024** ([Chatarmin 2026](https://chatarmin.com/en/blog/whats-app-api-pricing)) — fluxo conversacional inbound vai pra R$ [redacted Tier 0]
- **Custo Meta Cloud 200 conv/mês Brasil:** ~R$ [redacted Tier 0] utility + R$ [redacted Tier 0] service ([Message Central](https://www.messagecentral.com/blog/whatsapp-business-api-pricing-in-brazil)) — **MAIS BARATO** que R$ [redacted Tier 0]/mês Z-API
...
- **BSP enterprise (ε)** R$ [redacted Tier 0]+/mês = 30× overkill pra perfil 50-200 conv/mês atual; reaberto via review_trigger se cliente pagante pedir SLA
...
- Custo monetário direto Wagner: **economia R$ [redacted Tier 0]/mês Z-API + R$ [redacted Tier 0] daemon CT 100 RAM/CPU desperdiçados**
...
| **(β)** Meta Cloud default + Z-API SaaS fallback opcional | Sem daemon próprio; Z-API responde pelo ban; oimpresso fora linha fogo | ✅ **ESCOLHIDA (β híbrida)** | Combina onboarding rápido oficial (Embedded Signup v4 5-15min) + safety-net Z-API pra legacy + custo R$ [redacted Tier 0]/mês default + risco ban zero default |
...
| **(ε)** BSP enterprise (Take Blip/Twilio/360dialog) | Terceiriza inteiro; alto custo, alto SLA | ❌ Rejeitada por ora | R$ [redacted Tier 0]+/mês Take Blip = 30× overkill perfil PME atual (50-200 conv/mês); reaberto via review_trigger se cliente pagante pedir |
...
- Não conflita com ADR 0140 JANA Pro porque **default Meta Cloud = custo marginal zero**. Z-API só entra em caso ativo (R$ [redacted Tier 0]/mês cobrado do cliente legacy, não do oimpresso).
...
| **Custo marginal zero JANA Pro ([ADR 0140](../decisions/0140-jana-pro-produto-comercial-saas.md))** | Meta Cloud: service messages (customer-initiated) unlimited grátis desde nov/2024. Utility templates R$ [redacted Tier 0]/conv BR ([Message Central 2026](https://www.messagecentral.com/blog/whatsapp-business-api-pricing-in-brazil)) — 500 triages JANA Pro = R$ [redacted Tier 0]/mês utility max. Margem 92-94% projetada [ADR 0140] preservada. |
...
| **Constituição v2 [ADR 0094]** | #1 Context as product (driver simples, sem daemon). #2 Tiered cost (Meta free + Z-API R$ [redacted Tier 0] só se ativo). #4 Loop fechado métrica (`whatsapp.messages.sent` OTel já existe). #5 SoC brutal (sem daemon CT 100 = menos uma fronteira). #6 Multi-tenant Tier 0 (Meta nativo). #7 Transparência (driver default oficial, sem TOS violation). #8 Confiabilidade fallback (Z-API opcional pra emergência). |
...
- Métricas observadas Prometheus + OTel:
  ...
  - `whatsapp.cost.centavos` — esperado: utility R$ [redacted Tier 0]/conv, service R$ [redacted Tier 0]
...
**Custo monetário direto:** **R$ [redacted Tier 0]** (Meta Cloud free service msg + R$ [redacted Tier 0]/mês utility típico). **Economia:** ~R$ [redacted Tier 0]/mês Z-API + RAM/CPU CT 100 desperdiçados (~2.4GB RAM Baileys daemon).
...
| Custo/conversa BR | R$ [redacted Tier 0]/mês fixo ÷ N conv (não-linear) | R$ [redacted Tier 0] utility / R$ [redacted Tier 0] service | > R$ [redacted Tier 0] utility médio = revisão pricing |
...
- **Custo Meta médio > R$ [redacted Tier 0]/mês/business sustained 60d** → reabrir avaliar BSP enterprise (Take Blip / Twilio markup)
...
2. **BSP enterprise pedido por cliente piloto pagante** ≥ R$ [redacted Tier 0]/mês recorrente assinado — reabrir avaliar Take Blip / Twilio / 360dialog
...
- Brief diário Wagner via WhatsApp = template utility R$ [redacted Tier 0] × 30 dias = **R$ [redacted Tier 0]/mês/cliente** (não R$ [redacted Tier 0]/dia × 30 = R$ [redacted Tier 0]/mês como previa modelo conservador ADR 0140)
- Triage ad-hoc cliente-initiated = service messages = **R$ [redacted Tier 0]/triage** (não R$ [redacted Tier 0] como previa)
- **Margem JANA Pro real pode ser 96-98% em vez de 92-94% projetado** — folga adicional ~R$ [redacted Tier 0]k/ano em 50 clientes Pro = R$ [redacted Tier 0]/mês a mais de margem
...
> **Wagner:** O caminho profissional do WhatsApp do oimpresso a partir de hoje é **Meta Cloud API como default universal + Z-API como driver opcional não-default + BaileysDriver/daemon Node OUT integral**. Decisão venceu α/γ/δ/ε porque (1) você reportou Baileys instável, (2) onda detecção Meta 2026 atinge whatsmeow igual Baileys (rewrite não resolve), (3) Embedded Signup v4 default Meta 5-15min derruba premissa "Z-API default por onboarding rápido" da ADR 0096 emenda 3, (4) service messages unlimited grátis desde nov/2024 destrava custo zero JANA Pro [ADR 0140] em margem 96-98% real (não 92-94% previsto). Próximo passo HOJE: SSH CT 100 `docker compose stop whatsapp-baileys` (você autorizou). Custo total ~18h suas + 16-24h codáveis IA + 37 dias relógio. Economia: R$ [redacted Tier 0]/mês Z-API + RAM CT 100. ADR pronta pra você aceitar/rejeitar em 5 min: [`2026-05-27-whatsapp-profissionalizacao-baileys-out.md`](../decisions/proposals/2026-05-27-whatsapp-profissionalizacao-baileys-out.md).
