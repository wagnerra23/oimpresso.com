...
**Nota de processo:** o [#3532](https://github.com/wagnerra23/oimpresso.com/pull/3532) (fix GLOB_BRACE) foi fechado sem merge às 20:57Z junto do merge do #3534 — aparentemente acidental (o "merge" aprovado cobria ambos; o bug seguia em `origin/main:139`). Reaberto com justificativa; os required renomeados pelo P14 (#3535, sessão paralela) exigiram commit vazio pra re-disparar CI com os nomes novos. Ambos mergeados 22:04-22:05Z (#3532 + hotfix #3545).

## Adendo lote 1 — run real por materialização + 4 rodadas de refutação (ledger G5)

Wagner aprovou o skim ("merge / aprovado"). Execução:

1. **Materialização em vez de re-roll:** os 5 BRIEFINGs commitados são **byte-a-byte o output do motor que Wagner skimou** (o dry-run devolve o conteúdo completo; escrever o que foi aprovado é mais fiel que re-chamar a LLM, que é não-determinística). `distilled_at: 2026-07-01`, zero custo LLM extra. Shrink por porta (by design, porta ≤1 página — ADR 0291 D-1; conteúdo antigo preservado no histórico git): Financeiro 17.7KB→4.0KB (77%) · Whatsapp 76% · Governance 65% · OficinaAuto 82% · Sells 39%.
2. **Refutação adversarial (protocolo G5), lote INTEIRO por rodada, sessão fresca, tier fable > gerador gpt-4o-mini≈haiku:**
   - **R1: REPROVADO 14,29%** (8/56) — padrão sistêmico: porta antiga datada reapresentada como estado ATUAL ("recentemente", "último PR 25/mai" com 13 commits posteriores) + mistranslations ("depuração"≠aposentadoria, paridade "funcional"≠visual, "todas as refs removidas" com 176 arquivos citando baileys) + gap US-WA-310 já entregue + fabricação de gaps (OficinaAuto).
   - **R2: REPROVADO 6,12%** (3/49) — capability inflada (anexos automáticos Pix/NFe/boleto: Pix AUSENTE no próprio CAPTERRA-INVENTARIO, classe-âncora inexistente), "última mudança" OficinaAuto omitindo ADR 0265 (erradicação locação — regressão catalogada!), e **troca de tenant no canário Sells (era biz=1, não biz=4 — gravidade ALTA)**.
   - **R3: REPROVADO 3,45%** (2/58) — paridade 9.8 era contaminação cross-módulo (doc de Sells; o real da medição 2026-05-20 é **9.5**/10) + data do wizard Embedded (05-28→05-27, PR #1768; o 05-28 tinha vindo da evidência de migration da R1 — as rodadas convergem em precisão).
   - **R4: APROVADO 1,82%** (1/55, PII 0) — ressalva não-bloqueante "Policies CRUD" corrigida com o wording prescrito pelo próprio refutador + 2 INCERTOs suavizados.
3. **Lições sobre o motor v1 (fila pra evolução do prompt — contrato ADR 0291):** (a) preservar datas das fontes em TODO claim de estado; (b) proibir advérbios de recência sem timestamp; (c) validar gaps contra código antes de publicar; (d) não copiar headers/H1 da porta antiga; (e) risco de contaminação cross-módulo em números.
4. Entries das 4 rodadas no [`governance/sdd-verification-ledger.json`](../../governance/sdd-verification-ledger.json) (append-only, reprovados registrados — §6 do protocolo).
