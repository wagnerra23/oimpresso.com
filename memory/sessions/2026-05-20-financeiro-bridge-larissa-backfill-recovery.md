...
description: Sessão tarde — Larissa biz=4 destrava "quem está devendo" via backfill SQL 3 fases após audit revelar Observer no-op silencioso por 12 dias. R$ [redacted Tier 0]k visível → R$ [redacted Tier 0] a receber real
...
```
paid: 15.764 sells (R$ [redacted Tier 0]) → vão virar status=quitado
due: 1.550 sells (R$ [redacted Tier 0])     → vão virar status=aberto (DINHEIRO REAL DEVENDO)
partial: 97 sells (R$ [redacted Tier 0])     → vão virar status=parcial
```
...
Resultado: 0 → **15.412 baixas biz=4** (R$ [redacted Tier 0] baixados historicamente).
...
total_a_receber: R$ [redacted Tier 0]
titulos_quitados: 15.772
total_quitado_histórico: R$ [redacted Tier 0]
```
...
Dados 100% coerentes com `transactions.invoice_no` + `contacts.name` + `transactions.transaction_date`. Cross-check totalizador: fin_titulos R$ [redacted Tier 0] vs cálculo direto via `transactions(payment_status IN ('due','partial'))` R$ [redacted Tier 0] = diferença 0.34% (R$ [redacted Tier 0]) explicada pelos 522 órfãos AP (`purchase status='received'`).
