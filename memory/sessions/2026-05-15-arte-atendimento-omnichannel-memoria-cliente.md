...
- **Gap fatal #2 — `AnalisarMensagemAgent` (PR #916) está aberto há dias mas adiado por custo.** Custo real estimado no próprio PR: **R$ [redacted Tier 0] one-time backlog (21k msgs) + R$ [redacted Tier 0]/mês/biz real-time**. Isso é trivial — não é razão honesta pra adiar. Razão honesta provável: medo de drift biz=1 sem dry-run validado. Mergear este PR é a **maior alavanca de impacto vs esforço da lista**.
...
- **Gap alto-impacto #4 — sem `Reply Suggestion` (MagicReply/Fin-style) baseada no perfil acumulado do cliente.** Tags + análise + memória cliente formam o substrato pra "sugerir resposta personalizada" no inbox — feature top vendável em PME BR (Take Blip cobra R$ [redacted Tier 0]/mês por isso).
...
- **Recomendação:** mergear PR #916 **com dry-run gated por Wagner**, antes de qualquer outra coisa. Custos travados em ~R$ [redacted Tier 0] totais no mês. Próxima ação hoje: `php artisan whatsapp:analyze-backlog --business=1 --channel=10 --since=24h --dry-run` e olhar o output.
...
- **Custo real travado**: R$ [redacted Tier 0] one-time + R$ [redacted Tier 0]/mês/biz. Comparar com R$ [redacted Tier 0]-1.500/mês cobrado por Take Blip pela mesma feature.
...
- ❌ **Não trocar gpt-4o-mini por modelo maior antes do dry-run.** R$ [redacted Tier 0] ainda não foi gasto — calibrar modelo sem dado de qualidade é cargo cult.
