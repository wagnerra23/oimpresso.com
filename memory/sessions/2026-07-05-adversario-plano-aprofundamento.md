...
| 2 | Alto | Claim sem evidência: eu disse "zero lente de segurança" | já existe um `03-security-review-quick.md` (2026-05-09) com achados (detalhe no escopo apartado da Onda 3) | canon corrigido: baseline referenciado no `AUDITORIA-SEGURANCA-ESCOPO.md` como 1º item da onda |
...
- Onda 3: conteúdo dual-use movido pra doc apartado (`AUDITORIA-SEGURANCA-ESCOPO.md`); baseline referenciado lá.
...
- **#2** confirmar mitigação do achado A-1 do baseline de segurança (detalhe no `AUDITORIA-SEGURANCA-ESCOPO.md`) — se ainda aberto, correção prioritária fora da fila das ondas.
