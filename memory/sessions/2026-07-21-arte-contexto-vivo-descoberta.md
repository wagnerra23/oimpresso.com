...
| 4 | **Co-locação como sinal** (`.charter.md`/`.casos.md` ao lado do `.tsx`) + **mapa por-tela GERADO** | `scripts/qa/screen-coverage-map.mjs`, `scripts/design/ancora.mjs` | A fronteira (Anthropic, Sourcegraph) diz: *"convenção de pasta + gerador vence mapa escrito à mão"*. Você faz exatamente isso. |
