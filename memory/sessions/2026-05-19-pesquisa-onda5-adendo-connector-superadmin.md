...
|---|---|---|---|
| `Plan "SaaS Oimpresso Premium R$ [redacted Tier 0]/mês"` em RB biz=1 | `Superadmin::Package` (Starter / Regular / Unlimited) — controla módulos via `custom_permissions` | Duplicar source-of-truth: Plan RB vs Package Superadmin | **Alto** — drift entre duas tabelas; qual ganha? Refator caro |
...
```
Wagner cadastra Package "Premium R$ [redacted Tier 0]" em /superadmin/packages (já existe UI)
    │
    ▼
