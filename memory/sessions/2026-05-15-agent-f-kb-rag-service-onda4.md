...
|-------------------------|----------------------------------------------------------------|--------------------|-----------------|-----------------|--------------------------------------------------------------------------|
| `ask` (sem cache)       | ~3.500 (system 400 + 6 fontes × ~500 + pergunta 100)            | ~400               | ~$0.000765      | **~R$ [redacted Tier 0]**  | `(3500*0.15 + 400*0.60) / 1e6 * 5`                                       |
| `ask` (cache hit)       | 0                                                              | 0                  | $0              | **R$ [redacted Tier 0]**     | Redis 1h TTL — corpus_hash invalida automaticamente quando bridge atualiza |
| `summarize`             | ~3.300 (system 200 + body 3000 + meta 100)                      | ~250               | ~$0.000645      | **~R$ [redacted Tier 0]**  | `(3300*0.15 + 250*0.60) / 1e6 * 5`                                       |
| `summarize` (cache hit) | 0                                                              | 0                  | $0              | **R$ [redacted Tier 0]**     | Redis 6h TTL — chave inclui `node.updated_at` + `sourceDoc.updated_at`    |
| `suggestMeta`           | ~2.500 (system 200 + body 2300)                                 | ~150               | ~$0.000465      | **~R$ [redacted Tier 0]**  | `(2500*0.15 + 150*0.60) / 1e6 * 5`                                       |

**Projeção mensal Wagner ROTA LIVRE (biz=4) — uso pesado:**
- 500 perguntas/mês → 500 × R$ [redacted Tier 0] = **R$ [redacted Tier 0]/mês**
- 200 summarize/mês → 200 × R$ [redacted Tier 0] = **R$ [redacted Tier 0]/mês**
- 100 suggestMeta/mês → 100 × R$ [redacted Tier 0] = **R$ [redacted Tier 0]/mês**
- **Total: ~R$ [redacted Tier 0]/mês por business em uso pesado** — desprezível vs receita.

**Alerta de explosão de custo:** monitorar `mcp_audit_log.custo_brl SUM by business_id` daily. Se um business passar R$ [redacted Tier 0]/mês, investigar (loop user / scraping / bug).
