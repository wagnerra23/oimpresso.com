...
- **Z-API (provider BR) NÃO resolve o problema** — Wagner observou rodando "muito bem" em outro cliente, mas auditoria 2026-05-15 mostra que **Z-API é wrapper Baileys-like com SaaS por cima**: sofre o mesmo blackbox LID. Doc Z-API admite literal: *"It is not possible to convert an `@lid` to a phone number"*. Driver oimpresso `ZapiDriver` já existe (72/100, pronto), mas o `ZapiWebhookController` ignora `senderLid/chatLid` — migrar SEM refactor de chave canônica reproduz o bug. Custo: R$ [redacted Tier 0]/mês fixo (Plano Ultimate, 1 instância, msgs ilimitadas) — mais barato que Cloud API ~R$ [redacted Tier 0]-120 mas com **Reclame Aqui 3.8/10 + suporte lag 37 dias + vendor lock-in alto**. Recomendação Z-API: **NÃO migrar biz=1 agora** — só ativar como fallback secundário canary em biz=99 sandbox se Wagner quiser comparar empiricamente.
...
| Custo/mês (BR, 1k msgs/mês) | utility ~$4 / marketing ~$62.5 (BSP markup +$3-10) ([Chatarmin pricing](https://chatarmin.com/en/blog/whats-app-api-pricing)) | $0 (lib + CT 100 ~R$ [redacted Tier 0]/mês prorata) | $0 | $0 |
...
|---|---|---|---|
| **Custo mensal ROTA LIVRE** | **R$ [redacted Tier 0] fixo** (Plano Ultimate, 1 instância, msgs ilimitadas, arquivos ≤100MB) ([z-api.io](https://z-api.io/)) | R$ [redacted Tier 0] marginal (CT 100 já existe) | ~R$ [redacted Tier 0]-120 (4.500 msgs × $0.0068 + BSP) |
...
**Quando ativar (opcional, decisão Wagner):** Z-API vira **fallback secundário sombra** em biz=99 sandbox por 30 dias canary (R$ [redacted Tier 0] × 1 mês = R$ [redacted Tier 0] total). Se uptime > 99,5% E métricas estáveis, promover a `fallback_priority=2` (atrás de Cloud API quando essa entrar).
...
**Insight:** o que Z-API "estabilizou" **não é Baileys lib em si** — é a **camada de orquestração anti-ban por cima** (queue + IP rotation + behavioral mimicry + warm-up). Essa camada custa **engenharia + infra real** (R$ [redacted Tier 0]/mês cliente pagar paga isso).
...
Calibrado pro caso **biz=4 ROTA LIVRE / Larissa (99% volume, ~50-200 msgs/dia, meta R$ [redacted Tier 0]mi/ano, prod hoje, multi-tenant Tier 0 IRREVOGÁVEL)**. Score 0-100% por dimensão; total ponderado embaixo.
...
| **D3. Custo total ROTA LIVRE/mês** | 10 | 100% (R$ [redacted Tier 0] marginal CT100) | 100% | 80% (precisa VPS RAM ≥4GB) | 90% (VPS Hetzner CX23 €3 ~R$ [redacted Tier 0]) | 75% (R$ [redacted Tier 0] fixo) | 50% (R$ [redacted Tier 0]-120 + BSP markup) |
...
- **Em 60-90 dias (após refactor `contact_lid` + canary Cloud API biz=99):** **Cloud API = 71%** assume liderança. Custo R$ [redacted Tier 0]-120/mês cabe na meta R$ [redacted Tier 0]mi/ano (~0,003% revenue).
- **Janela intermediária (30 dias):** Z-API canary biz=99 sombra ganha empate com Baileys 7.x final que ainda não saiu — Wagner pode validar empiricamente em biz=99 sandbox por R$ [redacted Tier 0] sem mexer prod.
...
- ROTA LIVRE atual ~150 msgs/dia × 30 = 4.500 msgs/mês × $0.004 = **~$18/mês utility = R$ [redacted Tier 0]-120/mês**
...
**Quando ativar:** se ban Baileys 6.7.9 → recovery > 4h Wagner OR margem mensal absorve R$ [redacted Tier 0] facilmente (provável dado meta ADR 0022 R$ [redacted Tier 0]mi/ano).
...
**Custos:**
- R$ [redacted Tier 0]/mês fixo (Plano Ultimate, 1 instância) × 1 mês canary = R$ ~100 total
...
- Custo previsível R$ [redacted Tier 0] fixo independente volume
...
**Custos:**
- VPS extra (Postgres + Redis dedicados pra Evolution): pode reusar Postgres CT 100 existente + Redis → **R$ [redacted Tier 0] marginal**
- Docker container `evolution-api:2.3.7` no CT 100: **R$ [redacted Tier 0]** (CT 100 tem capacidade)
...
5. **Z-API NÃO é solução pro cross-contact** — é trade de risco (self-host CT 100 zero-custo) por SaaS BR (R$ [redacted Tier 0]/mês + Reclame Aqui 3.8/10 + vendor lock-in). Se Wagner quiser comparar, faz em biz=99 sandbox sombra, **nunca biz=1 prod sem refactor `contact_lid` canônico antes**.
