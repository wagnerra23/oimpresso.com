...
**Multi-documento por venda** (`transaction_documents` poly): 1 venda → N notas (nfe55 + nfse56 + mdfe58). O caso comvis canônico = banner (NF-e 55) + instalação (NFS-e 56) + transporte se >R$ [redacted Tier 0] (MDF-e 58), tudo em 1 transaction.
...
## Atualização PR2 — [CC] 2026-06-01 (Claude Code)

Verificação no repo durante a implementação: o catálogo `contact_addresses` +
`app/ContactAddress.php` (§3 "JÁ EXISTE no repo") na verdade vive na branch
**não-mergeada** `feat/crm-078-multiplos-enderecos` (US-CRM-078 PR1: model +
migration + Pest cross-tenant), **não** em `main`/`feat/staging-ct100`.

A consumação no Sells (§3 "Direção do P0") foi implementada como **PR2** sobre
essa branch (`feat/crm-078-pr2-sells-endereco`): seção `sec-entrega` de 1ª classe
em `Sells/Create.tsx` lendo `contact.addresses[]` (Destinatário `is_default` ↔
Local de entrega `is_shipping` + "Outro"), hint de MDF-e por cidade,
`ContactController@getCustomers` eager-load `addresses` (Tier 0 HasBusinessScope).
Emissão fiscal real (`<entrega>`/cMun via `city_code` no `NfeService`) = **PR3**.
