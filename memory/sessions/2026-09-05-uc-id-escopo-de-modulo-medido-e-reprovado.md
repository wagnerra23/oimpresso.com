...
4. **Afirmei ao [W] que "o CI do repositório está parado há ~12h30", com tabela — e era falso.** Duas sondas erradas: (i) `actions/runs?status=completed&per_page=5` não devolve "os 5 mais recentemente concluídos" (ordena por `created_at`, filtra depois), então o `updated_at` do topo não era o último run concluído; (ii) "os 60 runs mais recentes estão `queued`" é **tautológico** — a listagem ordena por criação, logo os mais novos são os que acabaram de nascer do meu push. A sonda certa (`status=in_progress` + contagem por janela) mostrou **24 rodando** e **7087 concluídos nas 12h anteriores**. Cheguei a checar o status oficial do GitHub para confirmar um incidente que não existia. Corrigido no mesmo turno, antes de virar canon ou decisão. É a **2ª instância de LC-08 nesta sessão** — cometida enquanto eu escrevia a lápide sobre medir a fonte errada.
