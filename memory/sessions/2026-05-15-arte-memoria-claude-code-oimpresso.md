...
2. **`.claude/rules/` path-scoped (G2) — adotar pattern Anthropic 2026?** 4-6 arquivos cobrindo Modules/PHP, Pages TSX, Migrations. Move conteúdo seletivo de `memory/reference/`. ROI ~R$ 11k/ano em LLM + menos noise. Esforço 3h. **Recomendação: SIM** (gate quantitativo após Fase 3: validar redução tokens via `claude-code-usage-self`).
