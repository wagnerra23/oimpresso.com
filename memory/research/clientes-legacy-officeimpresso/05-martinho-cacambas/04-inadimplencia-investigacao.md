...
A inadimplência reportada **R$ 4,82M = 76,7% da receita 12m** **NÃO é** cliente da Martinho que não paga hoje — é **lixo contábil histórico não-limpo desde 2015**. Três sinais decisivos:

1. **83% do valor vencido (R$ 3,88M de R$ 4,82M) tem > 365 dias de atraso**, com média de **2.564 dias (≈ 7 anos)** e título mais antigo de **2015-10-22**. Isso é **backlog não-baixado**, não cobrança recente atrasada.
...
3. **20% dos títulos vencidos têm `RAZAOSOCIAL` órfã** (não existe em PESSOAS — grafia/cadastro inconsistente, R$ 860k) e **6.585 títulos em `INATIVO CANCELADA` somam R$ 61,85M** (12× mais valor cancelado do que ativo vencido) — sinal de **operação manual com correções caóticas**.
...
|---|---:|---:|---:|
| 0-30 dias (recente) | 59 | R$ 78.188,91 | 1,6% |
| 30-60 dias | 145 | R$ 160.357,76 | 3,3% |
| 60-180 dias | 498 | R$ 564.302,00 | 11,7% |
| 180-365 dias | 81 | R$ 138.099,57 | 2,9% |
| **> 365 dias (abandono)** | **3.812** | **R$ 3.878.695,36** | **80,5%** |

- **N total vencidos:** 4.595 títulos
- **Média atraso:** **2.564 dias** (≈ 7 anos)
- **Min vencto:** 2015-10-22 · **Max vencto:** 2026-05-09
- **Total vencido:** R$ 4.819.643,60

**Interpretação:** 4 de 5 títulos vencidos é fóssil de 2015-2019. O atraso "real" recente (até 60 dias) é apenas R$ 238k = 4,9% do total reportado. **A inadimplência atual ≠ inadimplência histórica.**
...
- **Top 1 (`Cliente_77639D`):** 8,4% do total · R$ 394.405,00 · 232 títulos · **vencendo desde 2016-02-19**
...
| Valor pago | R$ 3.118.612,21 |
| Valor vencido | R$ 718.175,83 |

**Interpretação:** boletos funcionam mas têm **26% sem baixa** (cancelados + abertos = 22%). Pode ser melhorado, mas não é catastrófico. O **valor vencido de boletos 12m (R$ 718k) é apenas 15% do total vencido (R$ 4,82M)** — confirma que os outros 85% são fóssil pré-12m.
...
|---|---:|---:|
| `INATIVO CANCELADA` | **6.585** | **R$ 61.854.054,53** |
| `ATIVO` | 4.595 | R$ 4.819.643,60 |
| `INATIVO AGRUPADO` | 415 | R$ 525.455,20 |
| `INATIVO EXCLUIDA` | 464 | R$ 521.121,78 |
| `INATIVO EXCLUIDO` | 272 | R$ 242.062,10 |
| `ATIVO AGRUPADO` | 91 | R$ 189.905,11 |
| `INATIVO EXC.AGRUPADO` | 9 | R$ 39.303,00 |
| `INATIVO PREVISÃO` | 13 | R$ 11.045,00 |
| `ATIVO PREVISAO` | 9 | R$ 8,00 |

**LANCAMENTO_FUTURO='S':** 105 títulos (8,00 vencidos · negligível) · **`ATIVO PREVISAO`:** 24 títulos · 8,00 vencidos · negligível.

**Interpretação:**
- **R$ 61,85M em `INATIVO CANCELADA`** (12× mais que ativos vencidos) — sinal claríssimo de que a equipe **cancela bastante mas não limpa retroativo**. Esses não contam pra inadimplência (status inativo) mas mostram **rotatividade comercial enorme** (orçamentos virando A Receber e depois cancelados).
...
| Valor órfão | **R$ 860.390,03 / R$ 4,82M = 17,85%** |
...
|---|---:|---|
| Inadimplência REAL (cobrança falha) | **15-20%** | apenas R$ 700-900k recentes (60d-12m) tem indício de não-baixa; resto é fóssil |
| USO RUIM histórico (não-limpo) | **65-75%** | 83% do "vencido" tem > 1 ano; backlog até 2015 |
| Drift VENDA vs FINANCEIRO | **10-15%** | 374 vendas sem lançamentos = R$ 1,64M de dado faltante a normalizar |
...
> "Você tem **R$ 4,82M no relatório de contas a receber vencido, mas R$ 3,88M são de 2015-2019** — não vai cobrar mais. O resto (R$ 940k de 12m recentes) sim. Nossa migração inclui ferramenta de write-off em batch que limpa esse histórico em ~3 semanas de uso (sua equipe processa 200/dia). Depois disso o relatório fica honesto e você sabe quanto realmente precisa cobrar."

**Quanto cobrar:**

| Item | Custo IA-pair Felipe | Custo Wagner | Valor real |
|---|---:|---:|---:|
| Importer + cleanup rules + tela revisão batch (P0×3) | 24h | 8h | **R$ 8.000** (one-time) |
| Asaas + lembrete (P1×2) | 12h | 4h | **R$ 3.000** (one-time) |
| Setup OficinaAuto vertical (PLACA + FSM 2 estados) | 16h | 4h | **R$ 4.000** (one-time) |
| Manutenção mensal núcleo + OficinaAuto | — | — | **R$ 350-500/mês** (recorrente) |
| **Total piloto** | | | **R$ 15.000 + R$ 400/mês** |

ROI pro cliente: 1 funcionária × 3 semanas × R$ 200/dia limpando manualmente = R$ 3k. Mas o **valor real está em parar de operar com "relatório mentindo"** (não saber quanto é cobrável de fato). Se cobrar 5% de R$ 940k recente = **R$ 47k de receita recuperada** justifica os R$ 15k facilmente.

### 4.4 Roadmap proposto pra abordagem Martinho

1. **Demo 1 — "Onde está seu dinheiro" (45min)**
   Wagner mostra o gráfico de aging desta investigação (R$ 3,88M é fóssil; R$ 940k é cobrável recente). Cliente sai com diagnóstico claro.
...
4. **Contrato piloto (R$ 15k + R$ 400/m)**
   Migração cutover paralela 30d. Critério de sucesso: relatório de contas a receber pós-cleanup ≤ R$ 1,5M (atual: R$ 4,82M).
