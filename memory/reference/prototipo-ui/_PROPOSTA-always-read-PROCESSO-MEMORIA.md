...
```markdown
4b. **Antes de tocar design-memory** (`prototipo-ui/**` · charters · `*.casos.md` · build visual): ler [`memory/reference/prototipo-ui/PROCESSO_MEMORIA_CC.md`](PROCESSO_MEMORIA_CC.md) (raiz do método anti-regressão — §5 REGRESSÕES PROIBIDAS + NÚCLEO 13 invariantes) + [`memory/LICOES_CC.md`](../../../memory/LICOES_CC.md) (L-01..L-25). No fim da build, rodar `node scripts/design/ds-guard.mjs <arquivos tocados>` (§8) e, ao formalizar, `node scripts/design/integrity-check.mjs` (§15). _REGRESSÃO É INACEITÁVEL._
```
...
```markdown
- **Design-memory (Cowork ↔ git):** antes de evoluir `prototipo-ui/**` ler [`PROCESSO_MEMORIA_CC.md`](PROCESSO_MEMORIA_CC.md) + [`memory/LICOES_CC.md`](../../../memory/LICOES_CC.md). Defesas mecânicas: `ds-guard.mjs` (§8 · paleta inventada/tela-na-raiz) e `integrity-check.mjs` (§15 · espinha sã). Anéis Avaliar→Testar→Adotar→Descartar; nada reprovado volta sem citar §5.
```
...
> Detalhe do que foi landeado: `memory/reference/prototipo-ui/README-DESTINO-PROCESSO-MEMORIA-2026-06-02.md`.
