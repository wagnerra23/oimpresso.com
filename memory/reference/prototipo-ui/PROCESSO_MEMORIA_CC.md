...
### 2026-08-08 — Descrever na §15 o NÍVEL DE ENFORCEMENT de cada IT ("advisory"/"duro") em vez de apontar pro script dono
- **o que foi tentado:** a tabela do §15 carregava o nível de enforcement junto da descrição — literalmente `IT2 … — advisory`.
- **por que caiu:** apodreceu **no mesmo dia em que foi escrita**. O [#4005](https://github.com/wagnerra23/oimpresso.com/pull/4005) escreveu "advisory" e o [#4037](https://github.com/wagnerra23/oimpresso.com/pull/4037) promoveu o IT2 a **DURO** — ambos em 2026-07-09 — e a tabela seguiu 30 dias afirmando o contrário do que a máquina faz. Medido nesta sessão comparando linha a linha com `integrity-check.mjs`: **4 das 8 linhas + o Veredito** discordavam (IT2 nível errado · IT5 omitia a janela de frescor · IT6 prometia checagem de paleta que o código não faz · IT7 prometia escopo maior que os 5 alvos reais · Veredito dizia "qualquer IT" onde o código diz "qualquer IT **duro**"). A única linha intacta é a do **IT8** — a única que **aponta pro dono** (`handoff:check`) em vez de restatear.
- **o limite (variante também proibida):** nenhum doc do método (§15, §9, §16, `STATUS.md`) **afirma** nível de enforcement, limiar numérico ou escopo de varredura de uma máquina. Aponta pro dono; se o valor for necessário, cita a **chave** que o define (ex.: `OIMPRESSO_BENCHMARK_STALE_DIAS`), nunca o número. ⚠️ **E não vire gate:** detectar enforcement-restateado por vocabulário foi **medido e reprovado** (130 falso-positivos) em `memory/proibicoes.md` §5 (2026-07-16 · classe LC-10) — a defesa aqui é **subtração** (a tabela para de afirmar), não YAML novo.

### 2026-08-08 — Fechar o IT8 vermelho com `handoff:baseline:write` (congelar a dívida pra "devolver o job ao verde")
- **o que foi tentado:** avaliado nesta sessão como o caminho rápido — a própria mensagem de erro do guard oferece (*"se a mudança é legítima e muda a dívida, rode `npm run handoff:baseline:write`"*).
- **por que caiu:** a dívida **não é legítima — é true-positive**. Os 3 `PROMPT_PARA_CODE_*` de 2026-07-10 ([#4096](https://github.com/wagnerra23/oimpresso.com/pull/4096)/[#4099](https://github.com/wagnerra23/oimpresso.com/pull/4099)) nunca foram citados na fila (nem acima nem abaixo da linha d'água — `grep` = 0 em `COWORK_NOTES.md`), e **um deles, `DS-DOMINIO-RETIRAR-DSV6`, não tem retorno em `CODE_NOTES.md`**: é exatamente a "tarefa invisível" que a §16 Regra 1 existe pra impedir, invisível há 29 dias. Congelar transformaria o último sinal vivo em silêncio permanente.
- **o limite (variante também proibida):** baseline de catraca só se regrava quando a mudança que ela acusa é **deliberada e correta** — nunca pra devolver um job ao verde. Vale pro `handoff-integrity-baseline.json` e qualquer irmão. Antes de regravar, **provar que o acusado é falso-positivo**; sendo verdadeiro, o conserto é **resolver a dívida** (arquivar com lápide o que pousou · enfileirar o que não pousou), e isso é decisão de [W]/[CL], não do baseline. Parente: `memory/proibicoes.md` §5 (2026-07-17, drift-sentinel — "não regrave o baseline sem provar que o instrumento mede o que diz medir").
...
### TESTE-07 · Auditoria de integridade com o processo 31 dias sem medição (o gatilho §12 disparando de verdade)
- **o que testei:** rodei a integridade inteira (IT1–IT8 + `ds-guard --all` + os selftests do handoff) numa sessão em que o **próprio IT5 já estava vermelho** — a pergunta não era "o método detecta?", era "**alguém reage** quando ele detecta?".
- **o que funcionou:** as máquinas estavam **certas e dispararam sozinhas**, sem um falso-positivo em 8 sondas. IT5 acusou staleness real (31d, última medição 2026-07-09). IT8 acusou 3 órfãos **reais**. O `handoff:selftest` provou que a catraca morde (fixture ruim → vermelho; abaixo-da-linha → ignorado; baseline → congelado). O `ds-guard --all` deu relatório de dívida (9 arquivos) **sem bloquear**, como desenhado no §8.
- **fraqueza achada (2):**
  1. **Ninguém escuta o vermelho.** IT5 ficou 31d vermelho e o IT8 **29 dias sem um único verde — 75 runs `failure`**, última verde 2026-07-03 (medido em `gh run list`, não inferido). Os dois vivem em job advisory, e **nenhuma máquina vigia "advisory vermelho há N dias"**. Quem trouxe foi **[W]** — não mecanismo. É o TESTE-04 de novo, deslocado: lá o elo fraco era *ler*; aqui é **reagir**.
  2. **A §15 descrevia errado o que as máquinas fazem** — 4 de 8 linhas + o Veredito (detalhe e limite na §5).
- **correção:** (2) corrigida aqui — a tabela para de afirmar enforcement e aponta pro dono. (1) **deliberadamente NÃO corrigida por mim:** subir a defesa um tier (§12 passo 3) é **promover gate**, decisão de [W] (ADR 0271/0275/0336, promoção exige mordida provada); e construir vigia de "advisory vermelho persistente" é máquina nova, que exige **FP medido antes** — run vermelha por flaky/rede contaria como alarme, e isso é medição, não palpite. Fica **reportado com o par candidato nomeado**, não armado. O mesmo vão já está registrado como candidato em `memory/LICOES_CODE.md` **LC-14** (estender o eixo 1 do `cron-watchdog.mjs` pra olhar `conclusion`, não só recência) — **não incrementei o contador de lá**: a *classe* da LC-14 é `ancora-estrutural-que-compara-formatacao`, e o vão-do-ouvinte é o agravante nomeado nela, não a classe. Inflar contador alheio seria o oposto do que o ledger mede.
- **⚠️ sessão paralela (ADR 0119):** enquanto esta rodava, **outra** sessão logou a própria linha do §11 no mesmo dia e mergeou primeiro ([#5433](https://github.com/wagnerra23/oimpresso.com/pull/5433), 1 linha, só o benchmark) — o IT5 já estava verde em `main` quando este PR foi reconciliado. As duas linhas ficam (append por sessão · precedente `2026-06-02 (b)`); esta é a `(b)`. **O que sobra de único aqui é a auditoria** (§15 + §5 + este TESTE), não o destravamento do IT5.
- **emenda a uma premissa da sessão irmã** (append-only — não reescrevo a linha dela): ela concluiu *"IT5 STALE não avermelha PR (`continue-on-error`; 8/8 `success`)"* medindo o `design-memory-gate.yml`, onde o step **é** advisory. Mas o T7 vive em **outro** workflow: `governance-script-tests.yml` L329 roda `design-memory-gate.test.mjs` **sem** `continue-on-error`, e ele assert que `integrity-check` sai 0. Provado por bite-test (`OIMPRESSO_BENCHMARK_STALE_DIAS=-1` → IT5 FAIL → rc=1 → teste rc=1). É a regra da **LC-14** / §5 2026-07-28: *workflow com job advisory + job que morde exige consultar os dois*. Nenhuma das duas medições estava errada — as duas estavam **incompletas em metades diferentes**.
- **nota:** **6.5/10** — cobertura ~9 (as 8 sondas certas, disparando, com selftest que morde) × execução ~5 (um mês sem medição + um true-positive ignorado 29d + [W] teve que trazer). **Dispara o gatilho §12** (confiança < 7).
...
| 2026-08-08 | **0%** (0 recidiva publicada; 2 quase-LC-08 barrados pela própria medição — "1280px" era **1440** no viewport real, e "tem wrapper" na fonte era **irrelevante** no runtime) | **0** ([W] não pegou erro; o defeito de DS foi pego pelo smoke R1 = verificador) | **queda: −1 classe** — `.os-btn` fora de escopo **não tem defesa que dispare** (ds-guard não cobre, visual-regression não pegou em ~2,5 meses) | 7 (smoke real + controle +/− no runtime; mas o fluxo vivo da Memória **não** foi exercitado — biz=1 sem fatos) | smoke R1 de `/ia/memoria` pós-fatia-D: render ok, console limpo. Achado colateral: os 3 shims deprecados (`Jana`/`Financeiro`/`Ponto` PrimaryButton) emitem `.os-btn`, e no CSS **embarcado** só existem regras `.sells-cowork .os-btn` ⇒ **11 telas** com primary sem `padding`/`radius`/`nowrap`, mascarado pelo `style` inline que aplica só a cor. Vivo desde 2026-05-22. Correção de premissa: IT5 STALE **não** avermelha PR (step `continue-on-error`; 8/8 runs `success`). |
| 2026-08-08 (b) | **0%** (0/1 — meu único erro foi afirmar "15 de 15 runs vermelhas" a partir de `gh run list --limit 15`, amostra truncada; corrigido ampliando pra 100 **antes** de virar registro. Não repete L-NN; é a classe LC-08 do canon pai) | **0 de detecção · 1 de escalação** — as máquinas acusaram sozinhas e certas; o que [W] teve que trazer foi o **vermelho que ninguém lia** | **0 defesa nova** (o conserto foi **subtração**, §5). A classe "advisory vermelho sem ouvinte" segue **sem** defesa — par candidato medido, **não** armado | **6.5** (cobertura ~9 × execução ~5) — **dispara §12** | sessão **paralela** à de cima (ADR 0119), mesmo dia, alvos diferentes: aqui foi auditoria §15/§12 (TESTE-07). IT8 vermelho **29d / 75 runs, 0 verdes** desde 2026-07-10 (última verde 07-03) e é **true-positive**: 3 órfãos, `DS-DOMINIO-RETIRAR-DSV6` sem retorno em `CODE_NOTES` = tarefa invisível. §15 corrigida (4 de 8 linhas + Veredito discordavam do `integrity-check.mjs`). Descartado congelar o baseline do IT8 (§5). §12 passo 3 (subir tier) = **[W]**. **Emenda à "correção de premissa" da linha acima** (append-only, não reescrevo a dela): ela vale pro `design-memory-gate.yml`, onde o step É `continue-on-error` — mas **não** pro `governance-script-tests.yml`, cujo step "Design-memory gate wired no CI" (L329) roda **sem** `continue-on-error` e assert `integrity-check` sair 0 (T7). Bite-test: `OIMPRESSO_BENCHMARK_STALE_DIAS=-1` → IT5 FAIL → `integrity-check` rc=1 → `design-memory-gate.test` rc=1. É a regra da LC-14 / §5 2026-07-28: **workflow com job advisory + job que morde exige consultar os dois**. _(Achado de passagem: `…STALE_DIAS=0` é engolido por `Number(x) \|\| 30` — o override não expressa zero.)_ |
| 2026-09-07 | **0%** L-NN (0 recidiva de design-memory). Meus 2 erros foram do canon PAI e ambos barrados ANTES de virar registro: `\` colapsado em heredoc Python (LC-26, 5ª) e `grep -E "\|"` (P4 do `block-sonda-que-mente`). Declarados, não escondidos — mesmo critério da linha 2026-08-08 (b) | **0** ([W] não pegou erro nenhum; o único defeito publicado — `FJ_GROUPS` — foi pego pelo **CI** = verificador, e o Autofix trouxe) | **0 defesa nova · 1 defesa forte DISPAROU**: o `aplicar-payload` recusou por **hash** o `colors_and_type.css` de 24/08 que o handoff traria por cima do runtime de 02/09 (ADR UI-0031). Downgrade de token barrado sem humano no meio — é o tipo de escape que antes só [W] pegava | **8** (cobertura ~9: as 2 rotas de máquina + 6 gates rodados nesta árvore × execução ~7: **fidelidade visual de tela nenhuma foi medida** — importar é transporte, não comparação) | import do Cowork inteiro de 07/09 ([#6940](https://github.com/wagnerra23/oimpresso.com/pull/6940)): bundle delta +26/~101/-0 sobre a base de 24/08, 49 docs pela rota fiel, **54 recusados** por serem canon de tela (PROTOCOL 10.4). Achado colateral com dono: o ciclo dividiu a Forja em 1 arquivo por tela e `FJ_GROUPS` migrou `forja-page.jsx` → `forja-atomos.jsx` — **`TrabalhoListaTest` UC-TRAB-14 avermelhou, e isso é a âncora funcionando**: o teste lê a FONTE, então a fonte mudar de forma TEM que doer. Conserto = seguir a âncora (conteúdo idêntico: 6 ids na mesma ordem, medido), não afrouxar o regex. Varri a família: 4 leituras reais do espelho em teste — 1 quebrada (corrigida), 1 intacta (`FORJA_ACTORS` segue em `forja-data.jsx`), 1 ponteiro podre em comentário (`PHASES` → `forja-quadro.jsx:33`, corrigido). IT5 do §15 já estava vermelho em `origin/main` **antes** deste PR (31d > 30) — medido em worktree de `origin/main`, não suposto; esta linha é a medição que ele cobra, não um carimbo pra silenciá-lo |
...
| Método/este | `PROCESSO_MEMORIA_CC.md` | `memory/reference/prototipo-ui/PROCESSO_MEMORIA_CC.md` | 2 | **início sempre** |
...
| Register de tela | `<Tela>.decisoes.md` | `prototipo-ui/cowork/Wagner/legado/<tela>/decisoes.md` | 2 (snapshot) | tocar a tela |
...
| IT2 | Todo `*.charter.md` em `resources/js/Pages/**` tem tela viva (`.tsx` irmão) | charter sem tela |
...
**Veredito:** IT **duro** falho → estrutura **comprometida**: parar e consertar antes de evoluir (vira Reestruturação §12 se reincidir). IT advisory falho **reporta, não derruba** — quem é qual, o script diz.
...
- 2026-08-08 · **TESTE-07 — auditoria §15/§12 + o vão do ouvinte:** sessão de integridade rodada com o IT5 já vermelho há 31d. As 8 sondas estavam **certas e dispararam**; o que faltou foi **reação** — IT8 acumulou **29 dias / 75 runs vermelhas sem uma verde** (3 órfãos reais, `DS-DOMINIO-RETIRAR-DSV6` = tarefa invisível sem retorno do [CL]) e quem trouxe foi [W], não máquina. Corrigida a §15, que **descrevia errado o próprio script** em 4 de 8 linhas + Veredito (a única linha intacta era a que apontava pro dono — §5 entrada 1). §5 sai do vácuo com 2 entradas; a 2ª barra congelar o baseline do IT8 pra "devolver o job ao verde". **Gatilho §12 disparado** (confiança 6.5 < 7): passos 1/2/4 executados aqui; **passo 3 (subir defesa um tier) é [W]** — promover gate advisory a required exige mordida provada (ADR 0271/0275/0336) e vigia de "advisory vermelho persistente" exige FP medido antes. Disposição dos 3 órfãos = [W]/[CL]. **Sessão paralela no mesmo dia** (ADR 0119): o [#5433](https://github.com/wagnerra23/oimpresso.com/pull/5433) logou a própria linha do §11 e destravou o IT5 **antes** deste PR — as duas coexistem (esta é a `(b)`), e o que sobra de único aqui é a **auditoria**, não o destravamento. Emendada (append-only) a premissa "IT5 STALE não avermelha PR": vale pro `design-memory-gate.yml`, não pro `governance-script-tests.yml` (T7, sem `continue-on-error`) — bite-test com `…STALE_DIAS=-1`.
