...
PR não foi aberto. Os 5 .tsx foram salvos como pinos F1 históricos em `prototipo-ui/cowork/Wagner/legado/financeiro-{tela}/page.tsx`. Os 5 controllers foram descartados.
...
**Como evitar:** se a tela já está em prod ([`memory/reference/prototipo-ui/TELAS_REVIEW_QUEUE.md`](TELAS_REVIEW_QUEUE.md) marca status `[x]`), **JAMAIS sobrescrever Controller existente**. F3 entrega Controllers de telas **novas**. Refator visual de tela existente = PR separado, baseado no Controller real (não no zero).
...
- [ ] Tela já em prod? `memory/reference/prototipo-ui/TELAS_REVIEW_QUEUE.md` marca `[x]` → não sobrescrever Controller, só refator visual em PR separado
