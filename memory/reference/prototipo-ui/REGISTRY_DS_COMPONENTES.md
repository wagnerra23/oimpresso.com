...
> fábrica) vivem em [`ds-v6/showcase.html`](../../../prototipo-ui/cowork/Wagner/legado/ds-v6/showcase.html). O **reuse-mapping** "kit `c-*` → componente
> React no repo" está em ``ds-v6/REUSE_MAPPING.md`` (arquivo histórico removido) — 8/11 já reusam o que está
> abaixo; 3 (`c-id` ficha-360 · `c-tl` unificada · `c-nba`) são buraco do DS (Tier-0, nascem na 1ª tela que
> os consome). A **receita** de como montar tela do kit: [`ds-v6/receita.html`](../../../prototipo-ui/cowork/Wagner/legado/ds-v6/receita.html).
...
| Componente | Import | Existe | Substitui (anti-pattern) |
|---|---|---|---|
| **Command** (motor) + **Popover** (shell) | `@/Components/ui/command` + `@/Components/ui/popover` | ✅ canon | qualquer **`<input role="combobox">`** / **`aria-autocomplete`** + `<ul role="listbox">` hand-rolado na tela |

> **Estado da migração (2026-07-15):** **3 de 5 migradas** e **testadas em prod** (screenshot: abrir·buscar·selecionar) — `PlanoContaCombobox` (Financeiro, `Popover`+`Command`, [#4302](https://github.com/wagnerra23/oimpresso.com/pull/4302)) · `GradeProductCombobox` (Purchase, `Command` inline async, [#4303](https://github.com/wagnerra23/oimpresso.com/pull/4303)) · `ClienteCombobox` (Financeiro, `Command` inline async+texto-livre, [#4304](https://github.com/wagnerra23/oimpresso.com/pull/4304)). **2 ficam como RESÍDUO INTENCIONAL** (decisão [W] 2026-07-15, NÃO force-migrar): `CustomerSearchAutocomplete` e `ProductSearchAutocomplete` (ambas em `Sells/_components`) — são **Tier 0 valor/estoque** (o `CustomerSearchAutocomplete` **entrega** o grupo de preço no `onSelect` e o parent `Sells/Create.tsx:500` reaplica — migrar o componente arrisca esse contrato; o `ProductSearchAutocomplete` adiciona item à venda), têm **hotfix do sufixo** (bug "Cliente padrãowagner", Wagner 2026-05-27) que o modelo controlado do `CommandInput` conflita, e a UX `selectedLabel`-no-input não mapeia limpo. O detector `--roles` segue listando-as como independentes (drift honesto); só migram numa sessão dedicada, com aceite Tier 0 + smoke pesado em prod. Ver `memory/proibicoes.md §5`.
