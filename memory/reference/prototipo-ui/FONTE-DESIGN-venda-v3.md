...
| Por que aqui e não em `design-oimpresso/` | o gate **required** `Ancora de design nao-shell` resolve `related_prototype` **dentro de `prototipo-ui/cowork/Wagner/`** (`anchor-content-check.mjs::anchorRelPath` corta o path até `cowork/`). Uma âncora fora daí classifica como `MISSING` e derruba o gate. |
