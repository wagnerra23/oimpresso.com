...
### ✅ Sincronizado
- `memory/reference/prototipo-ui/STATUS.md` — espinha viva (fiel ao Cowork; ver ⚠️ drift DS abaixo)
- `memory/reference/prototipo-ui/CARTA_DESIGN_CC.md` — carta subordinada (ref ADR ajustada → 0247)
...
`readme_handoff_block_missing` flag um **gap real**: o `memory/reference/prototipo-ui/README.md` canônico **não tem** o marcador `<!-- HANDOFF-ENTRY -->` que o STATUS.md (L-18) exige. Sem ele, um Share→Handoff entrega o projeto mas o Code não acha a fila. **Fix é conteúdo do loop [CC]** — sugiro [CC] regenerar o README preservando o bloco "🤖 Claude Code — COMECE AQUI" + o marcador.
...
|---|---|---|
| Método/raiz do processo | `memory/reference/prototipo-ui/PROCESSO_MEMORIA_CC.md` | criado (vizinho de `PROTOCOL.md`/`CLAUDE_DESIGN_BRIEFING.md`) |
| Charter Produção/Oficina | `prototipo-ui/cowork/Wagner/legado/producao-oficina/charter.md` | criado (convenção pasta-por-tela, ex `clientes/`) |
| Register Produção/Oficina | `prototipo-ui/cowork/Wagner/legado/producao-oficina/decisoes.md` | criado (irmão do charter → IT2 do PROCESSO passa) |
...
### Charter atualizado: charters Vendas/Compras mirrorados em `prototipo-ui/cowork/Wagner/legado/`
...
[CL] commitou **verbatim** o charter `memory/reference/prototipo-ui/CHARTER_GOVERNANCA_CC_JANA.md` (export Cowork, irmão de `CHARTER_GOVERNANCA_W.md` + `CHARTER_CHAMPION_AGENTES.md`). Espelha pro git a **conclusão** do report `rep-cc-vs-jana` (`metricas.html`) — pro raciocínio rumo a 9.7 ficar durável e re-derivável do `main`.
...
Dos 2 docs de Casos de Uso (Vendas UC-V/R/C · Oficina UC-01..10) gerei, **só nas telas canon**, `PRECISA TER` na charter + GUARD Pest `uc-<id>` + check `protocol_freshness` (advisory no health-check, espelha `review-freshness` #2078, ratchet baseline). Fonte única: `scripts/design/audit/uc-registry.json`.
...
Fecha o "primeiro hop" do loop zero-paste: um `.md` em `prototipo-ui/cowork/Wagner/handoffs/` é assinado por uma Action e vira `pending` via tool MCP, sem o [W] colar nada nem computar HMAC. Auditei contra o `main` fresco (§10.4): a Fase 0/1 já estava no `main` — faltava só o que **assina e dispara**.
...
- `ds-v6/tokens.css` **aposentado/deletado** → git mirror `prototipo-ui/cowork/Wagner/legado/ds-v6/` ainda tem **8 arquivos**; DS vivo `019dd02f` não tem ds-v6 (é do ERP `019dcfd3`, sem acesso de escrita/listagem meu).
...
| `scripts/design/audit/GOLDEN-REFERENCE.md` | *"AP1–AP10 … (Camada 4 · KB-9.75)"* |
...
- Landa `memory/reference/prototipo-ui/METODO-KB-9.75.html` **verbatim** — corpo não reescrito
...
| Hanken Grotesk | `prototipo-ui/cowork/Wagner/legado/ds-v6/tokens.css` (`--sans`) | ❌ **zero** ocorrências fora do kit — nunca entrou no repo |
...
**Não editei `prototipo-ui/cowork/Wagner/legado/ds-v6/tokens.css`** — é export do Cowork, e a regra de ouro do
...
`prototipo-ui/cowork/Wagner/**` é espelho de leitura (ADR 0374) e edição minha some no próximo
...
Como sempre: não editei `prototipo-ui/cowork/Wagner/**` pra "consertar" nada — é espelho de leitura
...
- **§8** afirma "17 arquivos `forja-*.jsx` + `forja-page.css` no `prototipo-ui/cowork/Wagner/`". O espelho
...
- **Repo:** `ancora.mjs` nas 4 telas → `n/a` classificado como *"declaração legítima — a tela nasce
  do DS"*. Nenhuma âncora do repo aponta para fora de `prototipo-ui/cowork/Wagner/` (medido: 88 `n/a` +
...
- **Não** criei protótipo `.jsx` em `prototipo-ui/cowork/Wagner/legado/<novo>`: o `cowork-ssot-guard` R3
  reprova (exit 1) fora do allowlist, cuja meta declarada é **zero**.
- **Não** escrevi no espelho `prototipo-ui/cowork/Wagner/`: é build-only do vivo (ADR 0374) — arquivo
