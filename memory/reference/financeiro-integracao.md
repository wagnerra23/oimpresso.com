...
**Status (2026-04-24):** spec-ready em `memory/requisitos/Financeiro/` (score audit 85+/100), scaffold criado em `Modules/Financeiro/` (Enabled, vazio), Onda 1 ainda não começou.
...
1. **Hooks DataController** (`Modules/Financeiro/Http/Controllers/DataController.php`):
   ...
   - `superadmin_package()` — Free/Pro R$ [redacted Tier 0]/Enterprise R$ [redacted Tier 0] + take rate 0,5%
...
- **`format_date()` mantém shift +3h INTENCIONAL** (ADR 0066); Larissa decorou horários históricos. NÃO resetar.
- Pré-popular form com `format_now_local()` (sem shift); `format_date()` é só para dados decorados pelo cliente.
- **`session('business.time_zone')` retorna null** (Eloquent dot-notation falha). Usar `session('business_timezone')`.
- **`disabled=false` no Form shim é OMITIDO automaticamente**; checar antes de "fix".
- **Toda tabela usa `language: { url: asset('locale/datatables/pt-BR.json') }`.**
...
- Ver schema DB + ultimatepos-integracao.md
