...
- [feedback-cowork-sync-now-prompt-stale.md](feedback-cowork-sync-now-prompt-stale.md) — **Sync now no Claude Design = comando digitado** (`sync now`+Enter, não botão); prompt do Cowork pós-sync pode vir STALE → revisar contra git antes de executar (caso 2026-05-30: mandava re-numerar ADR 0238 já existente + renomear colisões 0235/0236 = violaria append-only Tier 0)
- [feedback-commits-delphi-svn.md](feedback-commits-delphi-svn.md) — **Wagner 2026-05-27 (revisada):** `D:\Programas\` (SVN compartilhada time inteiro, `wr2.com.br:8777`) é **READ-ONLY pro Claude**. NÃO comita SVN Delphi — princípio 4 Constituição v2 (Loop fechado por métrica) + Wagner não recompila/redistribui .exe ([ADR 0113](../decisions/0113-integracao-delphi-laravel-ads-3-caminhos.md)). `svn.exe` (SlikSvn) instalado só pra leitura (info/log/status/diff/blame). Wagner comita manual via TortoiseSVN quando decidir recompilar.
- [feedback-claude-worktree-isolation.md](feedback-claude-worktree-isolation.md) — **Wagner 2026-05-27:** Claude edita canon do oimpresso via **atomic Bash chain** (stash + checkout main + edit + commit + push + checkout original + stash pop em sequência mínima com branch guard). Wagner trabalha em paralelo trocando branches; 3 incidentes documentados nesta sessão (commits caíram em fix/cliente-drawer + chore/whatsapp-post-merge-cleanup). Pegadinha: `.claude/worktrees/` é gitignored — worktrees ad-hoc do harness não servem como isolation real. Worktree REAL fora de `.claude/worktrees/` resolve (ex `D:/oimpresso-claude-canon`).
