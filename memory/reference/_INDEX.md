...
- [feedback-habilitar-modulo-por-business.md](feedback-habilitar-modulo-por-business.md) — **Tier 0 IRREVOGÁVEL** (lado a lado com `business_id` global scope em [proibicoes.md](../proibicoes.md)). **2 CAMADAS** canônicas pra esconder/liberar módulo por business: (1) `/superadmin/packages/{id}/edit` pra módulos nWidart `X_module` — 24+ chaves listadas; (2) `/business/settings` pra módulos core `enabled_modules` — 13 keys listadas (pegadinha: sem switch business, só user da própria biz edita; 3 caminhos pra contornar). Checklist Wagner recorrente + pattern criar gate quando módulo nWidart não tem. NUNCA `if ($business_id === N) return`. Wagner regra 2026-05-18.
