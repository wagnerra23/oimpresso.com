...
- [feedback-commits-delphi-svn.md](feedback-commits-delphi-svn.md) — **Wagner 2026-05-27:** após cada tarefa em `D:\Programas\` (SVN working copy Delphi WR Comercial), commit SVN automático em PT-BR pra histórico granular. svn.exe via SlikSvn em `C:\Program Files\SlikSvn\bin\`. Atenção publication-policy: commit SVN = visível IMEDIATAMENTE no servidor central (sem stage local).
