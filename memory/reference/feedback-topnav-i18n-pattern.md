...
2. **Chave i18n**: `'label' => 'jana::jana.menu.dashboard'` — `LegacyMenuAdapter::resolveLabel()` em `app/Services/LegacyMenuAdapter.php#180` chama `trans()` se a string contém `::`
...
- Pra fallback gracioso: se a chave não existe, `trans()` retorna a chave bruta — o menu vai mostrar "jana::jana.menu.dashboard" literal, o que é feio mas não quebra
