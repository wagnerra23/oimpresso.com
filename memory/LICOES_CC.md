...
- **Regra:** **antes de qualquer "sync"/ponte pro Code, ler o estado EFETIVO no git** — `memory/reference/prototipo-ui/SYNC_LOG.md` + `CODE_NOTES.md` + `PROTOCOL.md` no `main` (não a memória local, que predata). Se o `SYNC_LOG`/`CODE_NOTES` recente já mostra a fila `[PROCESSADO]`/devolvida, **não re-sincronizar**. Ingerir o retorno do [CL] e atualizar a espinha local **antes** de propor. A memória local é cache, o git é a verdade (§10.3: o que não está commitado é invisível; o que ESTÁ commitado manda).
- **Ref:** sessão 2026-05-31-reanalise-protocolo; `memory/reference/prototipo-ui/SYNC_LOG.md` (00:45 shift + 07:00 processamento); `memory/reference/prototipo-ui/CODE_NOTES.md` (veredito §10.4); `AUTOMACAO-LOOP-AUTONOMO.md`; PROTOCOL §2 overlay + §10.3/§10.4; L-09/L-11.
...
## L-28 — Medir foco por foco-PROGRAMÁTICO + `:focus-visible` = gate CEGO (foco de script não dispara :focus-visible)
- **Erro (2026-07-20):** ao mecanizar G15 (foco visível, WCAG 2.4.7) no `qa-conformance.js`, a 1ª versão focava cada controle via `.focus()` e lia `getComputedStyle`, guardando por `el.matches(':focus-visible')`. No engine do smoke o **foco programático não dispara `:focus-visible` nem em input de texto** → todo controle caía em "indeterminável", `measured=0`, e o controle-negativo deu **`CEGO`** (o gate não conseguia ficar vermelho). Suposição errada: "Chromium dispara :focus-visible em inputs no foco de script".
- **Sintoma:** gate de a11y que reporta só `⬜/na`/indeterminável em toda tela; o `.negative()` do gate diz `CEGO` (não `discrimina`). Teatro — gate que não pode reprovar (família §5 "verde que não fica vermelho").
- **Regra:** foco visível **NÃO** se mede focando-e-lendo (`:focus-visible` depende de heurística de entrada por teclado, não reproduzível por `.focus()`). Medir por **rule-scan determinístico** (varrer regras `:focus`/`:focus-visible`, detectar o anti-padrão `outline:none` sem repor indicador) — mesma metodologia de G3/G7. E **provar com o controle-negativo ANTES de declarar pronto**: foi ele que pegou o gate cego, não a leitura do código (anti-§2: recibo, não afirmação).
- **Ref:** `prototipo-ui/cowork/Wagner/qa-conformance.js` (G14 contraste AA + G15 foco, v2.5); smoke datado 2026-07-20 (harness `qa-test-root`: G14 flagou `1.92:1`, G15 rule-scan `pass>bug:fail>pass`). Origem: adversário refutou o doc "loop de conformidade do Claude Design" (§2 sem recibo, citação Pix2Fact fabricada) — G14/G15 são o conserto do único gap real (a11y).
