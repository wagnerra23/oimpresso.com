...
**IA:** sem pacote IA instalado em `composer.lock` (`openai-php/laravel` foi removido). Drivers do Copiloto ainda referenciam `OpenAI\Laravel\Facades\OpenAI` em [Modules/Copiloto/Services/Ai/OpenAiDirectDriver.php](Modules/Copiloto/Services/Ai/OpenAiDirectDriver.php), portanto **Copiloto só roda em `COPILOTO_AI_DRY_RUN=true`** (devolve fixtures). **Stack-alvo (decidida 2026-04-26 sessão 16, formalizada em ADR 0031 + 0032):** **Prism PHP** (wrapper LLM) + **Vizra ADK** (framework de agente) + **Mem0 REST** (memória especializada). Roadmap 7 sprints — sprint 1 = Prism swap (resolve dry_run); sprints 2-3 = Vizra ADK (sessions/tools/traces); sprints 4-5 = Mem0 REST (memória semântica Tier 6-7); sprint 6 = Vizra Cloud opcional. Comparativo completo: [memory/comparativos/stack_agente_php_vizra_prism_mem0_capterra_2026_04_26.md](memory/comparativos/stack_agente_php_vizra_prism_mem0_capterra_2026_04_26.md).
