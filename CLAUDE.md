...
- **Acesso ao design (v2) — Code É o designer-agente** ([ADR 0282](memory/decisions/0282-protocolo-v2-colapso-ratificacao.md) + [PROTOCOL §0.1](prototipo-ui/PROTOCOL.md)): tenho acesso **completo** ao design por 3 vias — protótipo Cowork no repo · tool `DesignSync` (projeto claude.ai/design fresco) · plugin Claude Design. Falta a fonte visual de uma tela? **GERO** (ancorado no DS canon) + valido por CI (visual-regression + PR UI Judge) — **NUNCA** "precisa vir do Cowork / esperar handoff / me autorize a desenhar". Soberania [W] Tier 0 = merge · produto · token/componente novo, não "posso desenhar". _(Origem: incidente 2026-07-15 wizard cartão Financeiro — o corpo v1 do PROTOCOL §1 venceu a leitura e me travei; §0.1 + esta linha resolvem.)_
