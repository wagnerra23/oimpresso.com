...
//   da tela de venda (Sells/Create via NumericInputPtBR). Já causou o Bug R$ [redacted Tier 0]k da
//   Larissa @ Rota Livre (biz=4, 2026-05-27): ela digitou R$ [redacted Tier 0] e o sistema gravou
//   R$ [redacted Tier 0] A correção foi este parser — mas até hoje rodava SEM teste algum.
...
describe('parseDecimalPtBR — REGRESSÃO Bug R$ [redacted Tier 0]k Larissa (2026-05-27) [NÃO REMOVER]', () => {
  ...
  it('"25" → 25 (R$ [redacted Tier 0] não vinte e cinco mil)', () => {
...
describe('parseDecimalPtBR — heurística do ponto (decimal en-US vs milhar pt-BR)', () => {
  it.each([
    ...
    ['3.50', 3.5], // 2 dígitos → decimal (R$ [redacted Tier 0] digitado com ponto)
    ['1.234', 1234], // 3 dígitos → milhar
    ['3.500', 3500], // 3 dígitos → milhar (R$ [redacted Tier 0] = três mil e quinhentos)
...
describe('round-trip — parse(format(x)) preserva valor (foco→blur não corrompe)', () => {
  ...
  // um total de R$ [redacted Tier 0] que volta pro input não pode virar outro número.
