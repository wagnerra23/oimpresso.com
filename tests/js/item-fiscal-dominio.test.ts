...
describe('NCM — 8 dígitos, obrigatório', () => {
  ...
  it('exige preenchimento e DIZ quantos dígitos faltam', () => {
...
describe('CFOP — 4 dígitos e o primeiro tem significado', () => {
  it('aceita entradas (1/2/3) e saídas (5/6/7)', () => {
    for (const cfop of ['1102', '2102', '3102', '5102', '6102', '7102']) {
      expect(validarCfop(cfop)).toBeNull();
    }
  });

  it('recusa primeiro dígito que não existe na tabela', () => {
  ...
  it('exige preenchimento e comprimento exato', () => {
...
describe('campos opcionais — vazio passa, preenchido é conferido', () => {
  ...
  it('cBenef: 2 letras da UF + 6 dígitos', () => {
...
describe('alíquota e redução — faixa 0..100', () => {
  ...
  it('recusa negativo e acima de 100', () => {
...
describe('COERÊNCIA CST × alíquota — o erro que o formato NÃO pega', () => {
  it('CST que declara não haver imposto não admite alíquota', () => {
  ...
  it('os mesmos CSTs passam quando a alíquota é zero', () => {
    for (const cst of CST_SEM_ALIQUOTA) {
      expect(erroDeCoerencia(`${cst} — algo`, '0')).toBeNull();
    }
  });

  it('CST 00 é tributado integralmente — alíquota zero é contradição no sentido oposto', () => {
  ...
  it('CST 102 (Simples) tem TRÊS dígitos e não pode ser lido como "10"', () => {
  ...
  it('a validação de FORMATO sozinha aprovaria o par incoerente — é por isso que a coerência existe', () => {
...
describe('errosFiscais — a aba inteira, em lista', () => {
  ...
  it('item corretamente preenchido não acusa nada', () => {
  ...
  it('pega a incoerência mesmo com todo o resto correto', () => {
