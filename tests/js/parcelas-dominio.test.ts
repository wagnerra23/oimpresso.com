...
describe('ratear — o invariante do centavo (Tier 0 · valor)', () => {
  it('UC-V330 · caminho A × caminho B: a soma bate com o total, para 1..48 parcelas', () => {
  ...
  it('UC-V331 · o caso que dá nome ao problema: 100 em 3 não perde o centavo', () => {
  ...
  it('UC-V330 · o resto vai para as PRIMEIRAS parcelas, uma unidade em cada', () => {
...
describe('quantidadeSaneada — entrada de texto não vira NaN nem laço infinito', () => {
  it('UC-V333 · texto lixo e vazio caem em 1', () => {
  ...
  it('UC-V333 · respeita o teto de 48', () => {
...
describe('soma e diferença — o que a tela usa para liberar o Confirmar', () => {
  it('UC-V334 · fecha no total quando as parcelas somam o total', () => {
  ...
  it('UC-V335 · não fecha quando falta ou sobra centavo — e diz de que lado', () => {
  ...
  it('UC-V337 · parse pt-BR: separador de milhar não infla o valor (guard do incidente num_uf)', () => {
...
describe('mesmoDiaNoMes — "vence no mesmo dia de cada mês" ≠ somar 30 dias', () => {
  it('UC-V332 · dia 31 é grampeado no último dia do mês curto, sem virar o mês seguinte', () => {
  ...
  it('UC-V332 · dia que existe em todo mês anda sem surpresa', () => {
    const base = new Date(2026, 0, 15);
    const mar = mesmoDiaNoMes(base, 2);
    expect([mar.getMonth(), mar.getDate()]).toEqual([2, 15]);
  });

  it('UC-V332 · contraste real com somar 30 dias: em 31/01 os dois métodos caem em MESES diferentes', () => {
...
describe('venceuAntesDeHoje — comparação por dia, não por instante', () => {
  it('UC-V336 · vencimento de hoje NÃO está vencido, mesmo com hora diferente', () => {
    const hojeTarde = new Date();
    hojeTarde.setHours(23, 59, 0, 0);
    expect(venceuAntesDeHoje(hojeTarde)).toBe(false);
  });

  it('UC-V336 · ontem está vencido; amanhã não', () => {
