...
it('com ctx Larissa, instructions inclui empresa + faturamento + cliente + metas', function () {
    ...
    expect($prompt)->toContain('2026-02: R$ [redacted Tier 0]');
    expect($prompt)->toContain('2026-03: R$ [redacted Tier 0]');
    expect($prompt)->toContain('2026-04: R$ [redacted Tier 0]');
...
it('com ctx + memoria recall, ambos aparecem (ordem: base → ctx → memoria)', function () {
    ...
    expect($prompt)->toContain('Larissa quer R$ [redacted Tier 0]mil/mês');
});
