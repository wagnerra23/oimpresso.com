...
| ~~A1~~ | ✅ | W | ~~A4 rodada 2: Validar Larissa~~ — 29-abr 09:06-09:25 chat real prod: 3 perguntas → 3 respostas distintas (caixa R$ [redacted Tier 0] ≠ bruto R$ [redacted Tier 0] ≠ líquido R$ [redacted Tier 0] abril) | sex 02-mai | 0.5 |
...
| ~~A1~~ | ✅ | W | ~~MEM-MCP-1.c: 5 tools + 2 Resources + 1 Prompt~~ — protocol vivo em prod (`c4dcb210`); 108 docs sincronizados; tools/list, tools/call, resources/list testados em prod com dados reais (decisions-search "ContextoNegocio" → 3 ADRs via FULLTEXT MySQL) | sex 02-mai | 1 |
| ~~A1~~ | ✅ | W | ~~MEM-MCP-1.d: RBAC Spatie + filtro scope~~ — McpScopesSeeder com 8 perms `copiloto.mcp.*` (commit `333f3ffe`); Wagner com Admin#1 ganhou todas; gate `copiloto.mcp.use` no McpAuthMiddleware retorna 403 + audit no_permission. Granularidade fina nos Tools (próx sprint) | seg 05-mai | 1 |
| ~~A2~~ | ✅ | W | ~~MEM-MCP-1.c-perf: Otimizar latência~~ — Octane+FrankenPHP em CT 100 (commits `6400fd0e`+`d8f17e56`); 60s+ timeouts → 1.5s warm; CT 100 32GB/16c via Tailscale Proxmox; Traefik fix maxIdleConnsPerHost 2→500 (commit `492e6aef`) eliminou 9/20 timeouts; bench final 20 paralelas externas: 20/20 ✅ em 3.2s wall total | seg 05-mai | 0.5 |
| ⏳ | TODO | W | **MEM-MCP-1.d-fine: granularidade fina por Tool** — DecisionsSearchTool checa `copiloto.mcp.decisions.read`, ClaudeCodeUsageSelfTool checa `copiloto.mcp.usage.self` etc. Hoje só gate único `copiloto.mcp.use` no middleware | qua 07-mai | 0.5 |
| ~~MEM-MCP-1.e~~ | ✅ | W | ~~Dashboard /copiloto/admin/governanca~~ — GovernancaService + Inertia page (commits `bebc1486`+`532f2156`) com 4 KPIs, latency p50/p95/p99, top tools, top users, denied por error_code, série diária; gate `copiloto.mcp.usage.all`; bug fix audit middleware (extrai params.name do JSON-RPC body) | sex 09-mai | 1 |
| ~~COP-CHAT-STREAM~~ | ✅ | W | ~~Streaming SSE chat~~ — endpoint `/copiloto/conversas/{id}/mensagens/stream` (commit `9fcf0e80`); bypass Vizra com laravel/ai Agent::stream (commit `d3cba46c`); TTFB 1.8s, 26 chunks/resposta token-por-token | qua 07-mai | 0.5 |
| ~~MEM-CHAT-ENT-1~~ | ✅ | W | ~~Chat enterprise via assistant-ui~~ — @assistant-ui/react integrado (commits `b1e2f67d`/`06ae8644`/`f7dd2af7`); markdown render + code highlight + welcome com 4 sugestões + Stop button + auto-scroll; smoke prod via Chrome OK (POST stream 200 + bolha CP renderizando markdown) | qua 07-mai | 1 |
| ⏳ | TODO | W | **MEM-CHAT-ENT-2: polish UI Copiloto** — avatar do user real (vez de "CP"), separadores de dia (Hoje/Ontem/12-abril), indicador "Copiloto está pensando..." pré-primeiro-chunk, sugestões iniciais por business | qui 08-mai | 0.5 |
| ⏳ | TODO | W | **MEM-CHAT-ENT-3: edit + regenerate** — UX edit user message (lib já tem MessagePrimitive.EditComposer); botão Regenerate na bolha assistant; ambos re-rodam `/mensagens/stream` | sex 09-mai | 1 |
| ⏳ | TODO | W | **MEM-CHAT-ENT-4: anexar imagem (vision)** — drag-drop file na composer; upload via Hostinger storage; payload OpenAI com `image_url`; suporta GPT-4o vision pra análise de fotos de NF, planilhas, dashboards | seg 12-mai | 1.5 |
| ⏳ | TODO | W | **MEM-CHAT-ENT-5: anexar PDF/Excel** — extração via barryvdh/laravel-dompdf inverso (PDF→texto) + maatwebsite/excel; envia conteúdo extraído como contexto adicional ao LLM | ter 13-mai | 1 |
| ⏳ | TODO | W | **MEM-GAP-1: Knowledge base por project com upload UI** — equivalente Anthropic Projects: tela /copiloto/admin/knowledge com upload PDF/MD/CSV, dedup SHA256 + indexação Meilisearch, "can-use vs can-edit" permissions | qua 14-mai | 2 |
| ⏳ | TODO | W | **MEM-GAP-2: Claude Projects shared (UI gerencia knowledge)** — interface read/write pra editar memory/decisions/ADRs sem precisar git push (Wagner edita inline + commit auto) | qui 15-mai | 1.5 |
| ⏳ | TODO | W | **MEM-GAP-3: File access restrictions** — Skills/Tools podem ter allowlist de paths que podem Read/Edit; bloqueio em runtime via middleware MCP | sex 16-mai | 1 |
| ⏳ | TODO | W | **MEM-GAP-4: Centralized policy enforcement** — settings.json server-managed (Anthropic Team v2.1.38+ feature): admin push policy → Claude Code do dev pega no fetch | seg 19-mai | 2 |
