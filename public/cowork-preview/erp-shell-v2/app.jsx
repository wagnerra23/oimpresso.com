...
function Header({ company, route, onSelectRoute, prodType, onProdType, chatTab, onChatTab, recTab, onRecTab }) {
  ...
  const isRecurring = route === "recurring";
  // Financeiro Cockpit V2 (R#1.a) gerencia sua própria navegação na col esquerda —
  // remove a tab strip L1 de sibling sub-módulos. Sub-módulos contínuam acessíveis
  // pela sidebar e por ⌘K.
  const isFinanceiroCockpit = route === "financeiro" || route === "fluxo" || route === "dre";
  const PROD_TYPES = [
  ...
  const REC_TABS = [
    { key: "assinaturas", label: "Assinaturas", color: "oklch(0.42 0.13 295)" },
    { key: "planos",      label: "Planos",      color: "oklch(0.45 0.13 250)" },
    { key: "faturas",     label: "Faturas",     color: "oklch(0.45 0.13 60)"  },
    { key: "config",      label: "Configurações", color: "oklch(0.50 0.02 80)" },
  ];
    ...
    <header className="topbar">
      <div className="topbar-l">
        ...
        <span className="topbar-area-label">
          {isProdutos ? "PRODUTOS" : isChat ? "JANA" : isRecurring ? "RECORRÊNCIA" : areaLabel}
        </span>
      </div>
      {isProdutos ? (
        ...
        <nav className="topbar-tabs" aria-label="Cobrança recorrente">
          {REC_TABS.map(t => {
            const active = (recTab || "assinaturas") === t.key;
            return (
              <button key={t.key}
                      className={"topbar-tab topbar-tab--rec" + (active ? " active" : "")}
                      onClick={() => onRecTab?.(t.key)}
                      style={active ? {borderBottomColor: t.color, color: t.color} : null}>
                <span>{t.label}</span>
              </button>
            );
        ...
        <div className="topbar-tabs topbar-tabs-empty" aria-hidden="true"/>
      ) : group ? (
...
// FinSidePage: wrapper minimalista para Fluxo e DRE quando acessados via sidebar
function FinSidePage({ Body, title, subtitle }) {
  if (!Body) return null;
  return (
    <div className="os-page fin-root fin-cockpit" data-screen-label={title}>
...
function App() {
  ...
  // Sub-rota da Cobrança Recorrente (Assinaturas | Planos | Faturas | Configurações)
  const [recTab, setRecTab] = useStateA(() => {
    try { return localStorage.getItem("oimpresso.rec.tab") || "assinaturas"; } catch (e) { return "assinaturas"; }
  });
  useEffectA(() => {
    try { localStorage.setItem("oimpresso.rec.tab", recTab); } catch (e) {}
  }, [recTab]);
  useEffectA(() => {
  ...
  // ─── Aplica tweaks do Financeiro no DOM (após render do FinanceiroPage) ───
  useEffectA(() => {
    const apply = () => {
      const el = document.querySelector(".fin-cockpit");
      if (!el) return;
      el.setAttribute("data-fin-refine", tweaks.finRefine ? "on" : "off");
      el.setAttribute("data-fin-hero",   tweaks.finHero || "default");
      el.setAttribute("data-fin-tom",    tweaks.finTom  || "neutral");
      el.setAttribute("data-fin-kb",     tweaks.finKb ? "on" : "off");
      el.setAttribute("data-print-date", new Date().toLocaleString("pt-BR"));
      // Densidade · seta as vars CSS que o fin-polish lê pra altura/padding da linha
      const d = tweaks.finDensity;
      const map = {
        compact:     { row: "30px", py: "5px" },
        comfortable: { row: "38px", py: "8px" },
        spacious:    { row: "48px", py: "12px" },
      };
      const v = map[d] || map.comfortable;
      el.style.setProperty("--finp-density-row", v.row);
      el.style.setProperty("--finp-density-py",  v.py);
    };
    apply();
    // Reaplica sempre que a rota muda (FinanceiroPage pode re-mountar)
    const t = setInterval(apply, 600);
    return () => clearInterval(t);
  }, [tweaks.finRefine, tweaks.finHero, tweaks.finTom, tweaks.finDensity, tweaks.finKb, route]);
  ...
  else if (route === "financeiro") content = <window.FinanceiroPage/>;
  else if (route === "fluxo")      content = <FinSidePage Body={window.TelaFluxo} title="Fluxo de caixa" subtitle="Maio 2026 · ROTA LIVRE"/>;
  else if (route === "dre")        content = <FinSidePage Body={window.TelaDRE}   title="DRE / Relatórios"  subtitle="Maio 2026 · ROTA LIVRE"/>;
  ...
  else if (route === "whatsapp") content = <window.WhatsappPage/>;
  else if (route === "recurring") content = <window.RecurringPage tab={recTab} onTab={setRecTab}/>;
  else if (route === "fiscal")    content = <window.FiscalCockpit onNavigate={handleSelectRoute}/>;
  else if (route === "nfe")       content = <window.FiscalNFePage/>;
  else if (route === "nfse")      content = <window.FiscalNFSePage/>;
  else if (route === "dfe")       content = <window.FiscalDFePage/>;
  else if (route === "fiscal_eventos") content = <window.FiscalEventosPage/>;
  else if (route === "fiscal_config")  content = <window.FiscalConfigPage/>;
  else if (route === "sped")      content = <window.FiscalSpedPage/>;
  else if ([
    "crm", "whatsapp", "cv", "relatorios", "assets",
    "catalogue", "brief", "repair", "manufacturing", "ads", "projects", "vestuario",
    "portalos", "auditoria"
  ].includes(route)) content = <window.MockupPage route={route}/>;
  ...
  return (
    <div className={"app app--sb-" + sbMode}>
      ...
      <div className="main">
        {route !== "recurring" && route !== "financeiro" && route !== "fluxo" && route !== "dre"
          && route !== "fiscal" && route !== "nfe" && route !== "nfse" && route !== "dfe"
          && route !== "fiscal_eventos" && route !== "fiscal_config" && route !== "sped" && (
          <Header company={company} route={route} onSelectRoute={handleSelectRoute}
                  ...
                  recTab={recTab} onRecTab={setRecTab}/>
        )}
      ...
      <TweaksPanel title="Tweaks">
        ...
        <TweakSection label="Financeiro · refino visual"/>
        <TweakToggle
          label="Polimento ligado"
          value={tweaks.finRefine}
          onChange={(v) => setTweak("finRefine", v)}/>
        <TweakRadio
          label="Densidade da linha"
          value={tweaks.finDensity}
          options={["compact","comfortable","spacious"]}
          onChange={(v) => setTweak("finDensity", v)}/>
        <TweakSelect
          label="Hero (saldo previsto)"
          value={tweaks.finHero}
          options={[
            { value: "default", label: "Mercury (padrão)" },
            { value: "compact", label: "Compacto · 1 linha" },
            { value: "cards",   label: "4 mini-cards" },
          ]}
          onChange={(v) => setTweak("finHero", v)}/>
        <TweakRadio
          label="Tom"
          value={tweaks.finTom}
          options={["warm","neutral","cool"]}
          onChange={(v) => setTweak("finTom", v)}/>
        <TweakToggle
          label="Seeds KB-9.75 (R#2–#4)"
          value={tweaks.finKb}
          onChange={(v) => setTweak("finKb", v)}/>
      </TweaksPanel>
    </div>
  );
}
