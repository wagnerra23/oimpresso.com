...
function hallucinationGoldenSet_governanca(): array
{
    return [
        ...
        // ── Wave 28 — 15 casos derivados de erros REAIS documentados em
        //    memory/reference/feedback-*.md (cada bug vira anti-alucinação). ──
        [
            'question' => 'Pode sugerir voltar pro Baileys 6.7.9 ou esperar a 7.0.0 final estável?',
            'answer' => 'Não. O Baileys 7.x é decisão IRREVERSÍVEL (Wagner 2026-05-15, regra Tier 0 comportamental). A versão rc atual É a de migração — não cabe propor downgrade de versão nem adiar a migração por issues abertas. Executar o plano; bugs reais documentados só DURANTE a migração.',
            'must_contain' => ['Baileys 7.x', 'IRREVERSÍVEL', 'migração'],
            'must_not_contain' => ['6.7.9', '7.0.0 final', 'rc quebrado'],
            'category' => 'constituicao_governanca',
        ],
        [
            'question' => 'Como habilitar/desabilitar um módulo para um business específico no oimpresso?',
            'answer' => '3 camadas canônicas (Tier 0 IRREVOGÁVEL, Wagner 2026-05-18): (1) pacote nWidart via UI Superadmin/Packages; (2) business.enabled_modules (core UltimatePOS) via /business/settings; (3) Spatie Permissions por role via /roles. Habilitar é compra de pacote no superadmin, via UI — nunca condicional por business no código.',
            'must_contain' => ['enabled_modules', 'Superadmin', 'Spatie', '3 camadas'],
            'must_not_contain' => ['if ($business_id === N) return', 'hardcode no controller', 'true só pra biz 4'],
            'category' => 'constituicao_governanca',
        ],
        [
            'question' => 'A auto-mem e a memória do projeto são privadas de cada dev?',
            'answer' => 'Não — no oimpresso toda memória canon é compartilhada com o time (Felipe/Maiara/Eliana/Luiz) via MCP server. Categorias (ADR 0131): CANON (git, time inteiro vê) vs LOCAL pessoal (so Wagner). Chamar de restrita ao autor é léxico errado — fere a transparência da Constituição v2.',
            'must_contain' => ['compartilhada', 'MCP', 'CANON', '0131'],
            'must_not_contain' => ['restrita a uma pessoa', 'só o autor enxerga', 'conhecimento privado do dev'],
            'category' => 'constituicao_governanca',
        ],
        [
            'question' => 'O indexador SPEC→mcp_tasks processa módulos meta _-prefixados como _DesignSystem?',
            'answer' => 'Não — o indexador MCP pula meta-módulos _-prefixados (_DesignSystem). As tasks git-canônicas desses módulos ficam invisíveis no tasks-list/MCP (descoberto 2026-05-31). Workaround: criar a task direto via tasks-create.',
            'must_contain' => ['pula', '_-prefixados', 'invisíveis'],
            'must_not_contain' => ['processa normalmente', 'sincroniza todos', 'aparece no tasks-list'],
            'category' => 'constituicao_governanca',
        ],
        [
            'question' => 'Qual gateway de cobrança (boleto/PIX) propor para o cliente?',
            'answer' => 'Inter PJ Banking (Modules/RecurringBilling via Inter API) é o caminho de cobrança do cliente. O Asaas fica no repo apenas como blueprint reusável pros próximos drivers banco (Bradesco/Itaú), não como caminho ativo de cliente. Cliente piloto ROTA LIVRE usa Inter (Wagner 2026-05-14).',
            'must_contain' => ['Inter PJ', 'blueprint', 'RecurringBilling'],
            'must_not_contain' => ['use Asaas pro cliente', 'Asaas é o caminho', 'propor Asaas'],
            'category' => 'constituicao_governanca',
        ],
        [
            'question' => 'Pode pagar comissão recorrente perpétua (% do MRR pra sempre) a um vendedor?',
            'answer' => 'Não — % de MRR perpétuo sem decay é suicídio financeiro. Padrão SaaS B2B BR: 10-20% upfront + 15-25% no MRR do ano 1 + decay (5-10% ano 2). Caso real Agrosys: proposta de 50% do MRR pra sempre foi recusada.',
            'must_contain' => ['decay', 'upfront', 'suicídio financeiro'],
            'must_not_contain' => ['perpétuo é aceitável', 'pode pagar pra sempre', 'sem decay é ok'],
            'category' => 'constituicao_governanca',
        ],
        [
            'question' => 'O lookup de CNPJ sobrescreve o telefone e o email já cadastrados do cliente?',
            'answer' => 'Os contatos não. Dados cadastrais oficiais (razão social, endereço, CNAE, regime) o lookup sobrescreve (Receita é fonte da verdade). Já os contatos pessoais (telefone, celular, WhatsApp, email) só preenche se em branco — nunca pisa no que o user digitou. Wagner PR #1419.',
            'must_contain' => ['só preenche se em branco', 'cadastrais', 'contatos'],
            'must_not_contain' => ['sobrescreve o telefone', 'apaga os contatos', 'pisa no email'],
            'category' => 'constituicao_governanca',
        ],
        [
            'question' => 'Por que um transaction_payment pode não gerar o fin_titulo_baixa correspondente?',
            'answer' => 'Observer no-op silencioso: quando o business não tem fin_contas_bancarias, o Observer da bridge Sells→Financeiro vira no-op sem erro nem log. Foi bug invisível em prod por 12 dias até a Larissa reportar. Lição: no-op silencioso é débito técnico ALTO — todo guard de pré-requisito precisa de log/alarme.',
            'must_contain' => ['Observer', 'no-op', 'fin_contas_bancarias'],
            'must_not_contain' => ['nunca acontece', 'sempre gera a baixa', 'impossível faltar'],
            'category' => 'constituicao_governanca',
        ],
        [
            'question' => 'As migrations rodam automaticamente no deploy do Hostinger (quick-sync)?',
            'answer' => 'Não. O quick-sync.yml faz git pull + cache clear, mas NÃO roda composer install nem as migrations. Toda PR com migration exige passo manual via SSH pós-merge (ssh + cd public_html + php artisan migrate). Hostinger ≠ CT 100 (ADR 0062).',
            'must_contain' => ['manual', 'quick-sync', 'SSH'],
            'must_not_contain' => ['roda automático', 'migra sozinho', 'migrate no deploy'],
            'category' => 'constituicao_governanca',
        ],
        [
            'question' => 'A Jana (IA) deve atuar dentro do módulo Fiscal?',
            'answer' => 'Sim — Wagner quer IA no Fiscal: briefing mensal da Jana + leitura de receita SEFAZ não-mapeada via Brain B + Q&A em linguagem natural no atalho de comando. Foi removida por engano no PR #1605 (interpretação errada de SoC). A tarefa fiscal seguinte é REATIVAR.',
            'must_contain' => ['briefing mensal', 'Fiscal', 'REATIVAR'],
            'must_not_contain' => ['IA não pertence ao fiscal', 'excluída de vez', 'sem IA no fiscal'],
            'category' => 'constituicao_governanca',
        ],
        [
            'question' => 'Posso usar (array) $model num Collection::map() pra passar Eloquent pro Inertia?',
            'answer' => 'Não. O cast (array) nativo expõe propriedades protected do Model com prefixo null-byte (chaves estranhas tipo *attributes, ~30 no total), e o React recebe undefined em todos os campos. Use $model->toArray() no map. Validado em prod biz=1 (2026-05-11).',
            'must_contain' => ['toArray()', 'null-byte', 'undefined'],
            'must_not_contain' => ['(array) funciona', 'cast direto é ok', 'array nativo serve'],
            'category' => 'constituicao_governanca',
        ],
        [
            'question' => 'Com Inertia::defer, a prop deferida já está disponível no first paint?',
            'answer' => 'Não — prop deferida chega undefined no first paint (antes do partial reload resolver). Null-guard é obrigatório: envolver em Deferred com fallback skeleton e checar null antes de .map/.length, senão dá TypeError em prod.',
            'must_contain' => ['undefined', 'first paint', 'Null-guard', 'Deferred'],
            'must_not_contain' => ['disponível imediatamente', 'sempre presente no render', 'sem precisar guard'],
            'category' => 'constituicao_governanca',
        ],
        [
            'question' => 'Por que um teste Pest em Modules/ lança "A facade root has not been set"?',
            'answer' => 'Falta o uses(Tests\\TestCase::class) explícito no topo do arquivo. Pest 4 + Laravel exige uses(Tests\\TestCase::class) por arquivo que use app(), config(), Artisan::call() ou qualquer facade — sem ele o app não é bootado. Descoberto na implementação do ADR 0216 (PR #1874).',
            'must_contain' => ['uses(Tests\\TestCase::class)', 'por arquivo', 'facade'],
            'must_not_contain' => ['é automático', 'global pro projeto', 'não precisa declarar'],
            'category' => 'constituicao_governanca',
        ],
        [
            'question' => 'Por que o cycles-close --rollover não moveu as tasks DOING pro novo cycle?',
            'answer' => 'Porque mcp_tasks.cycle_id estava NULL nessas tasks (órfãs de cycle) — o rollover só move tasks linkadas ao cycle antigo. É drift duplo: nominal (rótulo do cycle) + estrutural (cycle_id NULL). Antes de cycles-close --rollover_to, validar quantas tasks têm cycle_id do cycle velho.',
            'must_contain' => ['cycle_id', 'NULL', 'rollover'],
            'must_not_contain' => ['move todas sozinho', 'rollover pega tudo', 'sempre migra as tasks'],
            'category' => 'constituicao_governanca',
        ],
        [
            'question' => 'Como defino o grupo de um item no sidebar — hardcode o label no Sidebar.tsx?',
            'answer' => 'Não se hardcoda no frontend. O grupo do sidebar vem do DataController do módulo (via LegacyMenuAdapter); o Sidebar.tsx só renderiza o que o backend declara. Regra Wagner 2026-05-19 (ADR 0094 SoC).',
            'must_contain' => ['DataController', 'LegacyMenuAdapter', 'backend declara'],
            'must_not_contain' => ['hardcode no Sidebar.tsx', 'label fixo no React', 'string no frontend'],
            'category' => 'constituicao_governanca',
        ],
    ];
}
