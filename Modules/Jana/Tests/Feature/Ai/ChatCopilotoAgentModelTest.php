...
// @covers-us US-COPI-144 — modelo do chat env-driven: ChatCopilotoAgent::model()
...
/**
 * R-COPI-144 — GUARD tests pro modelo env-driven do chat (US-COPI-144).
 *
...
it('R-COPI-144-001 — sem config → model() é null (cai no default do provider, legado)', function () {
    config()->set('copiloto.chat_model', null);

    expect(chatAgentModel()->model())->toBeNull();
});

it('R-COPI-144-002 — string vazia → null (não força "", que quebraria o request)', function () {
    config()->set('copiloto.chat_model', '');

    expect(chatAgentModel()->model())->toBeNull();
});

it('R-COPI-144-003 — JANA_CHAT_MODEL=gpt-4o → model() devolve gpt-4o (liga só o chat)', function () {
