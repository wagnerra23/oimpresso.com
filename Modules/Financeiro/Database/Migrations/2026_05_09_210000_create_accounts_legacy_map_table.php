 ...
 *
 * Decisão: ADR 0118 (segregação dominios externos) + proibições.md (não
 * modificar tabelas core UltimatePOS sem bridge table).
 *
 * Permite UPSERT idempotente de imports legacy via UNIQUE
 * (legacy_source, legacy_id, business_id) sem mexer no schema core.
 *
 * Multi-tenant Tier 0 (ADR 0093) — business_id global scope obrigatório.
 *
 * Casos de uso atuais:
 *  - Migração one-shot Delphi WR Comercial CONTAS → accounts
