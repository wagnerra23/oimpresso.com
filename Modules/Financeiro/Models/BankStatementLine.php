 ...
 *
 * MOTIVO (re-impl #2045 · Tier 0 hardening, ADR 0093): a tabela só era
 * acessada via `DB::table(...)` cru no ConciliacaoController, sem o NET do
 * global scope BusinessScope. O isolamento multi-tenant dependia 100% de cada
 * query repetir `where('business_id', …)` na mão — qualquer query futura que
 * esquecesse vazaria cross-tenant. Esta Model adiciona o global scope por
 * business_id por cima (defesa em profundidade), mantendo o
 * `where('business_id', …)` explícito dos callers como segunda camada (padrão
 * do módulo).
 *
 * ESCOPO (re-impl sobre ADR 0236): cobre SÓ a origem OFX
 * (`fin_bank_statement_lines`). A origem API (`fin_extrato_lancamentos`, Fase 1
 * ADR 0236) continua via `DB::table` no controller — tabela e schema distintos.
 *
