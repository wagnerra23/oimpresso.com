...
/**
 * D8 Security Wave 15 — FormRequest extraído de ReminderController::store.
 *
 * Padroniza validação que estava inline ($request->validate) seguindo pattern
 * canônico NfeBrasil (Modules/NfeBrasil/Http/Requests/StoreEmissaoRequest.php).
 *
 * Multi-tenant (ADR 0093): authorize() apenas valida sessão — scope business_id
 * segue no Controller via session('business.id') + Reminder::where('business_id'..).
 */
