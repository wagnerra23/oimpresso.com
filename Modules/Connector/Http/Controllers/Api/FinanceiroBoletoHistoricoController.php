...
use Carbon\Carbon;
use App\FinanceiroBoletoHistorico;

/**
 * @group Financeiro Boleto Historico Management
 * @authenticated
 *
 * APIs para gerenciar historico de boletos financeiros.
 */
class FinanceiroBoletoHistoricoController extends ApiController
{
    /**
     * Sincronizar Financeiro Boleto Historico
     *
     * @bodyParam data array required Lista de historicos para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncFinanceiroBoletoHistorico(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $historico = FinanceiroBoletoHistorico::where('business_id', $business_id)
                    ...
                    if ($historico) {
                        ...
                        $historico = FinanceiroBoletoHistorico::create([
            ...
            Log::error('Erro na sincronização de FinanceiroBoletoHistorico: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Financeiro Boleto Historico atualizado até uma data
     */
    public function getFinanceiroBoletoHistoricoUntilDate(Request $request)
    {
        ...
        try {
            ...
            $historicos = FinanceiroBoletoHistorico::where('business_id', $business_id)
            ...
            Log::error('Erro ao buscar FinanceiroBoletoHistorico: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
