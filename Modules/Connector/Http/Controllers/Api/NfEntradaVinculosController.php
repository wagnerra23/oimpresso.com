...
use Carbon\Carbon;
use App\NfEntradaVinculos;

/**
 * @group NF Entrada Vinculos Management
 * @authenticated
 *
 * APIs para gerenciar NF Entrada Vinculos.
 */
class NfEntradaVinculosController extends ApiController
{
    /**
     * Sincronizar NF Entrada Vinculos
     *
     * @bodyParam data array required Lista de vinculos para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncNfEntradaVinculos(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $vinculo = NfEntradaVinculos::where('business_id', $business_id)
                    ...
                    if ($vinculo) {
                        $updatedAt = $vinculo->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'deleted_at' => $vinculo->deleted_at,
                                'officeimpresso_codigo' => $vinculo->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $vinculo->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }
                        ...
                        $vinculo = NfEntradaVinculos::create([
            ...
            Log::error('Erro na sincronização de NF Entrada Vinculos: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar NF Entrada Vinculos atualizados até uma data
     */
    public function getNfEntradaVinculosUntilDate(Request $request)
    {
        ...
        try {
            ...
            $vinculos = NfEntradaVinculos::where('business_id', $business_id)
                ->where('updated_at', '>', $date)
                ->get();

            $response = $vinculos->map(function ($vinculo) {
                return [
                    'id' => $vinculo->id,
            ...
            Log::error('Erro ao buscar NF Entrada Vinculos: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
