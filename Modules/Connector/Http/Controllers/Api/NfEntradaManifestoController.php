...
use Carbon\Carbon;
use App\NfEntradaManifesto;

/**
 * @group NF Entrada Manifesto Management
 * @authenticated
 *
 * APIs para gerenciar NF Entrada Manifesto.
 */
class NfEntradaManifestoController extends ApiController
{
    /**
     * Sincronizar NF Entrada Manifesto
     *
     * @bodyParam data array required Lista de manifestos para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncNfEntradaManifesto(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $manifesto = NfEntradaManifesto::where('business_id', $business_id)
                    ...
                    if ($manifesto) {
                        $updatedAt = $manifesto->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'id' => $manifesto->id,
                                'updated_at' => $manifesto->updated_at->format('Y-m-d H:i:s'),
                                'deleted_at' => $manifesto->deleted_at,
                                'officeimpresso_codigo' => $manifesto->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $manifesto->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }
                        ...
                        $manifesto = NfEntradaManifesto::create([
            ...
            Log::error('Erro na sincronização de NF Entrada Manifesto: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar NF Entrada Manifesto atualizado até uma data
     */
    public function getNfEntradaManifestoUntilDate(Request $request)
    {
        ...
        try {
            ...
            $manifestos = NfEntradaManifesto::where('business_id', $business_id)
                ->where('updated_at', '>', $date)
                ->get();

            $response = $manifestos->map(function ($manifesto) {
                return [
                    'id' => $manifesto->id,
            ...
            Log::error('Erro ao buscar NF Entrada Manifesto: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
