...
use Carbon\Carbon;
use App\ProducaoMotivo;

/**
 * @group Produção Motivo Management
 * @authenticated
 *
 * APIs para gerenciar os motivos de produção.
 */
class ProducaoMotivoController extends ApiController
{
    /**
     * Sincronizar Motivos de Produção
     *
     * @bodyParam data array required Lista de motivos para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncProducaoMotivo(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $motivo = ProducaoMotivo::where('business_id', $business_id)
            ...
            Log::error('Erro na sincronização de Produção Motivo: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Motivos de Produção atualizado até uma data
     */
    public function getProducaoMotivoUntilDate(Request $request)
    {
        ...
        try {
            ...
            $motivos = ProducaoMotivo::where('business_id', $business_id)
            ...
            Log::error('Erro ao buscar Motivos de Produção: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
