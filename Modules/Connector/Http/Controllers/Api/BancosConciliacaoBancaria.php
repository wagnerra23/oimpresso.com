...
use Carbon\Carbon;
use App\BancosConciliacaoBancaria;

/**
 * @group Bancos Conciliacao Bancaria Management
 * @authenticated
 *
 * APIs para gerenciar Bancos Conciliacao Bancaria.
 */
class BancosConciliacaoBancariaController extends ApiController
{
    /**
     * Sincronizar Bancos Conciliacao Bancaria
     *
     * @bodyParam data array required Lista de conciliações bancárias para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncBancosConciliacaoBancaria(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $conciliacao = BancosConciliacaoBancaria::where('business_id', $business_id)
                    ...
                    if ($conciliacao) {
                        $updatedAt = $conciliacao->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'id' => $conciliacao->id,
                                'updated_at' => $conciliacao->updated_at->format('Y-m-d H:i:s'),
                                'deleted_at' => $conciliacao->deleted_at,
                                'officeimpresso_codigo' => $conciliacao->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $conciliacao->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }
                        ...
                        $conciliacao = BancosConciliacaoBancaria::create([
            ...
            Log::error('Erro na sincronização de Bancos Conciliacao Bancaria: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Bancos Conciliacao Bancaria atualizado até uma data
     */
    public function getBancosConciliacaoBancariaUntilDate(Request $request)
    {
        ...
        try {
            ...
            $concilicoes = BancosConciliacaoBancaria::where('business_id', $business_id)
                ->where('updated_at', '>', $date)
                ->get();

            $response = $concilicoes->map(function ($conciliacao) {
                return [
                    'id' => $conciliacao->id,
            ...
            Log::error('Erro ao buscar Bancos Conciliacao Bancaria: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
