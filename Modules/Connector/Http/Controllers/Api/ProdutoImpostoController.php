...
/**
 * @group Produto Imposto Management
 * @authenticated
 *
 * APIs para gerenciar Produto Imposto.
 */
class ProdutoImpostoController extends ApiController
{
    /**
     * Sincronizar Produto Imposto
     *
     * @bodyParam data array required Lista de produtos impostos para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncProdutoImposto(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $produtoImposto = ProdutoImposto::where('business_id', $business_id)
                    ...
                    if ($produtoImposto) {
                        $updatedAt = $produtoImposto->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'id' => $produtoImposto->id,
                                'updated_at' => $produtoImposto->updated_at->format('Y-m-d H:i:s'),
                                'deleted_at' => $produtoImposto->deleted_at,
                                'officeimpresso_codigo' => $produtoImposto->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $produtoImposto->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }

                        $produtoImposto->update([
                        ...
                        $produtoImposto = ProdutoImposto::create([
            ...
            Log::error('Erro na sincronização de ProdutoImposto: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Produto Imposto atualizado até uma data
     */
    public function getProdutoImpostoUntilDate(Request $request)
    {
        ...
        try {
            ...
            $produtosImpostos = ProdutoImposto::where('business_id', $business_id)
                ->where('updated_at', '>', $date)
                ->get();

            $response = $produtosImpostos->map(function ($produtoImposto) {
                return [
                    'id' => $produtoImposto->id,
                    'descricao' => $produtoImposto->descricao,
                    'officeimpresso_codigo' => $produtoImposto->officeimpresso_codigo,
                    'officeimpresso_dt_alteracao' => $produtoImposto->officeimpresso_dt_alteracao,
                    'updated_at' => $produtoImposto->updated_at->format('Y-m-d H:i:s'),
            ...
            Log::error('Erro ao buscar Produto Imposto: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
