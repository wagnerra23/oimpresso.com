...
use Carbon\Carbon;
use App\ContratoTipo;

/**
 * @group Contrato Tipo Management
 * @authenticated
 *
 * APIs para gerenciar Contrato Tipo.
 */
class ContratoTipoController extends ApiController
{
    /**
     * Sincronizar Contrato Tipo
     *
     * @bodyParam data array required Lista de tipos de contrato para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncContratoTipo(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $contratoTipo = ContratoTipo::where('business_id', $business_id)
                    ...
                    if ($contratoTipo) {
                        $updatedAt = $contratoTipo->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'id' => $contratoTipo->id,
                                'updated_at' => $contratoTipo->updated_at->format('Y-m-d H:i:s'),
                                'deleted_at' => $contratoTipo->deleted_at,
                                'officeimpresso_codigo' => $contratoTipo->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $contratoTipo->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }
                        ...
                        $contratoTipo = ContratoTipo::create([
            ...
            Log::error('Erro na sincronização de ContratoTipo: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Contrato Tipo atualizado até uma data
     */
    public function getContratoTipoUntilDate(Request $request)
    {
        ...
        try {
            ...
            $contratoTipos = ContratoTipo::where('business_id', $business_id)
                ->where('updated_at', '>', $date)
                ->get();

            $response = $contratoTipos->map(function ($contratoTipo) {
                return [
                    'id' => $contratoTipo->id,
                    'descricao' => $contratoTipo->descricao,
                    'officeimpresso_codigo' => $contratoTipo->officeimpresso_codigo,
                    'officeimpresso_dt_alteracao' => $contratoTipo->officeimpresso_dt_alteracao,
                    'updated_at' => $contratoTipo->updated_at->format('Y-m-d H:i:s'),
            ...
            Log::error('Erro ao buscar Contrato Tipo: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
