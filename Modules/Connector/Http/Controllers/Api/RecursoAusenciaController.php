...
use Carbon\Carbon;
use App\RecursoAusencia;

/**
 * @group RecursoAusencia Management
 * @authenticated
 *
 * APIs para gerenciar RecursoAusencia.
 */
class RecursoAusenciaController extends ApiController
{
    /**
     * Sincronizar RecursoAusencia
     *
     ...
     * }
     */
    public function syncRecursoAusencia(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $registro = RecursoAusencia::where('business_id', $business_id)
                    ...
                    if ($registro) {
                        ...
                        $registro = RecursoAusencia::create([
            ...
            Log::error('Erro na sincronização de RecursosAusencia: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar RecursosAusencia atualizados até uma data
     */
    public function getRecursoAusenciaUntilDate(Request $request)
    {
        ...
        try {
            ...
            $registros = RecursoAusencia::where('business_id', $business_id)
            ...
            Log::error('Erro ao buscar RecursosAusencia: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
