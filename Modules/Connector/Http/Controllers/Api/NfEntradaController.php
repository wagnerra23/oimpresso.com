...
/**
 * @group NF Entrada Management
 * @authenticated
 *
 * APIs para gerenciar Notas Fiscais de Entrada.
 */
class NfEntradaController extends ApiController
{
    /**
     * Sincronizar Notas Fiscais de Entrada
     *
     * @bodyParam data array required Lista de notas fiscais de entrada para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncNfEntrada(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $nfEntrada = NfEntrada::where('business_id', $business_id)
                    ...
                    if ($nfEntrada) {
                        $updatedAt = $nfEntrada->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'id' => $nfEntrada->id,
                                'updated_at' => $nfEntrada->updated_at->format('Y-m-d H:i:s'),
                                'deleted_at' => $nfEntrada->deleted_at,
                                'officeimpresso_codigo' => $nfEntrada->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $nfEntrada->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }
            ...
            Log::error('Erro na sincronização de NF Entrada: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Notas Fiscais de Entrada atualizadas até uma data
     */
    public function getNfEntradaUntilDate(Request $request)
    {
        ...
        try {
            ...
            $nfEntradas = NfEntrada::where('business_id', $business_id)
                ->where('updated_at', '>', $date)
                ->get();

            $response = $nfEntradas->map(function ($nfEntrada) {
                return [
                    'id' => $nfEntrada->id,
                    'descricao' => $nfEntrada->descricao,
                    'officeimpresso_codigo' => $nfEntrada->officeimpresso_codigo,
                    'officeimpresso_dt_alteracao' => $nfEntrada->officeimpresso_dt_alteracao,
                    'updated_at' => $nfEntrada->updated_at->format('Y-m-d H:i:s'),
            ...
            Log::error('Erro ao buscar NF Entrada: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
