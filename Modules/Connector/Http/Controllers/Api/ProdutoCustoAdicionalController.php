...
use Carbon\Carbon;
use App\CustoAdicional;

/**
 * @group CustoAdicional Management
 * @authenticated
 *
 * APIs for managing CustoAdicional.
 */
class ProdutoCustoAdicionalController extends ApiController
{
    /**
     * Synchronize CustoAdicional
     */
    public function syncCustoAdicional(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $record = CustoAdicional::where('business_id', $business_id)
            ...
            Log::error('Erro na sincronização de CustoAdicional: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
