...
/**
 * @group CentroTrabalho Management
 * @authenticated
 *
 * APIs for managing CentroTrabalho.
 */
class ProdutoCentroTrabalhoController extends ApiController
{
    /**
     * Synchronize CentroTrabalho
     */
    public function syncCentroTrabalho(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $record = CentroTrabalho::where('business_id', $business_id)
            ...
            Log::error('Erro na sincronização de CentroTrabalho: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
