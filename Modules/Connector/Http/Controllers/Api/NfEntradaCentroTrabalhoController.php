...
use Carbon\Carbon;
use App\NfEntradaCentroTrabalho;

/**
 * @group NF Entrada Centro Trabalho Management
 * @authenticated
 *
 * APIs para gerenciar NF Entrada Centro Trabalho.
 */
class NfEntradaCentroTrabalhoController extends ApiController
{
    /**
     * Sincronizar NF Entrada Centro Trabalho
     *
     ...
     * }
     */
    public function syncNfEntradaCentroTrabalho(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $centroTrabalho = NfEntradaCentroTrabalho::where('business_id', $business_id)
                    ...
                    if ($centroTrabalho) {
                        ...
                        $centroTrabalho = NfEntradaCentroTrabalho::create([
            ...
            Log::error('Erro na sincronização de NF Entrada Centro Trabalho: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar NF Entrada Centro Trabalho atualizado até uma data
     */
    public function getNfEntradaCentroTrabalhoUntilDate(Request $request)
    {
        ...
        try {
            ...
            $centrosTrabalho = NfEntradaCentroTrabalho::where('business_id', $business_id)
                ->where('updated_at', '>', $date)
                ->get();

            $response = $centrosTrabalho->map(function ($centroTrabalho) {
                return [
                    'id' => $centroTrabalho->id,
            ...
            Log::error('Erro ao buscar NF Entrada Centro Trabalho: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
