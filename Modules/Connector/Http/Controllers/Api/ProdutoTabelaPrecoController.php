...
/**
 * @group TabelaPreco Management
 * @authenticated
 *
 * APIs for managing TabelaPreco.
 */
class ProdutoTabelaPrecoController extends ApiController
{
    /**
     * Synchronize TabelaPreco
     */
    public function syncTabelaPreco(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $record = TabelaPreco::where('business_id', $business_id)
            ...
            Log::error('Erro na sincronização de TabelaPreco: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
