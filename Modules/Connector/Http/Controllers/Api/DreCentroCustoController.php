...
use Carbon\Carbon;
use App\DreCentroCusto;

/**
 * @group DRE Centro Custo Management
 * @authenticated
 *
 * APIs para gerenciar DRE Centro Custo.
 */
class DreCentroCustoController extends ApiController
{
    /**
     * Sincronizar DRE Centro Custo
     *
     * @bodyParam data array required Lista de DRE Centro Custos para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncDreCentroCusto(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $dreCentroCusto = DreCentroCusto::where('business_id', $business_id)
                    ...
                    if ($dreCentroCusto) {
                        $updatedAt = $dreCentroCusto->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'id' => $dreCentroCusto->id,
                                'updated_at' => $dreCentroCusto->updated_at->format('Y-m-d H:i:s'),
                                'deleted_at' => $dreCentroCusto->deleted_at,
                                'officeimpresso_codigo' => $dreCentroCusto->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $dreCentroCusto->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }

                        $dreCentroCusto->update([
                        ...
                        $dreCentroCusto = DreCentroCusto::create([
            ...
            Log::error('Erro na sincronização de DreCentroCusto: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar DRE Centro Custo atualizado até uma data
     */
    public function getDreCentroCustoUntilDate(Request $request)
    {
        ...
        try {
            ...
            $dreCentroCustos = DreCentroCusto::where('business_id', $business_id)
                ->where('updated_at', '>', $date)
                ->get();

            $response = $dreCentroCustos->map(function ($dreCentroCusto) {
                return [
                    'id' => $dreCentroCusto->id,
                    'descricao' => $dreCentroCusto->descricao,
                    'officeimpresso_codigo' => $dreCentroCusto->officeimpresso_codigo,
                    'officeimpresso_dt_alteracao' => $dreCentroCusto->officeimpresso_dt_alteracao,
                    'updated_at' => $dreCentroCusto->updated_at->format('Y-m-d H:i:s'),
            ...
            Log::error('Erro ao buscar Dre Centro Custo: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
