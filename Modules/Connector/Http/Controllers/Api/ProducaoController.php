...
/**
 * @group Produção Management
 * @authenticated
 *
 * APIs para gerenciar Produção.
 */
class ProducaoController extends ApiController
{
     ...
     *
     * @bodyParam data array required Lista de produções para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncProducao(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $producao = Producao::where('business_id', $business_id)
                    ...
                    if ($producao) {
                        $updatedAt = $producao->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'deleted_at' => $producao->deleted_at,
            ...
            Log::error('Erro na sincronização de Produção: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Produção atualizado até uma data
     */
    public function getProducaoUntilDate(Request $request)
    {
        ...
        try {
            ...
            $producoes = Producao::where('business_id', $business_id)
                ->where('updated_at', '>', $date)
                ->get();

            $response = $producoes->map(function ($producao) {
                return [
                    'id' => $producao->id,
            ...
            Log::error('Erro ao buscar Produção: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
