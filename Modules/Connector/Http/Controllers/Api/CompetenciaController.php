...
/**
 * @group Competencia Management
 * @authenticated
 *
 * APIs para gerenciar Competencia.
 */
class CompetenciaController extends ApiController
{
     ...
     *
     * @bodyParam data array required Lista de competencias para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncCompetencia(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $competencia = Competencia::where('business_id', $business_id)
                    ...
                    if ($competencia) {
                        $updatedAt = $competencia->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'id' => $competencia->id,
                                'updated_at' => $competencia->updated_at->format('Y-m-d H:i:s'),
                                'deleted_at' => $competencia->deleted_at,
                                'officeimpresso_codigo' => $competencia->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $competencia->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }
            ...
            Log::error('Erro na sincronização de Competencia: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Competencias atualizadas até uma data
     */
    public function getCompetenciaUntilDate(Request $request)
    {
        ...
        try {
            ...
            $competencias = Competencia::where('business_id', $business_id)
                ->where('updated_at', '>', $date)
                ->get();

            $response = $competencias->map(function ($competencia) {
                return [
                    'id' => $competencia->id,
            ...
            Log::error('Erro ao buscar Competencias: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
