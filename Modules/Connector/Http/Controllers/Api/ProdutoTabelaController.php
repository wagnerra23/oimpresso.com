...
/**
 * @group Tabela Management
 * @authenticated
 *
 * APIs for managing Tabela.
 */
class ProdutoTabelaController extends ApiController
{
    /**
     * Synchronize Tabela
     */
    public function syncTabela(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $record = Tabela::where('business_id', $business_id)
            ...
            Log::error('Erro na sincronização de Tabela: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
