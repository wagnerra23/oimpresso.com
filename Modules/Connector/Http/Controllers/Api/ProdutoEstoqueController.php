...
/**
 * @group ProdutoEstoque Management
 * @authenticated
 *
 * APIs para gerenciar ProdutoEstoque.
 */
class ProdutoEstoqueController extends ApiController
{
    /**
     * Sincronizar ProdutoEstoque
     *
     ...
     * }
     */
    public function syncProdutoEstoque(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $registro = ProdutoEstoque::where('business_id', $business_id)
                    ...
                    if ($registro) {
                        ...
                        $registro = ProdutoEstoque::create([
            ...
            Log::error('Erro na sincronização de ProdutoEstoque: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar ProdutoEstoque atualizado até uma data
     */
    public function getProdutoEstoqueUntilDate(Request $request)
    {
        ...
        try {
            ...
            $registros = ProdutoEstoque::where('business_id', $business_id)
            ...
            Log::error('Erro ao buscar ProdutoEstoque: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
