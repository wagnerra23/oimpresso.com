...
/**
 * @group Licenciamento Management
 * @authenticated
 *
 * APIs para gerenciar licenciamentos.
 */
class LicenciamentoController extends ApiController
{
    public function syncLicenciamento(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $licenciamento = Licenciamento::where('business_id', $business_id)
                    ...
                    if ($licenciamento) {
                        $updatedAt = $licenciamento->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'id' => $licenciamento->id,
                                'updated_at' => $licenciamento->updated_at->format('Y-m-d H:i:s'),
                                'deleted_at' => $licenciamento->deleted_at,
                                'officeimpresso_codigo' => $licenciamento->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $licenciamento->officeimpresso_dt_alteracao,
                            ];
                        }
            ...
            Log::error('Erro na sincronização de Licenciamentos: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    public function getSyncLicenciamentoUntilDate(Request $request)
    {
        ...
        try {
            ...
            $licenciamentos = Licenciamento::where('business_id', $business_id)
                                           ->where('updated_at', '>', $date)
                                           ->get();

            $response = $licenciamentos->map(function ($licenciamento) {
                return [
                    'id' => $licenciamento->id,
            ...
            Log::error('Erro ao buscar licenciamentos: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
