...
/**
 * @group Markup Management
 * @authenticated
 *
 * APIs for managing Markup.
 */
class ProdutoMarkupController extends ApiController
{
    /**
     * Synchronize Markup
     */
    public function syncMarkup(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $record = Markup::where('business_id', $business_id)
            ...
            Log::error('Erro na sincronização de Markup: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
