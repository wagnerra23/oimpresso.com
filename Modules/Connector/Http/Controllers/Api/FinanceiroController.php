...
/**
 * @group Financeiro Management
 * @authenticated
 *
 * APIs para gerenciar Financeiro.
 */
class FinanceiroController extends ApiController
{
    /**
     * Sincronizar Financeiro
     *
     * @bodyParam data array required Lista de registros financeiros para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncFinanceiro(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $financeiro = Financeiro::where('business_id', $business_id)
                    ...
                    if ($financeiro) {
                        $updatedAt = $financeiro->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'id' => $financeiro->id,
                                'updated_at' => $financeiro->updated_at->format('Y-m-d H:i:s'),
                                'deleted_at' => $financeiro->deleted_at,
                                'officeimpresso_codigo' => $financeiro->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $financeiro->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }
            ...
            Log::error('Erro na sincronização de Financeiro: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Financeiro atualizado até uma data
     */
    public function getFinanceiroUntilDate(Request $request)
    {
        ...
        try {
            ...
            $financeiros = Financeiro::where('business_id', $business_id)
                ->where('updated_at', '>', $date)
                ->get();

            $response = $financeiros->map(function ($financeiro) {
                return [
                    'id' => $financeiro->id,
            ...
            Log::error('Erro ao buscar Financeiro: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
