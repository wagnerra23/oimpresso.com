...
use Carbon\Carbon;
use App\ProdutoEstoqueLocal;

/**
 * @group ProdutoEstoqueLocal Management
 * @authenticated
 *
 * APIs para gerenciar ProdutoEstoqueLocal.
 */
class ProdutoEstoqueLocalController extends ApiController
{
    /**
     * Sincronizar ProdutoEstoqueLocal
     *
     ...
     * }
     */
    public function syncProdutoEstoqueLocal(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $registro = ProdutoEstoqueLocal::where('business_id', $business_id)
                    ...
                    if ($registro) {
                        ...
                        $registro = ProdutoEstoqueLocal::create([
            ...
            Log::error('Erro na sincronização de ProdutoEstoqueLocal: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar ProdutoEstoqueLocal atualizado até uma data
     */
    public function getProdutoEstoqueLocalUntilDate(Request $request)
    {
        ...
        try {
            ...
            $registros = ProdutoEstoqueLocal::where('business_id', $business_id)
            ...
            Log::error('Erro ao buscar ProdutoEstoqueLocal: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
