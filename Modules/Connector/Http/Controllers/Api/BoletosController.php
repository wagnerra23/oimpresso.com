...
/**
 * @group Boletos Management
 * @authenticated
 *
 * APIs para gerenciar boletos.
 */
class BoletosController extends ApiController
{
     ...
     * }
     */
    public function syncBoletos(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $boleto = Boletos::where('business_id', $business_id)
            ...
            Log::error('Erro na sincronização de boletos: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar boletos atualizados até uma data
     */
    public function getBoletosUntilDate(Request $request)
    {
        ...
        try {
            ...
            $boletos = Boletos::where('business_id', $business_id)
