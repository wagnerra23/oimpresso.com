...
use Carbon\Carbon;
use App\ProducaoSituacao;

/**
 * @group Produção Situação Management
 * @authenticated
 *
 * APIs para gerenciar Situações de Produção.
 */
class ProducaoSituacaoController extends ApiController
{
    /**
     * Sincronizar Situações de Produção
     *
     * @bodyParam data array required Lista de situações para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncProducaoSituacao(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $situacao = ProducaoSituacao::where('business_id', $business_id)
                    ...
                    if ($situacao) {
                        $updatedAt = $situacao->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'id' => $situacao->id,
                                'updated_at' => $situacao->updated_at->format('Y-m-d H:i:s'),
                                'deleted_at' => $situacao->deleted_at,
                                'officeimpresso_codigo' => $situacao->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $situacao->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }
                        ...
                        $situacao = ProducaoSituacao::create([
            ...
            Log::error('Erro na sincronização de Produção Situação: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Situações de Produção atualizadas até uma data
     */
    public function getProducaoSituacaoUntilDate(Request $request)
    {
        ...
        try {
            ...
            $situacoes = ProducaoSituacao::where('business_id', $business_id)
                ->where('updated_at', '>', $date)
                ->get();

            $response = $situacoes->map(function ($situacao) {
                return [
                    'id' => $situacao->id,
            ...
            Log::error('Erro ao buscar Situações de Produção: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
