...
use Carbon\Carbon;
use App\NfEntradaDespesa;

/**
 * @group NF Entrada Despesa Management
 * @authenticated
 *
 * APIs para gerenciar NF Entrada Despesa.
 */
class NfEntradaDespesaController extends ApiController
{
    /**
     * Sincronizar NF Entrada Despesa
     */
    public function syncNfEntradaDespesa(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $despesa = NfEntradaDespesa::where('business_id', $business_id)
                    ...
                    if ($despesa) {
                        $updatedAt = $despesa->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'deleted_at' => $despesa->deleted_at,
                                'officeimpresso_codigo' => $despesa->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $despesa->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }
                        ...
                        $despesa = NfEntradaDespesa::create([
            ...
            Log::error('Erro na sincronização de NF Entrada Despesa: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar NF Entrada Despesa atualizado até uma data
     */
    public function getNfEntradaDespesaUntilDate(Request $request)
    {
        ...
        try {
            ...
            $despesas = NfEntradaDespesa::where('business_id', $business_id)
                ->where('updated_at', '>', $date)
                ->get();

            $response = $despesas->map(function ($despesa) {
                return [
                    'id' => $despesa->id,
            ...
            Log::error('Erro ao buscar NF Entrada Despesa: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
