...
use Carbon\Carbon;
use App\NFEntradaProdutosCustoAd;

/**
 * @group NF Entrada Produtos Custo Ad Management
 * @authenticated
 *
 * APIs para gerenciar NF Entrada Produtos Custo Ad.
 */
class NFEntradaProdutosCustoAdController extends ApiController
{
    /**
     * Sincronizar NF Entrada Produtos Custo Ad
     *
     ...
     * }
     */
    public function syncNFEntradaProdutosCustoAd(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $produtoCustoAd = NFEntradaProdutosCustoAd::where('business_id', $business_id)
                    ...
                    if ($produtoCustoAd) {
                        $updatedAt = $produtoCustoAd->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'id' => $produtoCustoAd->id,
                                'updated_at' => $produtoCustoAd->updated_at->format('Y-m-d H:i:s'),
                                'deleted_at' => $produtoCustoAd->deleted_at,
                                'officeimpresso_codigo' => $produtoCustoAd->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $produtoCustoAd->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }

                        $produtoCustoAd->update([
                        ...
                        $produtoCustoAd = NFEntradaProdutosCustoAd::create([
            ...
            Log::error('Erro na sincronização de NF Entrada Produtos Custo Ad: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar NF Entrada Produtos Custo Ad atualizado até uma data
     */
    public function getNFEntradaProdutosCustoAdUntilDate(Request $request)
    {
        ...
        try {
            ...
            $produtosCustoAd = NFEntradaProdutosCustoAd::where('business_id', $business_id)
                ->where('updated_at', '>', $date)
                ->get();

            $response = $produtosCustoAd->map(function ($produto) {
                return [
                    'id' => $produto->id,
            ...
            Log::error('Erro ao buscar NF Entrada Produtos Custo Ad: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
