...
/**
 * @group Produção OS Management
 * @authenticated
 *
 * APIs para gerenciar Produção OS.
 */
class ProducaoOsController extends ApiController
{
    /**
     * Sincronizar Produção OS
     *
     * @bodyParam data array required Lista de ordens de serviço para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncProducaoOs(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $producaoOs = ProducaoOs::where('business_id', $business_id)
                    ...
                    if ($producaoOs) {
                        $updatedAt = $producaoOs->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'id' => $producaoOs->id,
                                'updated_at' => $producaoOs->updated_at->format('Y-m-d H:i:s'),
                                'deleted_at' => $producaoOs->deleted_at,
                                'officeimpresso_codigo' => $producaoOs->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $producaoOs->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }
            ...
            Log::error('Erro na sincronização de Produção OS: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Produção OS atualizadas até uma data
     */
    public function getProducaoOsUntilDate(Request $request)
    {
        ...
        try {
            ...
            $response = $producaoOs->map(function ($os) {
                return [
                    'id' => $os->id,
                    'descricao' => $os->descricao,
                    'officeimpresso_codigo' => $os->officeimpresso_codigo,
                    'officeimpresso_dt_alteracao' => $os->officeimpresso_dt_alteracao,
                    'updated_at' => $os->updated_at->format('Y-m-d H:i:s'),
            ...
            Log::error('Erro ao buscar Produção OS: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
