...
/**
 * @group Projeto Management
 * @authenticated
 *
 * APIs para gerenciar Projeto.
 */
class ProjetoController extends ApiController
{
     ...
     * }
     */
    public function syncProjeto(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $registro = Projeto::where('business_id', $business_id)
            ...
            Log::error('Erro na sincronização de Projetos: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Projetos atualizados até uma data
     */
    public function getProjetoUntilDate(Request $request)
    {
        ...
        try {
            ...
            $registros = Projeto::where('business_id', $business_id)
