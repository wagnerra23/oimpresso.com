...
use Carbon\Carbon;
use App\ProdutoMovimento;

/**
 * @group Produto Movimento Management
 * @authenticated
 *
 * APIs para gerenciar Produto Movimento.
 */
class ProdutoMovimentoController extends ApiController
{
    /**
     * Sincronizar Produto Movimento
     *
     * @bodyParam data array required Lista de movimentos de produtos para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncProdutoMovimento(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $movimento = ProdutoMovimento::where('business_id', $business_id)
                    ...
                    if ($movimento) {
                        ...
                        $movimento = ProdutoMovimento::create([
            ...
            Log::error('Erro na sincronização de Produto Movimento: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Produto Movimento atualizado até uma data
     */
    public function getProdutoMovimentoUntilDate(Request $request)
    {
        ...
        try {
            ...
            $movimentos = ProdutoMovimento::where('business_id', $business_id)
            ...
            Log::error('Erro ao buscar Produto Movimento: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
