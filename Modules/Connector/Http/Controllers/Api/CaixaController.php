...
/**
 * @group Caixa Management
 * @authenticated
 *
 * APIs para gerenciar Caixa.
 */
class CaixaController extends ApiController
{
     ...
     * Sincronizar Caixa
     *
     * @bodyParam data array required Lista de caixas para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncCaixa(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $caixa = Caixa::where('business_id', $business_id)
                    ...
                    if ($caixa) {
                        $updatedAt = $caixa->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'id' => $caixa->id,
                                'updated_at' => $caixa->updated_at->format('Y-m-d H:i:s'),
                                'deleted_at' => $caixa->deleted_at,
                                'officeimpresso_codigo' => $caixa->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $caixa->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }
            ...
            Log::error('Erro na sincronização de Caixa: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Caixa atualizado até uma data
     */
    public function getCaixaUntilDate(Request $request)
    {
        ...
        try {
            ...
            $caixas = Caixa::where('business_id', $business_id)
            ...
            Log::error('Erro ao buscar Caixa: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
