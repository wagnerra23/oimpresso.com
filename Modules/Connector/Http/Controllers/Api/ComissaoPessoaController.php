...
use Carbon\Carbon;
use App\ComissaoPessoa;

/**
 * @group Comissao Pessoa Management
 * @authenticated
 *
 * APIs para gerenciar Comissao Pessoa.
 */
class ComissaoPessoaController extends ApiController
{
    /**
     * Sincronizar Comissao Pessoa
     *
     * @bodyParam data array required Lista de comissao pessoas para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncComissaoPessoa(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $comissaoPessoa = ComissaoPessoa::where('business_id', $business_id)
                    ...
                    if ($comissaoPessoa) {
                        $updatedAt = $comissaoPessoa->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'id' => $comissaoPessoa->id,
                                'updated_at' => $comissaoPessoa->updated_at->format('Y-m-d H:i:s'),
                                'deleted_at' => $comissaoPessoa->deleted_at,
                                'officeimpresso_codigo' => $comissaoPessoa->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $comissaoPessoa->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }

                        $comissaoPessoa->update([
                        ...
                        $comissaoPessoa = ComissaoPessoa::create([
            ...
            Log::error('Erro na sincronização de ComissaoPessoa: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Comissao Pessoa atualizado até uma data
     */
    public function getComissaoPessoaUntilDate(Request $request)
    {
        ...
        try {
            ...
            $comissaoPessoas = ComissaoPessoa::where('business_id', $business_id)
                ->where('updated_at', '>', $date)
                ->get();

            $response = $comissaoPessoas->map(function ($comissaoPessoa) {
                return [
                    'id' => $comissaoPessoa->id,
                    'descricao' => $comissaoPessoa->descricao,
                    'officeimpresso_codigo' => $comissaoPessoa->officeimpresso_codigo,
                    'officeimpresso_dt_alteracao' => $comissaoPessoa->officeimpresso_dt_alteracao,
                    'updated_at' => $comissaoPessoa->updated_at->format('Y-m-d H:i:s'),
            ...
            Log::error('Erro ao buscar Comissao Pessoa: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
