...
use Carbon\Carbon;
use App\FinanceiroHistAgrupamento;

/**
 * @group Financeiro Hist Agrupamento Management
 * @authenticated
 *
 * APIs para gerenciar Financeiro Hist Agrupamento.
 */
class FinanceiroHistAgrupamentoController extends ApiController
{
    /**
     * Sincronizar Financeiro Hist Agrupamento
     *
     * @bodyParam data array required Lista de registros para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncFinanceiroHistAgrupamento(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $record = FinanceiroHistAgrupamento::where('business_id', $business_id)
                    ...
                    if ($record) {
                        ...
                        $record = FinanceiroHistAgrupamento::create([
            ...
            Log::error('Erro na sincronização de FinanceiroHistAgrupamento: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Financeiro Hist Agrupamento atualizado até uma data
     */
    public function getFinanceiroHistAgrupamentoUntilDate(Request $request)
    {
        ...
        try {
            ...
            $records = FinanceiroHistAgrupamento::where('business_id', $business_id)
            ...
            Log::error('Erro ao buscar Financeiro Hist Agrupamento: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
