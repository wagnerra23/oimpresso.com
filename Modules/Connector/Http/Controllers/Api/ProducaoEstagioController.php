...
use Carbon\Carbon;
use App\ProducaoEstagio;

/**
 * @group Produção Estágio Management
 * @authenticated
 *
 * APIs para gerenciar Produção Estágio.
 */
class ProducaoEstagioController extends ApiController
{
    /**
     * Sincronizar Produção Estágio
     *
     * @bodyParam data array required Lista de estágios de produção para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncProducaoEstagio(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $producaoEstagio = ProducaoEstagio::where('business_id', $business_id)
                    ...
                    if ($producaoEstagio) {
                        $updatedAt = $producaoEstagio->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'id' => $producaoEstagio->id,
                                'updated_at' => $producaoEstagio->updated_at->format('Y-m-d H:i:s'),
                                'deleted_at' => $producaoEstagio->deleted_at,
                                'officeimpresso_codigo' => $producaoEstagio->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $producaoEstagio->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }

                        $producaoEstagio->update([
                        ...
                        $producaoEstagio = ProducaoEstagio::create([
            ...
            Log::error('Erro na sincronização de Produção Estágio: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Produção Estágio atualizado até uma data
     */
    public function getProducaoEstagioUntilDate(Request $request)
    {
        ...
        try {
            ...
            $response = $producaoEstagio->map(function ($estagio) {
                return [
                    'id' => $estagio->id,
            ...
            Log::error('Erro ao buscar Produção Estágio: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
