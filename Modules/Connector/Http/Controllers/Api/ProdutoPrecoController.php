...
/**
 * @group Preco Management
 * @authenticated
 *
 * APIs for managing Preco.
 */
class ProdutoPrecoController extends ApiController
{
    /**
     * Synchronize Preco
     */
    public function syncPreco(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $variacao = variation_templates::where('business_id', $business_id);
                    $variacao->is_dummy = 0;
                    $variacao->save();
;


                    $record = Preco::where('business_id', $business_id)
            ...
            Log::error('Erro na sincronização do Produto: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
