...
use Carbon\Carbon;
use App\NfEntradaTipo;

/**
 * @group NF Entrada Tipo Management
 * @authenticated
 *
 * APIs para gerenciar NF Entrada Tipo.
 */
class NfEntradaTipoController extends ApiController
{
    /**
     * Sincronizar NF Entrada Tipo
     *
     * @bodyParam data array required Lista de tipos de entrada para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncNfEntradaTipo(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $nfEntradaTipo = NfEntradaTipo::where('business_id', $business_id)
                    ...
                    if ($nfEntradaTipo) {
                        $updatedAt = $nfEntradaTipo->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'id' => $nfEntradaTipo->id,
                                'updated_at' => $nfEntradaTipo->updated_at->format('Y-m-d H:i:s'),
                                'deleted_at' => $nfEntradaTipo->deleted_at,
                                'officeimpresso_codigo' => $nfEntradaTipo->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $nfEntradaTipo->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }

                        $nfEntradaTipo->update([
                        ...
                        $nfEntradaTipo = NfEntradaTipo::create([
            ...
            Log::error('Erro na sincronização de NF Entrada Tipo: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar NF Entrada Tipo atualizado até uma data
     */
    public function getNfEntradaTipoUntilDate(Request $request)
    {
        ...
        try {
            ...
            $nfEntradaTipos = NfEntradaTipo::where('business_id', $business_id)
                ->where('updated_at', '>', $date)
                ->get();

            $response = $nfEntradaTipos->map(function ($nfEntradaTipo) {
                return [
                    'id' => $nfEntradaTipo->id,
                    'descricao' => $nfEntradaTipo->descricao,
                    'officeimpresso_codigo' => $nfEntradaTipo->officeimpresso_codigo,
                    'officeimpresso_dt_alteracao' => $nfEntradaTipo->officeimpresso_dt_alteracao,
                    'updated_at' => $nfEntradaTipo->updated_at->format('Y-m-d H:i:s'),
            ...
            Log::error('Erro ao buscar NF Entrada Tipo: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
