...
class ContasController extends ApiController
{
     ...
     * Sincronizar Contas
     *
     * @bodyParam data array required Lista de contas para sincronizar.
     * @response {
     *   "status": "completed",
    ...
    public function syncContas(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $conta = Contas::where('business_id', $business_id)
                    ...
                    if ($conta) {
                        $updatedAt = $conta->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'id' => $conta->id,
                                'updated_at' => $conta->updated_at->format('Y-m-d H:i:s'),
                                'deleted_at' => $conta->deleted_at,
                                'officeimpresso_codigo' => $conta->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $conta->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }
            ...
            Log::error('Erro na sincronização de Contas: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Contas atualizadas até uma data
     */
    public function getContasUntilDate(Request $request)
    {
        ...
        try {
            ...
            $contas = Contas::where('business_id', $business_id)
                ->where('updated_at', '>', $date)
                ->get();

            $response = $contas->map(function ($conta) {
                return [
                    'id' => $conta->id,
            ...
            Log::error('Erro ao buscar Contas: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
