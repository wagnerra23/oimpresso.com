...
use Carbon\Carbon;
use App\ConciliacaoBancariaFinanceiro;

/**
 * @group Conciliacao Bancaria Financeiro Management
 * @authenticated
 *
 * APIs para gerenciar Conciliacao Bancaria Financeiro.
 */
class ConciliacaoBancariaFinanceiroController extends ApiController
{
    /**
     * Sincronizar Conciliacao Bancaria Financeiro
     *
     * @bodyParam data array required Lista de conciliações para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncConciliacaoBancariaFinanceiro(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $record = ConciliacaoBancariaFinanceiro::where('business_id', $business_id)
                    ...
                    if ($record) {
                        ...
                        $record = ConciliacaoBancariaFinanceiro::create([
            ...
            Log::error('Erro na sincronização de ConciliacaoBancariaFinanceiro: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Conciliacao Bancaria Financeiro atualizado até uma data
     */
    public function getConciliacaoBancariaFinanceiroUntilDate(Request $request)
    {
        ...
        try {
            ...
            $records = ConciliacaoBancariaFinanceiro::where('business_id', $business_id)
            ...
            Log::error('Erro ao buscar Conciliacao Bancaria Financeiro: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
