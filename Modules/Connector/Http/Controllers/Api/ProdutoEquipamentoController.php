...
/**
 * @group Equipamento Management
 * @authenticated
 *
 * APIs for managing Equipamento.
 */
class ProdutoEquipamentoController extends ApiController
{
    /**
     * Synchronize Equipamento
     */
    public function syncEquipamento(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $record = Equipamento::where('business_id', $business_id)
            ...
            Log::error('Erro na sincronização de Equipamento: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
