...
/**
 * @group Usuario Mensagem Management
 * @authenticated
 *
 * APIs para gerenciar Usuario Mensagem.
 */
class UsuarioMensagemController extends ApiController
{
    /**
     * Sincronizar Usuario Mensagem
     */
    public function syncUsuarioMensagem(Request $request)
    {
        try {
            // Validação dos dados
            ...
            foreach ($data as $item) {
                try {
                    $usuarioMensagem = UsuarioMensagem::where('business_id', $business_id)
                    ...
                    if ($usuarioMensagem) {
                        $updatedAt = $usuarioMensagem->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'id' => $usuarioMensagem->id,
                                'updated_at' => $usuarioMensagem->updated_at->format('Y-m-d H:i:s'),
                                'deleted_at' => $usuarioMensagem->deleted_at,
                                'officeimpresso_codigo' => $usuarioMensagem->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $usuarioMensagem->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }

                        // Atualizar registro existente
                        $usuarioMensagem->update([
                        ...
                        $usuarioMensagem = UsuarioMensagem::create([
            ...
            Log::error('Erro na sincronização de Usuario Mensagem: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Obter Usuario Mensagem atualizados até uma data específica
     */
    public function getSyncUsuarioMensagemUntilDate(Request $request)
    {
        ...
        try {
            ...
            $mensagens = UsuarioMensagem::where('business_id', $business_id)
                ->where('updated_at', '>', $date)
                ->get();

            $response = $mensagens->map(function ($mensagem) {
                return [
                    'id' => $mensagem->id,
                    'descricao' => $mensagem->descricao,
                    'officeimpresso_codigo' => $mensagem->officeimpresso_codigo,
                    'officeimpresso_dt_alteracao' => $mensagem->officeimpresso_dt_alteracao,
                    'updated_at' => $mensagem->updated_at->format('Y-m-d H:i:s'),
            ...
            Log::error('Erro ao buscar Usuario Mensagem: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
