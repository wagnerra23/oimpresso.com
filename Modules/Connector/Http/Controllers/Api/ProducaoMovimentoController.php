...
use Carbon\Carbon;
use App\ProducaoMovimento;

/**
 * @group Produção Movimento Management
 * @authenticated
 *
 * APIs para gerenciar Produção Movimento.
 */
class ProducaoMovimentoController extends ApiController
{
    /**
     * Sincronizar Produção Movimento
     *
     * @bodyParam data array required Lista de movimentos de produção para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncProducaoMovimento(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $movimento = ProducaoMovimento::where('business_id', $business_id)
                    ...
                    if ($movimento) {
                        $updatedAt = $movimento->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'id' => $movimento->id,
                                'updated_at' => $movimento->updated_at->format('Y-m-d H:i:s'),
                                'deleted_at' => $movimento->deleted_at,
                                'officeimpresso_codigo' => $movimento->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $movimento->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }
                        ...
                        $movimento = ProducaoMovimento::create([
            ...
            Log::error('Erro na sincronização de Produção Movimento: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Produção Movimento atualizado até uma data
     */
    public function getProducaoMovimentoUntilDate(Request $request)
    {
        ...
        try {
            ...
            $movimentos = ProducaoMovimento::where('business_id', $business_id)
                ->where('updated_at', '>', $date)
                ->get();

            $response = $movimentos->map(function ($movimento) {
                return [
                    'id' => $movimento->id,
            ...
            Log::error('Erro ao buscar Produção Movimento: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
