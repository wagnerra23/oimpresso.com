...
use Carbon\Carbon;
use App\FinanceiroBoleto;

/**
 * @group Financeiro Boleto Management
 * @authenticated
 *
 * APIs para gerenciar Financeiro Boleto.
 */
class FinanceiroBoletoController extends ApiController
{
    /**
     * Sincronizar Financeiro Boleto
     *
     * @bodyParam data array required Lista de boletos para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncFinanceiroBoleto(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $boleto = FinanceiroBoleto::where('business_id', $business_id)
                    ...
                    if ($boleto) {
                        $updatedAt = $boleto->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'id' => $boleto->id,
                                'updated_at' => $boleto->updated_at->format('Y-m-d H:i:s'),
                                'deleted_at' => $boleto->deleted_at,
                                'officeimpresso_codigo' => $boleto->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $boleto->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }
                        ...
                        $boleto = FinanceiroBoleto::create([
            ...
            Log::error('Erro na sincronização de FinanceiroBoleto: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Financeiro Boleto atualizado até uma data
     */
    public function getFinanceiroBoletoUntilDate(Request $request)
    {
        ...
        try {
            ...
            $boletos = FinanceiroBoleto::where('business_id', $business_id)
                ->where('updated_at', '>', $date)
                ->get();

            $response = $boletos->map(function ($boleto) {
                return [
                    'id' => $boleto->id,
            ...
            Log::error('Erro ao buscar Financeiro Boleto: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
