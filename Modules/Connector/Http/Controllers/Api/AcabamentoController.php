...
/**
 * @group Acabamento Management
 * @authenticated
 *
 * APIs para gerenciar acabamentos.
 */
class AcabamentoController extends ApiController
{
    /**
     * Sincronizar Acabamentos
     *
     * @bodyParam data array required Lista de acabamentos para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncAcabamento(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $acabamento = Acabamento::where('business_id', $business_id)
                    ...
                    if ($acabamento) {
                        $updatedAt = $acabamento->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'id' => $acabamento->id,
                                'updated_at' => $acabamento->updated_at->format('Y-m-d H:i:s'),
                                'deleted_at' => $acabamento->deleted_at,
                                'officeimpresso_codigo' => $acabamento->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $acabamento->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }
            ...
            Log::error('Erro na sincronização de Acabamentos: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Acabamentos atualizados até uma data
     */
    public function getAcabamentoUntilDate(Request $request)
    {
        ...
        try {
            ...
            $acabamentos = Acabamento::where('business_id', $business_id)
                ->where('updated_at', '>', $date)
                ->get();

            $response = $acabamentos->map(function ($acabamento) {
                return [
                    'id' => $acabamento->id,
            ...
            Log::error('Erro ao buscar Acabamentos: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
