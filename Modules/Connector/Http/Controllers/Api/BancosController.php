...
/**
 * @group Bancos Management
 * @authenticated
 *
 * APIs para gerenciar Bancos.
 */
class BancosController extends ApiController
{
    /**
     * Sincronizar Bancos
     *
     * @bodyParam data array required Lista de bancos para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncBancos(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $banco = Bancos::where('business_id', $business_id)
                    ...
                    if ($banco) {
                        $updatedAt = $banco->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'id' => $banco->id,
                                'updated_at' => $banco->updated_at->format('Y-m-d H:i:s'),
                                'deleted_at' => $banco->deleted_at,
                                'officeimpresso_codigo' => $banco->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $banco->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }
            ...
            Log::error('Erro na sincronização de Bancos: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Bancos atualizado até uma data
     */
    public function getBancosUntilDate(Request $request)
    {
        ...
        try {
            ...
            $bancos = Bancos::where('business_id', $business_id)
                ->where('updated_at', '>', $date)
                ->get();

            $response = $bancos->map(function ($banco) {
                return [
                    'id' => $banco->id,
            ...
            Log::error('Erro ao buscar Bancos: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
