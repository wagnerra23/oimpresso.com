...
/**
 * @group Contrato Management
 * @authenticated
 *
 * APIs para gerenciar Contratos.
 */
class ContratoController extends ApiController
{
     ...
     *
     * @bodyParam data array required Lista de contratos para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncContrato(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $contrato = Contrato::where('business_id', $business_id)
                    ...
                    if ($contrato) {
                        $updatedAt = $contrato->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'id' => $contrato->id,
                                'updated_at' => $contrato->updated_at->format('Y-m-d H:i:s'),
                                'deleted_at' => $contrato->deleted_at,
                                'officeimpresso_codigo' => $contrato->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $contrato->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }
            ...
            Log::error('Erro na sincronização de Contrato: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Contratos atualizados até uma data
     */
    public function getContratoUntilDate(Request $request)
    {
        ...
        try {
            ...
            $contratos = Contrato::where('business_id', $business_id)
                ->where('updated_at', '>', $date)
                ->get();

            $response = $contratos->map(function ($contrato) {
                return [
                    'id' => $contrato->id,
            ...
            Log::error('Erro ao buscar Contratos: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
