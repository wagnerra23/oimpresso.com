...
/**
 * @group Centro Custo Management
 * @authenticated
 *
 * APIs para gerenciar Centro Custo.
 */
class CentroCustoController extends ApiController
{
    /**
     * Sincronizar Centro Custo
     *
     * @bodyParam data array required Lista de centros de custo para sincronizar.
     * @response {
     *   "status": "completed",
     ...
     * }
     */
    public function syncCentroCusto(Request $request)
    {
        try {
            ...
            foreach ($data as $item) {
                try {
                    $centroCusto = CentroCusto::where('business_id', $business_id)
                    ...
                    if ($centroCusto) {
                        $updatedAt = $centroCusto->updated_at->format('Y-m-d H:i:s');
                        ...
                        if ($updatedAt !== $oimpressoUpdatedAt) {
                            $response[] = [
                                ...
                                'id' => $centroCusto->id,
                                'updated_at' => $centroCusto->updated_at->format('Y-m-d H:i:s'),
                                'deleted_at' => $centroCusto->deleted_at,
                                'officeimpresso_codigo' => $centroCusto->officeimpresso_codigo,
                                'officeimpresso_dt_alteracao' => $centroCusto->officeimpresso_dt_alteracao,
                            ];
                            continue;
                        }
            ...
            Log::error('Erro na sincronização de Centro Custo: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * Listar Centro Custo atualizado até uma data
     */
    public function getCentroCustoUntilDate(Request $request)
    {
        ...
        try {
            ...
            $centroCustos = CentroCusto::where('business_id', $business_id)
                ->where('updated_at', '>', $date)
                ->get();

            $response = $centroCustos->map(function ($centroCusto) {
                return [
                    'id' => $centroCusto->id,
            ...
            Log::error('Erro ao buscar Centro Custo: ' . $e->getMessage());
            return response()->json([
                'status' => 'error',
