...
/**
 * Varre resources/js/Pages/**\/*.tsx procurando blocos `@memcofre` e
...
{
    protected $signature = 'memcofre:sync-pages
                            {--dry : Não grava — só mostra o que seria sincronizado}';

    protected $description = 'Varre telas React (.tsx) procurando blocos @memcofre e popula docs_pages';
    ...
    public function handle(): int
    {
        ...
        $this->info("Com bloco @memcofre:   {$found}");
        $this->line("Sem bloco @memcofre:   {$missing}");
        if (! $this->option('dry')) {
    ...
    {
        // Procura a linha '// @memcofre' e coleta linhas seguintes '// campo: valor'
        ...
        foreach ($lines as $line) {
            ...
            if (! $inBlock) {
                if (preg_match('/^\/\/\s*@memcofre\b/', $trimmed)) {
                    $inBlock = true;
                }
                continue;
            }
