...
use Modules\MemCofre\Entities\DocValidationRun;

/**
 * Valida a integridade da documentação do MemCofre.
 *
 ...
 *   4. PAGE_NO_META:   .tsx sem bloco @memcofre
...
class DocValidator
{
    ...
    public function validate(?string $onlyModule = null): array
    {
        ...
        // 4. PAGE_NO_META: .tsx sem @memcofre (roda uma vez só, global)
        if (! $onlyModule) {
            ...
            if (File::isDirectory($pagesRoot)) {
                ...
                foreach ($it as $f) {
                    ...
                    if ($head === false || ! str_contains($head, '@memcofre')) {
                        ...
                        $issues[] = [
                            ...
                            'message' => "Tela {$relative} não tem bloco @memcofre.",
                        ];
                    }
                }
            }
