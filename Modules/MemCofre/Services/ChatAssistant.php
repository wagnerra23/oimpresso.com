...
class ChatAssistant
{
    ...
    public function ask(string $question, ?string $moduleContext = null): array
    {
        ...
        if (empty($snippets)) {
            return [
                'reply'       => "Não encontrei nada relevante nos documentos do MemCofre sobre isso. Tente termos diferentes ou especifique um módulo.",
                'sources'     => [],
                'mode'        => 'offline',
                'tokens_used' => null,
            ];
    ...
    {
        $lines = ["Encontrei " . count($snippets) . " trecho(s) relevante(s) no MemCofre:\n"];
    ...
    {
        return (bool) config('memcofre.ai.enabled', false) && env('OPENAI_API_KEY');
    }
    ...
    protected function askWithAi(string $question, array $snippets): array
    {
        ...
        $systemPrompt = <<<SYS
Você é o assistente do MemCofre — sistema de documentação viva do projeto OI Impresso (stack Laravel 9.51 + PHP 8.4 + React/Inertia + Tailwind 4).

Responda em português brasileiro, curto e direto. Use markdown quando ajudar (listas, code blocks).

Consulte EXCLUSIVAMENTE os trechos fornecidos abaixo. Se a resposta não estiver neles, diga "não encontrei no MemCofre" e sugira onde procurar (módulo, tipo de arquivo).
...
SYS;
        ...
        try {
            $result = \OpenAI\Laravel\Facades\OpenAI::chat()->create([
                'model'       => (string) config('memcofre.ai.model', 'gpt-4o-mini'),
                'messages'    => [
                    ['role' => 'system', 'content' => $systemPrompt],
                    ['role' => 'user',   'content' => $question],
                ],
                'temperature' => (float) config('memcofre.ai.temperature', 0.2),
                'max_tokens'  => (int) config('memcofre.ai.max_tokens', 800),
            ]);
