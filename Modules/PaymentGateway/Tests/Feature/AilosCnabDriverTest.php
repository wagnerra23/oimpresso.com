 ...
 *
 * Cobertura AilosCnabDriver (driver fino sobre CnabBoletoAdapter Onda 4f.0):
 *   - key/supports
...
function setupAilosCnabSchema(): void
{
...
beforeEach(function () {
    ...
    $this->cred = PaymentGatewayCredential::query()->create([
        'business_id'  => 1,
        'gateway_key'  => 'ailos_cnab',
        'ambiente'     => 'production',
        'ativo'        => true,
        'nome_display' => 'Ailos (ex-Cecred) CNAB cooperativa SC',
        'config_json'  => [
            'agencia'             => '0101',
            'agenciaDv'           => '0',
            'conta'               => '012345',
            'contaDv'             => '6',
            'convenio'            => '1234567',
            'carteira'            => '1', // Cooperativa Ailos cobrança simples
    ...
    $this->driver = new AilosCnabDriver();
});

// ─── key / supports ──────────────────────────────────────────────────────

it('has gateway key ailos_cnab', function () {
    expect($this->driver->key())->toBe('ailos_cnab');
});
...
it('throws on emitirPix (CNAB não suporta PIX)', function () {
    $input = new EmitirCobrancaInput(
        ...
        descricao: 'x', idempotencyKey: 'ailos-pix-k1',
    );
...
it('throws on cobrarCartao (CNAB não suporta cartão)', function () {
    $input = new EmitirCobrancaInput(
        businessId: 1, contactId: 1, valorCentavos: 1000,
        vencimento: new DateTimeImmutable('+5 days'),
        descricao: 'x', idempotencyKey: 'ailos-card-k1',
    );
...
it('emitirBoleto gera cnab remessa e retorna nosso numero', function () {
    ...
    expect($result->payloadGateway['gateway_key'])->toBe('ailos_cnab');
...
it('emitirBoleto lança CredentialMisconfiguredException sem agencia', function () {
    ...
    $input = new EmitirCobrancaInput(
        businessId: 1, contactId: 1, valorCentavos: 1000,
        vencimento: new DateTimeImmutable('+5 days'),
        descricao: 'x', idempotencyKey: 'ailos-bad-001',
    );
...
it('respeita business id global scope (biz=2 não enxerga credencial biz=1)', function () {
    ...
    $input = new EmitirCobrancaInput(
        ...
        descricao: 'Boleto biz 2', idempotencyKey: 'ailos-biz2-001',
        meta: [
