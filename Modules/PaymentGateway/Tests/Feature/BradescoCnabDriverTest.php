 ...
 *
 * Cobertura BradescoCnabDriver (driver fino sobre CnabBoletoAdapter Onda 4f.0):
 *   - key/supports
...
function setupBradescoCnabSchema(): void
{
...
beforeEach(function () {
    ...
    $this->cred = PaymentGatewayCredential::query()->create([
        ...
        'ambiente'     => 'production',
        'ativo'        => true,
        'nome_display' => 'Bradesco CNAB PME',
        'config_json'  => [
            'agencia'             => '1234',
            'conta'               => '0567890',
            'carteira'            => '09', // PME mais comum
    ...
    $this->driver = new BradescoCnabDriver();
});

// ─── key / supports ──────────────────────────────────────────────────────

it('has gateway key bradesco_cnab', function () {
...
it('throws on emitirPix (CNAB não suporta PIX)', function () {
    $input = new EmitirCobrancaInput(
        ...
        descricao: 'x', idempotencyKey: 'bradesco-pix-k1',
    );
...
it('throws on cobrarCartao (CNAB não suporta cartão)', function () {
    $input = new EmitirCobrancaInput(
        businessId: 1, contactId: 1, valorCentavos: 1000,
        vencimento: new DateTimeImmutable('+5 days'),
        descricao: 'x', idempotencyKey: 'bradesco-card-k1',
    );
...
it('emitirBoleto gera cnab remessa e retorna nosso numero', function () {
    $input = new EmitirCobrancaInput(
        ...
        idempotencyKey: 'bradesco-cob-001',
        meta: [
...
it('emitirBoleto lança CredentialMisconfiguredException sem agencia', function () {
    ...
    $input = new EmitirCobrancaInput(
        businessId: 1, contactId: 1, valorCentavos: 1000,
        vencimento: new DateTimeImmutable('+5 days'),
        descricao: 'x', idempotencyKey: 'bradesco-bad-001',
    );
...
it('respeita business id global scope (biz=2 não enxerga credencial biz=1)', function () {
    ...
    $idsBiz1 = PaymentGatewayCredential::query()
        ->where('gateway_key', 'bradesco_cnab')
    ...
    $input = new EmitirCobrancaInput(
        ...
        descricao: 'Boleto biz 2', idempotencyKey: 'bradesco-biz2-001',
        meta: [
