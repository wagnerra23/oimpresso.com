 ...
 *
 * Cobertura CaixaCnabDriver (driver fino sobre CnabBoletoAdapter Onda 4f.0):
 *   - key/supports
...
function setupCaixaCnabSchema(): void
{
...
beforeEach(function () {
    ...
    $this->cred = PaymentGatewayCredential::query()->create([
        'business_id'  => 1,
        'gateway_key'  => 'caixa_cnab',
        'ambiente'     => 'production',
        'ativo'        => true,
        'nome_display' => 'Caixa SIGCB PJ',
        'config_json'  => [
            'agencia'             => '4321',
            'codigo_cliente'      => '123456',
            'carteira'            => 'RG', // SIGCB Cobrança Registrada
    ...
    $this->driver = new CaixaCnabDriver();
});

// ─── key / supports ──────────────────────────────────────────────────────

it('has gateway key caixa_cnab', function () {
    expect($this->driver->key())->toBe('caixa_cnab');
});
...
it('throws on emitirPix (CNAB não suporta PIX)', function () {
    $input = new EmitirCobrancaInput(
        ...
        descricao: 'x', idempotencyKey: 'caixa-pix-k1',
    );
...
it('throws on cobrarCartao (CNAB não suporta cartão)', function () {
    $input = new EmitirCobrancaInput(
        businessId: 1, contactId: 1, valorCentavos: 1000,
        vencimento: new DateTimeImmutable('+5 days'),
        descricao: 'x', idempotencyKey: 'caixa-card-k1',
    );
...
it('emitirBoleto gera cnab remessa e retorna nosso numero', function () {
    ...
    expect($result->payloadGateway['gateway_key'])->toBe('caixa_cnab');
...
it('emitirBoleto lança CredentialMisconfiguredException sem agencia', function () {
    ...
    $input = new EmitirCobrancaInput(
        businessId: 1, contactId: 1, valorCentavos: 1000,
        vencimento: new DateTimeImmutable('+5 days'),
        descricao: 'x', idempotencyKey: 'caixa-bad-001',
    );
...
it('respeita business id global scope (biz=2 não enxerga credencial biz=1)', function () {
    ...
    $input = new EmitirCobrancaInput(
        ...
        descricao: 'Boleto biz 2', idempotencyKey: 'caixa-biz2-001',
        meta: [
