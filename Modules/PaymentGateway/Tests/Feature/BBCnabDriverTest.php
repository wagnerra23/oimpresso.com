 ...
 *
 * Cobertura BBCnabDriver (driver fino sobre CnabBoletoAdapter Onda 4f.0):
 *   - key/supports
...
function setupBBCnabSchema(): void
{
...
beforeEach(function () {
    ...
    $this->cred = PaymentGatewayCredential::query()->create([
        ...
        'ambiente'     => 'production',
        'ativo'        => true,
        'nome_display' => 'BB CNAB PJ Convenio',
        'config_json'  => [
            'agencia'             => '3478',
            'conta'               => '0012345',
            'convenio'            => '1234567',
            'carteira'            => '17', // Cobrança Simples Eletrônica BB (mais comum PJ)
    ...
    $this->driver = new BBCnabDriver();
});

// ─── key / supports ──────────────────────────────────────────────────────

it('has gateway key bb_cnab', function () {
    expect($this->driver->key())->toBe('bb_cnab');
});
...
it('throws on emitirPix (CNAB não suporta PIX)', function () {
    $input = new EmitirCobrancaInput(
        ...
        descricao: 'x', idempotencyKey: 'bb-pix-k1',
    );
...
it('throws on cobrarCartao (CNAB não suporta cartão)', function () {
    $input = new EmitirCobrancaInput(
        businessId: 1, contactId: 1, valorCentavos: 1000,
        vencimento: new DateTimeImmutable('+5 days'),
        descricao: 'x', idempotencyKey: 'bb-card-k1',
    );
...
it('emitirBoleto gera cnab remessa e retorna nosso numero', function () {
    ...
    expect($result->payloadGateway['gateway_key'])->toBe('bb_cnab');
...
it('emitirBoleto lança CredentialMisconfiguredException sem agencia', function () {
    ...
    $input = new EmitirCobrancaInput(
        businessId: 1, contactId: 1, valorCentavos: 1000,
        vencimento: new DateTimeImmutable('+5 days'),
        descricao: 'x', idempotencyKey: 'bb-bad-001',
    );
...
it('respeita business id global scope (biz=2 não enxerga credencial biz=1)', function () {
    ...
    $input = new EmitirCobrancaInput(
        ...
        descricao: 'Boleto biz 2', idempotencyKey: 'bb-biz2-001',
        meta: [
