...
class CloneProposalRepository {
    ...
    public function clone ($data = []) {
        ...
        $proposal = \Modules\Grow\Models\Proposal::Where('doc_id', $data['doc_id'])->first();
        ...
        //clone line items
        if ($lineitems = \Modules\Grow\Models\Lineitem::Where('lineitemresource_id', $estimate->bill_estimateid)->Where('lineitemresource_type', 'estimate')->orderBy('lineitem_position', 'asc')->get()) {
    ...
    public function validateData($data) {
        ...
        if ($data['docresource_type'] == 'client') {
            if (\Modules\Grow\Models\Client::Where('client_id', $data['docresource_id'])->doesntExist()) {
        ...
        if ($data['docresource_type'] == 'client') {
            if ($user = \Modules\Grow\Models\User::Where('clientid', $data['docresource_id'])->Where('account_owner', 'yes')->first()) {
        ...
        if ($data['docresource_type'] == 'lead') {
            if ($lead = \Modules\Grow\Models\Lead::Where('lead_id', $data['docresource_id'])->first()) {
