...
use Modules\Grow\Models\EventTracking;
...
class EventTrackingRepository {
    ...
    function recordEvent($event = '', $users = '', $event_id = '') {
        ...
        if ($users instanceof \Modules\Grow\Models\User) {
            $userlist[] = $users->id;
            $users = $userlist;
        }
        ...
        foreach ($users as $userid) {
            ...
            if ($userid != auth()->id() && $event['event_notification_category'] != '') {
                ...
                if ($user = \Modules\Grow\Models\User::Where('id', $userid)->first()) {

                    //get users role
                    $role = \Modules\Grow\Models\Role::Where('role_id', $user->role_id)->first();
