...
class ProjectRepository {
    ...
    public function search($id = '', $data = []) {
        ...
        if (request('action') == 'search') {
            if ($fields = \Modules\Grow\Models\CustomField::Where('customfields_type', 'projects')->Where('customfields_show_filter_panel', 'yes')->get()) {
    ...
    public function switchPermissionsProtocol($type = '') {
        ...
        if ($type == 'user_roles') {
            ...
            \Modules\Grow\Models\ProjectAssigned::getQuery()->delete();
        }

        //set assigned users based on category users
        if ($type == 'category_based') {
            //get all categories
            $categories = \Modules\Grow\Models\Category::Where('category_type', 'project')->get();
            //loop through
    ...
    public function assignCategoryUsers($category_id = '', $project_id = '') {
        ...
        \Modules\Grow\Models\ProjectAssigned::Where('projectsassigned_projectid', $project_id)->delete();

        //get the category
        if ($category = \Modules\Grow\Models\Category::Where('category_type', 'project')->where('category_id', $category_id)->first()) {
            //category users
            $users = $category->users;
            //assign
            foreach ($users as $user) {
    ...
    public function unassignCategoryProjects($category_id = '') {
        ...
        if ($category = \Modules\Grow\Models\Category::Where('category_type', 'project')->where('category_id', $category_id)->first()) {
            ...
            foreach ($projects as $project) {
                if ($project->project_type == 'project') {
                    \Modules\Grow\Models\ProjectAssigned::Where('projectsassigned_projectid', $project->project_id)->delete();
                }
            }
        }
    }
    ...
    public function usersProjects($id) {
        ...
        //get the user
        if (!$user = \Modules\Grow\Models\User::Where('id', $id)->With('role')->first()) {
            $projects->where('project_type', 'foobar'); //just to ensure we atleast return an object
            return $projects->get();
        }
    ...
    public function getDefaultFilesFolder($id = '') {
        ...
        if (is_numeric($id) && $folder = \Modules\Grow\Models\FileFolder::Where('filefolder_projectid', $id)->Where('filefolder_default', 'yes')->first()) {
