...
use Modules\Grow\Models\Invoice;
...
class InvoiceRepository {
    ...
    public function refreshInvoice($invoice) {
        ...
        if (!$invoice instanceof \Modules\Grow\Models\Invoice) {
            return false;
        }
    ...
    public function saveLineItems($bill_invoiceid = '') {
        ...
        //delete line items
        \Modules\Grow\Models\Lineitem::Where('lineitemresource_type', 'invoice')
        ...
        if (is_array(request('js_item_description'))) {
            foreach (request('js_item_description') as $key => $description) {
                ...
                if (request('js_item_linked_type')[$key] == 'expense' && request('js_item_linked_id')[$key]) {
                    \Modules\Grow\Models\Expense::where('expense_id', request('js_item_linked_id')[$key])
                        ->update([
                ...
                if (request('js_item_linked_type')[$key] == 'timer' && is_numeric(request('js_item_linked_id')[$key])) {
                    ...
                    \Modules\Grow\Models\Timer::whereIn('timer_id', $timers)
                        ->update([
    ...
    public function unlinkItems($bill_invoiceid = '') {
        ...
        \Modules\Grow\Models\Expense::where('expense_billable_invoiceid', $bill_invoiceid)
            ->update([
        ...
        //[unlink][billed task]
        \Modules\Grow\Models\Timer::where('timer_billing_invoiceid', $bill_invoiceid)
            ->update([
    ...
    public function processAffiliateEarnings($invoice_id = '') {
        ...
        $affiliates = \Modules\Grow\Models\Custom\CSAffiliateProject::Where('cs_affiliate_project_projectid', $invoice->bill_projectid)
            ->where('cs_affiliate_project_status', 'active')
            ->get();

        //get the invoice
        $project = \Modules\Grow\Models\Project::Where('project_id', $invoice->bill_projectid)->first();

        //reccord earnings
        foreach ($affiliates as $affiliate) {

            //record earning
            $earning = new \Modules\Grow\Models\Custom\CSAffiliateEarning();
