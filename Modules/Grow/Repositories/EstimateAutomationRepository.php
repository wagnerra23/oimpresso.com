...
class EstimateAutomationRepository {
    ...
    public function __construct(
        ...
        //system settings
        $this->settings = \Modules\Grow\Models\Settings::Where('settings_id', 1)->first();
    ...
    public function createProject($estimate) {
        ...
        if (is_numeric($estimate->estimate_automation_log_created_project_id)) {
            if (\Modules\Grow\Models\Project::Where('project_id', $estimate->estimate_automation_log_created_project_id)->exists()) {
        ...
        $project = new \Modules\Grow\Models\Project();
    ...
    public function assignProject($project, $estimate) {
        ...
        foreach ($assigned_users as $assigned_user) {
            $assigned = new \Modules\Grow\Models\ProjectAssigned();
        ...
        //email
        foreach ($assigned_users as $assigned_user) {
            if ($user = \Modules\Grow\Models\User::Where('id', $assigned_user->automationassigned_userid)->first()) {
    ...
    public function createTasks($project, $estimate) {
        ...
        $milestone = \Modules\Grow\Models\Milestone::Where('milestone_projectid', $project->project_id)->Where('milestone_type', 'uncategorised')->first();

        //get estimate line items
        if ($items = \Modules\Grow\Models\Lineitem::Where('lineitemresource_type', 'estimate')->where('lineitemresource_id', $estimate->bill_estimateid)->get()) {
            ...
            foreach ($items as $item) {
                ...
                if (is_numeric($item->lineitem_linked_product_id) && $product_tasks = \Modules\Grow\Models\ProductTask::Where('product_task_itemid', $item->lineitem_linked_product_id)->get()) {
                    ...
                    foreach ($product_tasks as $task_template) {
                        $task = new \Modules\Grow\Models\Task();
    ...
    public function createTaskDependencies($project, $product_tasks_list, $product_tasks_map) {
        ...
        if ($dependencies = \Modules\Grow\Models\ProductTasksDependency::WhereIn('product_task_dependency_taskid', $product_tasks_list)->get()) {
            ...
            foreach ($dependencies as $dependency) {
                $task_dependency = new \Modules\Grow\Models\TaskDependency();
    ...
    public function assignTaskUsers($task, $project, $obj, $type) {
        ...
        if ($type == 'specified') {
            ...
            foreach ($assigned_users as $project_user) {
                if (\Modules\Grow\Models\ProjectAssigned::Where('projectsassigned_projectid', $project->project_id)
        ...
        $users = [];
        foreach ($assigned_users as $assigned_user) {
            $assigned = new \Modules\Grow\Models\TaskAssigned();
        ...
        foreach ($users as $user_id) {
            if ($user = \Modules\Grow\Models\User::Where('id', $user_id)->first()) {
                $data = [
                    'event_creatorid' => 0,
                    'event_item' => 'assigned',
                    'event_item_id' => '',
    ...
    public function createInvoice($estimate, $project) {
        ...
        if (is_numeric($estimate->estimate_automation_log_created_invoice_id)) {
            if (\Modules\Grow\Models\Invoice::Where('bill_invoiceid', $estimate->estimate_automation_log_created_invoice_id)->exists()) {
                //info
        ...
        if ($client = \Modules\Grow\Models\Client::Where('client_id', $estimate->bill_clientid)->first()) {
    ...
    public function copyAttchments($project, $estimate) {
        ...
        if (!$default_folder = \Modules\Grow\Models\FileFolder::Where('filefolder_projectid', $project->project_id)->Where('filefolder_default', 'yes')->first()) {
        ...
        //get all the files
        if ($files = \Modules\Grow\Models\File::Where('fileresource_type', 'estimate')->Where('fileresource_id', $estimate->bill_estimateid)->get()) {
