...
class ContractRepository {
    ...
    public function contactDetails($doc_id = '') {
        ...
        if ($contract = \Modules\Grow\Models\Contract::Where('doc_id', $doc_id)->first()) {
            //client contract
            if ($contract->docresource_type == 'client') {
                //company name
                if ($client = \Modules\Grow\Models\Client::Where('client_id', $contract->doc_client_id)->first()) {
                    $details['company_name'] = $client->client_company_name;
                }
                //first name
                if ($user = \Modules\Grow\Models\User::Where('clientid', $contract->doc_client_id)->where('account_owner', 'yes')->first()) {
            ...
            if ($contract->docresource_type == 'lead') {
                //company name
                if ($lead = \Modules\Grow\Models\Lead::Where('lead_id', $contract->doc_lead_id)->first()) {
    ...
    public function visibilitySignatures($contract = '', $view = '') {
        ...
        //validate
        if (!$contract instanceof \Modules\Grow\Models\Contract) {
            return false;
        }
    ...
    public function publish($id) {
        ...
        //get the project
        if (!$document = \Modules\Grow\Models\Contract::Where('doc_id', $id)->first()) {
