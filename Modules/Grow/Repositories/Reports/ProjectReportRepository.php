...
use Modules\Grow\Models\Client;
...
class ProjectReportRepository {
    ...
    public function getCategory($type = '') {
        ...
        //get all categories
        $categories = \Modules\Grow\Models\Category::Where('category_type', 'project')->orderBy('category_name', 'asc')->get();

        foreach ($categories as $category) {

            //start projects query
            $project_ids = \Modules\Grow\Models\Project::where('project_categoryid', $category->category_id)
            ...
            //count_projects_completed
            $count_projects_completed = \Modules\Grow\Models\Project::where('project_creatorid', $category->category_id)
                ->where('project_type', 'project')
            ...
            //get project ids
            $count_projects_pending = \Modules\Grow\Models\Project::where('project_creatorid', $category->category_id)
                ->where('project_type', 'project')
            ...
            //count_tasks_due
            $count_tasks_due = \Modules\Grow\Models\Task::whereIn('task_projectid', $project_ids)->whereNotIn('task_status', [2])->count();

            //count_tasks_completed
            $count_tasks_completed = \Modules\Grow\Models\Task::whereIn('task_projectid', $project_ids)->where('task_status', 2)->count();

            //sum_hours
            $sum_hours = \Modules\Grow\Models\Timer::whereIn('timer_projectid', $project_ids)->where('timer_status', 'stopped')->sum('timer_time');

            //sum_expenses
            $sum_expenses = \Modules\Grow\Models\Expense::whereIn('expense_projectid', $project_ids)->sum('expense_amount');

            //sum_invoices
            $sum_invoices = \Modules\Grow\Models\Invoice::whereIn('bill_projectid', $project_ids)->whereNotIn('bill_status', ['draft'])->sum('bill_final_amount');

            //sum_payments
            $sum_payments = \Modules\Grow\Models\Payment::whereIn('payment_projectid', $project_ids)->sum('payment_amount');
