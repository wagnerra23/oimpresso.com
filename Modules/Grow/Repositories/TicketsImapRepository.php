...
class TicketsImapRepository {
    ...
    public function trackEmail($email, $folder, $category) {
        ...
        //save
        $imaplog = new \Modules\Grow\Models\ImapLog();
    ...
    public function getThreadTicket($references = []) {
        ...
        if ($ticket = \Modules\Grow\Models\Ticket::whereIn('ticket_imap_sender_email_id', $references)->first()) {
        ...
        //look for a ticket reply with a message id that matches the reference array
        if ($ticket_reply = \Modules\Grow\Models\TicketReply::whereIn('ticketreply_imap_sender_email_id', $references)->first()) {
            if ($ticket = \Modules\Grow\Models\Ticket::where('ticket_id', $ticket_reply->ticketreply_ticketid)->first()) {
                Log::info("IMAP - a ticket was found that matches this email thread - ticket id (" . $ticket->ticket_id . ")", ['process' => '[tickets-imap-process-emails]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
                return $ticket;
            }
        }
    ...
    public function newTicket($email = '', $category = '') {
        ...
        try {
            ...
            //create a new ticket
            $ticket = new \Modules\Grow\Models\Ticket();
            ...
            foreach ($attachments as $attachment) {
                try {
                    ...
                    if ($file_content_id && $file_content_disposition == 'inline') {
                        ...
                        $new_attachment = new \Modules\Grow\Models\Attachment();
    ...
    public function newReply($email = '', $ticket = '') {
        ...
        try {
            ...
            $reply = new \Modules\Grow\Models\TicketReply();
            ...
            if ($status = \Modules\Grow\Models\TicketStatus::Where('ticketstatus_use_for_client_replied', 'yes')->first()) {
    ...
    public function getUser($email) {
        ...
        if ($user = \Modules\Grow\Models\User::Where('email', $from_email)->first()) {
            ...
            $user = new \Modules\Grow\Models\User();
            $user->type = 'contact';
            $user->creatorid = 0;
