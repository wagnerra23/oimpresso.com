...
class CloneContractRepository {
    ...
    public function clone ($data = []) {
        ...
        $contract = \Modules\Grow\Models\Contract::Where('doc_id', $data['doc_id'])->first();

        //get client
        $client = \Modules\Grow\Models\Client::Where('client_id', $data['doc_client_id'])->first();
        ...
        //clone estimate
        if ($estimate = \Modules\Grow\Models\Estimate::Where('bill_estimate_type', 'document')->Where('bill_contractid', $data['doc_id'])->first()) {
            ...
            if ($lineitems = \Modules\Grow\Models\Lineitem::Where('lineitemresource_type', 'estimate')->Where('lineitemresource_id', $estimate->bill_estimateid)->orderBy('lineitem_position', 'asc')->get()) {
    ...
    public function validateData($data) {
        ...
        //validate client
        if (\Modules\Grow\Models\Client::Where('client_id', $data['doc_client_id'])->doesntExist()) {
        ...
        if (is_numeric($data['doc_project_id'])) {
            if (\Modules\Grow\Models\Project::Where('project_id', $data['doc_project_id'])->doesntExist()) {
        ...
        if ($user = \Modules\Grow\Models\User::Where('clientid', $data['doc_client_id'])->Where('account_owner', 'yes')->first()) {
