...
class DestroyRepository {
    ...
    public function destroyProject($project_id) {
        ...
        //delete file folders
        \Modules\Grow\Models\FileFolder::Where('filefolder_projectid', $project->project_id)->delete();
        ...
        \Modules\Grow\Models\EmailQueue::Where('emailqueue_resourcetype', 'project')->Where('emailqueue_resourceid', $project->project_id)->delete();

        //unlink estimate automations
        \Modules\Grow\Models\Estimate::where('estimate_automation_log_created_project_id', $project->project_id)
    ...
    public function destroyClient($client_id) {
        ...
        if (!$client = \Modules\Grow\Models\Client::Where('client_id', $client_id)->first()) {
        ...
        //delete proposals
        \Modules\Grow\Models\Proposal::Where('doc_client_id', $client_id)->getQuery()->delete();

        //delete contracts
        \Modules\Grow\Models\Contract::Where('doc_client_id', $client_id)->getQuery()->delete();
        ...
        //delete queued emails
        \Modules\Grow\Models\EmailQueue::Where('emailqueue_resourcetype', 'client')->Where('emailqueue_resourceid', $client->client_id)->delete();
    ...
    public function destroyTicket($ticket_id) {
        ...
        if (!$ticket = \Modules\Grow\Models\Ticket::Where('ticket_id', $ticket_id)->first()) {
        ...
        //delete queued emails
        \Modules\Grow\Models\EmailQueue::Where('emailqueue_resourcetype', 'ticket')->Where('emailqueue_resourceid', $ticket_id)->delete();

        //delete events
        \Modules\Grow\Models\Event::Where('event_parent_type', 'ticket')->Where('event_parent_id', $ticket_id)->delete();
    ...
    public function destroyTask($task_id) {
        ...
        if ($events = \Modules\Grow\Models\Event::Where('event_parent_type', 'task')->Where('event_parent_id', $task_id)->get()) {
            foreach ($events as $event) {
                $event->trackings()->delete();
                $event->delete();
            }
        ...
        //delete queued emails
        \Modules\Grow\Models\EmailQueue::Where('emailqueue_resourcetype', 'task')->Where('emailqueue_resourceid', $task_id)->delete();
    ...
    public function destroyExpense($expense_id) {
        ...
        //delete queued emails
        \Modules\Grow\Models\EmailQueue::Where('emailqueue_resourcetype', 'expense')->Where('emailqueue_resourceid', $expense_id)->delete();
    ...
    public function destroyComment($comment_id) {
        ...
        if (!$comment = \Modules\Grow\Models\Comment::Where('comment_id', $comment_id)->first()) {
        ...
        //delete events and events tracking
        if ($events = \Modules\Grow\Models\Event::Where('event_item', 'comment')->Where('event_item_id', $comment_id)->get()) {
            foreach ($events as $event) {
                $event->trackings()->delete();
                $event->delete();
            }
    ...
    public function destroyFile($file_id) {
        ...
        //get file and validate
        if (!$file = \Modules\Grow\Models\File::Where('file_id', $file_id)->first()) {
        ...
        //delete events and events tracking
        if ($events = \Modules\Grow\Models\Event::Where('event_item', 'file')->Where('event_item_id', $file_id)->get()) {
            foreach ($events as $event) {
                $event->trackings()->delete();
                $event->delete();
            }
        ...
        \Modules\Grow\Models\Project::where('project_cover_file_id', $file_id)
            ->update([
    ...
    public function destroyTaskAttachment($attachment_id) {
        ...
        if (!$attachment = \Modules\Grow\Models\Attachment::Where('attachment_id', $attachment_id)->first()) {
        ...
        //remove cover from projects that originally used this image as a cover
        \Modules\Grow\Models\Task::where('task_id', $attachment->attachmentresource_id)
            ->update([
                'task_cover_image' => 'no',
    ...
    public function destroyInvoice($invoice_id) {
        ...
        if (!$invoice = \Modules\Grow\Models\Invoice::Where('bill_invoiceid', $invoice_id)->first()) {
        ...
        \Modules\Grow\Models\Expense::where('expense_billable_invoiceid', $invoice_id)->update([
            'expense_billing_status' => 'not_invoiced',
            'expense_billable_invoiceid' => NULL,
        ]);

        //update linked timers to 'not invoiced'
        \Modules\Grow\Models\Timer::where('timer_billing_invoiceid', $invoice_id)->update([
            'timer_billing_status' => 'not_invoiced',
            'timer_billing_invoiceid' => NULL,
        ]);

        \Modules\Grow\Models\Estimate::where('bill_converted_to_invoice_invoiceid', $invoice_id)->update([
            'bill_converted_to_invoice' => null,
            'bill_converted_to_invoice_invoiceid' => NULL,
        ]);

        //delete events
        \Modules\Grow\Models\Event::Where('event_parent_type', 'invoice')->where('event_parent_id', $invoice_id)->delete();
        \Modules\Grow\Models\EventTracking::Where('parent_type', 'invoice')->where('parent_id', $invoice_id)->delete();

        //delete queued emails
        \Modules\Grow\Models\EmailQueue::Where('emailqueue_resourcetype', 'invoice')->Where('emailqueue_resourceid', $invoice_id)->delete();

        //unlink estimate automations
        \Modules\Grow\Models\Estimate::where('estimate_automation_log_created_invoice_id', $invoice_id)
    ...
    public function destroyEstimate($estimate_id) {
        ...
        if (!$estimate = \Modules\Grow\Models\Estimate::Where('bill_estimateid', $estimate_id)->first()) {
        ...
        //delete events
        \Modules\Grow\Models\Event::Where('event_parent_type', 'estimate')->where('event_parent_id', $estimate_id)->delete();
        \Modules\Grow\Models\EventTracking::Where('parent_type', 'estimate')->where('parent_id', $estimate_id)->delete();

        //delete queued emails
        \Modules\Grow\Models\EmailQueue::Where('emailqueue_resourcetype', 'estimate')->Where('emailqueue_resourceid', $estimate_id)->delete();

        //delete automation - assigned users
        \Modules\Grow\Models\AutomationAssigned::Where('automationassigned_resource_type', 'estimate')->Where('automationassigned_resource_id', $estimate_id)->delete();
    ...
    public function destroyLead($lead_id) {
        ...
        //delete events and events tracking
        if ($events = \Modules\Grow\Models\Event::Where('event_parent_type', 'lead')->Where('event_parent_id', $lead_id)->get()) {
            foreach ($events as $event) {
                $event->trackings()->delete();
                $event->delete();
            }
        ...
        //delete queued emails
        \Modules\Grow\Models\EmailQueue::Where('emailqueue_resourcetype', 'lead')->Where('emailqueue_resourceid', $lead_id)->delete();
    ...
    public function destroySubscription($id) {
        ...
        if (!$subscription = \Modules\Grow\Models\Subscription::Where('subscription_id', $id)->first()) {
        ...
        //delete events and events tracking
        if ($events = \Modules\Grow\Models\Event::Where('event_item', 'subscription')->Where('event_item_id', $id)->get()) {
            foreach ($events as $event) {
                $event->trackings()->delete();
                $event->delete();
            }
        ...
        if ($invoices = \Modules\Grow\Models\Invoice::Where('bill_subscriptionid', $id)->get()) {
            foreach ($invoices as $invoice) {
                $this->destroyInvoice($invoice->bill_invoiceid);
            }
        }
    ...
    public function destroyProposal($doc_id) {
        ...
        //get file and validate
        if (!$proposal = \Modules\Grow\Models\Proposal::Where('doc_id', $doc_id)->first()) {
        ...
        //delete events and events tracking
        if ($events = \Modules\Grow\Models\Event::Where('event_item', 'proposal')->Where('event_item_id', $doc_id)->get()) {
            foreach ($events as $event) {
                $event->trackings()->delete();
                $event->delete();
            }
