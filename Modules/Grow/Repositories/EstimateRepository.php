...
use Modules\Grow\Models\Estimate;
...
class EstimateRepository {
    ...
    public function createProposalEstimate($id) {
        ...
        if (\Modules\Grow\Models\Estimate::Where('bill_proposalid', $id)->Where('bill_estimate_type', 'document')->exists()) {
            return;
        }
    ...
    public function createContractEstimate($id) {
        ...
        //check that we do not already have an estimate
        if (\Modules\Grow\Models\Estimate::Where('bill_contractid', $id)->Where('bill_estimate_type', 'document')->exists()) {
            return;
        }
    ...
    public function refreshEstimate($estimate) {
        ...
        if (!$estimate instanceof \Modules\Grow\Models\Estimate) {
    ...
    public function saveLineItems($bill_estimateid = '') {
        ...
        //get the estimate
        if (!$estimate = \Modules\Grow\Models\Estimate::Where('bill_estimateid', $bill_estimateid)->first()) {
        ...
        //delete line items
        \Modules\Grow\Models\Lineitem::Where('lineitemresource_type', 'estimate')
    ...
    public function convertEstimateToInvoice($id) {
        ...
        //get the line items
        $lines = \Modules\Grow\Models\Lineitem::Where('lineitemresource_type', 'estimate')->where('lineitemresource_id', $id)->get();

        //get the line items
        $taxes = \Modules\Grow\Models\Tax::Where('taxresource_type', 'estimate')->where('taxresource_id', $id)->get();

        //create an invocie
        $invoice = new \Modules\Grow\Models\Invoice();
        ...
        foreach ($lines as $line) {
            ...
            if ($estimate->bill_tax_type == 'inline') {
                ...
                $taxes = \Modules\Grow\Models\Tax::Where('tax_lineitem_id', $line->lineitem_id)->get();
                foreach ($taxes as $tax_x) {
                    //get clean tax item for cloning
                    if ($tax = \Modules\Grow\Models\Tax::Where('tax_id', $tax_x->tax_id)->first()) {
