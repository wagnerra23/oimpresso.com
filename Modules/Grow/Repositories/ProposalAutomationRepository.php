...
class ProposalAutomationRepository {
    ...
    public function createProject($proposal) {
        ...
        if (is_numeric($proposal->proposal_automation_log_created_project_id)) {
            if (\Modules\Grow\Models\Project::Where('project_id', $proposal->proposal_automation_log_created_project_id)->exists()) {
        ...
        \Modules\Grow\Models\Proposal::where('doc_id', $proposal->doc_id)
            ->update([
    ...
    public function createTasks($project, $proposal) {
        ...
        if (!$estimate = \Modules\Grow\Models\Estimate::Where('bill_proposalid', $proposal->doc_id)->first()) {
    ...
    public function createInvoice($proposal, $project) {
        ...
        if (is_numeric($proposal->proposal_automation_log_created_invoice_id)) {
            if (\Modules\Grow\Models\Invoice::Where('bill_invoiceid', $proposal->proposal_automation_log_created_invoice_id)->exists()) {
                //info
        ...
        if ($client = \Modules\Grow\Models\Client::Where('client_id', $proposal->doc_client_id)->first()) {
    ...
    public function emailInvoice($bill_invoiceid) {
        ...
        if (!$invoice = \Modules\Grow\Models\Invoice::Where('bill_invoiceid', $bill_invoiceid)->first()) {
