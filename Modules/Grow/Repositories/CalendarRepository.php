...
use Modules\Grow\Models\CalendarEvent;
use Modules\Grow\Models\Project;
use Modules\Grow\Models\Reminder;
...
class CalendarRepository {
    ...
    public function projectEvents($all_events = [], $id = '') {
        ...
        foreach ($rows as $project) {
            ...
            if ($id) {
                if ($files = \Modules\Grow\Models\File::Where('fileresource_type', 'project')
            ...
            if ($id) {
                //created by
                $event['extendedProps']['creator'] = \Modules\Grow\Models\User::Where('id', $project->project_creatorid)->first();
    ...
    public function taskEvents($all_events = [], $id = '') {
        ...
        foreach ($rows as $task) {
            ...
            //get file attachments
            if ($id) {
                if ($files = \Modules\Grow\Models\Attachment::Where('attachmentresource_type', 'task')
            ...
            if ($id) {
                //created by
                $event['extendedProps']['creator'] = \Modules\Grow\Models\User::Where('id', $task->task_creatorid)->first();
    ...
    public function calendarEvents($all_events = [], $id = '') {
        ...
        foreach ($rows as $calendarevent) {
            ...
            //get file attachments
            if ($id) {
                if ($files = \Modules\Grow\Models\Attachment::Where('attachmentresource_type', 'calendarevent')
            ...
            if ($id) {
                //created by
                $event['extendedProps']['creator'] = \Modules\Grow\Models\User::Where('id', $calendarevent->calendar_event_creatorid)->first();
    ...
    public function updateEvent($data = []) {
        ...
        //fetch event
        if (!$event = \Modules\Grow\Models\CalendarEvent::Where('calendar_event_uniqueid', $data['event_id'])->first()) {
    ...
    public function updateProject($data = []) {
        ...
        //fetch the project
        if (!$project = \Modules\Grow\Models\Project::Where('project_uniqueid', $data['event_id'])->first()) {
        ...
        if (request()->filled('attachments')) {
            ...
            $default_folder = \Modules\Grow\Models\FileFolder::Where('filefolder_projectid', request('fileresource_id'))
    ...
    public function updateTask($data = []) {
        ...
        //fetch the task
        if (!$task = \Modules\Grow\Models\Task::Where('task_uniqueid', $data['event_id'])->first()) {
