...
class CloneEstimateRepository {
    ...
    public function clone ($data = []) {
        ...
        $estimate = \Modules\Grow\Models\Estimate::Where('bill_estimateid', $data['estimate_id'])->first();
        ...
        foreach ($payload['lineitems'] as $lineitem_x) {
            ...
            if (!$lineitem = \Modules\Grow\Models\Lineitem::Where('lineitem_id', $lineitem_x->lineitem_id)->first()) {
                //skip it
                $continue;
            }
            ...
            if ($estimate->bill_tax_type == 'inline') {
                ...
                $taxes = \Modules\Grow\Models\Tax::Where('tax_lineitem_id', $lineitem_x->lineitem_id)->get();
    ...
    private function validateData($data = []) {
        ...
        //check client exists
        if (!$client = \Modules\Grow\Models\Client::Where('client_id', $data['client_id'])->first()) {
        ...
        if (isset($data['project_id']) && is_numeric($data['project_id'])) {
            if (!$project = \Modules\Grow\Models\Project::Where('project_id', $data['project_id'])->first()) {
