...
class CloneProjectRepository {
    ...
    public function clone ($data = []) {
        ...
        $project = \Modules\Grow\Models\Project::Where('project_id', $data['project_id'])->first();
    ...
    public function cloneTemplate($data = []) {
        ...
        //get clean the object for cloning
        if (!$new_project = \Modules\Grow\Models\Project::Where('project_id', $data['new_project_id'])->first()) {
        ...
        //get clean the object for cloning
        if (!$template = \Modules\Grow\Models\Project::Where('project_id', $data['template_project_id'])->first()) {
    ...
    private function copyMilestones($old_project, $new_project) {
        ...
        if ($this->data['copy_milestones'] == 'on') {
            if ($milestones = \Modules\Grow\Models\Milestone::Where('milestone_projectid', $old_project->project_id)->get()) {
    ...
    private function copyTasks($old_project, $new_project, $mileston_map) {
        ...
        if ($this->data['copy_tasks'] == 'on' && $this->data['copy_milestones'] == 'on') {
            ...
            if ($tasks = \Modules\Grow\Models\Task::Where('task_projectid', $old_project->project_id)->get()) {
                foreach ($tasks as $task) {
                    ...
                    if ($this->data['copy_tasks_checklist'] == 'on') {
                        if ($checklists = \Modules\Grow\Models\Checklist::Where('checklistresource_type', 'task')->Where('checklistresource_id', $task->task_id)->get()) {
                    ...
                    if ($this->data['copy_tasks_files'] == 'on') {
                        if ($attachments = \Modules\Grow\Models\Attachment::Where('attachmentresource_type', 'task')->Where('attachmentresource_id', $task->task_id)->get()) {
            ...
            //copy dependencies
            if ($dependencies = \Modules\Grow\Models\TaskDependency::Where('tasksdependency_projectid', $old_project->project_id)->get()) {
                foreach ($dependencies as $dependency) {

                    //find new dependant task
                    if ($dependant_task = \Modules\Grow\Models\Task::Where('task_cloning_original_task_id', $dependency->tasksdependency_taskid)->first()) {

                        //find new blocking task
                        if ($blocking_task = \Modules\Grow\Models\Task::Where('task_cloning_original_task_id', $dependency->tasksdependency_blockerid)->first()) {
    ...
    private function copyInvoices($old_project, $new_project) {
        ...
        if ($this->data['copy_invoices'] == 'on') {
            if ($invoices = \Modules\Grow\Models\Invoice::Where('bill_projectid', $old_project->project_id)->get()) {
    ...
    private function copyEstimates($old_project, $new_project) {
        ...
        if ($this->data['copy_estimates'] == 'on') {
            if ($estimates = \Modules\Grow\Models\Estimate::Where('bill_projectid', $old_project->project_id)->get()) {
    ...
    private function copyFolders($old_project, $new_project) {
        ...
        $folders = \Modules\Grow\Models\FileFolder::Where('filefolder_projectid', $old_project->project_id)->get();

        //delete any folders that may have been created in the project controller steps
        \Modules\Grow\Models\FileFolder::Where('filefolder_projectid', $new_project->project_id)->delete();
    ...
    private function copyFiles($old_project, $new_project, $folder_mapping) {
        ...
        if ($this->data['copy_files'] == 'on') {
            if ($files = \Modules\Grow\Models\File::Where('fileresource_type', 'project')->Where('fileresource_id', $old_project->project_id)->get()) {
    ...
    private function copyTags($old_project, $new_project) {
        ...
        //we are copying
        if ($tags = \Modules\Grow\Models\Tag::Where('tagresource_type', 'project')->Where('tagresource_id', $old_project->project_id)->get()) {
            foreach ($tags as $tag) {
