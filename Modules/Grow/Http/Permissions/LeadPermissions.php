...
class LeadPermissions {
    ...
    public function check($action = '', $lead = '') {
        ...
        //GET THE RESOURCE
        if (is_numeric($lead)) {
            if (!$lead = \Modules\Grow\Models\Lead::Where('lead_id', $lead)->first()) {
        ...
        if ($lead instanceof \Modules\Grow\Models\Lead || $lead instanceof \Illuminate\Pagination\LengthAwarePaginator) {
        ...
        if ($action == 'users') {
            $list = [];
            $users = \Modules\Grow\Models\User::with('role')->get();
