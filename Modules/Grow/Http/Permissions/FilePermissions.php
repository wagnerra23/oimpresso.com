...
class FilePermissions {
    ...
    public function check($action = '', $file = '') {
        ...
        //GET THE RESOURCE
        if (is_numeric($file)) {
            if (!$file = \Modules\Grow\Models\File::Where('file_id', $file)->first()) {
        ...
        if ($file instanceof \Modules\Grow\Models\File || $file instanceof \Illuminate\Pagination\LengthAwarePaginator) {
            //all is ok
        } else {
            Log::error("the file coud not be found", ['process' => '[permissions][file]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
