...
class NotePermissions {
    ...
    public function check($action = '', $note = '') {
        ...
        //GET THE RESOURCE
        if (is_numeric($note)) {
            if (!$note = \Modules\Grow\Models\Note::Where('note_id', $note)->first()) {
        ...
        if ($note instanceof \Modules\Grow\Models\Note || $note instanceof \Illuminate\Pagination\LengthAwarePaginator) {
            //all is ok
        } else {
            Log::error("the note coud not be found", ['process' => '[permissions][note]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
