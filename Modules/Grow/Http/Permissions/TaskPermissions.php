...
namespace Modules\Grow\Http\Permissions;
...
class TaskPermissions {
    ...
    public function check($action = '', $task = '', $project = '', $assigned_users = '', $assigned_project_users = '', $project_managers = '') {
        ...
        //GET THE RESOURCE
        if (is_numeric($task)) {
            if (!$task = \Modules\Grow\Models\Task::Where('task_id', $task)->first()) {
        ...
        if ($task instanceof \Modules\Grow\Models\Task || $task instanceof \Illuminate\Pagination\LengthAwarePaginator) {
        ...
        if ($action == 'users') {
            ...
            $list = [];
            $users = \Modules\Grow\Models\User::Where('status', 'active')->with('role')->get();
