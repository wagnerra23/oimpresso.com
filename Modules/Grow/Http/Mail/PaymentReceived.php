...
class PaymentReceived extends Mailable {
    ...
    public function build() {
        ...
        if ($this->user->type == 'team') {
            if (!$template = \Modules\Grow\Models\EmailTemplate::Where('emailtemplate_name', 'New Payment')->first()) {
                return false;
            }
        }
        ...
        if ($this->user->type == 'client') {
            if (!$template = \Modules\Grow\Models\EmailTemplate::Where('emailtemplate_name', 'Thank You For Payment')->first()) {
                return false;
            }
        }

        //validate
        if (!$this->obj instanceof \Modules\Grow\Models\Invoice || !$this->user instanceof \Modules\Grow\Models\User) {
            return false;
        }
