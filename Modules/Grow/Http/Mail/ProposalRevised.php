...
class ProposalRevised extends Mailable {
    ...
    public function build() {
        ...
        //email template
        if (!$template = \Modules\Grow\Models\EmailTemplate::Where('emailtemplate_name', 'Proposal Revised')->first()) {
            return false;
        }

        //validate
        if (!$this->obj instanceof \Modules\Grow\Models\Proposal) {
            return false;
        }
        ...
        if ($this->data['user_type'] == 'client') {
            //validate
            if (!$this->user instanceof \Modules\Grow\Models\User) {
                return false;
            }
        ...
        if ($this->data['user_type'] == 'lead') {
            //validate
            if (!$this->user instanceof \Modules\Grow\Models\Lead) {
                return false;
            }
            //set vars
