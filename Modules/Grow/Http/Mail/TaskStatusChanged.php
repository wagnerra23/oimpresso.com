...
class TaskStatusChanged extends Mailable {
    ...
    public function build() {
        ...
        //email template
        if (!$template = \Modules\Grow\Models\EmailTemplate::Where('emailtemplate_name', 'Task Status Change')->first()) {
            return false;
        }

        //validate
        if (!$this->obj instanceof \Modules\Grow\Models\Task || !$this->user instanceof \Modules\Grow\Models\User) {
            return false;
        }
        ...
        //get the task status
        if($task_status = \Modules\Grow\Models\TaskStatus::Where('taskstatus_id', $this->obj->task_status)->first()){
        ...
        $queue = new \Modules\Grow\Models\EmailQueue();
