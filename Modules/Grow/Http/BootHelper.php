...
function middlewareBootSettings() {
    ...
    foreach (\Modules\Grow\Models\LeadStatus::get() as $status) {
    ...
    foreach (\Modules\Grow\Models\TaskStatus::get() as $status) {
    ...
    foreach (\Modules\Grow\Models\TicketStatus::get() as $status) {
    ...
    //all team members
    $settings['team_members'] = \Modules\Grow\Models\User::Where('type', 'team')->Where('status', 'active')->get();

    //the main admin user
    $settings['main_admin_user'] = \Modules\Grow\Models\User::Where('id', 1)->first();
    ...
    $enabled_modules = \Modules\Grow\Models\Module::where('module_status', 'enabled')->pluck('module_name')->toArray();
...
function middlewareBootMail() {
    ...
    //get email signature
    if ($template = \Modules\Grow\Models\EmailTemplate::Where('emailtemplate_name', 'Email Signature')->first()) {
        $email_signature = $template->emailtemplate_body;
    }

    //get email footer
    if ($template = \Modules\Grow\Models\EmailTemplate::Where('emailtemplate_name', 'Email Footer')->first()) {
...
function middlewareSaaSBootMail() {
    ...
    $landlord_settings = \Modules\Grow\Models\Landlord\Settings::On('landlord')->Where('settings_id', 'default')->first();
