...
class ItemsExport implements FromCollection, WithHeadings, WithMapping {
    ...
    public function map($items): array{
        ...
        $count_sold = \Modules\Grow\Models\Lineitem::where('lineitem_linked_product_id', $items->item_id)
            ->join('invoices', 'lineitems.lineitemresource_id', '=', 'invoices.bill_invoiceid')
            ->where('lineitemresource_type', 'invoice')
            ->where('bill_status', 'paid')
            ->count() ?? 0;

        $sum_sold = \Modules\Grow\Models\Lineitem::where('lineitem_linked_product_id', $items->item_id)
            ->join('invoices', 'lineitems.lineitemresource_id', '=', 'invoices.bill_invoiceid')
            ->where('lineitemresource_type', 'invoice')
            ->where('bill_status', 'paid')
