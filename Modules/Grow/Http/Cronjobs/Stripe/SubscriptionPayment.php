...
class SubscriptionPayment {
    ...
    public function __invoke(
         ...
         */
        $limit = 1;
        if ($webhooks = \Modules\Grow\Models\Webhook::Where('webhooks_gateway_name', 'stripe')
            ...
            foreach ($webhooks as $webhook) {
                ...
                if (!$session = \Modules\Grow\Models\PaymentSession::Where('session_gateway_ref', $webhook->webhooks_matching_reference)->first()) {
                    //log error
                ...
                if (!$subscription = \Modules\Grow\Models\Subscription::Where('subscription_id', $session->session_subscription)->first()) {
                    //log error
                ...
                //create line item
                $line = new \Modules\Grow\Models\Lineitem();
                ...
                //create new payment
                $payment = new \Modules\Grow\Models\Payment();
                 ...
                 * --------------------------------------------------------------------------*/
                if ($user = \Modules\Grow\Models\User::Where('id', $session->session_creatorid)->first()) {
                    //payment thank you
                    $mail = new \App\Mail\PaymentReceived($user, $email_data, $invoice);
                    $mail->build();
                    //subscription started
