...
class ReminderCron {
    ...
    public function __invoke(
        ...
        $reminders = \Modules\Grow\Models\Reminder::Where('reminder_datetime', '<', $now)
        ...
        foreach ($reminders as $reminder) {
            ...
            //queue email
            if ($user = \Modules\Grow\Models\User::Where('id', $reminder->reminder_userid)->first()) {
                ...
                if ($reminder->reminderresource_type == 'client') {
                    ...
                    if ($client = \Modules\Grow\Models\Client::Where('client_id', $reminder->reminderresource_id)->first()) {
                        $data['linked_item_title'] = $client->client_company_name;
                        $data['linked_item_url'] = url('/clients/' . $client->client_id);
                        //primary user
                        if ($user = \Modules\Grow\Models\User::Where('clientid', $client->client_id)->where('account_owner', 'yes')->first()) {
                ...
                if ($reminder->reminderresource_type == 'project') {
                    ...
                    //create other meta data
                    if ($project = \Modules\Grow\Models\Project::Where('project_id', $reminder->reminderresource_id)->first()) {
                ...
                if ($reminder->reminderresource_type == 'task') {
                    ...
                    //create other meta data
                    if ($task = \Modules\Grow\Models\Task::Where('task_id', $reminder->reminderresource_id)->first()) {
                ...
                if ($reminder->reminderresource_type == 'lead') {
                    ...
                    //create other meta data
                    if ($lead = \Modules\Grow\Models\Lead::Where('lead_id', $reminder->reminderresource_id)->first()) {
                ...
                if ($reminder->reminderresource_type == 'invoice') {
                    ...
                    //create other meta data
                    if ($invoice = \Modules\Grow\Models\Invoice::Where('bill_invoiceid', $reminder->reminderresource_id)->first()) {
                ...
                if ($reminder->reminderresource_type == 'estimate') {
                    ...
                    //create other meta data
                    if ($estimate = \Modules\Grow\Models\Estimate::Where('bill_estimateid', $reminder->reminderresource_id)->first()) {
                ...
                if ($reminder->reminderresource_type == 'ticket') {
                    ...
                    //create other meta data
                    if ($ticket = \Modules\Grow\Models\Ticket::Where('ticket_id', $reminder->reminderresource_id)->first()) {
                ...
                if ($reminder->reminderresource_type == 'subscription') {
                    ...
                    //create other meta data
                    if ($subscription = \Modules\Grow\Models\Subscription::Where('subscription_id', $reminder->reminderresource_id)->first()) {
        ...
        \Modules\Grow\Models\Settings::where('settings_id', 1)
            ->update([
