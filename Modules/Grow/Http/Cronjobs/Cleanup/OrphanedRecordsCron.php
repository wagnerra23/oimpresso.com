...
class OrphanedRecordsCron {
    ...
    public function cleanupTasks() {
        ...
        if ($tasks = \Modules\Grow\Models\Task::WhereNotIn('task_projectid', function ($query) {
    ...
    public function cleanupProjects() {
        ...
        ------------------------------------------------------------------------------------------ */
        \Modules\Grow\Models\Project::whereNotIn('project_categoryid', function ($query) {
    ...
    public function cleanupPayments() {
        ...
        if (Schema::hasColumn('payments', 'payment_type')) {
            \Modules\Grow\Models\Payment::whereNotIn('payment_invoiceid', function ($query) {
        ...
        ------------------------------------------------------------------------------------------ */
        if (Schema::hasColumn('payments', 'payment_type')) {
            \Modules\Grow\Models\Payment::whereNotIn('payment_subscriptionid', function ($query) {
        ...
        ------------------------------------------------------------------------------------------ */
        \Modules\Grow\Models\Payment::Where('payment_amount', 0)->delete();
    ...
    public function cleanupTimers() {
        ...
        \Modules\Grow\Models\Timer::WhereNotIn('timer_clientid', function ($query) {
            $query->select('client_id')->from('clients');
        })->delete();

        //all timers with invalid project
        \Modules\Grow\Models\Timer::WhereNotIn('timer_projectid', function ($query) {
            $query->select('project_id')->from('projects');
        })->delete();

        //all timers with invalid task
        \Modules\Grow\Models\Timer::WhereNotIn('timer_taskid', function ($query) {
    ...
    public function cleanupInvoices() {
        ...
        if (Schema::hasColumn('invoices', 'bill_uniqueid')) {
            \Modules\Grow\Models\Invoice::where('bill_uniqueid', '')->OrWhere('bill_uniqueid', null)
                ->update([
    ...
    public function cleanupEstimates() {
        ...
        if (Schema::hasColumn('estimates', 'bill_uniqueid')) {
            \Modules\Grow\Models\Estimate::where('bill_uniqueid', '')->OrWhere('bill_uniqueid', null)
                ->update([
                    'bill_uniqueid' => str_unique(),
                ]);
        }
