...
class FixesCron {
    ...
    public function projectTemplateFolders() {
        ...
        if (!$default_folder = \Modules\Grow\Models\FileFolder::Where('filefolder_default', 'yes')->Where('filefolder_system', 'yes')->first()) {
            return;
        }

        //get all [project templates] that do not have a [default file folder]
        if ($projects = \Modules\Grow\Models\Project::leftJoin('file_folders', 'file_folders.filefolder_projectid', '=', 'projects.project_id')
            ...
            foreach ($projects as $project) {
                ...
                \Modules\Grow\Models\File::where('fileresource_type', 'project')
    ...
    public function missingTimezones() {
        ...
        //users
        \Modules\Grow\Models\User::whereNull('timezone')
        ->orWhere('timezone', '')
        ->update(['timezone' => config('system.settings_system_timezone')]);
        
        //tasks
        \Modules\Grow\Models\Task::whereNull('task_calendar_timezone')
        ->orWhere('task_calendar_timezone', '')
        ->update(['task_calendar_timezone' => config('system.settings_system_timezone')]);
        
        //projects
        \Modules\Grow\Models\Project::whereNull('project_calendar_timezone')
