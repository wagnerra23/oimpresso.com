...
class ImapTicketRepliesCron {
    ...
    public function __invoke() {
        ...
        \Modules\Grow\Models\EmailQueue::Where('emailqueue_to', '')->delete();
         ...
         */
        $limit = 5;
        if ($emails = \Modules\Grow\Models\EmailQueue::where(function ($query) {
            ...
            foreach ($emails as $email) {
                ...
                if (!is_numeric($email->emailqueue_resourceid)) {
                    ...
                    \Modules\Grow\Models\EmailQueue::Where('emailqueue_id', $email->emailqueue_id)->delete();
                    continue;
                }

                //validation - get the reply
                if (!$reply = \Modules\Grow\Models\TicketReply::Where('ticketreply_id', $email->emailqueue_resourceid)->first()) {
                ...
                if (!$ticket = \Modules\Grow\Models\Ticket::Where('ticket_id', $reply->ticketreply_ticketid)->first()) {
                ...
                try {
                    ...
                    //log email
                    $log = new \Modules\Grow\Models\EmailLog();
    ...
    public function getPayload($email, $ticket, $reply) {
        ...
        if (!$category = \Modules\Grow\Models\Category::Where('category_type', 'ticket')->Where('category_id', $ticket->ticket_categoryid)->first()) {
        ...
        if ($last_reply = \Modules\Grow\Models\TicketReply::Where('ticketreply_ticketid', $ticket->ticket_id)
        ...
        if ($ticket_replies = \Modules\Grow\Models\TicketReply::Where('ticketreply_ticketid', $ticket->ticket_id)->get()) {
        ...
        //get attachments
        $attachments = \Modules\Grow\Models\Attachment::Where('attachmentresource_type', 'ticketreply')->Where('attachmentresource_id', $reply->ticketreply_id)->get();
