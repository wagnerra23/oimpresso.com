...
namespace Modules\Grow\Http\Middleware\Templates\Proposals;
use Closure;
use Log;
    ...
    private function fronteEnd() {
        ...
        if (request()->filled('proposalresource_id') && request()->filled('proposalresource_type')) {
            ...
            if (request('proposalresource_type') == 'project') {
                if ($project = \Modules\Grow\Models\Project::Where('project_id', request('proposalresource_id'))->first()) {
            ...
            if (request('proposalresource_type') == 'client') {
                if ($client = \Modules\Grow\Models\Client::Where('client_id', request('proposalresource_id'))->first()) {
                    ...
                    $projects = \Modules\Grow\Models\Project::Where('project_clientid', request('proposalresource_id'))->get();
                    config(
                        [
