...
namespace Modules\Grow\Http\Middleware\Templates\Contracts;
use Closure;
use Log;
    ...
    private function fronteEnd() {
        ...
        if (request()->filled('contractresource_id') && request()->filled('contractresource_type')) {
            ...
            if (request('contractresource_type') == 'project') {
                if ($project = \Modules\Grow\Models\Project::Where('project_id', request('contractresource_id'))->first()) {
            ...
            if (request('contractresource_type') == 'client') {
                if ($client = \Modules\Grow\Models\Client::Where('client_id', request('contractresource_id'))->first()) {
                    ...
                    $projects = \Modules\Grow\Models\Project::Where('project_clientid', request('contractresource_id'))->get();
                    config(
                        [
