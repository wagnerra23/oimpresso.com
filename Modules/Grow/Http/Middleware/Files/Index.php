...
namespace Modules\Grow\Http\Middleware\Files;
            ...
            if (config('system.settings2_file_folders_status') == 'enabled') {
                if (is_numeric(request('fileresource_id'))) {
                    if (!request()->filled('filter_folderid')) {
                        ...
                        if ($folder = \Modules\Grow\Models\FileFolder::Where('filefolder_projectid', request('fileresource_id'))->where('filefolder_default', 'yes')->first()) {
                            request()->merge([
