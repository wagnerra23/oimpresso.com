...
namespace Modules\Grow\Http\Middleware\Tickets;
use Closure;
use Log;
...
class DownloadAttachment {
    ...
    public function handle($request, Closure $next) {
        ...
        if (!$attachment = \Modules\Grow\Models\Attachment::Where('attachment_uniqiueid', $uniqueid)->first()) {
        ...
        if (in_array($attachment->attachmentresource_type, ['ticket', 'ticketreply'])) {
            ...
            if (auth()->user()->is_client) {
                if ($attachment->attachmentresource_type == 'ticket') {
                    if ($ticket = \Modules\Grow\Models\Ticket::Where('ticket_id', $attachment->attachmentresource_id)->first()) {
                ...
                if ($attachment->attachmentresource_type == 'ticketreply') {
                    if ($reply = \Modules\Grow\Models\TicketReply::Where('ticketreply_id', $attachment->attachmentresource_id)->first()) {
