...
namespace Modules\Grow\Http\Middleware\Fooos;
use Closure;
use Log;
    ...
    private function fronteEnd() {
        ...
        if (request()->filled('foooresource_id') && request()->filled('foooresource_type')) {
            ...
            if (request('foooresource_type') == 'project') {
                if ($project = \Modules\Grow\Models\Project::Where('project_id', request('foooresource_id'))->first()) {
            ...
            if (request('foooresource_type') == 'client') {
                if ($client = \Modules\Grow\Models\Client::Where('client_id', request('foooresource_id'))->first()) {
                    ...
                    $projects = \Modules\Grow\Models\Project::Where('project_clientid', request('foooresource_id'))->get();
                    config(
                        [
