...
namespace Modules\Grow\Http\Middleware\General;
use Cache;
use Closure;
...
class General {
    ...
    private function myTimer() {
        ...
        //get the task
        if ($task = \Modules\Grow\Models\Task::where('task_id', $timer->timer_taskid)->first()) {
    ...
    private function myReminders() {
        ...
        if (\Modules\Grow\Models\Reminder::Where('reminder_status', 'due')->where('reminder_userid', auth()->id())->exists()) {
            request()->merge([
