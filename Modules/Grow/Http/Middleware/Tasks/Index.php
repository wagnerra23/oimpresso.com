...
class Index {
    ...
    public function handle($request, Closure $next) {
        ...
        if (auth()->user()->is_team) {
            ...
            if (request()->filled('taskresource_type') == 'project' && request()->filled('taskresource_id')) {
                if ($project = \Modules\Grow\Models\Project::Where('project_id', request('taskresource_id'))->first()) {
                    //template project
    ...
    private function fronteEnd() {
        ...
        if (request('taskresource_type') == '' || request('taskresource_type') == 'client') {
            ...
            if (auth()->user()->is_team) {
                if (auth()->user()->role->role_tasks >= 2 || \Modules\Grow\Models\ProjectManager::Where('projectsmanager_userid', auth()->id())->exists()) {
                    config([
                        //visibility
