...
namespace Modules\Grow\Http\Middleware\Expenses;
use Closure;
use Log;
...
class Edit {
    ...
    public function handle($request, Closure $next) {
        ...
        if ($expense_id == '' || !$expense = \Modules\Grow\Models\Expense::Where('expense_id', $expense_id)->first()) {
    ...
    private function fronteEnd($expense) {
        ...
        if (request()->filled('expenseresource_id') && request()->filled('expenseresource_type')) {
            ...
            if (request('expenseresource_type') == 'project') {
                if ($project = \Modules\Grow\Models\Project::Where('project_id', request('expenseresource_id'))->first()) {
            ...
            if (request('expenseresource_type') == 'client') {
                if ($client = \Modules\Grow\Models\Client::Where('client_id', request('expenseresource_id'))->first()) {
                    ...
                    $projects = \Modules\Grow\Models\Project::Where('project_clientid', request('expenseresource_id'))->get();
                    config(
                        [
