...
namespace Modules\Grow\Http\Middleware\Kbcategories;
use Closure;
use Log;
...
class Destroy {
    ...
    public function handle($request, Closure $next) {
        ...
        //does the foo exist
        if (!$category = \Modules\Grow\Models\Category::Where('category_id', $category_id)->Where('category_type', 'knowledgebase')->first()) {
            abort(409, __('lang.category_not_found'));
        }

        //check if empty
        if(\Modules\Grow\Models\Knowledgebase::Where('knowledgebase_categoryid', $category_id)->count() != 0){
