...
namespace Modules\Grow\Http\Middleware\Canned;
use Closure;
use Log;
    ...
    private function fronteEnd() {
        ...
        if (request()->filled('cannedresource_id') && request()->filled('cannedresource_type')) {
            ...
            if (request('cannedresource_type') == 'project') {
                if ($project = \Modules\Grow\Models\Project::Where('project_id', request('cannedresource_id'))->first()) {
            ...
            if (request('cannedresource_type') == 'client') {
                if ($client = \Modules\Grow\Models\Client::Where('client_id', request('cannedresource_id'))->first()) {
                    ...
                    $projects = \Modules\Grow\Models\Project::Where('project_clientid', request('cannedresource_id'))->get();
                    config(
                        [
