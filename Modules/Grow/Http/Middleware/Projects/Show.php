...
namespace Modules\Grow\Http\Middleware\Projects;
...
class Show {
    ...
    private function fronteEnd($project = '') {
        ...
        $count = \Modules\Grow\Models\CustomField::where('customfields_type', 'projects')->where('customfields_standard_form_status', 'enabled')->where('customfields_status', 'enabled')->count();
        if ($count > 0) {
            config([
