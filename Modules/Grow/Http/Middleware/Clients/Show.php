...
class Show {
    ...
    public function handle($request, Closure $next) {
        ...
        if ($client_id == '' || !$client = \Modules\Grow\Models\Client::Where('client_id', $client_id)->first()) {
    ...
    private function fronteEnd() {
        ...
        $count = \Modules\Grow\Models\CustomField::where('customfields_type', 'clients')->where('customfields_standard_form_status', 'enabled')->where('customfields_status', 'enabled')->count();
        if ($count > 0) {
            config([
