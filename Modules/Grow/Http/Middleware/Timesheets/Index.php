...
namespace Modules\Grow\Http\Middleware\Timesheets;

use Modules\Grow\Models\Timesheet;
use Closure;
use Log;
    ...
    public function handle($request, Closure $next) {
        ...
        if (auth()->user()->is_client) {
            if (request()->ajax() && request()->filled('timesheetresource_id')) {
                if ($project = \Modules\Grow\Models\Project::Where('project_id', request('timesheetresource_id'))->first()) {
