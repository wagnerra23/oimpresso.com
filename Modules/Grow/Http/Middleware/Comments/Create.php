...
namespace Modules\Grow\Http\Middleware\Comments;
...
class Create {
    ...
    private function fronteEnd() {
        ...
        //project resource
        if (request('commentresource_type') == 'project') {
            if ($project = \Modules\Grow\Models\Project::Where('project_id', request('commentresource_id'))->first()) {
        ...
        if (request('commentresource_type') == 'task') {
            if ($task = \Modules\Grow\Models\Task::Where('task_id', request('commentresource_id'))->first()) {
                //add some form fields data
                request()->merge([
