...
namespace Modules\Grow\Http\Middleware\Subscriptions;
use Closure;
use Log;
    ...
    private function fronteEnd() {
        ...
        if (request()->filled('subscriptionresource_id') && request()->filled('subscriptionresource_type')) {
            ...
            if (request('subscriptionresource_type') == 'project') {
                if ($project = \Modules\Grow\Models\Project::Where('project_id', request('subscriptionresource_id'))->first()) {
            ...
            if (request('subscriptionresource_type') == 'client') {
                if ($client = \Modules\Grow\Models\Client::Where('client_id', request('subscriptionresource_id'))->first()) {
                    ...
                    $projects = \Modules\Grow\Models\Project::Where('project_clientid', request('subscriptionresource_id'))->get();
                    config(
                        [
