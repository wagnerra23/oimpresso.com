...
use Modules\Grow\Http\Requests\Projects\ProjectUpdateAutomation;
use Modules\Grow\Http\Requests\Projects\ProjectValidation;
use Modules\Grow\Http\Responses\Common\ChangeCategoryResponse;
use Modules\Grow\Http\Responses\Projects\PinningResponse;
use Modules\Grow\Http\Responses\Projects\ActivateResponse;
use Modules\Grow\Http\Responses\Projects\ArchiveResponse;
use Modules\Grow\Http\Responses\Projects\BulkChangeStatusResponse;
use Modules\Grow\Http\Responses\Projects\BulkChangeStatusUpdateResponse;
use Modules\Grow\Http\Responses\Projects\ChangeAssignedResponse;
use Modules\Grow\Http\Responses\Projects\ChangeAssignedUpdateResponse;
use Modules\Grow\Http\Responses\Projects\ChangeCategoryUpdateResponse;
use Modules\Grow\Http\Responses\Projects\ChangeCoverResponse;
use Modules\Grow\Http\Responses\Projects\ChangeStatusResponse;
use Modules\Grow\Http\Responses\Projects\CommonResponse;
use Modules\Grow\Http\Responses\Projects\CreateCloneResponse;
use Modules\Grow\Http\Responses\Projects\CreateResponse;
use Modules\Grow\Http\Responses\Projects\DestroyResponse;
use Modules\Grow\Http\Responses\Projects\DetailsResponse;
use Modules\Grow\Http\Responses\Projects\EditAutomationResponse;
use Modules\Grow\Http\Responses\Projects\EditResponse;
use Modules\Grow\Http\Responses\Projects\PrefillProjectResponse;
use Modules\Grow\Http\Responses\Projects\RemoveCoverResponse;
use Modules\Grow\Http\Responses\Projects\SetFileCoverResponse;
use Modules\Grow\Http\Responses\Projects\ShowDynamicResponse;
use Modules\Grow\Http\Responses\Projects\ShowResponse;
use Modules\Grow\Http\Responses\Projects\StoreCloneResponse;
use Modules\Grow\Http\Responses\Projects\StoreResponse;
use Modules\Grow\Http\Responses\Projects\UpdateAutomationResponse;
use Modules\Grow\Http\Responses\Projects\UpdateDetailsResponse;
use Modules\Grow\Http\Responses\Projects\UpdateProgressResponse;
use Modules\Grow\Http\Responses\Projects\UpdateResponse;
use Modules\Grow\Http\Responses\Projects\Views\CardResponse;
use Modules\Grow\Http\Responses\Projects\Views\ListResponse;
...
use Modules\Grow\Repositories\CloneProjectRepository;
...
use Modules\Grow\Repositories\FileFolderRepository;
use Modules\Grow\Repositories\FileRepository;
use Modules\Grow\Repositories\MilestoneCategoryRepository;
use Modules\Grow\Repositories\MilestoneRepository;
use Modules\Grow\Repositories\PinnedRepository;
use Modules\Grow\Repositories\ProjectAssignedRepository;
use Modules\Grow\Repositories\ProjectAutomationRepository;
use Modules\Grow\Repositories\ProjectManagerRepository;
...
class Projects extends Controller {
    ...
    public function show(TimerRepository $timerrepo, $id) {
        ...
        //clients contacts
        $contacts = \Modules\Grow\Models\User::where('clientid', $project['project_clientid'])->where('type', 'client')->get();
         ...
         *
         * -------------------------------------------------------------------------------*/
        \Modules\Grow\Models\EventTracking::where('resource_id', $id)
    ...
    public function customFieldValidationFailed() {
        ...
        //custom field validation
        $fields = \Modules\Grow\Models\CustomField::Where('customfields_type', 'projects')->get();
        $errors = '';
        foreach ($fields as $field) {
    ...
    public function changeStatus() {
        ...
        //get the project
        $project = \Modules\Grow\Models\Project::Where('project_id', request()->route('project'))->first();
    ...
    public function archive($id) {
        ...
        $project = \Modules\Grow\Models\Project::Where('project_id', $id)->first();
    ...
    public function changeCategoryUpdate(CategoryRepository $categoryrepo) {
        ...
        foreach (request('ids') as $project_id => $value) {
            if ($value == 'on') {
                $project = \Modules\Grow\Models\Project::Where('project_id', $project_id)->first();
    ...
    private function applyPermissions($project = '') {
        ...
        if ($project instanceof \Modules\Grow\Models\Project) {
            //edit permissions
    ...
    public function storeClone(CloneProjectRepository $clonerepo, $id) {
        ...
        //get the invoice
        if (!$project = \Modules\Grow\Models\Project::Where('project_id', $id)->first()) {
            abort(404);
        }
    ...
    public function prefillProject() {
        ...
        //get the template
        $template = \Modules\Grow\Models\Project::Where('project_id', request('id'))->first();
    ...
    public function changeProgressUpdate($id) {
        ...
        \Modules\Grow\Models\Project::where('project_id', $id)
            ->update([
    ...
    public function setFileBasedCoverImage($id) {
        ...
        //validate file
        if (!$file = \Modules\Grow\Models\File::Where('file_id', request('file'))->first()) {
            abort(404);
        }
    ...
    public function BulkchangeAssignedUpdate(ProjectAssignedRepository $assignedrepo) {
        ...
        foreach (request('ids') as $project_id => $value) {
            if ($value == 'on') {
                ...
                if (!$project = \Modules\Grow\Models\Project::Where('project_id', $project_id)->first()) {
                    continue;
                }
    ...
    private function pageSettings($section = '', $data = []) {
        ...
        if (request()->filled('filter_category')) {
            if ($category = \Modules\Grow\Models\Category::Where('category_type', 'project')->where('category_id', request('filter_category'))->first()) {
                $page['crumbs'] = [
                    __('lang.projects'),
