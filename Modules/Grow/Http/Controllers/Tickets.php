...
use Modules\Grow\Http\Requests\Tickets\TicketStoreUpdate;
use Modules\Grow\Http\Responses\Tickets\ArchiveRestoreResponse;
use Modules\Grow\Http\Responses\Tickets\ChangeStatusResponse;
use Modules\Grow\Http\Responses\Tickets\CreateResponse;
use Modules\Grow\Http\Responses\Tickets\DeleteReplyResponse;
use Modules\Grow\Http\Responses\Tickets\DestroyResponse;
use Modules\Grow\Http\Responses\Tickets\EditReplyResponse;
use Modules\Grow\Http\Responses\Tickets\EditResponse;
use Modules\Grow\Http\Responses\Tickets\EditTagsResponse;
use Modules\Grow\Http\Responses\Tickets\IndexResponse;
use Modules\Grow\Http\Responses\Tickets\PinningResponse;
use Modules\Grow\Http\Responses\Tickets\ReplyResponse;
use Modules\Grow\Http\Responses\Tickets\ShowResponse;
use Modules\Grow\Http\Responses\Tickets\StoreReplyResponse;
use Modules\Grow\Http\Responses\Tickets\StoreResponse;
use Modules\Grow\Http\Responses\Tickets\UpdateReplyResponse;
use Modules\Grow\Http\Responses\Tickets\UpdateResponse;
...
use Modules\Grow\Repositories\TicketReplyRepository;
use Modules\Grow\Repositories\TicketRepository;
...
class Tickets extends Controller {
    ...
    public function index(CategoryRepository $categoryrepo) {
        ...
        //statuses
        $statuses = \Modules\Grow\Models\TicketStatus::orderBy('ticketstatus_position', 'asc')->get();
    ...
    public function create(CategoryRepository $categoryrepo) {
        ...
        if (auth()->user()->is_client) {
            $clients_projects = \Modules\Grow\Models\Project::Where('project_clientid', auth()->user()->clientid)->get();
        } else {
            $clients_projects = [];
        }

        //canned categories
        $canned_categories = \Modules\Grow\Models\Category::Where('category_type', 'canned')->orderBy('category_name', 'asc')->get();

        //recently used canned responses
        $canned_recently_used = \Modules\Grow\Models\CannedRecentlyUsed::Where('cannedrecent_userid', auth()->id())
    ...
    public function edit(CategoryRepository $categoryrepo, $id) {
        ...
        //projects
        $projects = \Modules\Grow\Models\Project::where('project_clientid', $ticket->ticket_clientid)->get();
    ...
    public function update(TicketStoreUpdate $request, $id) {
        ...
        //get ticket
        $ticket = \Modules\Grow\Models\Ticket::Where('ticket_id', $id)->first();
        ...
        if (is_array(request('attachments'))) {
            foreach (request('attachments') as $attachment_id => $value) {
                ...
                if ($value == 'on' && is_numeric($attachment_id)) {
                    ...
                    if ($attachment = \Modules\Grow\Models\Attachment::find($attachment_id)) {
    ...
    public function getCustomFields($obj = '') {
        ...
        if ($obj instanceof \Modules\Grow\Models\Ticket) {
    ...
    public function customFieldValidationFailed() {
        ...
        //custom field validation
        $fields = \Modules\Grow\Models\CustomField::Where('customfields_type', 'tickets')->get();
        $errors = '';
        foreach ($fields as $field) {
    ...
    public function editReply($id) {
        ...
        $reply = \Modules\Grow\Models\TicketReply::Where('ticketreply_id', $id)->first();
    ...
    public function deleteReply($id) {
        ...
        \Modules\Grow\Models\EmailQueue::Where('emailqueue_resourcetype', 'ticket-reply')->Where('emailqueue_resourceid', $id)->delete();
    ...
    public function changeStatusUpdate() {
        ...
        foreach (request('ids') as $id => $value) {
            if ($value == 'on') {
                \Modules\Grow\Models\Ticket::where('ticket_id', $id)
                    ->update([
    ...
    private function statsWidget($data = array()) {
        ...
        foreach ($statuses as $status) {
            $stat = [
                'value' => \Modules\Grow\Models\Ticket::where('ticket_status', $status->ticketstatus_id)->count(),
