...
use Modules\Grow\Http\Requests\Settings\Tweak\TweakValidation;
use Modules\Grow\Http\Responses\Settings\Tweak\IndexResponse;
use Modules\Grow\Http\Responses\Settings\Tweak\UpdateResponse;
...
class Tweak extends Controller {
    ...
    public function update(TweakValidation $request) {
        ...
        if (request('settings2_tweak_imap_reset_stuck_in_processing') == 'on') {
            \Modules\Grow\Models\Category::where('category_type', 'ticket')
                ->update([
