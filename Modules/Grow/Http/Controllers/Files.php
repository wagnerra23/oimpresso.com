...
use Modules\Grow\Http\Responses\Files\BulkDownloadResponse;
use Modules\Grow\Http\Responses\Files\CopyFilesResponse;
use Modules\Grow\Http\Responses\Files\CreateFolderResponse;
use Modules\Grow\Http\Responses\Files\CreateResponse;
use Modules\Grow\Http\Responses\Files\DeleteFolderResponse;
use Modules\Grow\Http\Responses\Files\DestroyResponse;
use Modules\Grow\Http\Responses\Files\EditFoldersResponse;
use Modules\Grow\Http\Responses\Files\EditTagsResponse;
use Modules\Grow\Http\Responses\Files\IndexResponse;
use Modules\Grow\Http\Responses\Files\MoveFilesResponse;
use Modules\Grow\Http\Responses\Files\ShowFoldersResponse;
use Modules\Grow\Http\Responses\Files\ShowMoveFilesResponse;
use Modules\Grow\Http\Responses\Files\StoreResponse;
use Modules\Grow\Http\Responses\Files\UpdateResponse;
use Modules\Grow\Http\Permissions\FilePermissions;
...
class Files extends Controller {
    ...
    public function index() {
        ...
        if (request()->filled('fileresource_type') && request()->filled('fileresource_id')) {
            \Modules\Grow\Models\EventTracking::where('resource_id', request('fileresource_id'))
        ...
        if (request('fileresource_type') == 'project' && request()->filled('fileresource_id')) {
            $folders = \Modules\Grow\Models\FileFolder::Where('filefolder_projectid', request('fileresource_id'))
        ...
        if (request()->filled('attachments')) {
            foreach (request('attachments') as $uniqueid => $file_name) {
                ...
                if (request('fileresource_type') == 'project' && request('fileresource_id') > 0) {
                    if ($project = \Modules\Grow\Models\Project::Where('project_id', request('fileresource_id'))->first()) {

                        //project folders are disabled. Put file in default folder
                        if (config('system.settings2_file_folders_status') == 'disabled') {
                            if ($default_folder = \Modules\Grow\Models\FileFolder::Where('filefolder_projectid', request('fileresource_id'))
    ...
    public function edit($id) {
        ...
        $file = \Modules\Grow\Models\File::Where('file_id', $id)->first();
    ...
    public function showImage() {
        ...
        if (request()->filled('file_id')) {
            if ($file = \Modules\Grow\Models\File::Where('file_uniqueid', request('file_id'))->first()) {
    ...
    public function download() {
        ...
        //get the file
        $file = \Modules\Grow\Models\File::Where('file_uniqueid', request('file_id'))->first();

        //mark events as read
        \Modules\Grow\Models\EventTracking::where('eventtracking_source_id', $file->file_id)
    ...
    public function downloadAttachment() {
        ...
        //get the file
        $attachment = \Modules\Grow\Models\Attachment::Where('attachment_uniqiueid', request('attachment_id'))->first();
    ...
    public function update($id) {
        ...
        if ($event = \Modules\Grow\Models\Event::Where('event_item', 'file')->Where('event_item_id', $id)->first()) {
    ...
    public function destroy(DestroyRepository $destroyrepo) {
        ...
        foreach (request('ids') as $id => $value) {
            ...
            if ($value == 'on') {
                ...
                //get numeric file id
                if (!is_numeric($id)) {
                    if ($file = \Modules\Grow\Models\File::Where('file_uniqueid', $id)->first()) {
                        $id = $file->file_id;
                    }
                }
    ...
    private function applyPermissions($file = '') {
        ...
        if ($file instanceof \Modules\Grow\Models\File) {
            //project files
    ...
    public function showFolders() {
        ...
        $folders = \Modules\Grow\Models\FileFolder::Where('filefolder_projectid', request('fileresource_id'))->orderBy('filefolder_name', 'asc')->get();
    ...
    public function storeFolder() {
            ...
            'filefolder_name' => [
                ...
                function ($attribute, $value, $fail) {
                    if (request()->filled('filefolder_name')) {
                        if (\Modules\Grow\Models\FileFolder::Where('filefolder_name', $value)->Where('filefolder_projectid', request('fileresource_id'))->exists()) {
        ...
        //store record
        $folder = new \Modules\Grow\Models\FileFolder();
    ...
    public function updateFolders() {
        ...
        if (is_array(request('filefolder_name'))) {
            ...
            //update
            foreach (request('filefolder_name') as $id => $value) {
                \Modules\Grow\Models\FileFolder::where('filefolder_id', $id)
    ...
    public function deleteFolder($id) {
        ...
        //validate
        if (!$folder = \Modules\Grow\Models\FileFolder::Where('filefolder_projectid', request('fileresource_id'))->Where('filefolder_id', $id)->first()) {
            abort(404);
        }
        ...
        if (request('confirm_hidden_fields') == 'on') {
            \Modules\Grow\Models\File::Where('file_folderid', $id)->delete();
        } else {
            //move files to the default folder
            if ($default_folder = \Modules\Grow\Models\FileFolder::Where('filefolder_projectid', request('fileresource_id'))->where('filefolder_default', 'yes')->first()) {
                \Modules\Grow\Models\File::where('file_folderid', $id)
    ...
    public function ShowMoveFiles() {
        ...
        //validate
        $folders = \Modules\Grow\Models\FileFolder::Where('filefolder_projectid', request('fileresource_id'))->get();
    ...
    public function moveFiles() {
        ...
        if (is_array(request('ids'))) {
            foreach (request('ids') as $unique_id => $value) {
                if ($value == 'on') {
                    if ($file = \Modules\Grow\Models\File::Where('file_uniqueid', $unique_id)->first()) {
    ...
    public function copyAction(ProjectRepository $projectrepo) {
        ...
        foreach (request('ids') as $file_uniqueid => $value) {
            if ($value == 'on') {
                if ($project = \Modules\Grow\Models\Project::Where('project_id', request('copy_target_project'))->first()) {
                    if ($file = \Modules\Grow\Models\File::Where('file_uniqueid', $file_uniqueid)->first()) {
                        //unique key
                        $unique_key = Str::random(50);
                        //directory
