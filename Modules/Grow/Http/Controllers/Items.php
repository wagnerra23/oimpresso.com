...
use Modules\Grow\Http\Requests\Items\ItemStoreUpdate;
use Modules\Grow\Http\Requests\Items\StoreUpdateTask;
use Modules\Grow\Http\Responses\Common\ChangeCategoryResponse;
use Modules\Grow\Http\Responses\Items\CategoryItemsResponse;
use Modules\Grow\Http\Responses\Items\ChangeCategoryUpdateResponse;
use Modules\Grow\Http\Responses\Items\CreateResponse;
use Modules\Grow\Http\Responses\Items\DestroyResponse;
use Modules\Grow\Http\Responses\Items\EditResponse;
use Modules\Grow\Http\Responses\Items\IndexResponse;
use Modules\Grow\Http\Responses\Items\PinningResponse;
use Modules\Grow\Http\Responses\Items\StoreResponse;
use Modules\Grow\Http\Responses\Items\Tasks\TasksCreateResponse;
use Modules\Grow\Http\Responses\Items\Tasks\TasksDeleteResponse;
use Modules\Grow\Http\Responses\Items\Tasks\TasksEditResponse;
use Modules\Grow\Http\Responses\Items\Tasks\TasksIndexResponse;
use Modules\Grow\Http\Responses\Items\UpdateResponse;
use Modules\Grow\Models\Category;
use Modules\Grow\Models\Item;
use Modules\Grow\Repositories\CategoryRepository;
use Modules\Grow\Repositories\ItemRepository;
use Modules\Grow\Repositories\PinnedRepository;
use Modules\Grow\Repositories\ProductTaskRepository;
use Modules\Grow\Repositories\UnitRepository;
...
class Items extends Controller {
    ...
    public function update(ItemStoreUpdate $request, UnitRepository $unitrepo, $id) {
        ...
        \Modules\Grow\Models\Lineitem::where('lineitem_linked_product_id', $id)
    ...
    public function destroy() {
        ...
        foreach (request('ids') as $id => $value) {
            ...
            if ($value == 'on') {
                //get the item
                $item = \Modules\Grow\Models\Item::Where('item_id', $id)->first();
                //delete client
                $item->delete();

                //delete any automation tasks
                if ($tasks = \Modules\Grow\Models\ProductTask::Where('product_task_itemid', $id)->get()) {
                    foreach ($tasks as $task) {
                        //delete assigned users
                        \Modules\Grow\Models\AutomationAssigned::Where('automationassigned_resource_type', 'product_task')
                            ->Where('automationassigned_resource_id', $task->product_task_id)->delete();

                        //delete dependencies
                        \Modules\Grow\Models\ProductTasksDependency::Where('product_task_dependency_taskid', $task->product_task_id)->delete();
    ...
    public function changeCategoryUpdate(CategoryRepository $categoryrepo) {
        ...
        foreach (request('ids') as $item_id => $value) {
            if ($value == 'on') {
                $item = \Modules\Grow\Models\Item::Where('item_id', $item_id)->first();
                //update the category
    ...
    public function storeTask(StoreUpdateTask $request) {
        ...
        //store record
        $task = new \Modules\Grow\Models\ProductTask();
        ...
        if (request()->filled('dependencies_cannot_complete') && is_array(request('dependencies_cannot_complete'))) {
            foreach (request('dependencies_cannot_complete') as $blocking_task_id) {
                $dependency = new \Modules\Grow\Models\ProductTasksDependency();
    ...
    public function editTask($id) {
        ...
        //get the foo
        if (!$task = \Modules\Grow\Models\ProductTask::Where('product_task_id', $id)->first()) {
            abort(404);
        }

        //assigned users
        $assigned_users = \Modules\Grow\Models\AutomationAssigned::Where('automationassigned_resource_type', 'product_task')
        ...
        $dependencies = \Modules\Grow\Models\ProductTasksDependency::Where('product_task_dependency_type', 'cannot_complete')
        ...
        $dependencies = \Modules\Grow\Models\ProductTasksDependency::Where('product_task_dependency_type', 'cannot_start')
            ->Where('product_task_dependency_taskid', $id)
            ->get();
    ...
    public function updateTask(StoreUpdateTask $request, $id) {
        ...
        //delete assigned users
        \Modules\Grow\Models\AutomationAssigned::Where('automationassigned_resource_id', 'product')
            ->Where('automationassigned_resource_id', $id)
            ->delete();
    ...
    public function destroyTask($id) {
        ...
        \Modules\Grow\Models\ProductTasksDependency::Where('product_task_dependency_blockerid', $task->id)->delete();
