...
use Modules\Grow\Http\Requests\Contracts\StoreUpdate;
use Modules\Grow\Http\Responses\Common\ChangeCategoryResponse;
use Modules\Grow\Http\Responses\Contracts\AttachProjectResponse;
use Modules\Grow\Http\Responses\Contracts\ChangeCategoryUpdateResponse;
use Modules\Grow\Http\Responses\Contracts\ChangeStatusResponse;
use Modules\Grow\Http\Responses\Contracts\CreateCloneResponse;
use Modules\Grow\Http\Responses\Contracts\CreateResponse;
use Modules\Grow\Http\Responses\Contracts\DestroyResponse;
use Modules\Grow\Http\Responses\Contracts\EmailResponse;
use Modules\Grow\Http\Responses\Contracts\IndexResponse;
use Modules\Grow\Http\Responses\Contracts\PinningResponse;
use Modules\Grow\Http\Responses\Contracts\PublishResponse;
use Modules\Grow\Http\Responses\Contracts\PublishScheduledResponse;
use Modules\Grow\Http\Responses\Contracts\SignatureResponse;
use Modules\Grow\Http\Responses\Contracts\StoreResponse;
...
use Modules\Grow\Models\Contract;
use Modules\Grow\Repositories\CategoryRepository;
use Modules\Grow\Repositories\CloneContractRepository;
use Modules\Grow\Repositories\ContractRepository;
...
class Contracts extends Controller {
    ...
    public function create(CategoryRepository $categoryrepo) {
        ...
        //templates
        $templates = \Modules\Grow\Models\ContractTemplate::orderBy('contract_template_id', 'ASC')->get();

        //we are on client page
        if (config('modal.type') == 'preset-client') {
            //get projects
            $projects = \Modules\Grow\Models\Project::Where('project_clientid', request('contractresource_id'))
    ...
    public function store(StoreUpdate $request) {
        ...
        //create the contract
        $contract = new \Modules\Grow\Models\Contract();
        ...
        if (is_numeric(request('contract_template'))) {
            if ($template = \Modules\Grow\Models\ContractTemplate::Where('contract_template_id', request('contract_template'))->first()) {
    ...
    public function show(EstimateGeneratorRepository $estimategenerator, $id) {
        ...
        //get the estimate
        if ($estimate = \Modules\Grow\Models\Estimate::Where('bill_contractid', $id)->Where('bill_estimate_type', 'document')->first()) {
            request()->merge([
    ...
    public function showPublic($id) {
        ...
        //get contract
        $contract = \Modules\Grow\Models\Contract::Where('doc_unique_id', $id)->first();
    ...
    public function editingContract(CategoryRepository $categoryrepo, $id) {
        ...
        if (!$estimate = \Modules\Grow\Models\Estimate::Where('bill_contractid', $id)->Where('bill_estimate_type', 'document')->first()) {
            ...
            $estimate = \Modules\Grow\Models\Estimate::Where('bill_contractid', $id)->Where('bill_estimate_type', 'document')->first();
        }
    ...
    public function destroy() {
        ...
        foreach (request('ids') as $id => $value) {
            ...
            if ($value == 'on') {
                //get the contract
                $contract = \Modules\Grow\Models\Contract::Where('doc_id', $id)->first();
                //delete client
                $contract->delete();
                //add to array
                $allrows[] = $id;
    ...
    public function changeCategoryUpdate(CategoryRepository $categoryrepo) {
        ...
        foreach (request('ids') as $contract_id => $value) {
            if ($value == 'on') {
                $contract = \Modules\Grow\Models\Contract::Where('doc_id', $contract_id)->first();
                //update the category
    ...
    public function publishScheduled($id) {
        ...
        if (!$contract = \Modules\Grow\Models\Contract::Where('doc_id', $id)->first()) {
            abort(404);
        }
    ...
    public function attachProjectUpdate() {
        ...
        $contract = \Modules\Grow\Models\Contract::Where('bill_contractid', request()->route('contract'))->first();
