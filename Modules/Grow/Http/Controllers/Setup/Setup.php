...
namespace Modules\Grow\Http\Controllers\Setup;

use Modules\Grow\Http\Controllers\Controller;
use Modules\Grow\Http\Controllers\Setup\DatabaseResponse;
use Modules\Grow\Http\Controllers\Setup\FinishResponse;
use Modules\Grow\Http\Controllers\Setup\IndexResponse;
use Modules\Grow\Http\Controllers\Setup\RequirementsResponse;
use Modules\Grow\Http\Controllers\Setup\SettingsResponse;
use Modules\Grow\Http\Controllers\Setup\UserResponse;
use Modules\Grow\Repositories\EnvRepository;
use Modules\Grow\Repositories\SpaceRepository;
use Exception;
...
class Setup extends Controller {
    ...
    public function updateSettings() {
        ...
        if (!$settings = \Modules\Grow\Models\Settings::Where('settings_id', 1)->first()) {
            abort(409, __('Error(1007): Unable to load database settings'));
        }

        //update default categories dates
        \Modules\Grow\Models\Category::where('category_system_default', 'yes')
            ->update(['category_created' => now()]);

        //update default categories dates
        \Modules\Grow\Models\KbCategories::where('kbcategory_system_default', 'yes')
    ...
    public function updateUser(EnvRepository $envrepo) {
        ...
        //update default user
        if (!$user = \Modules\Grow\Models\User::Where('id', 1)->first()) {
    ...
    public function createUserSpace($user) {
        ...
        $space = new \Modules\Grow\Models\Project();
