...
use Modules\Grow\Http\Requests\Estimates\EstimateClone;
use Modules\Grow\Http\Requests\Estimates\EstimateSave;
use Modules\Grow\Http\Requests\Estimates\EstimateStoreUpdate;
use Modules\Grow\Http\Requests\Estimates\EstimateUpdateAutomation;
use Modules\Grow\Http\Responses\Common\ChangeCategoryResponse;
use Modules\Grow\Http\Responses\Estimates\AcceptResponse;
use Modules\Grow\Http\Responses\Estimates\AttachFilesResponse;
use Modules\Grow\Http\Responses\Estimates\AttachProjectResponse;
use Modules\Grow\Http\Responses\Estimates\ChangeCategoryUpdateResponse;
use Modules\Grow\Http\Responses\Estimates\ChangeStatusResponse;
use Modules\Grow\Http\Responses\Estimates\ConvertToEstimate;
use Modules\Grow\Http\Responses\Estimates\CreateCloneResponse;
use Modules\Grow\Http\Responses\Estimates\CreateResponse;
use Modules\Grow\Http\Responses\Estimates\DeclineResponse;
use Modules\Grow\Http\Responses\Estimates\DestroyResponse;
use Modules\Grow\Http\Responses\Estimates\DocumentEditingResponse;
use Modules\Grow\Http\Responses\Estimates\EditAutomationResponse;
use Modules\Grow\Http\Responses\Estimates\EditResponse;
use Modules\Grow\Http\Responses\Estimates\IndexResponse;
use Modules\Grow\Http\Responses\Estimates\PDFResponse;
use Modules\Grow\Http\Responses\Estimates\PinningResponse;
use Modules\Grow\Http\Responses\Estimates\PublishResponse;
use Modules\Grow\Http\Responses\Estimates\PublishRevisedResponse;
use Modules\Grow\Http\Responses\Estimates\PublishScheduledResponse;
use Modules\Grow\Http\Responses\Estimates\ResendResponse;
use Modules\Grow\Http\Responses\Estimates\SaveResponse;
use Modules\Grow\Http\Responses\Estimates\ShowPublicResponse;
use Modules\Grow\Http\Responses\Estimates\ShowResponse;
use Modules\Grow\Http\Responses\Estimates\StoreCloneResponse;
use Modules\Grow\Http\Responses\Estimates\StoreResponse;
use Modules\Grow\Http\Responses\Estimates\UpdateAutomationResponse;
use Modules\Grow\Http\Responses\Estimates\UpdateResponse;
use Modules\Grow\Http\Responses\Estimates\UpdateTaxtypeResponse;
use Modules\Grow\Repositories\CategoryRepository;
use Modules\Grow\Repositories\ClientRepository;
use Modules\Grow\Repositories\CloneEstimateRepository;
...
use Modules\Grow\Repositories\EstimateAutomationRepository;
use Modules\Grow\Repositories\EstimateGeneratorRepository;
use Modules\Grow\Repositories\EstimateRepository;
use Modules\Grow\Repositories\EventRepository;
use Modules\Grow\Repositories\EventTrackingRepository;
use Modules\Grow\Repositories\FileRepository;
use Modules\Grow\Repositories\LineitemRepository;
use Modules\Grow\Repositories\PinnedRepository;
use Modules\Grow\Repositories\ProjectRepository;
use Modules\Grow\Repositories\PublishEstimateRepository;
use Modules\Grow\Repositories\TagRepository;
use Modules\Grow\Repositories\TaxRepository;
...
class Estimates extends Controller {
    ...
    public function store(EstimateStoreUpdate $request, ClientRepository $clientrepo) {
        ...
        //get new estimate
        $estimate = \Modules\Grow\Models\Estimate::Where('bill_estimateid', $bill_estimateid)->first();
    ...
    public function applyDefaultAutomation($estimate) {
        ...
        $assigned_users = \Modules\Grow\Models\AutomationAssigned::Where('automationassigned_resource_type', 'estimate')
            ->Where('automationassigned_resource_id', 0)
            ->get();

        $assigned = [];
        foreach ($assigned_users as $user) {
            $assigned = new \Modules\Grow\Models\AutomationAssigned();
    ...
    public function show($id) {
        ...
        if (auth()->user()->is_client) {
            \Modules\Grow\Models\Estimate::where('bill_estimateid', $id)
                ->update(['bill_viewed_by_client' => 'yes']);
        }

        //custom fields
        $payload['customfields'] = \Modules\Grow\Models\CustomField::Where('customfields_type', 'clients')->get();

        //get estimate files
        $payload['files'] = \Modules\Grow\Models\File::Where('fileresource_type', 'estimate')->Where('fileresource_id', $id)->orderBy('file_filename', 'ASC')->get();
    ...
    public function showPublic($id) {
        ...
        //get estimate
        $estimate = \Modules\Grow\Models\Estimate::Where('bill_uniqueid', $id)->first();
    ...
    private function updateEstimateTax($bill_estimateid = '') {
        ...
        \Modules\Grow\Models\Tax::Where('taxresource_type', 'estimate')
    ...
    public function publishScheduledEstimate($id) {
        ...
        if (!$estimate = \Modules\Grow\Models\Estimate::Where('bill_estimateid', $id)->first()) {
            abort(404);
        }
    ...
    public function publishRevisedEstimate($id) {
        ...
        \Modules\Grow\Models\Estimate::where('bill_estimateid', $estimate->bill_estimateid)
    ...
    public function acceptEstimate(EstimateAutomationRepository $automationrepo, $id) {
        ...
        $user = \Modules\Grow\Models\User::Where('clientid', $estimate->bill_clientid)->Where('account_owner', 'yes')->first();
    ...
    public function changeStatus() {
        ...
        //get the estimate
        $estimate = \Modules\Grow\Models\Estimate::Where('bill_estimateid', request()->route('estimate'))->first();
    ...
    public function changeStatusUpdate(EstimateAutomationRepository $automationrepo) {
        ...
        if (request('bill_status') == 'revised' || request('bill_status') == 'new') {
            \Modules\Grow\Models\Estimate::where('bill_estimateid', request()->route('estimate'))
    ...
    public function attachProjectUpdate() {
        ...
        if (!$project = \Modules\Grow\Models\Project::Where('project_id', request('attach_project_id'))->first()) {
            abort(409, __('lang.item_not_found'));
        }
    ...
    public function createClone(CategoryRepository $categoryrepo, $id) {
        ...
        //get the estimate
        $estimate = \Modules\Grow\Models\estimate::Where('bill_estimateid', $id)->first();
    ...
    public function updateAutomation(EstimateUpdateAutomation $request, $id) {
        ...
        \Modules\Grow\Models\AutomationAssigned::Where('automationassigned_resource_type', 'estimate')
            ->Where('automationassigned_resource_id', $id)
            ->delete();
    ...
    public function attachFiles(Request $request, FileRepository $filerepo, $id) {
        ...
        if ($file = $request->file('file')) {
            ...
            //save the file
            $file = new \Modules\Grow\Models\File();
    ...
    public function deleteFile() {
        ...
        //get the record
        if (!$file = \Modules\Grow\Models\File::Where('file_uniqueid', request('file_uniqueid'))->first()) {
            abort(403);
        }
    ...
    public function updateTaxType($id) {
        ...
        if (request('bill_tax_type') != $estimate->bill_tax_type) {
            //delete all taxes
            \Modules\Grow\Models\Tax::Where('taxresource_type', 'estimate')->Where('taxresource_id', $id)->delete();
            ...
            if (request('bill_tax_type') == 'inline') {
                ...
                if ($zero_rate_tax = \Modules\Grow\Models\TaxRate::Where('taxrate_uniqueid', 'zero-rated-tax-rate')->first()) {
                    //get all line items
                    if ($lineitems = \Modules\Grow\Models\Lineitem::Where('lineitemresource_type', 'estimate')->Where('lineitemresource_id', $id)->get()) {
                        foreach ($lineitems as $lineitem) {
                            $tax = new \Modules\Grow\Models\Tax();
    ...
    public function getClientCustomFields($obj = '') {
        ...
        if ($obj instanceof \Modules\Grow\Models\Project) {
