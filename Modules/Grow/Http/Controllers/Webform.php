...
use Modules\Grow\Http\Responses\Webform\SaveResponse;
...
class Webform extends Controller {
    ...
    public function saveForm($id) {
        ...
        if ($last = \Modules\Grow\Models\Lead::orderBy('lead_position', 'desc')->first()) {
        ...
        $lead = new \Modules\Grow\Models\Lead();
        ...
        //assign users
        $assigned_users = [];
        $preset_users = \Modules\Grow\Models\WebformAssigned::Where('webformassigned_formid', $id)->get();
        foreach ($preset_users as $preset_user) {
            $assigned = new \Modules\Grow\Models\LeadAssigned;
        ...
        if ($webform->webform_notify_admin == 'yes') {
            ...
            if ($users = \Modules\Grow\Models\User::Where('role_id', 1)->get()) {
