...
use Modules\Grow\Http\Responses\Reminders\CloseCardResponse;
use Modules\Grow\Http\Responses\Reminders\CreateCardResponse;
use Modules\Grow\Http\Responses\Reminders\CreateResponse;
use Modules\Grow\Http\Responses\Reminders\DestroyCardResponse;
use Modules\Grow\Http\Responses\Reminders\DestroyResponse;
use Modules\Grow\Http\Responses\Reminders\EditCardResponse;
use Modules\Grow\Http\Responses\Reminders\EditResponse;
use Modules\Grow\Http\Responses\Reminders\ShowCardResponse;
use Modules\Grow\Http\Responses\Reminders\ShowResponse;
use Modules\Grow\Http\Responses\Reminders\ShowTopnavResponse;
use Modules\Grow\Http\Responses\Reminders\StoreCardResponse;
use Modules\Grow\Http\Responses\Reminders\StoreResponse;
use Modules\Grow\Http\Responses\Reminders\DeleteReminderResponse;
...
class Reminders extends Controller {
    ...
    public function show() {
        ...
        //no reminder found
        if (!$reminder = \Modules\Grow\Models\Reminder::Where('reminderresource_type', request('resource_type'))
    ...
    public function edit() {
        ...
        if (!$reminder = \Modules\Grow\Models\Reminder::Where('reminder_id', request('reminder_id'))
    ...
    public function store() {
        ...
        \Modules\Grow\Models\Reminder::Where('reminderresource_type', request('resource_type'))
            ->where('reminderresource_id', request('resource_id'))
            ->where('reminder_userid', auth()->id())
            ->delete();

        //save the reminder
        $reminder = new \Modules\Grow\Models\Reminder();
    ...
    public function getResourceItem($resource_type = '', $resource_id = '') {
        ...
        if ($resource_type == 'client') {
            if (!$client = \Modules\Grow\Models\Client::Where('client_id', $resource_id)->first()) {
                return false;
            }
        ...
        if ($resource_type == 'project') {
            if (!$project = \Modules\Grow\Models\Project::Where('project_id', $resource_id)->first()) {
                return false;
            }
        ...
        if ($resource_type == 'invoice') {
            if (!$invoice = \Modules\Grow\Models\Invoice::Where('bill_invoiceid', $resource_id)->first()) {
                return false;
            }
        ...
        if ($resource_type == 'estimate') {
            if (!$estimate = \Modules\Grow\Models\Estimate::Where('bill_estimateid', $resource_id)->first()) {
                return false;
            }
        ...
        if ($resource_type == 'task') {
            if (!$task = \Modules\Grow\Models\Task::Where('task_id', $resource_id)->first()) {
                return false;
            }
        ...
        if ($resource_type == 'lead') {
            if (!$lead = \Modules\Grow\Models\Lead::Where('lead_id', $resource_id)->first()) {
                return false;
            }
        ...
        if ($resource_type == 'ticket') {
            if (!$ticket = \Modules\Grow\Models\Ticket::Where('ticket_id', $resource_id)->first()) {
                return false;
            }
        ...
        if ($resource_type == 'proposal') {
            if (!$doc = \Modules\Grow\Models\Proposal::Where('doc_id', $resource_id)->first()) {
                return false;
            }
        ...
        if ($resource_type == 'contract') {
            if (!$doc = \Modules\Grow\Models\Contract::Where('doc_id', $resource_id)->first()) {
                return false;
            }
            return $doc->doc_title;
        }
    ...
    public function delete() {
        ...
        //delete any existing reminders for this user
        \Modules\Grow\Models\Reminder::Where('reminder_id', request('reminder_id'))
            ->where('reminder_userid', auth()->id())
            ->delete();
    ...
    public function close() {
        ...
        if (request()->filled('resource_id') && request()->filled('resource_type')) {
            if (\Modules\Grow\Models\Reminder::Where('reminderresource_type', request('resource_type'))
                ->where('reminderresource_id', request('resource_id'))
                ->where('reminder_userid', auth()->id())
                ->first()) {
                return $this->show();
    ...
    public function topNavFeed() {
        ...
        $reminders = \Modules\Grow\Models\Reminder::Where('reminder_userid', auth()->id())
            ->where('reminder_status', $status)
            ->orderBy('reminder_datetime', 'DESC')
            ->get();

        //count due reminders
        $count_due = \Modules\Grow\Models\Reminder::Where('reminder_userid', auth()->id())
    ...
    public function deleteReminder($id) {
        ...
        //delete record
        if (is_numeric($id)) {
            \Modules\Grow\Models\Reminder::Where('reminder_id', $id)
    ...
    public function deleteAllReminders() {
        ...
        //delete record
        \Modules\Grow\Models\Reminder::Where('reminder_userid', auth()->id())->where('reminder_status', 'due')->delete();
