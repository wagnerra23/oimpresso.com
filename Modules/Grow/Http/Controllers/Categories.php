...
use Modules\Grow\Http\Responses\Categories\CreateResponse;
use Modules\Grow\Http\Responses\Categories\DestroyResponse;
use Modules\Grow\Http\Responses\Categories\EditResponse;
use Modules\Grow\Http\Responses\Categories\IndexResponse;
use Modules\Grow\Http\Responses\Categories\StoreResponse;
use Modules\Grow\Http\Responses\Categories\UpdateResponse;
...
class Categories extends Controller {
    ...
    public function store() {
        ...
        //check duplicates
        if (\Modules\Grow\Models\Category::where('category_name', request('category_name'))
    ...
    public function update(ProjectRepository $projectrepo, $id) {
        ...
        if (is_numeric(request('migrate')) && $category = \Modules\Grow\Models\Category::Where('category_id', request('migrate'))->first()) {
            ...
            //projects
            \Modules\Grow\Models\Project::where('project_categoryid', $id)
                ->update(['project_categoryid' => request('migrate')]);

            //clients
            \Modules\Grow\Models\Client::where('client_categoryid', $id)
                ->update(['client_categoryid' => request('migrate')]);

            //contracts
            \Modules\Grow\Models\Contract::where('doc_categoryid', $id)
                ->update(['doc_categoryid' => request('migrate')]);

            //expenses
            \Modules\Grow\Models\Expense::where('expense_categoryid', $id)
                ->update(['expense_categoryid' => request('migrate')]);

            //invoices
            \Modules\Grow\Models\Invoice::where('bill_categoryid', $id)
                ->update(['bill_categoryid' => request('migrate')]);

            //leads
            \Modules\Grow\Models\Lead::where('lead_categoryid', $id)
                ->update(['lead_categoryid' => request('migrate')]);

            //tickets
            \Modules\Grow\Models\Ticket::where('ticket_categoryid', $id)
                ->update(['ticket_categoryid' => request('migrate')]);

            //items
            \Modules\Grow\Models\Item::where('item_categoryid', $id)
                ->update(['item_categoryid' => request('migrate')]);

            //estimates
            \Modules\Grow\Models\Estimate::where('bill_categoryid', $id)
                ->update(['bill_categoryid' => request('migrate')]);
    ...
    public function destroy($id) {
        ...
        //get record
        if (!\Modules\Grow\Models\Category::find($id)) {
            abort(409, __('lang.error_request_could_not_be_completed'));
        }
        ...
        \Modules\Grow\Models\CategoryUser::Where('categoryuser_categoryid', $id)->delete();
    ...
    public function showTeam($id) {
        ...
        //get the users
        $users = \Modules\Grow\Models\CategoryUser::Where('categoryuser_categoryid', $id)->get();
    ...
    public function updateTeam(ProjectRepository $projectrepo, $id) {
        ...
        //get the category
        if (!$category = \Modules\Grow\Models\Category::Where('category_id', $id)->first()) {
            abort(404);
        }
        ...
        if (request()->filled('users')) {
            foreach (request('users') as $user) {
                $categoryuser = new \Modules\Grow\Models\CategoryUser();
