...
use Modules\Grow\Http\Requests\Invoices\InvoiceClone;
use Modules\Grow\Http\Requests\Invoices\InvoiceRecurrringSettings;
use Modules\Grow\Http\Requests\Invoices\InvoiceSave;
use Modules\Grow\Http\Requests\Invoices\InvoiceStoreUpdate;
use Modules\Grow\Http\Responses\Common\ChangeCategoryResponse;
use Modules\Grow\Http\Responses\Invoices\AttachFilesResponse;
use Modules\Grow\Http\Responses\Invoices\AttachProjectResponse;
use Modules\Grow\Http\Responses\Invoices\ChangeCategoryUpdateResponse;
use Modules\Grow\Http\Responses\Invoices\CreateCloneResponse;
use Modules\Grow\Http\Responses\Invoices\CreateResponse;
use Modules\Grow\Http\Responses\Invoices\DestroyResponse;
use Modules\Grow\Http\Responses\Invoices\EditResponse;
use Modules\Grow\Http\Responses\Invoices\IndexResponse;
use Modules\Grow\Http\Responses\Invoices\PDFResponse;
use Modules\Grow\Http\Responses\Invoices\PinningResponse;
use Modules\Grow\Http\Responses\Invoices\PublishResponse;
use Modules\Grow\Http\Responses\Invoices\PublishScheduledResponse;
use Modules\Grow\Http\Responses\Invoices\RecurringSettingsResponse;
use Modules\Grow\Http\Responses\Invoices\ResendResponse;
use Modules\Grow\Http\Responses\Invoices\SaveResponse;
use Modules\Grow\Http\Responses\Invoices\ShowResponse;
use Modules\Grow\Http\Responses\Invoices\StoreCloneResponse;
use Modules\Grow\Http\Responses\Invoices\StoreResponse;
use Modules\Grow\Http\Responses\Invoices\UpdateResponse;
use Modules\Grow\Http\Responses\Invoices\UpdateTaxtypeResponse;
use Modules\Grow\Http\Responses\Pay\MolliePaymentResponse;
use Modules\Grow\Http\Responses\Pay\PaypalPaymentResponse;
use Modules\Grow\Http\Responses\Pay\PaystackPaymentResponse;
use Modules\Grow\Http\Responses\Pay\RazorpayPaymentResponse;
use Modules\Grow\Http\Responses\Pay\StripePaymentResponse;
use Modules\Grow\Http\Responses\Pay\TapPaymentResponse;
use Modules\Grow\Repositories\CategoryRepository;
use Modules\Grow\Repositories\ClientRepository;
use Modules\Grow\Repositories\CloneInvoiceRepository;
...
use Modules\Grow\Repositories\InvoiceGeneratorRepository;
use Modules\Grow\Repositories\InvoiceRepository;
use Modules\Grow\Repositories\LineitemRepository;
use Modules\Grow\Repositories\MolliePaymentRepository;
use Modules\Grow\Repositories\PaypalPaymentRepository;
use Modules\Grow\Repositories\PaystackPaymentRepository;
use Modules\Grow\Repositories\ProjectRepository;
use Modules\Grow\Repositories\PublishInvoiceRepository;
use Modules\Grow\Repositories\RazorpayPaymentRepository;
use Modules\Grow\Repositories\StripePaymentRepository;
use Modules\Grow\Repositories\TagRepository;
use Modules\Grow\Repositories\TapPaymentRepository;
...
class Invoices extends Controller {
    ...
    public function show($id) {
        ...
        if (auth()->user()->is_client) {
            \Modules\Grow\Models\Invoice::where('bill_invoiceid', $id)
        ...
        //get estimate files
        $payload['files'] = \Modules\Grow\Models\File::Where('fileresource_type', 'invoice')->Where('fileresource_id', $id)->orderBy('file_filename', 'ASC')->get();
    ...
    private function updateInvoiceTax($bill_invoiceid = '') {
        ...
        \Modules\Grow\Models\Tax::Where('taxresource_type', 'invoice')
    ...
    public function publishScheduledInvoice($id) {
        ...
        if (!$invoice = \Modules\Grow\Models\Invoice::Where('bill_invoiceid', $id)->first()) {
            abort(404);
        }
    ...
    public function changeCategoryUpdate(CategoryRepository $categoryrepo) {
        ...
        foreach (request('ids') as $bill_invoiceid => $value) {
            if ($value == 'on') {
                $invoice = \Modules\Grow\Models\Invoice::Where('bill_invoiceid', $bill_invoiceid)->first();
                //update the category
    ...
    public function attachProjectUpdate() {
        ...
        $invoice = \Modules\Grow\Models\Invoice::Where('bill_invoiceid', request()->route('invoice'))->first();
        ...
        if ($payments = \Modules\Grow\Models\Payment::Where('payment_invoiceid', request()->route('invoice'))->get()) {
    ...
    public function createClone(CategoryRepository $categoryrepo, $id) {
        ...
        //get the invoice
        $invoice = \Modules\Grow\Models\Invoice::Where('bill_invoiceid', $id)->first();
    ...
    public function emailClient() {
        ...
        //validate the invoice exists
        $invoice = \Modules\Grow\Models\Invoice::Where('bill_invoiceid', request('id'))->first();
    ...
    public function expensesPayload($bill_invoiceid) {
        ...
        if (is_array(request('expense_payload'))) {
            foreach (request('expense_payload') as $expense_id) {
                //get the expense
                if ($expense = \Modules\Grow\Models\Expense::Where('expense_id', $expense_id)->first()) {
    ...
    public function updateTaxType($id) {
        ...
        if (request('bill_tax_type') != $invoice->bill_tax_type) {
            //delete all taxes
            \Modules\Grow\Models\Tax::Where('taxresource_type', 'invoice')->Where('taxresource_id', $id)->delete();
            ...
            if (request('bill_tax_type') == 'inline') {
                ...
                if ($zero_rate_tax = \Modules\Grow\Models\TaxRate::Where('taxrate_uniqueid', 'zero-rated-tax-rate')->first()) {
                    //get all line items
                    if ($lineitems = \Modules\Grow\Models\Lineitem::Where('lineitemresource_type', 'invoice')->Where('lineitemresource_id', $id)->get()) {
