...
namespace Modules\Grow\Http\Controllers\Webmail;

use Modules\Grow\Http\Controllers\Controller;
use Modules\Grow\Http\Responses\Webmail\ComposeResponse;
use Modules\Grow\Http\Responses\Webmail\SendResponse;
...
class Compose extends Controller {
    ...
    public function compose() {
        ...
        if (request('resource_type') == 'client' && is_numeric(request('resource_id'))) {
            ...
            if ($client = $clients->first()) {
                if ($users = \Modules\Grow\Models\User::Where('clientid', request('resource_id'))->orderBy('account_owner', 'DESC')->get()) {
        ...
        if (request('resource_type') == 'user' && is_numeric(request('resource_id'))) {
            if ($user = \Modules\Grow\Models\User::Where('id', request('resource_id'))->first()) {
                $recipients['users'][] = [
                    'name' => $user->first_name . ' ' . $user->last_name,
                    'email' => $user->email,
                ];
        ...
        if (request('resource_type') == 'lead' && is_numeric(request('resource_id'))) {
            if ($lead = \Modules\Grow\Models\Lead::Where('lead_id', request('resource_id'))->first()) {
                $recipients['users'][] = [
        ...
        //get email templates
        $templates = \Modules\Grow\Models\WebmailTemplate::Where('webmail_template_type', request('webmail_template_type'))->orderBy('webmail_template_name', 'ASC')->get();
    ...
    public function send() {
         ...
         * ----------------------------------------------*/
        $email = new \Modules\Grow\Models\EmailQueue();
    ...
    public function prefillTemplate() {
        ...
        if (!$template = \Modules\Grow\Models\WebmailTemplate::Where('webmail_template_id', request('id'))->first()) {
            abort(404);
        }
