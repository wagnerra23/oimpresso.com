...
use Modules\Grow\Http\Responses\Milestones\CreateResponse;
use Modules\Grow\Http\Responses\Milestones\DestroyResponse;
use Modules\Grow\Http\Responses\Milestones\EditResponse;
use Modules\Grow\Http\Responses\Milestones\IndexResponse;
use Modules\Grow\Http\Responses\Milestones\StoreResponse;
use Modules\Grow\Http\Responses\Milestones\UpdateResponse;
...
class Milestones extends Controller {
    ...
    public function store() {
        ...
        //avoid duplicate
        if (\Modules\Grow\Models\Milestone::where('milestone_title', request('milestone_title'))
    ...
    public function destroy($id) {
        ...
        if (request('delete_milestone_tasks') == 'on') {
            ...
            if ($default = \Modules\Grow\Models\Milestone::Where('milestone_projectid', $milestone->milestone_projectid)->Where('milestone_type', 'uncategorised')->first()) {
                //update
                \Modules\Grow\Models\Task::Where('task_milestoneid', $milestone->milestone_id)->update(['task_milestoneid' => $default->milestone_id]);
            }
        }
    ...
    public function updatePositions() {
        ...
        foreach (request('sort-milestones') as $key => $id) {
            if (is_numeric($id)) {
                \Modules\Grow\Models\Milestone::where('milestone_id', $id)->update(['milestone_position' => $i]);
            }
            $i++;
        }
