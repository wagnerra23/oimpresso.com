...
use Modules\Grow\Http\Requests\Proposals\ProposalUpdateAutomation;
use Modules\Grow\Http\Requests\Proposals\StoreUpdate;
use Modules\Grow\Http\Responses\Common\ChangeCategoryResponse;
use Modules\Grow\Http\Responses\Documents\ShowEditResponse;
use Modules\Grow\Http\Responses\Documents\ShowPreviewResponse;
use Modules\Grow\Http\Responses\Proposals\ChangeCategoryUpdateResponse;
use Modules\Grow\Http\Responses\Proposals\ChangeStatusResponse;
use Modules\Grow\Http\Responses\Proposals\CreateCloneResponse;
use Modules\Grow\Http\Responses\Proposals\CreateResponse;
use Modules\Grow\Http\Responses\Proposals\DestroyResponse;
use Modules\Grow\Http\Responses\Proposals\EditAutomationResponse;
use Modules\Grow\Http\Responses\Proposals\EmailResponse;
use Modules\Grow\Http\Responses\Proposals\IndexResponse;
use Modules\Grow\Http\Responses\Proposals\PinningResponse;
use Modules\Grow\Http\Responses\Proposals\PublishResponse;
use Modules\Grow\Http\Responses\Proposals\PublishScheduledResponse;
use Modules\Grow\Http\Responses\Proposals\StoreResponse;
use Modules\Grow\Http\Responses\Proposals\UpdateAutomationResponse;
use Modules\Grow\Models\Category;
use Modules\Grow\Models\Proposal;
use Modules\Grow\Repositories\CategoryRepository;
use Modules\Grow\Repositories\CloneProposalRepository;
...
use Modules\Grow\Repositories\ProposalAutomationRepository;
use Modules\Grow\Repositories\ProposalRepository;
...
class Proposals extends Controller {
    ...
    public function create(CategoryRepository $categoryrepo) {
        ...
        //templates
        $templates = \Modules\Grow\Models\ProposalTemplate::orderBy('proposal_template_id', 'ASC')->get();
    ...
    public function store(StoreUpdate $request) {
        ...
        if (request('customer-selection-type') == 'client') {
            if (!$client = \Modules\Grow\Models\Client::Where('client_id', request('doc_client_id'))->first()) {
        ...
        if (request('customer-selection-type') == 'lead') {
            if (!$lead = \Modules\Grow\Models\Lead::Where('lead_id', request('doc_lead_id'))->first()) {
        ...
        //create the proposal
        $proposal = new \Modules\Grow\Models\Proposal();
        ...
        if (is_numeric(request('proposal_template'))) {
            if ($template = \Modules\Grow\Models\ProposalTemplate::Where('proposal_template_id', request('proposal_template'))->first()) {
    ...
    public function applyDefaultAutomation($proposal) {
        ...
        $assigned_users = \Modules\Grow\Models\AutomationAssigned::Where('automationassigned_resource_type', 'proposal')
    ...
    public function updateAutomation(ProposalUpdateAutomation $request, $id) {
        ...
        //get the project
        $proposal = \Modules\Grow\Models\Proposal::Where('doc_id', $id)->first();
        ...
        \Modules\Grow\Models\AutomationAssigned::Where('automationassigned_resource_type', 'proposal')
            ->Where('automationassigned_resource_id', $id)
            ->delete();
    ...
    public function show(EstimateGeneratorRepository $estimategenerator, $id) {
        ...
        //get the estimate
        if ($estimate = \Modules\Grow\Models\Estimate::Where('bill_proposalid', $id)->Where('bill_estimate_type', 'document')->first()) {
            request()->merge([
        ...
        //custom fields
        $customfields = \Modules\Grow\Models\CustomField::Where('customfields_type', 'clients')->get();
    ...
    public function showPublic(EstimateGeneratorRepository $estimategenerator, $id) {
        ...
        //get the proposal
        $doc = \Modules\Grow\Models\Proposal::Where('doc_unique_id', $id)->first();
    ...
    public function editingProposal(CategoryRepository $categoryrepo, $id) {
        ...
        if (!$estimate = \Modules\Grow\Models\Estimate::Where('bill_proposalid', $id)->Where('bill_estimate_type', 'document')->first()) {
            ...
            $estimate = \Modules\Grow\Models\Estimate::Where('bill_proposalid', $id)->Where('bill_estimate_type', 'document')->first();
        }
    ...
    public function changeCategoryUpdate(CategoryRepository $categoryrepo) {
        ...
        foreach (request('ids') as $proposal_id => $value) {
            if ($value == 'on') {
                $proposal = \Modules\Grow\Models\Proposal::Where('doc_id', $proposal_id)->first();
                //update the category
    ...
    public function publishScheduled($id) {
        ...
        if (!$proposal = \Modules\Grow\Models\Proposal::Where('doc_id', $id)->first()) {
            abort(404);
        }
    ...
    public function resendEmail($id) {
        ...
        if ($document->docresource_type == 'lead') {
            if ($lead = \Modules\Grow\Models\Lead::Where('lead_id', $document->doc_lead_id)->first()) {
                $data = [
                    'user_type' => 'lead',
                    'proposal_value' => $value,
                ];
    ...
    public function sign($id) {
        ...
        //get the proposal
        $proposal = \Modules\Grow\Models\Proposal::Where('doc_unique_id', $id)->first();
