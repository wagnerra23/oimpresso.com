...
use Modules\Grow\Http\Requests\Clients\ClientStoreValidation;
use Modules\Grow\Http\Responses\Clients\CommonResponse;
use Modules\Grow\Http\Responses\Clients\CreateResponse;
use Modules\Grow\Http\Responses\Clients\DestroyResponse;
use Modules\Grow\Http\Responses\Clients\DetailsResponse;
use Modules\Grow\Http\Responses\Clients\EditLogoResponse;
use Modules\Grow\Http\Responses\Clients\EditResponse;
use Modules\Grow\Http\Responses\Clients\IndexResponse;
use Modules\Grow\Http\Responses\Clients\ShowDynamicResponse;
use Modules\Grow\Http\Responses\Clients\ShowResponse;
use Modules\Grow\Http\Responses\Clients\StoreResponse;
use Modules\Grow\Http\Responses\Clients\UpdateDetailsResponse;
use Modules\Grow\Http\Responses\Clients\UpdateOwnerResponse;
use Modules\Grow\Http\Responses\Clients\UpdateResponse;
use Modules\Grow\Http\Responses\Clients\PinningResponse;
...
class Clients extends Controller {
    ...
    public function show($id) {
        ...
        $owner = \Modules\Grow\Models\User::Where('clientid', $id)
    ...
    public function update($id) {
        ...
        if (request('client_status') == 'suspended') {
            if ($users = \Modules\Grow\Models\User::Where('clientid', $id)->get()) {
                //each user - logout
                foreach ($users as $user) {
                    \Modules\Grow\Models\Session::Where('user_id', $user->id)->delete();
                }
            }
        }
    ...
    public function getCustomFields($obj = '') {
        ...
        if ($obj instanceof \Modules\Grow\Models\Client) {
    ...
    public function customFieldValidationFailed() {
        ...
        //custom field validation
        $fields = \Modules\Grow\Models\CustomField::Where('customfields_type', 'clients')->get();
        $errors = '';
        foreach ($fields as $field) {
    ...
    public function updateDescription() {
        ...
        //get the client
        $client = \Modules\Grow\Models\client::Where('client_id', request()->route('client'))->first();
    ...
    public function changeAccountOwner($id) {
        ...
        //get the foo
        if (!$users = \Modules\Grow\Models\User::Where('clientid', $id)->orderBy('first_name', 'DESC')->get()) {
            abort(409);
        }
    ...
    public function changeAccountOwnerUpdate($id) {
        ...
        //get the user
        if (!$user = \Modules\Grow\Models\User::Where('id', request('user_id'))->Where('clientid', $id)->first()) {
            abort(404);
        }

        //reset existing account owner
        \Modules\Grow\Models\User::where('clientid', $id)->update(['account_owner' => 'no']);
