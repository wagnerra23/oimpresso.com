...
use Modules\Grow\Http\Requests\Tasks\TaskRecurrringSettings;
use Modules\Grow\Http\Requests\Tasks\TaskStoreUpdate;
use Modules\Grow\Http\Responses\Tasks\ActivateResponse;
use Modules\Grow\Http\Responses\Tasks\ArchiveResponse;
use Modules\Grow\Http\Responses\Tasks\AttachFilesResponse;
use Modules\Grow\Http\Responses\Tasks\ChecklistResponse;
use Modules\Grow\Http\Responses\Tasks\CloneResponse;
use Modules\Grow\Http\Responses\Tasks\CloneStoreResponse;
use Modules\Grow\Http\Responses\Tasks\contentResponse;
use Modules\Grow\Http\Responses\Tasks\CreateResponse;
use Modules\Grow\Http\Responses\Tasks\DeleteTaskDependencyResponse;
use Modules\Grow\Http\Responses\Tasks\DestroyResponse;
use Modules\Grow\Http\Responses\Tasks\IndexKanbanResponse;
use Modules\Grow\Http\Responses\Tasks\IndexListResponse;
use Modules\Grow\Http\Responses\Tasks\RecurringSettingsResponse;
use Modules\Grow\Http\Responses\Tasks\ShowResponse;
use Modules\Grow\Http\Responses\Tasks\StoreChecklistResponse;
use Modules\Grow\Http\Responses\Tasks\StoreCommentResponse;
use Modules\Grow\Http\Responses\Tasks\StoreResponse;
use Modules\Grow\Http\Responses\Tasks\StoreTaskDependencyResponse;
use Modules\Grow\Http\Responses\Tasks\TimerStartResponse;
use Modules\Grow\Http\Responses\Tasks\TimerStopResponse;
use Modules\Grow\Http\Responses\Tasks\UpdateChecklistResponse;
use Modules\Grow\Http\Responses\Tasks\UpdateErrorResponse;
use Modules\Grow\Http\Responses\Tasks\UpdateLockedResponse;
use Modules\Grow\Http\Responses\Tasks\UpdateResponse;
use Modules\Grow\Http\Responses\Tasks\UpdateStatusLockedResponse;
use Modules\Grow\Http\Responses\Tasks\UpdateStatusResponse;
use Modules\Grow\Http\Responses\Tasks\UpdateTagsResponse;
use Modules\Grow\Models\Checklist;
use Modules\Grow\Models\Comment;
use Modules\Grow\Models\Task;
use Modules\Grow\Models\Timer;
use Modules\Grow\Http\Permissions\AttachmentPermissions;
use Modules\Grow\Http\Permissions\ChecklistPermissions;
use Modules\Grow\Http\Permissions\CommentPermissions;
use Modules\Grow\Http\Permissions\ProjectPermissions;
use Modules\Grow\Http\Permissions\TaskPermissions;
use Modules\Grow\Repositories\AttachmentRepository;
use Modules\Grow\Repositories\CategoryRepository;
use Modules\Grow\Repositories\ChecklistRepository;
use Modules\Grow\Repositories\CommentRepository;
use Modules\Grow\Repositories\CustomFieldsRepository;
use Modules\Grow\Repositories\DestroyRepository;
use Modules\Grow\Repositories\EmailerRepository;
use Modules\Grow\Repositories\EventRepository;
use Modules\Grow\Repositories\EventTrackingRepository;
use Modules\Grow\Repositories\ProjectAssignedRepository;
use Modules\Grow\Repositories\ProjectRepository;
use Modules\Grow\Repositories\TagRepository;
use Modules\Grow\Repositories\TaskAssignedRepository;
use Modules\Grow\Repositories\TaskDependencyRepository;
use Modules\Grow\Repositories\TaskRepository;
use Modules\Grow\Repositories\TaskStatusRepository;
use Modules\Grow\Repositories\TimerRepository;
use Modules\Grow\Repositories\UserRepository;
use Modules\Grow\Http\Rules\CheckBox;
...
use Validator;
use Modules\Grow\Http\Responses\Tasks\PinningResponse;
use Modules\Grow\Repositories\PinnedRepository;
...
class Tasks extends Controller {
    ...
    public function indexList() {
        ...
        $statuses = \Modules\Grow\Models\TaskStatus::all();
        ...
        if (request()->filled('taskresource_id') && request('taskresource_type') == 'project') {
            $milestones = \Modules\Grow\Models\Milestone::Where('milestone_projectid', request('taskresource_id'))->get();
        }

        $priorities = \Modules\Grow\Models\TaskPriority::orderBy('taskpriority_position', 'asc')->get();
    ...
    private function taskBoards() {
        ...
        $statuses = \Modules\Grow\Models\TaskStatus::orderBy('taskstatus_position', 'asc')->get();
    ...
    public function getCustomFields($obj = '') {
        ...
        if ($obj instanceof \Modules\Grow\Models\Task) {
    ...
    public function store(TaskStoreUpdate $request, TaskAssignedRepository $assignedrepo) {
        ...
        $project = \Modules\Grow\Models\Project::find(request('task_projectid'));
        ...
        if (request()->filled('task_milestoneid')) {
            if (!\Modules\Grow\Models\Milestone::where('milestone_id', request('task_milestoneid'))
        ...
        if (!request()->filled('task_milestoneid')) {
            if ($milestone = \Modules\Grow\Models\Milestone::where('milestone_type', 'uncategorised')
                ->where('milestone_projectid', request('task_projectid'))->first()) {
                request()->merge([
        ...
        foreach ($assigned_users as $assigned_user_id) {
            if ($assigned_user = \Modules\Grow\Models\User::Where('id', $assigned_user_id)->first()) {
    ...
    public function customFieldValidationFailed() {
        ...
        //custom field validation
        $fields = \Modules\Grow\Models\CustomField::Where('customfields_type', 'tasks')->get();
        $errors = '';
        foreach ($fields as $field) {
    ...
    public function show(
        ...
        //get clients users
        $client_users = \Modules\Grow\Models\User::Where('clientid', $task->task_clientid)->orderBy('first_name', 'asc')->get();
        ...
        //milestone
        $milestones = \Modules\Grow\Models\Milestone::Where('milestone_projectid', $task->task_projectid)->get();

        //page settings
        $page = $this->pageSettings('task', $task);

        //mark events as read
        \Modules\Grow\Models\EventTracking::where('parent_id', $id)
            ->where('parent_type', 'task')
            ->where('eventtracking_userid', auth()->id())
            ->update(['eventtracking_status' => 'read']);

        //get users reminders
        if ($reminder = \Modules\Grow\Models\Reminder::Where('reminderresource_type', 'task')
        ...
        $project_tasks = \Modules\Grow\Models\Task::Where('task_projectid', $task->task_projectid)->orderBy('task_title', 'ASC')->get();
    ...
    private function processTask($task = '') {
        ...
        if ($task instanceof \Modules\Grow\Models\Task) {
    ...
    public function updateStatus(ProjectPermissions $projectpermissions, $id) {
        ...
        $validator = Validator::make(request()->all(), [
            'task_status' => [
                'required',
                function ($attribute, $value, $fail) {
                    if (\Modules\Grow\Models\TaskStatus::Where('taskstatus_id', $value)->doesntExist()) {
        ...
        if ($old_status != request('task_status')) {
            if ($first_task = \Modules\Grow\Models\Task::Where('task_status', request('task_status'))->orderBy('task_position', 'ASC')->first()) {
    ...
    public function updatePriority($id) {
        ...
        //get the priority
        $priority = \Modules\Grow\Models\TaskPriority::Where('taskpriority_id', request('task_priority'))->first();
    ...
    public function updateMilestone($id) {
        ...
        //validate
        if (!\Modules\Grow\Models\Milestone::Where('milestone_id', request('task_milestoneid'))->where('milestone_projectid', $task->task_projectid)->exists()) {
    ...
    public function updateAssigned(TaskAssignedRepository $assignedrepo, $id) {
        ...
        if (request()->filled('assigned')) {
            foreach (request('assigned') as $user_id => $value) {
                if ($value == 'on') {
                    ...
                    if (\Modules\Grow\Models\User::Where('id', $user_id)->doesntExist()) {
                        return new UpdateResponse([
                            'type' => 'update-assigned',
                            'tasks' => $tasks,
                            'task' => $task,
        ...
        foreach ($currently_assigned as $current_user) {
            if (!in_array($current_user, $newly_signed_users)) {
                ...
                \Modules\Grow\Models\Timer::where('timer_taskid', $id)->where('timer_creatorid', $current_user)
    ...
    public function StoreChecklist(ChecklistRepository $checklistrepo, $id) {
        ...
        //get next position
        if ($last = \Modules\Grow\Models\Checklist::Where('checklistresource_type', 'task')
    ...
    public function UpdateChecklist(ChecklistRepository $checklistrepo, $id) {
        ...
        //update checklist
        $checklist = \Modules\Grow\Models\Checklist::Where('checklist_id', $id)->first();
    ...
    public function deleteAttachment() {
        ...
        $attachment = \Modules\Grow\Models\Attachment::Where('attachment_uniqiueid', request()->route('uniqueid'))->first();
        ...
        if ($task = \Modules\Grow\Models\Task::Where('task_id', $task_id)->first()) {
    ...
    private function applyCommentPermissions($comment = '') {
        ...
        if ($comment instanceof \Modules\Grow\Models\Comment) {
            //delete permissions
    ...
    private function applyAttachmentPermissions($attachment = '', $task = []) {
        ...
        //sanity - make sure this is a valid object
        if ($attachment instanceof \Modules\Grow\Models\Attachment) {
            //delete permissions
    ...
    private function applyChecklistPermissions($checklist = '') {
        ...
        //sanity - make sure this is a valid object
        if ($checklist instanceof \Modules\Grow\Models\Checklist) {
            //delete permissions
    ...
    public function updatePosition() {
        ...
        if (\Modules\Grow\Models\TaskStatus::Where('taskstatus_id', request('status'))->doesntExist()) {
            abort(409, __('lang.error_request_could_not_be_completed'));
        }
    ...
    public function archive($id) {
        ...
        $task = \Modules\Grow\Models\Task::Where('task_id', $id)->first();
    ...
    public function updateCustomFields($id) {
        ...
        //update
        foreach ($fields as $field) {
            \Modules\Grow\Models\Task::where('task_id', $id)
                ->update([
    ...
    public function showMyNotes($id) {
        ...
        //get tasks
        if ($note = \Modules\Grow\Models\Note::Where('noteresource_type', 'task')
    ...
    public function editMyNotes($id) {
        ...
        //get tasks
        $note = \Modules\Grow\Models\Note::Where('noteresource_type', 'task')
            ->Where('noteresource_id', $id)
            ->Where('note_creatorid', auth()->id())->first();
    ...
    public function deleteMyNotes($id) {
        ...
        \Modules\Grow\Models\Note::Where('noteresource_type', 'task')
            ->where('noteresource_id', $id)
    ...
    public function updateMyNotes($id) {
        ...
        //create note
        $note = new \Modules\Grow\Models\Note();
    ...
    public function cloneStore(ProjectRepository $projectrepo, ProjectPermissions $projectpermissions, TaskAssignedRepository $assignedrepo, $id) {
        ...
        //project exists
        if (\Modules\Grow\Models\Project::Where('project_id', request('project_id'))->doesntExist()) {
            abort(409, __('lang.project') . ' - ' . __('lang.is_invalid'));
        }

        //project exists
        if (\Modules\Grow\Models\Milestone::Where('milestone_id', request('task_milestoneid'))->where('milestone_projectid', request('project_id'))->doesntExist()) {
        ...
        if (auth()->user()->is_team) {
            if (auth()->user()->is_admin) {
                $projects = \Modules\Grow\Models\Project::Where('project_type', 'project')->get();
                $project_list = [];
        ...
        //project
        $project = \Modules\Grow\Models\Project::Where('project_id', request('project_id'))->first();
    ...
    public function stopRecurring() {
        ...
        //get the task
        $task = \Modules\Grow\Models\Task::Where('task_id', request()->route('task'))->first();
    ...
    public function storeDependency(TaskDependencyRepository $dependencyrepo, $id) {
        ...
        if (!$blocking_task = \Modules\Grow\Models\Task::Where('task_id', request('tasksdependency_blockerid'))->first()) {
        ...
        if (\Modules\Grow\Models\TaskDependency::Where('tasksdependency_taskid', $id)
        ...
        $dependency = new \Modules\Grow\Models\TaskDependency();
    ...
    public function refreshDependencies($task) {
        ...
        if ($task->task_status == 2) {
            \Modules\Grow\Models\TaskDependency::where('tasksdependency_blockerid', $task->task_id)
    ...
    public function deleteDependency($id) {
        ...
        //delete record
        \Modules\Grow\Models\TaskDependency::Where('tasksdependency_taskid', $id)->where('tasksdependency_id', request('dependency_id'))->delete();
    ...
    public function addCoverImage($id) {
        ...
        //validate image
        if (!$image = \Modules\Grow\Models\Attachment::Where('attachment_uniqiueid', $attachment_uniqiueid)->Where('attachment_type', 'image')->Where('attachmentresource_id', $id)->first()) {
            abort(409, __('lang.error_request_could_not_be_completed'));
        }
    ...
    private function statsWidget($data = array()) {
        ...
        foreach (config('task_statuses') as $status) {
            $stat = [
                'value' => \Modules\Grow\Models\Task::where('task_status', $status->taskstatus_id)->count(),
