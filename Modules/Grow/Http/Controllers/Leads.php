...
use Modules\Grow\Http\Requests\Leads\LeadConvert;
use Modules\Grow\Http\Requests\Leads\LeadStoreUpdate;
use Modules\Grow\Http\Responses\Common\ChangeCategoryResponse;
use Modules\Grow\Http\Responses\Leads\ActivateResponse;
use Modules\Grow\Http\Responses\Leads\ArchiveResponse;
use Modules\Grow\Http\Responses\Leads\AttachFilesResponse;
use Modules\Grow\Http\Responses\Leads\BulkChangeStatusResponse;
use Modules\Grow\Http\Responses\Leads\BulkChangeStatusUpdateResponse;
use Modules\Grow\Http\Responses\Leads\ChangeAssignedResponse;
use Modules\Grow\Http\Responses\Leads\ChangeAssignedUpdateResponse;
use Modules\Grow\Http\Responses\Leads\ChangeCategoryUpdateResponse;
use Modules\Grow\Http\Responses\Leads\ChangeStatusResponse;
use Modules\Grow\Http\Responses\Leads\ChecklistResponse;
use Modules\Grow\Http\Responses\Leads\CloneResponse;
use Modules\Grow\Http\Responses\Leads\CloneStoreResponse;
use Modules\Grow\Http\Responses\Leads\contentResponse;
use Modules\Grow\Http\Responses\Leads\ConvertDetailsResponse;
use Modules\Grow\Http\Responses\Leads\ConvertLeadResponse;
use Modules\Grow\Http\Responses\Leads\CreateResponse;
use Modules\Grow\Http\Responses\Leads\DestroyResponse;
use Modules\Grow\Http\Responses\Leads\IndexKanbanResponse;
use Modules\Grow\Http\Responses\Leads\IndexListResponse;
use Modules\Grow\Http\Responses\Leads\PinningResponse;
use Modules\Grow\Http\Responses\Leads\ShowResponse;
use Modules\Grow\Http\Responses\Leads\StoreChecklistResponse;
use Modules\Grow\Http\Responses\Leads\StoreCommentResponse;
use Modules\Grow\Http\Responses\Leads\StoreResponse;
use Modules\Grow\Http\Responses\Leads\UpdateChecklistResponse;
use Modules\Grow\Http\Responses\Leads\UpdateErrorResponse;
use Modules\Grow\Http\Responses\Leads\UpdateResponse;
use Modules\Grow\Http\Responses\Leads\UpdateStatusResponse;
use Modules\Grow\Http\Responses\Leads\UpdateTagsResponse;
use Modules\Grow\Models\Checklist;
use Modules\Grow\Models\Comment;
use Modules\Grow\Models\Lead;
...
use Modules\Grow\Http\Permissions\LeadPermissions;
...
use Modules\Grow\Repositories\ClientRepository;
...
use Modules\Grow\Repositories\LeadAssignedRepository;
use Modules\Grow\Repositories\LeadRepository;
...
use Modules\Grow\Http\Rules\NoTags;
...
class Leads extends Controller {
    ...
    public function indexList() {
        ...
        $statuses = \Modules\Grow\Models\LeadStatus::all();
    ...
    private function leadBoards() {
        ...
        $statuses = \Modules\Grow\Models\LeadStatus::orderBy('leadstatus_position', 'asc')->get();
    ...
    public function create(CategoryRepository $categoryrepo) {
        ...
        $sources = \Modules\Grow\Models\LeadSources::all();
    ...
    public function getCustomFields($obj = '') {
        ...
        if ($obj instanceof \Modules\Grow\Models\Lead) {
    ...
    public function customFieldValidationFailed() {
        ...
        //custom field validation
        $fields = \Modules\Grow\Models\CustomField::Where('customfields_type', 'leads')->get();
        $errors = '';
        foreach ($fields as $field) {
    ...
    public function show(
        ...
        //get users reminders
        if ($reminder = \Modules\Grow\Models\Reminder::Where('reminderresource_type', 'lead')
    ...
    private function processLead($lead = '') {
        ...
        if ($lead instanceof \Modules\Grow\Models\Lead) {
    ...
    public function changeCategoryUpdate(CategoryRepository $categoryrepo) {
        ...
        if (!\Modules\Grow\Models\Category::Where('category_id', request('category'))
        ...
        foreach (request('ids') as $lead_id => $value) {
            if ($value == 'on') {
                $lead = \Modules\Grow\Models\Lead::Where('lead_id', $lead_id)->first();
                //update the category
    ...
    public function changeStatus() {
        ...
        //get the lead
        $lead = \Modules\Grow\Models\Lead::Where('lead_id', request()->route('lead'))->first();
    ...
    public function changeStatusUpdate() {
        ...
        $contacts = \Modules\Grow\Models\User::where('clientid', $lead['lead_clientid'])->where('type', 'client')->get();
    ...
    public function deleteAttachment() {
        ...
        if ($lead = \Modules\Grow\Models\Lead::Where('lead_id', $lead_id)->first()) {
    ...
    public function StoreChecklist(ChecklistRepository $checklistrepo, $id) {
        ...
        //get next position
        if ($last = \Modules\Grow\Models\Checklist::Where('checklistresource_type', 'lead')
    ...
    public function updateStatus($id) {
        ...
        //validate
        if (!\Modules\Grow\Models\LeadStatus::Where('leadstatus_id', request('lead_status'))->exists()) {
        ...
        $statuses = \Modules\Grow\Models\LeadStatus::Where('leadstatus_id', request('lead_status'))->first();
    ...
    public function updateCategory($id) {
        ...
        //validate
        if (!\Modules\Grow\Models\Category::Where('category_id', request('lead_categoryid'))->Where('category_type', 'lead')->exists()) {
            //show error and reset values
            return new UpdateErrorResponse([
        ...
        $categories = \Modules\Grow\Models\Category::Where('category_id', request('lead_categoryid'))->Where('category_type', 'lead')->first();
        ...
        if (request()->filled('assigned')) {
            foreach (request('assigned') as $user_id => $value) {
                if ($value == 'on') {
                    ...
                    if (\Modules\Grow\Models\User::Where('id', $user_id)->Where('type', 'team')->doesntExist()) {
                        return new UpdateResponse([
                            'type' => 'update-assigned',
                            'leads' => $leads,
                            'assigned' => $assignedrepo->getAssigned($id),
    ...
    public function convertDetails($id) {
        ...
        //check if file exists in the database
        if (!$lead = \Modules\Grow\Models\Lead::Where('lead_id', $id)->first()) {
            return;
        }
    ...
    public function convertLead(LeadConvert $request, ClientRepository $clientrepo, UserRepository $userrepo, $id) {
        ...
        //create new customer
        if (\Modules\Grow\Models\Client::where('client_created_from_leadid', $id)->exists()) {
            abort(409, __('lang.client_already_exists'));
        }

        //check for duplicate user - ignore [contact] type users
        if (\Modules\Grow\Models\User::Where('email', request('email'))->WhereIn('type', ['client', 'team'])->first()) {
        ...
        \Modules\Grow\Models\Proposal::where('docresource_type', 'lead')->where('doc_lead_id', $id)
            ->update([
    ...
    public function archive($id) {
        ...
        $lead = \Modules\Grow\Models\Lead::Where('lead_id', $id)->first();
    ...
    public function updateCustomFields($id) {
        ...
        //update
        foreach ($fields as $field) {
            \Modules\Grow\Models\Lead::where('lead_id', $id)
                ->update([
    ...
    public function showMyNotes($id) {
        ...
        //get leads
        if ($note = \Modules\Grow\Models\Note::Where('noteresource_type', 'lead')
    ...
    public function editMyNotes($id) {
        ...
        //get leads
        $note = \Modules\Grow\Models\Note::Where('noteresource_type', 'lead')
            ->Where('noteresource_id', $id)
            ->Where('note_creatorid', auth()->id())->first();
    ...
    public function deleteMyNotes($id) {
        ...
        \Modules\Grow\Models\Note::Where('noteresource_type', 'lead')
            ->where('noteresource_id', $id)
    ...
    public function showLogs($id) {
        ...
        $logs = \Modules\Grow\Models\Leadlog::Where('leadlog_leadid', $id)->orderBy('leadlog_id', 'DESC')->get();
    ...
    private function statsWidget($data = array()) {
        ...
        foreach ($statuses as $status) {
            ...
            $count = \Modules\Grow\Models\Lead::where('lead_status', $status->leadstatus_id)->count();
            $sum = \Modules\Grow\Models\Lead::where('lead_status', $status->leadstatus_id)->sum('lead_value');
