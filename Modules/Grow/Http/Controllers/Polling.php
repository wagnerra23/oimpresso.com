...
use Modules\Grow\Http\Responses\Polling\ActiveTimerResponse;
use Modules\Grow\Http\Responses\Polling\GeneralResponse;
use Modules\Grow\Http\Responses\Polling\TimersResponse;
...
class Polling extends Controller {
    ...
    public function generalPoll() {
        ...
        //get users unread notifications
        $count_reminders = \Modules\Grow\Models\Reminder::where('reminder_userid', auth()->id())->where('reminder_status', 'due')->count();

        //count unread instant messages
        $count_unread_messages = \Modules\Grow\Models\MessagesTracking::where('messagestracking_user_unique_id', auth()->user()->unique_id)
    ...
    public function timersPoll(TimerRepository $timerrepo) {
        ...
        if (is_array(request('timers'))) {
            ...
            $timers = \Modules\Grow\Models\Timer::selectRaw('timer_taskid, timer_time, timer_status')
        ...
        if ($timer = \Modules\Grow\Models\Timer::where('timer_status', 'running')->where('timer_creatorid', auth()->id())->first()) {
    ...
    public function activeTimerPoll(TimerRepository $timerrepo) {
        ...
        //get the task
        $task = \Modules\Grow\Models\Task::Where('task_id', $timer->timer_taskid)->first();
