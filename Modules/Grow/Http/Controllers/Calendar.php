...
use Modules\Grow\Http\Requests\Calendar\StoreUpdate;
use Modules\Grow\Http\Responses\Calendar\CreateResponse;
use Modules\Grow\Http\Responses\Calendar\DeleteFileResponse;
use Modules\Grow\Http\Responses\Calendar\DestroyResponse;
use Modules\Grow\Http\Responses\Calendar\IndexResponse;
use Modules\Grow\Http\Responses\Calendar\ShowResponse;
use Modules\Grow\Http\Responses\Calendar\StoreResponse;
use Modules\Grow\Http\Responses\Calendar\UpdateResponse;
...
use Modules\Grow\Repositories\CalendarRepository;
...
class Calendar extends Controller {
    ...
    public function index() {
        ...
        if (request('calendar_action') == 'user-preferences') {
            \Modules\Grow\Models\User::where('id', auth()->id())
                ->update([
    ...
    public function store(StoreUpdate $request) {
        ...
        //new record
        $new_event = new \Modules\Grow\Models\CalendarEvent();
        ...
        if (request()->filled('sharing_team_members')) {
            foreach (request('sharing_team_members') as $user_id) {
                $sharing = new \Modules\Grow\Models\CalenderEventSharing();
    ...
    public function destroy(DestroyRepository $destroyrepo, $id) {
         ...
         * ----------------------------------------------------*/
        if (request('resource_type') == 'calendarevent') {
            if ($event = \Modules\Grow\Models\CalendarEvent::Where('calendar_event_uniqueid', $id)->first()) {
                //delete assigned
                \Modules\Grow\Models\CalenderEventSharing::Where('calendarsharing_eventid', $event->calendar_event_id)->delete();
                //delete record
                \Modules\Grow\Models\CalendarEvent::Where('calendar_event_uniqueid', $id)->delete();
            }
         ...
         * ----------------------------------------------------*/
        if (request('resource_type') == 'project') {
            if ($project = \Modules\Grow\Models\Project::Where('project_uniqueid', $id)->first()) {
         ...
         * ----------------------------------------------------*/
        if (request('resource_type') == 'task') {
            if ($task = \Modules\Grow\Models\Task::Where('task_uniqueid', $id)->first()) {
    ...
    public function deleteFiles(ProjectPermissions $projectpermissions, TaskPermissions $taskpermissions, DestroyRepository $destroyrepo, $id) {
        ...
        if (request('type') == 'attachment') {
            if (!$attachment = \Modules\Grow\Models\Attachment::Where('attachment_uniqiueid', $id)->first()) {
                abort(404);
            }
