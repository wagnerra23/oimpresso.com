...
it('UC-FSPED-10 · flag default = true em produção (segurança audit sênior R1)', function () {
    expect(config('fiscal.sped_simples_only_lock'))->toBeTrue(
        'default deve ser true até GAP-FISCAL-003 eliminar hardcodes',
    );
});

it('UC-FSPED-10 · config key vive em config/fiscal.php (não em config/app.php nem hardcoded)', function () {
...
it('UC-FSPED-10 · SpedController referencia a flag no método gerar', function () {
