 ...
 *   - Audit LGPD via ForjaAuditService (D7.b)
 *
...
class ProjectsController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $this->makeAuditService($request)->log(
            event: ForjaAuditService::EVENT_PROJECT_CREATED,
    ...
    public function decompose(Request $request, int $id, ProjectDecomposerService $service): RedirectResponse
    {
        ...
        if ($result['success']) {
            ...
            $this->makeAuditService($request)->log(
                event: ForjaAuditService::EVENT_PROJECT_DECOMPOSED,
    ...
    /**
     * Factory do ForjaAuditService com $businessId resolvido da session.
     */
    private function makeAuditService(Request $request): ForjaAuditService
    {
        $businessId = (int) $request->session()->get('user.business_id', 1);

        return app()->makeWith(ForjaAuditService::class, ['businessId' => $businessId]);
    }
}
