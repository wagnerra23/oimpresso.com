...
- **O contador de vencidas lê `due_date` REAL**, não a janela que o `toGanttTasks` inventa quando falta data (`start + 3d`, `Gantt.tsx` :196). Aquele default existe para a barra ter largura, não para afirmar prazo; contar vencida em cima dele transformaria um default de desenho em fato de negócio. Consequência **medida em produção em 2026-09-03**, e ela corrige a previsão que este charter fazia: eu escrevera que o contador "normalmente mostra 0"; ele mostra **5**. Das 7 tasks com `due_date`, **5 já venceram** — o contador está dizendo algo útil, não zero. A previsão era afirmação sem medição (LC-08); o comportamento do código estava certo desde o início.
