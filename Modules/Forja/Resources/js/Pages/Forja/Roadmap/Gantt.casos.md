...
- **[BACKLOG] O contador "N com prazo vencido" lê `due_date` real, nunca a janela derivada** (PARIDADE §11 Onda 6) — o `toGanttTasks` inventa `start + 3d` quando falta data, e isso existe pra a barra ter largura, não pra afirmar prazo. A asserção é de cliente (o `useMemo` de `vencidas`), então precisa de render test do `.tsx` ou spec Playwright. ⚠️ Difícil de flagrar por olho: com **7 de 1186** tasks com `due_date` em produção (medido 2026-09-03), o contador certo e o errado só divergem quando alguém preenche prazo — e aí já está em produção.
- **[BACKLOG] Sem `jana.mcp.tasks.write`, nem o parágrafo-âncora nem a hint prometem arrastar** — `can_edit=false` é payload testável, mas a asserção que importa é o texto renderizado (afordância falsa, LC-15). Mesmo motivo do caso do drag acima: é de cliente.
