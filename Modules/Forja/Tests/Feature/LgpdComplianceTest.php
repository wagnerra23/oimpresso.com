...
/**
 * Testa D7 LGPD compliance do Módulo Forja — Wave 16 push (2026-05-16).
 *
 * Cobre 3 sub-dimensões da matriz Governance V3:
 * - D7.a (4 pts): PiiRedactor aplicado em audit logs (ForjaAuditService)
 * - D7.b (3 pts): Audit trail via Spatie activity_log (Forja **não tem Entities
 ...
 * @see Modules\Forja\Config\retention.php
 * @see Modules\Forja\Services\ForjaAuditService
 */
...
it('config/retention.php existe e é array válido (D7.c declaração)', function () {
    $configPath = base_path('Modules/Forja/Config/retention.php');
...
it('retention.php declara TTL pras 7 entidades PM canônicas (D7.c cobertura)', function () {
    $config = require base_path('Modules/Forja/Config/retention.php');
...
// ------------------------------------------------------------------
// D7.b — Audit trail via ForjaAuditService (3 pts)
// ------------------------------------------------------------------

it('ForjaAuditService existe e exige business_id no constructor (Tier 0)', function () {
    expect(class_exists(ForjaAuditService::class))->toBeTrue();

    // Constructor exige business_id > 0
    expect(fn () => new ForjaAuditService(0, app(PiiRedactor::class)))
        ->toThrow(\InvalidArgumentException::class);

    expect(fn () => new ForjaAuditService(-1, app(PiiRedactor::class)))
        ->toThrow(\InvalidArgumentException::class);
});

it('ForjaAuditService aceita business_id válido', function () {
    $service = new ForjaAuditService(PMG_LGPD_BIZ_WAGNER, app(PiiRedactor::class));
    expect($service)->toBeInstanceOf(ForjaAuditService::class);
});

it('ForjaAuditService declara eventos canônicos do domínio', function () {
    $reflection = new ReflectionClass(ForjaAuditService::class);
...
it('ForjaAuditService LOG_NAME é "project-mgmt" (canônico Spatie)', function () {
    expect(ForjaAuditService::LOG_NAME)->toBe('project-mgmt');
});

it('ForjaAuditService rejeita event não-canônico (defense)', function () {
    $service = new ForjaAuditService(PMG_LGPD_BIZ_WAGNER, app(PiiRedactor::class));
...
it('ForjaAuditService redaciona PII em description antes de persistir', function () {
    $service = new ForjaAuditService(PMG_LGPD_BIZ_WAGNER, app(PiiRedactor::class));
...
it('Admin/ProjectsController importa ProjectService + ForjaAuditService (D4+D7 integração)', function () {
    $contents = file_get_contents(
        base_path('Modules/Forja/Http/Controllers/Admin/ProjectsController.php')
    );

    expect($contents)
        ->toContain('use Modules\\Forja\\Services\\ProjectService;')
        ->toContain('use Modules\\Forja\\Services\\ForjaAuditService;');
});

it('ForjaAuditService.log() retorna Activity model (smoke, mockado em memória)', function () {
