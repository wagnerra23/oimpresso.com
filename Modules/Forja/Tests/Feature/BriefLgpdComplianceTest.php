 ...
 * @see Modules/Forja/Services/BriefGeneratorService.php
...
it('config retention.php existe e declara entidades canonicas', function () {
    ...
    expect(file_exists($configPath))->toBeTrue('retention.php nao existe em Modules/Forja/Config/');
...
it('config brief.redact_pii_before_llm publicado no namespace brief.*', function () {
    ...
    expect($val)->toBeTrue('mergeConfigFrom no ForjaServiceProvider falhou (Config/brief-retention.php sob a chave brief.*)');
