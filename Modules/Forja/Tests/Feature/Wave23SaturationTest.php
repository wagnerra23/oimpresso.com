...
/**
 * Wave 23 SATURATION Forja — F1 + F2 reuse + F3 Perf gap 72→≥80.
 *
 ...
 *
 * @see Modules\Forja\Services\ProjectService
 * @see Modules\Forja\Console\Commands\ForjaHealthCommand
 */

it('F1 confirmacao: CustomerJourneyTest Wave 18 tem >=5 cenarios', function () {
    $file = base_path('Modules/Forja/Tests/Feature/CustomerJourneyTest.php');
...
it('F3 Perf: ProjectService usa OtelHelper::spanBiz canon (D9 hot-path)', function () {
    $source = file_get_contents(base_path('Modules/Forja/Services/ProjectService.php'));
...
it('F6 ForjaHealthCommand registrado + signature canon', function () {
    ...
    $cmd = app(ForjaHealthCommand::class);
    expect($cmd)->toBeInstanceOf(ForjaHealthCommand::class);
