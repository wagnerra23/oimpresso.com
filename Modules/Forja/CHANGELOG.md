...
- **`Tests/Feature/Wave27SaturationTest.php`** (10 cenários, 0.27s avg) — saturação polish ≥90:
  ...
  - D9 span `ForjaAuditService.log` canônico — helpers `logTaskStatusChange`/`logTaskComment` delegam → span propagado
...
- D9.a OTel (W17): `ProjectService` 6 spans + `ForjaAuditService.log` 1 span.
...
- D9.a OTel: `ProjectService` + `ForjaAuditService` envoltos em `OtelHelper::spanBiz` desde Wave 17.
...
- D9.a Services já tinham `OtelHelper::spanBiz` cobertura desde Wave 17 (ProjectService + ForjaAuditService)
...
- ProjectService + ForjaAuditService já tinham `OtelHelper::spanBiz` cobertura desde Wave 17 (D9.a saturated)
