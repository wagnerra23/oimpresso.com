...
/**
 * project-mgmt:health — Health check do módulo Forja (D9.c — Wave 17 saturação 97%).
 *
 ...
 */
class ForjaHealthCommand extends Command
{
    ...
    protected $description = 'Health check do módulo Forja — 4 sinais (ADR 0155 D9.c, Wave 17).';
    ...
    private function checkProjectsTable(): array
    {
        if (! Schema::hasTable('mcp_projects')) {
            return $this->makeCheck('projects_table_present', 'FAIL', 0, '1',
                ...
                'Rode migrations Jana — Forja opera sobre models McpProject.'
            );
        }
    ...
    private function outputTable(array $checks, array $summary, ?int $businessId, bool $alert): int
    {
        ...
        $this->line('');
        $this->info('Forja Health Check — ' . now()->toDateTimeString());
