 ...
 * controller/request KB é `can:jana.mcp.memory.manage`. Os kb.* granulares são
 ...
 * 'jana.mcp.memory.manage'; os kb.* ficam ao lado como intenção futura
...
it('POST /kb/nodes without kb.write returns 403', function () {
    kbActAsUser(bizId: 1, permissions: ['kb.view']);  // sem 'jana.mcp.memory.manage' → 403 no gate coarse V1 (e sem kb.write pós-rename)
...
it('POST /kb/nodes/{slug}/restore restores soft-deleted (perm: kb.restore)', function () {
    kbActAsUser(bizId: 1, permissions: ['jana.mcp.memory.manage', 'kb.view', 'kb.restore']);
