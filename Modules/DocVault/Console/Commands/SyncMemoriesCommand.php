 ...
 *   php artisan docvault:sync-memories --dry
 *   php artisan docvault:sync-memories --commit  (faz git commit após sync)
 */
class SyncMemoriesCommand extends Command
{
    protected $signature = 'docvault:sync-memories
    ...
    {
        $src = (string) config('docvault.memory.claude_dir');
        ...
        if ($this->option('commit') && $hasChanges && ! $this->option('dry')) {
            ...
            $msg = "chore(docvault): sync Claude memory ({$date})\n\n+{$added} ~{$changed} -{$removed}";
