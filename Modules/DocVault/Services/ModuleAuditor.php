 ...
 *   C09 PAGES_ANNOTATED        — todo .tsx do módulo tem @docvault
...
class ModuleAuditor
{
    ...
    protected function c09PagesAnnotated(string $moduleName): ?array
    {
        ...
        foreach ($it as $f) {
            ...
            if ($head && str_contains($head, '@docvault')) $annotated++;
        }

        if ($total === 0) return null;
        if ($annotated < $total) {
            return ['level' => 'warning', 'message' => "{$annotated}/{$total} telas .tsx com @docvault — faltam " . ($total - $annotated) . "."];
        }
        return null;
    }
    ...
    {
        // Convenção: DocVault → DocVault/, PontoWr2 → Ponto/, Essentials → Essentials/
