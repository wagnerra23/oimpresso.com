<script type="text/javascript">
	$(document).ready( function () {
	    ...
	    productions_table = $('#productions_table').DataTable({
	        ...
	        aaSorting: [[0, 'desc']],
	        ajax: {
	            url: '{{action("\Modules\Boleto\Http\Controllers\ProductionController@index")}}',
	            data: function(d) {
    	...
    	recipe_table = $('#recipe_table').DataTable({
	        processing: true,
	        serverSide: true,
	        ajax: '{{action("\Modules\Boleto\Http\Controllers\RecipeController@index")}}',
	        columnDefs: [
	            {
	                targets: [0, 5, 6, 7],
	                orderable: false,
    ...
    function addIngredientRow(variation_id, search_element) {
    	...
    	$.ajax({
            url: "/boleto/get-ingredient-row/" + variation_id + '?row_index=' + row_index + '&row_ig_index=' + row_ig_index + '&sort_order=' + sort_order,
            dataType: 'html',
	...
	$(document).on('submit', '#recipe_form', function (e) {
		...
		if (ingredients_length < 1) {
			toastr.error('@lang("boleto::lang.please_add_ingredients")');
			e.preventDefault();
			return false;
		}
	});
	...
	$(document).on('click', 'button#add_ingredient_group', function() {
    	...
    	$.ajax({
            url: "/boleto/ingredient-group-form" + '?ig_index=' + ig_index,
            dataType: 'html',
            success: function(result) {
            	var el = $(result);
                $('#box_group').append(el);
    ...
    if(selected_rows.length > 0){
        swal({
            ...
            if (willDelete) {
                ...
                $.ajax({
                    method: "post",
                    url: "/boleto/update-product-prices",
                    dataType: 'json',
                    data: data,
                    success: function(result) {
...
$(document).on('change', '#choose_product_form #variation_id', function() {
    ...
    if (variation_id) {
        $.ajax({
            method: 'get',
            url: "/boleto/is-recipe-exist/" + variation_id,
            dataType: 'json',
            success: function(result) {
                if (result == 1) {
