...
- **Qualidade versionada que morde o CI** — evals de recall e RAGAS (`jana:recall-eval`, `jana:ragas-real-eval`), com baseline em `governance/` que reprova regressão. O `jana:drift-sentinel` **não** mede a qualidade da Jana e nunca mediu: ele pontua `faithfulness(pergunta, gabarito, gabarito)`, que dá `1.0` por construção (51/51 medido no CT 100, [US-COPI-143](memory/requisitos/Jana/SPEC.md#US-COPI-143)). O que ele vigia de verdade é o **juiz**: se cair abaixo de 1.0, quem quebrou foi o avaliador LLM (modelo depreciado, prompt regredido, API mudada) — o que desambigua uma queda do `ragas-real-eval` entre "a Jana piorou" e "o juiz quebrou".
