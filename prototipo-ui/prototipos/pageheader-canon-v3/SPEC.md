...
| **AP18** | **`rows.filter()` no frontend pra calcular counter de tab** | `rows` traz só tipo ativo (server-side filtered via `?type=X`). Filtrar por outros tipos retorna 0 — counter mostra `Todos 31 · Clientes 0 · Fornec. 0 · Equipe 0 · Repr. 0` ridículo. Tentar reconstruir verdade de uma amostra parcial. | Backend devolve `tab_counts` via `Inertia::defer` (todos counts em 1 query COUNT GROUP BY). Frontend só renderiza. NUNCA `filter()` no front sobre lista server-side filtered. Test plan: trocar `?type=X` → counter deve ficar igual. |
| **AP19** | **Sidebar com popup-menu dropdown sub-items** (`Menu::dropdown('Label', $sub => $sub->url(...)->url(...))`) | Duplica navegação — popup do sidebar mostra `Fornecedores · Clientes`, header da tela JÁ tem essas tabs. Viola "uma única forma de acessar uma página" (Constituição UI v2 ADR UI-0013). Padrão v2 banido por ADR 0180 (2026-05-21). | Item sidebar é SINGLE-LINK (`'href' => '/destino'`). Sub-views vão pra tabs no PageHeader Zona C via contrato `items()` v2 ADR 0180. Power-user usa Cmd+K. "Sidebar é mapa de DESTINOS, não AÇÕES." |
| **AP20** | **Primary button com hue per grupo do módulo** (`oklch(0.55 0.15 ${hueDoGrupo})`) | Pattern v3 pré-ADR-0190 fazia primary herdar cor do grupo (Financeiro=verde, Pessoas=limão, Cadastro=ciano). Inconsistência funcional: usuário aprende 11 cores de "ação principal" em vez de 1. | Primary INTERNO das telas é SEMPRE roxo médio universal `oklch(0.55 0.15 295)`. Hue per grupo aplica APENAS no sidebar (agrupamento visual decorativo). Pattern Linear/Notion/Vercel/Stripe: sidebar varia, primary CTA = única cor. [ADR 0190](../../../memory/decisions/0190-primary-button-roxo-universal-295.md). |
