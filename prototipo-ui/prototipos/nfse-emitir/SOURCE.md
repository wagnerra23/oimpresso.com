...
⚠️ Mesma regra do [`nfe-tributacao`](../nfe-tributacao/SOURCE.md) §1: **`Nfse/Emitir.tsx` já existe em produção** (401 linhas, `US-NFSE-009`, permissão `nfse.emit`) e declara `related_prototype: n/a (herda PT-02 Formulário)`, que o `ancora.mjs` reconhece como declaração legítima.
...
- Tokens **medidos no DS**, não copiados de outro protótipo — ver o achado em [`nfe-tributacao/SOURCE.md` §5](../nfe-tributacao/SOURCE.md) (8 dos 14 do `prototipos/perfil/` não existem e resolvem vazio).
