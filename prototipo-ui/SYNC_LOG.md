...
2026-06-03 [CC] export design "PROMPT_PARA_CODE_LIGAR-FISCAL-REAL-MARTINHO.md" (prototipo-ui-patch) — cutover fiscal REAL controlado biz=164: Passo 0 (per-business?) + prep código + emissão teste homolog autônomo [CL]; credenciais+flip prod+checkpoint SEFAZ = [W]
2026-06-03 [CL] Passo 0 entregue — NF-e 55 já per-business (business.ambiente); **NFS-e era GLOBAL** (env NFSE_AMBIENTE no bind do SnNfseAdapter) → flip vazaria pra ROTA LIVRE. Fix per-business implementado (montarPayload + adapter + 4 Pest isolamento). Detalhe+checklist [W] em CODE_NOTES.md
...
2026-06-03 [CL] **tarefa de design FECHADA** (lado Code) — NFS-e resolve ambiente per-business, cutover biz=164 destravado SEM tocar outros tenants. ⚠️ ZERO fiscal real: continua 100% homologação até [W] subir cert + flipar business.ambiente/nfse_provider_configs.ambiente da biz=164 + confirmar 1ª nota real no portal SEFAZ
2026-06-03 14:16 [CD] handoff bundle "oimpresso-erp-comunicacao-visual" (claude.ai/design, 40 chats) → Norte - Fluxo do Caminhao.html (North Star: 7 cenas caminhao ponta-a-ponta, costura entre modulos como heroi). Origem chat40 (dark theme + view Fila + DS-tempero stage-* + CRM Ficha Frota)
2026-06-03 14:16 [CL] implementou _Showcase/Norte.tsx (port verbatim dos 3 arquivos React-UMD+Babel) + rota /showcase/norte superadmin · commit 98cd2711f · tokens dark escopados .nx-root (DS-GUARD, stage-* com TODO) · deploy staging CT100 (deploy.sh + route:clear + restart) · VERIFICADO autenticado https://staging.oimpresso.com/showcase/norte cenas 1+6 · NAO e canon DS, e peca de visao showcase
