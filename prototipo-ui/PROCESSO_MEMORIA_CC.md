...
| 2026-08-08 | **0%** (0 erros na sessão — manutenção do próprio método, sem build visual) | **0** (quem pegou foi a máquina, não [W]: o IT5 vermelhou sozinho) | sem defesa nova (IT5 disparou sozinho; nenhuma classe nova mecanizada) | 8 (cadeia medida ponta-a-ponta, antes→depois nos DOIS instrumentos; cobertura estreita — 1 linha de doc, zero defesa nova) | IT5 vermelho por **TEMPO**, não por diff (31d > 30d). Cadeia medida: `IT5 stale` → `design-memory-gate.test.mjs` **T7 FAIL** → job `governance script tests` vermelho em **todo** PR (o workflow dispara em `pull_request: branches:[main]` **sem** filtro `paths:`) — detectado no #5422, que não toca `prototipo-ui/` (medido: 0 arquivos sob `prototipo-ui/` no diff dele). Medido 2026-08-08: o job **não** consta em `governance/required-checks-baseline.json` → não bloqueia merge, mas treina o time a ignorar vermelho. Desde que #4018 ensinou staleness ao IT5 (2026-07-09), **2 de 2** episódios foram temporais e curados por append (#4023 → este). **Resíduo declarado, NÃO consertado aqui:** o IT5 prova que existe uma linha datada escrita à mão — não que a medição aconteceu (família §5 2026-07-01 `last_validated` / 2026-07-09 `verificado_em`); e §11 pede log **por sessão**, mas o registro tem **3 linhas em 67 dias**. |
