...
3. **Piso intocável:** posso reescrever o processo, **nunca abaixo do piso** — espinha always-read (STATUS→PROCESSO) + soberania de [W] (constituição/ADR/token = só [W]).
   > **🔁 Emenda Onda A (proposta #2874 · [W] aprovou 2026-06-16 · ADR a numerar por [W]):** a **autoridade** da espinha migra pro **git (SSOT — ADR 0238/0239)**; `STATUS.md`/`MEMORY_INDEX.md` continuam **always-read** porém **cache derivado, não-autoritativo** (divergiu do git → o git vence). **O limite (anti-regressão TESTE-02):** (a) a metade **soberania-[W]** deste piso **NÃO muda** — constituição/ADR/token seguem só-[W]; (b) "always-read" **não** vira "pode parar de ler" — continua obrigatório, só perde o status de *autoridade*. Reversível: `git revert` + restaurar header do `STATUS.md`. Carve-outs: `memory-health` (segredo+colisão-ADR) fica ligado; nenhuma "Regra 6" removida.
