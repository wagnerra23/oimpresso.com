...
| 2026-08-08 | **0%** (0/1 — meu único erro foi afirmar "15 de 15 runs vermelhas" a partir de `gh run list --limit 15`, amostra truncada; corrigido ampliando pra 100 **antes** de virar registro. Não repete L-NN; é a classe LC-08 do canon pai) | **0 de detecção · 1 de escalação** — as máquinas acusaram sozinhas e certas; o que [W] teve que trazer foi o **vermelho que ninguém lia** | **0 defesa nova** (o conserto foi **subtração**, §5). A classe "advisory vermelho sem ouvinte" segue **sem** defesa — par candidato medido, **não** armado | **6.5** (cobertura ~9 × execução ~5) — **dispara §12** | auditoria §15/§12 (TESTE-07). IT5 stale **31d**; IT8 vermelho **29d / 75 runs, 0 verdes** desde 2026-07-10 (última verde 07-03) e é **true-positive**: 3 órfãos, `DS-DOMINIO-RETIRAR-DSV6` sem retorno em `CODE_NOTES` = tarefa invisível. §15 corrigida (4 de 8 linhas + Veredito discordavam do `integrity-check.mjs`). Descartado congelar o baseline do IT8 (§5). §12 passo 3 (subir tier) = **[W]**. |
