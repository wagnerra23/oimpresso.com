...
// ──────────────────────────────────────────────────────────────────────────
const FinHero = ({ tela, setTela, onCmdK, onNew }) => (
  <>
    <div className="os-page-h">
      <div className="os-page-h-l">
        <h1>Financeiro</h1>
        <p>
          Maio 2026 · ROTA LIVRE
      ...
      <div className="os-page-h-r">
        ...
        <button className="os-btn ghost"><I.Refresh size={12}/> Conciliar</button>
    ...
    {/* Sub-nav das 5 telas internas (Visão unificada / Fluxo / Conciliação / DRE / Plano) */}
...
const FilterBar = ({ tab, setTab, counts, query, setQuery, period, setPeriod }) => (
  <div className="os-toolbar fin-toolbar">
    <div className="os-tabs">
      {TABS.map((t) => (
        <button key={t.id}
          className={"os-tab" + (tab === t.id ? " active" : "") + (t.id === "late" ? " warn" : "")}
          onClick={() => setTab(t.id)}>
          {t.label}
          {counts[t.id] != null && <span className="os-tab-n">{counts[t.id]}</span>}
        </button>
      ))}
    </div>
    <FilterPill icon={I.Calendar} label="Período" value={period}/>
    <FilterPill icon={I.Tag} label="Categoria" value="Todas"/>
    <FilterPill icon={I.Building} label="Contraparte" value="Todas"/>
    <FilterPill icon={I.Bank} label="Conta" value="Itaú PJ · 4521"/>
