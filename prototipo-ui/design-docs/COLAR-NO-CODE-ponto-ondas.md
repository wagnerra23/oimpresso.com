...
> ## ⚠️ ERRATA [CL] — corrigida em 2026-09-05 (a 1ª versão, de 04/09, tinha 3 erros próprios)
>
> **Este bloco é VOLÁTIL por construção.** Ele vive numa cópia pousada do espelho, e
> `aplicar-payload.mjs` sobrescreve o arquivo inteiro (`writeFileSync`, sem merge) no próximo
> export — o applier avisa (`⚠️ PERDE N LINHAS`) mas escreve mesmo assim. **O durável é
> [`PONTE_CL_ponto-recibo-rede-2026-09-04.md`](PONTE_CL_ponto-recibo-rede-2026-09-04.md)**, que não
> tem gêmeo no vivo. Se este bloco sumir, a correção continua lá.
>
> **A regra que a 1ª versão desta errata violou, e que vale para o PLACAR inteiro:** número que
> outro sistema sabe melhor **não se restateia** — cita-se o comando. Os quatro números do PLACAR
> abaixo (⛓, E2E, a11y, VRT) mudaram em **menos de 24 h**. Portanto:
>
> | pergunta | comando que responde | NÃO leia do PLACAR |
> |---|---|---|
> | quantos UC ⛓ o Ponto tem? | `npm run casos:report` | o `18` do PLACAR e o `10` da 1ª errata **estão os dois velhos** — em 2026-09-05 são **0**: a frente 2 foi executada (#6785, #6794, #6802, #6823, #6844) |
> | quantas telas do Ponto têm VRT/a11y? | `node -e "…visreg-screens.json…"` (filtro `screen` começando em `Ponto`) | o `0` do PLACAR e o `4` da 1ª errata — em 2026-09-05 são **11 de 21 telas** (#6793, que ao abrir as 7 novas achou **6 violações CRITICAL**) |
> | quantas rotas o smoke E2E abre? | `grep -n -i ponto tests/Browser/CoreScreens/AuthBridgeSmokeTest.php` | o `1 de 4` da 1ª errata: o denominador `4` era do registry VRT, não das telas. São **21 telas** — logo **1 de 21** |
>
> **E o erro conceitual, que é o pior dos três:** a 1ª errata disse que o UC ⛓ *"tem teste que roda
> e passa"*. **Falso.** O predicado é `!tituloCorpus.includes(uc) && testCorpus.includes(uc)`
> ([`casos-coverage-guard.mjs:259`](../../scripts/casos-coverage-guard.mjs)), e o comentário ao lado
> diz literal *"citado em ALGUM teste"* — `testCorpus` é o **conteúdo inteiro** do arquivo, então o
> id pode estar só num comentário. ⛓ **não prova execução**; prova citação. O gloss inflou para
> baixo o custo da frente 2 — a direção que a despriorizaria. Segue verdadeiro o resto: não é
> violação, não entra no baseline, não muda exit code (`process.exit(0)` incondicional).
>
> **Duas omissões materiais da 1ª errata:** a lane `visual-regression` foi **demovida de required a
> advisory** em 2026-08-26 por decisão [W] — ela roda e mostra vermelho, **não bloqueia merge**; e o
> axe roda em `level: 0`, que é **só critical**, não "a11y coberta".
>
> **O que segue intacto, e nunca esteve em questão:** as **6 decisões de [W]** do bloco RESÍDUO e as
> frentes 5–7 (Fechamento · Conformidade · REP-P). Nada aqui as toca.
...
|---|---|---|---|---|
| **1** | Rede mínima (VRT + E2E de fumaça) | **0 E2E · 0 a11y · 0 VRT** — único módulo grande zerado nas quatro ⚠️ **ERRATA [CL]: não é 0 — mas o número muda toda semana; rode o comando do bloco ERRATA (em 05/09: VRT/a11y **11 de 21**, E2E **1 de 21**)** | `Pages/Ponto/{Espelho/Index,Espelho/Show,Dashboard/Index}.tsx` | 🟢 nenhuma |
| **2** | Dívida de comportamento: **18 UC ⛓** (citados só em docblock) ⚠️ **ERRATA [CL]: em 05/09 são **0** (frente executada). E ⛓ prova CITAÇÃO, não execução — ver bloco ERRATA** | 3º maior do repo | `Modules/Ponto/Tests/Feature/**` (16 arquivos) | 🟢 nenhuma |
...
UC ⛓ (só em docblock) ............ 18        → 0 é a meta      ⚠️ ERRATA: 0 em 05/09 — rode casos:report
Contratos ........................ 2 / 4     (painel, espelho)
E2E · a11y · VRT ................. 0 · 0 · 0 ← o bloqueio real do fechamento
                                             ⚠️ ERRATA: 1 · 11 · 11 de 21 telas em 05/09 (lane advisory)
...
|---|---:|---:|---:|---|
| 1 · Rede mínima (1 spec E2E + 1 baseline/config VRT) ⚠️ **ERRATA [CL]: 0 novos + 1 editado — estender `AuthBridgeSmokeTest.php`; VRT/a11y já cobrem 11 de 21** | 2 | 0 | **2** | 🟢 |
| 2 · Desamarrar 18 UC ⛓ (2 lotes) ⚠️ **ERRATA [CL]: JÁ FEITA em 05/09 (0 UC ⛓) — não reabrir** | 0 | 6 | **6** | 🟢 |
...
| **total** | **34** | **11** | **45** ⚠️ **ERRATA [CL]: o total caduca a cada onda entregue — recontar antes de usar; os 27 travados é que não mudam** | **27 travados** |
...
| 2 | 0 UC ⛓ | `casos:report` | **18 ⛓** | **0** em 05/09 ✅ — e ⛓ prova citação, não execução |
| 3 | Baseline visual | VRT das telas âncora commitado | **0** | **11 de 21** (lane advisory desde 26/08) |
| 4 | E2E de fumaça | as telas abrem com `ponto.access` | **0** | **1 de 21** telas |
| 5 | a11y | nenhum sinal só-por-cor; nó só-ícone com label | score **74** | axe **level 0 = só critical**, 11 de 21 |
...
| 3 | **Rede zerada é o bloqueio real:** sem VRT/E2E, "não mudei layout" é opinião — e este é o módulo com obrigação legal (Portaria 671/2021) ⚠️ **ERRATA [CL]: a rede não está zerada (VRT/a11y 11 de 21 em 05/09), mas a lane é advisory desde 26/08 — não bloqueia merge. O bloqueio real são as 6 decisões da linha 1** | verificação ausente | frente 1 (destravada) |
