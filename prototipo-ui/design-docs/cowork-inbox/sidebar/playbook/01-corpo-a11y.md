...
3. **Aposentar `SidebarTabs` (l.77) · `SidebarChat` (l.95) · `ConvRow` (l.150)** — **RESÍDUO-2 respondido em 2026-09-10: REMOVER.** Medição que fechou a decisão: os três têm **zero call sites** no projeto (`ConvRow` só é usado dentro de `SidebarChat`, que ninguém renderiza — ilha fechada); o vivo os removeu em 2026-05-05 (UI-0011 single-pane, conv switcher em `Pages/Copiloto/Chat.tsx`).
   - Remover junto o **CSS órfão** de `styles.css`: `.sb-tabs` (l.277) · `.sb-conv` / `.sb-conv:hover` / `.sb-conv.active` / `.sb-conv-t` (l.389-402) · `.sb-bullet` se não sobrar consumidor (**conferir antes** — `sb-bullet` pode ser usado por outro componente).
   - **A 01 NÃO toca `app.jsx`** (Lei 1 — é prefixo da 02). As props `tab`/`onTab`/`activeConvId`/`onSelectConv` continuam chegando ao `Sidebar` sem uso; deixe a assinatura recebendo-as com um comentário `// morto — cortado na thread 02` e registre no `_saida`. Quem corta o estado na origem é a 02.
   - Achado da mesma medição, **fora deste prefixo**: `ChatPage` (`app.jsx:72`) também é morta — a rota `chat` renderiza `window.JanaPage` — e arrasta `ConvTabsBar` + `Thread` + `LinkedAppsPanel`. Não é da 01: anotar no `_saida` e deixar pra 02.
...
PARAR SE          : (a) trocar div→button mudar o layout medido → parar e reportar (é mudança de alvo, precisa [W])
                    (b) remover o CSS quebrar outro consumidor (`.sb-bullet`) → manter a regra e dizer no _saida
```
