...
>
> **Living mirror, not a frozen snapshot — e espelho NÃO é fonte.** This claude.ai/design project is a **derived mirror / vitrine** of the git DS. **git é SSOT** (ADR 0239) e `claude.ai/design` é classificado **NÃO-fonte** (ADR 0315 Eixo A + ADR 0299): a direção livre é **push git→design** (`node scripts/design-sync/ds-push.mjs`, com gate VALOR:0 e sentinela `ds-mirror-drift`); a direção **design→git é opt-in explícito do Wagner**, com triagem, nunca automática. O que sai daqui entra no repo como **proposta auditada** (ver [`HANDOFF.md`](HANDOFF.md)), não como canon. The "DS v6 frozen snapshot" (`prototipo-ui/cowork/Wagner/ds-v6`, jun/2026) is kept only as a historical reference. See the transition proposal `memory/decisions/proposals/2026-07-09-ds-transicao-congelado-para-vivo-git-ssot.md` (+ ADR 0315).
...
- **GitHub:** [`wagnerra23/oimpresso.com`](https://github.com/wagnerra23/oimpresso.com) @ `main` — última leitura **2026-08-24** (árvore `29a59c1ce1d3`; git is SSOT). Ver [`github.md`](github.md).
- ⚠️ **Errata de paths (2026-08-24, verificada no `main`):** o shell é `resources/js/Layouts/AppShellV2.tsx` (não `AppShell.tsx`); **não existe** `Components/shared/ponto/`; `ModuleTopNav.tsx` foi substituído por `shared/PageHeaderTabs.tsx` · `shared/SubNav.tsx` · `shared/PageHeaderModuleNav.tsx`; `Components/shared/` tem 16 arquivos e `Components/ui/` 32. Confira a árvore antes de citar um path — a lista abaixo é histórica.
- Key files read:
