...
- **Extrair SEMPRE fora do repo** (ex: `~/Downloads/_handoff-<slug>-staging/`). O bundle traz `project/CLAUDE.md` + `project/CONSTITUICAO.md` + `project/memory/decisions/*.md` + charters próprios — extrair dentro do repo **sobrescreve o canon** (confirmado no teste). Só espelhe a fonte visual da tela específica em `prototipo-ui/prototipos/<tela>/` na hora de aplicar.
...
- **Sem manifesto/versão entre ZIPs:** havia 5 handoffs de nome quase idêntico (`1poop`/`userperfil`/`ahaoa`/`user`/sem-sufixo) sem diff entre eles. Registre **origem + data + nº de entries** do bundle escolhido no GAP-SPEC/SYNC_LOG; pra bundle recém-baixado o ponto de referência da Fase 0 **não é `git log`** (nunca entrou no git) — é o **diff patch × tela viva**.
...
```powershell
$zip = "C:\caminho\handoff.zip"; $staging = "C:\Users\<u>\Downloads\_handoff-<slug>-staging"
if (Test-Path $staging) { Remove-Item $staging -Recurse -Force }   # FORA do repo
