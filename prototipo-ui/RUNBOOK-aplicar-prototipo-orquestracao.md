...
- **Regras duras (violação = inventário rejeitado):**
  ...
  6. Prod = o que está NO AR: confirmar o SHA de produção (SSH) == `origin/main` antes de tratar a fonte como espelho da prod — **e, pra assets, o CHUNK do manifest** (git pull chega antes do build Vite publicar: janela em que o código está lá mas o bundle servido é o velho; caso real 2026-07-07, print do botão "quebrado" tirado do chunk antigo minutos antes do novo publicar).
  7. **Geometria emergente — propriedade pontual não pega quebra de layout** ([W] 2026-07-07, botão "Novo título" em 3 linhas): medir bg/padding/radius certos NÃO prova que o componente não quebrou — a quebra nasce da interação conteúdo×container. Pra todo componente interativo (botão/chip/input/pill), a sonda mede TAMBÉM: bounding-box (w×h), nº de linhas renderizadas do texto (`getClientRects().length` do text node) e overflow (`scrollWidth > clientWidth`). `linhas > 1` onde o protótipo tem 1 = **DIVERGE**, mesmo com todas as cores idênticas.

  **MECANISMO ⚙️ das regras 4/5/7 (não sonda-na-mão): `node prototipo-ui/style-fingerprint.mjs`** ([W] 2026-07-07 *"tem método pra isso?"* → sim, este). Sonda por amostragem = viés do que o agente lembra de medir; o fingerprint extrai o VETOR COMPLETO (bbox, nº de linhas, overflow, fonte, peso, cores computadas, bg efetivo, radius, borda) de TODO elemento visível-com-texto/interativo, por tema, e o comparador diffa exaustivamente: `--snippet` (roda em qualquer página renderizada → JSON) · `--compare proto.json prod.json` (vereditos IDENTICO/DIVERGE/SO_PROTO/SO_PROD, exit 1 se DIVERGE) · `--selftest` (fixtures com divergências plantadas — 2-linhas, cor, radius, só-de-um-lado — comparador provado pelos dois lados, L-31). Limite v1 declarado no header: matching por texto+tag (elementos sem texto ficam fora — contados, não silenciados). Lacuna nova de dimensão → vira CAMPO novo do vetor, não lembrança.
...
- **Saída por tela:** (a) **GAP-SPEC** em `memory/requisitos/<Mod>/<tela>-gap.md` (template abaixo); (b) **anchor-map por região** `<tela>.map.json`: por PARTE, o bloco do protótipo (+range de linha) ↔ arquivo/range da tela viva, carregando o **sha do protótipo** que o gerou. O map.json faz a sessão de aplicação ler só os trechos (economia real) e permite **invalidar** o gap quando o protótipo re-exporta (Fase 4 aborta se o sha mudou → regenera).
  - **MECANISMO ⚙️ (não JSON escrito à mão) — novo 2026-07-09:** `node prototipo-ui/gerar-map.mjs <tela>-gap.md` deriva o ESQUELETO do `<tela>.map.json` a partir da tabela de PARTES do gap.md (reusa `parsePartes` de `gerar-contrato.mjs` — mesma fonte que o contrato região-a-região, 1 fato = 1 lugar) + resolve os arquivos do protótipo/tela-viva das frontmatter `prototipo:`/`tela_viva:` + calcula `prototipo_sha` (git log do arquivo-fonte). **Nunca fabrica linha:** `prototipo.linhas`/`vivo.linhas` nascem `"TODO"` — o agente da Fase 1 preenche com `grep -n` real (regra dura acima, "Âncora dupla obrigatória"). Cobre **todas** as partes, não só as acionáveis (o 4º veredito "vivo à frente" também precisa de âncora dupla pra provar a alegação). Verificação (âncora existe? sha ficou stale? % de telas mapeadas?) é do gate irmão `node scripts/governance/design-code-map-check.mjs --check --strict` (advisory) — mesma separação gerar/verificar do W1 região-a-região.

#### Deconflito dos 3 eixos de mapa design↔código (2026-07-09 — NÃO confundir; cada mapa = 1 eixo, 1 fato = 1 lugar)
| Mapa | Eixo | Mapeia | Verifica |
|---|---|---|---|
| [`component-registry.json`](component-registry.json) | **componente** (reusável, N telas) | bloco `.cw-*`/`.os-*` → componente React real (`variant_map`, exports verificados). **Este é o "Code Connect" do projeto** — mesma natureza do `*.figma.tsx`: âncora ESTÁVEL no componente, sobrevive a refactor | `component-registry-check.mjs` |
| [`cowork-map.json`](cowork-map.json) | **arquivo** (ingestão) | glob do bundle → destino em `cowork/` (roteamento físico, não semântica) | `cowork-ssot-guard.mjs` |
| `memory/requisitos/**/<tela>.map.json` | **região de UMA tela** (zero reuso entre telas) | parte do gap.md → range protótipo ↔ range vivo + `prototipo_sha` (parente do `anchor-lint`, não do Code Connect) | `design-code-map-check.mjs` |

Os ids convergem por construção: `gerar-map.mjs` e `gerar-contrato.mjs` derivam ambos `id = slug(parte)` do MESMO gap.md — a parte do map.json e a seção `data-contract="<id>"` do contrato-de-tela nomeiam a mesma região. **Âncora estável do lado VIVO (implementado 2026-07-09, PR-B do deconflito):** range de linha é INFORMATIVO (frágil — refactor desloca linhas em silêncio; staleness por sha só cobre o lado do protótipo); a âncora verificável é `vivo.ancora: true` + `data-contract="<id>"` no `.tsx` — declarada e ausente = **DRIFT** (o checker pega o refactor que removeu a âncora); não-declarada = "linha-only", contada no resumo, nunca punida (backfill não vira castigo); âncora presente-mas-não-declarada = nudge WARN (declare e trave de graça).
...
- **`node scripts/governance/design-code-map-check.mjs --check --strict`** (novo 2026-07-09) → âncoras do `<tela>.map.json` batem com o disco + `prototipo_sha` não ficou STALE (protótipo re-exportou sem regenerar) + imprime a % de telas com mapping versionado (métrica gradeável do roadmap #1). Advisory (ADR 0271/0275); rodado em CI por `design-memory-gate.yml`.
...
- **Atrás** no que é commodity comprável: ponte design↔código (~30% na avaliação de 2026-06-22 — sem Figma Code Connect) e tokens (~35% — `oklch→Tailwind` na cabeça do agente, sem DTCG/Style Dictionary). Detalhe: [memory/sessions/2026-06-22-arte-design-to-code-sdd.md](../memory/sessions/2026-06-22-arte-design-to-code-sdd.md).
- **#1 saiu de PROSA pra MECANISMO (2026-07-09):** a ponte design↔código deixou de ser "re-derivada por tela em prosa" — `prototipo-ui/gerar-map.mjs` deriva o `<tela>.map.json` do gap.md (âncoras nunca fabricadas, `prototipo_sha` calculado) e `scripts/governance/design-code-map-check.mjs` verifica âncora-existe + STALE-por-sha + **a % de telas mapeadas** (a métrica gradeável que faltava — "contável por arquivo"). **Honesto sobre o número:** o mecanismo existe, o BACKFILL não — rodando o checker em 2026-07-09 contra o repo real: **1/15 telas com gap.md têm `.map.json` (7%)**, e o único map.json existente (`Financeiro/unificado.map.json`, escrito à mão em 2026-07-01) já estava **STALE** no dia em que o checker nasceu (o `financeiro-page.jsx` mudou depois e ninguém regenerou — a prova viva de por que o gate precisa existir). Adoção é a próxima Onda: rodar `gerar-map.mjs` nos 14 gap.md restantes + promover `design-code-map-check` de advisory a required quando a cobertura amadurecer (mesmo caminho do `contrato-de-tela`).
...
- **Roadmap de adoção (impacto×esforço):** #1 `<tela>.map.json` **(mecanismo implementado 2026-07-09 — gerar-map.mjs + design-code-map-check.mjs; backfill das 14 telas restantes pendente)** → #2 tokens DTCG/Style Dictionary → #3 Storybook + VRT como pré-filtro do gate humano → #4 tornar `contrato-de-tela` required quando maduro.
...
## Fonte única executável (NÃO caçar constante em prosa)
> **`node prototipo-ui/protocolo.config.mjs`** é o painel do protocolo: os 2 IDs de projeto Cowork (telas vs DS — só por ID, ADR 0325), o `STAGING_DIR`/`MIRROR_DIR` fixos, e o mapa fase→comando desta página. Scripts **importam** dele; o agente **lê 1 arquivo** em vez de caçar em INDEX §0.2 + ADR 0325 + runbooks. `--selftest` trava drift (IDs distintos + todo script do mapa existe) e roda no `design-memory-gate` do CI. Origem: Wagner 2026-07-09 *"o que mais precisa ficar procurando? tudo tem que estar documentado para o processo"*.

## Refs
- [`gerar-map.mjs`](gerar-map.mjs) / [`../scripts/governance/design-code-map-check.mjs`](../scripts/governance/design-code-map-check.mjs) — gera/verifica o `<tela>.map.json` (ponte design↔código persistente, Fase 1/5)
- [`protocolo.config.mjs`](protocolo.config.mjs) — **fonte única** (IDs, paths, mapa de fases · `--selftest` no CI)
