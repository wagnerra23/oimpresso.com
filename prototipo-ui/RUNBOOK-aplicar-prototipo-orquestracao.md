...
- **MECANISMO ⚙️ (não receita-na-mão) — novo 2026-06-30:** `node prototipo-ui/importar-bundle.mjs "<zip>"` faz a Fase −1 inteira como máquina: extrai num **temp**, verifica **extraídos == entries** (conta independente, não circular), **troca atômica** pro destino fixo (o staging velho só morre quando o novo está provado — fecha o *delete-before-verify*), e chama `detectar-telas` no fim. `--selftest` no CI. A receita PowerShell manual (§"Receita de import (Windows-safe)" abaixo) é a ESPEC do que a máquina faz — **use a máquina**.
