...
Os ids convergem por construção: `gerar-map.mjs` e `gerar-contrato.mjs` derivam ambos `id = slug(parte)` do MESMO gap.md — a parte do map.json e a seção `data-contract="<id>"` do contrato-de-tela nomeiam a mesma região. **Limite honesto do map.json (registrado 2026-07-09):** a âncora do lado VIVO é range de linha (frágil — refactor da tela desloca as linhas em silêncio; o checker detecta staleness só do lado do protótipo, via `prototipo_sha`). Fix planejado (PR-B): validar a âncora estável `data-contract="<id>"` no `vivo.arquivo` (os ids já batem por construção), range de linha vira informativo.
