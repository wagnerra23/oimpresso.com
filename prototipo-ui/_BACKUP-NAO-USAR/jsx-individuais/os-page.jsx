...
function OsRow({ os, selected, onSelect, onOpen, checked, onCheck }) {
  return (
    <tr className={`os-row ${selected?"selected":""} ${os.urgent?"urgent":""}`} onClick={() => onOpen(os)}>
      ...
      <td className="os-cell-stage"><OsStageBadge stage={os.stage}/></td>
    ...
    <div className="os-page">
      <div className="os-page-h">
        <div className="os-page-h-l">
          ...
          <p>Pedidos em andamento, em produção e entregues — todos os módulos.</p>
        </div>
        <div className="os-page-h-r">
          <button className="os-btn ghost"><I.search size={13}/> Filtros avançados</button>
      ...
      <div className="os-stats">
        ...
        <div className={`os-stat ${stats.atrasadas>0?"warn":""}`}>
          <small>Atrasadas</small>
          <b>{stats.atrasadas}</b>
        </div>
        <div className="os-stat">
          <small>Valor em aberto</small>
          <b className="mono">R$ {stats.valorAberto.toLocaleString('pt-BR',{minimumFractionDigits:2})}</b>
        </div>
        <div className="os-stat muted">
      ...
      <div className="os-toolbar">
        <div className="os-tabs">
          {stageFilters.map(f => (
            <button key={f.id}
              className={`os-tab ${stage===f.id?"active":""} ${f.warn?"warn":""}`}
              onClick={() => setStage(f.id)}>
              {f.label}
              {f.n != null && <span className="os-tab-n">{f.n}</span>}
            </button>
          ))}
        </div>
  ...
  return (
    <>
      ...
      <aside className="os-drawer">
        ...
        <div className="os-drawer-section">
          <h3>Etapa atual</h3>
          <div className="os-stages-flow">
            {OS_DATA.OS_STAGES.filter(s => !["cancelado"].includes(s.id)).map(s => {
              const idx = OS_DATA.OS_STAGES.findIndex(x => x.id === s.id);
              const cur = OS_DATA.OS_STAGES.findIndex(x => x.id === os.stage);
              const state = idx < cur ? "done" : idx === cur ? "current" : "todo";
              return (
                <div key={s.id} className={`os-stage-step ${state}`}>
