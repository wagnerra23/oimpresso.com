  ...
  useEffectV(() => {
    const onKey = (e) => {
      ...
      if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === "k") {
        e.preventDefault(); setPalOpen(true);
      }
      if (e.key === "Escape") {
      ...
      if (!inField && !palOpen && (e.key === "n" || e.key === "N")) {
        e.preventDefault(); setCreateOpen(true);
      }
    };
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [palOpen, viewsOpen, openId, createOpen]);

  // filtro composto: saved view → busca → status
  const view = VENDAS_SAVED_VIEWS.find(v => v.id === savedView) || VENDAS_SAVED_VIEWS[0];
  const filtered = useMemoV(() => {
    let out = VENDAS_LIST.filter(view.filter);
    if (query.trim()) {
    ...
    return out;
  }, [VENDAS_LIST, view, query, statusF]);
    ...
    <div className={`os-page vendas-page vendas-aplus`} data-vista={vista}>
      ...
      <header className="os-head">
        ...
        <div className="os-head-r">
          <div className="vd-vista" role="group" aria-label="Vista">
          ...
          <div className="vd-views" ref={viewsRef}>
            ...
            {viewsOpen && (
              <div className="vd-views-menu">
                {VENDAS_SAVED_VIEWS.map(sv => (
                  <div key={sv.id}
                       className={`vd-views-item ${savedView===sv.id ? "active" : ""}`}
                       onClick={()=>{ setSavedView(sv.id); setViewsOpen(false); }}>
                    {sv.label}
                    <span className="ct">{VENDAS_LIST.filter(sv.filter).length}</span>
                  </div>
                ))}
      ...
      <div className="os-kpis vd-kpis">
        ...
        <div className="os-kpi">
          ...
          <span className="os-kpi-sub">{arItems.length} vendas em aberto</span>
      ...
      <div className="os-tabs">
        ...
        <div className="os-tabs-r">
          <div className="os-search">
            <I.search size={12}/>
            <input type="text" placeholder="Buscar venda, cliente, chave SEFAZ..."
        ...
        <table className="os-table vendas-table vd-aplus-table">
          ...
          <tbody>
            ...
            {!loading && filtered.map(v => {
              ...
              return (
                <tr key={v.id}
                    className={"os-row" + (v.urgent ? " urgent" : "") + (sel ? " selected" : "")}
            ...
            {!loading && filtered.length === 0 && (
              <tr><td colSpan={11} className="os-empty">
                Nenhuma venda encontrada nesta vista. Use <kbd>N</kbd> pra criar ou <kbd>⌘K</kbd> pra buscar.
              </td></tr>
            )}
          </tbody>
        </table>
      ...
      <div className={`vd-pal-bd ${palOpen ? "on" : ""}`} onClick={()=>setPalOpen(false)}>
        <div className="vd-pal" onClick={e=>e.stopPropagation()}>
          ...
          <div className="vd-pal-list">
            {palGroups.length === 0 && <div className="vd-pal-empty">Nada encontrado.</div>}
            {palGroups.map((g, i) =>
              g.header
      ...
      <aside className="os-drawer wide vd-drawer-aplus" onClick={e => e.stopPropagation()}>
        ...
        <nav className="vd-drawer-tabs">
          {[
            ...
            <button key={t.k} className={`vd-drawer-tab ${tab===t.k ? "on" : ""}`} onClick={()=>setTab(t.k)}>
              {t.l}{t.ct !== null && t.ct > 0 && <span className="vd-drawer-tab-ct">{t.ct}</span>}
            </button>
          ))}
        </nav>
            ...
            <section className="vd-section">
              ...
              <div className="vd-pay-meta">
                <dl>
                  ...
                  <dt>Prazo</dt><dd>{v.payTerm || 0} dias</dd>
          ...
          {tab === "timeline" && (
            <section className="vd-section">
              ...
              <div className="vd-tline">
                <div className="vd-tline-it">
                  <small>{dateBR} {v.time}</small>
                  <b>Venda registrada por {s.name || v.seller}</b>
                </div>
                {v.fsm >= 1 && <div className="vd-tline-it">
                  <small>{dateBR} {v.time}</small>
                  <b>Pedido confirmado</b>
                </div>}
                {v.fiscal?.nfe?.status === "ok" && <div className="vd-tline-it">
                ...
                {v.fiscal?.nfse?.status === "ok" && <div className="vd-tline-it">
                ...
                {v.fsm >= 3 && <div className="vd-tline-it"><small>—</small><b>Produção concluída · entregue</b></div>}
                {v.fsm === 4 && <div className="vd-tline-it"><small>—</small><b>Pagamento confirmado</b></div>}
              </div>
            </section>
          )}
