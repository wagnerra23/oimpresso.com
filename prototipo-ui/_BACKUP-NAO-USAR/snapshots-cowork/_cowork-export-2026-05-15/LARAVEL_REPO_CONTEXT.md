...
- **Camada C (memória):** `MemoriaContrato` interface PHP com 3 drivers — **`MeilisearchDriver` DEFAULT** (self-hosted, R$ [redacted Tier 0]/mês recorrente — local rodando + Hostinger v1.10.3 instalado), `Mem0RestDriver` upgrade **condicional sprint 8+** (managed $25-300/mês — só quando trigger ativar), `NullMemoriaDriver` em dev. **Replanejamento por ADR 0036** após confirmar Meilisearch funcional na sessão 17 — economiza R$ [redacted Tier 0]-18.000/ano até comprovar tese. **`pgvector` rejeitado** — exige PostgreSQL (não temos). Ver ADRs 0033/0036.
