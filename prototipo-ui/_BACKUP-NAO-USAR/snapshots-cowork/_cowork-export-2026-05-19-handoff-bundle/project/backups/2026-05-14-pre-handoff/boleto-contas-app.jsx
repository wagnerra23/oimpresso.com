...
// Aprovado por [W] 2026-05-11 (escopo via questions_v2).
// Shape compatível com modelos reais do repo:
//   - fin_boleto_remessas (status gerado_mock|gerado|enviado|registrado|pago|vencido|cancelado)
//   - fin_contas_bancarias (complemento 1:1 de core accounts)
//   - rb_boleto_credentials (config_json com client_id/secret/cert criptografados)
// Persona Eliana [E] — densidade alta, 1280×1024, atalhos teclado.
// Tokens: emerald=pago/ativo, amber=vencendo/sandbox, rose=vencido, stone=neutro.
  ...
  return (
    <>
      ...
      <div className="p-6 grid grid-cols-4 gap-3">
        <KpiCard tone="dark" label="A receber" value={brl(totais.valor_aberto)} sub={`${totais.qtd_aberto} boletos registrados`} icon={I.receipt} />
...
function App() {
  ...
  return (
    <div className="min-h-screen flex">
      <Sidebar active={tela} onChange={setTela} />
      <main className="flex-1 flex flex-col min-w-0">
        {tela === 'boletos' && <TelaBoletos />}
        {tela === 'contas' && <TelaContas />}
        {['unificado','fluxo','conciliacao','dre','plano'].includes(tela) && (
          <>
            <Header title={({unificado:'Visão unificada',fluxo:'Fluxo de caixa',conciliacao:'Conciliação',dre:'DRE / Relatórios',plano:'Plano de contas'})[tela]} breadcrumb="já entregue em Financeiro.html" right={null} />
            <div className="flex-1 grid place-items-center text-stone-400 text-[13px]">
              tela já desenhada em <span className="fontmono mx-1 text-stone-600">Financeiro.html</span> — pulando neste protótipo
            </div>
          </>
        )}
      </main>
