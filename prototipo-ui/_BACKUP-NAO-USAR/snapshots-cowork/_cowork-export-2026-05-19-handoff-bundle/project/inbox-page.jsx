    ...
    <div className="os-page om-page" data-screen-label="01 Caixa unificada" data-testid="caixa-unif-page">
      <div className="os-page-h">
        ...
        <div className="os-page-h-r">
          ...
          <span className="om-kbd-hint" title="Atalhos">
            <span className="om-kbd">⌘K</span>
            <span className="om-kbd">?</span>
          </span>
        </div>
      </div>
      ...
      <div className="om-shell" data-mobile-view={mobileView}>
            ...
            <>
              <header className="om-thread-h">
                ...
                {!isPreview && (
                  <div className="om-thread-h-r">
                    {convQueue && window.SLAPill && <window.SLAPill conv={conv} queue={convQueue} size="lg"/>}
                    {window.SummarizeThreadDialog && (
                      <button className="om-ai-thread-btn" onClick={() => setSummarizeOpen(true)} title="Resumir conversa (IA)">
                        <span className="om-suggest-spark">✦</span>Resumir
                      </button>
                    )}
                    {window.AskInboxDialog && (
                      <button className="om-ai-thread-btn" onClick={() => { setAskInitialQuery(""); setAskOpen(true); }} title="Perguntar ao histórico deste cliente">
                        <span className="om-suggest-spark">✦</span>Perguntar
                      </button>
                    )}
                    {window.InboxPresenterMode && (
                      <button className="os-btn ghost" onClick={() => setPresenterOpen(true)} title="Modo apresentação (reunião)">Apresentar</button>
                    )}
                    {window.InboxTranscriptDialog && (
                      <button className="os-btn ghost" onClick={() => setTranscriptOpen(true)} title="Transcript PDF com header Oimpresso">Transcript</button>
                    )}
                    <button
              ...
              <div className={"om-input " + (internalMode ? "internal" : "")}>
                  ...
                  <window.SuggestReplyButton
                    ...
                    onApply={(out) => {
                      ...
                      if (out.queue_suggestion && out.queue_suggestion !== conv.queue) {
                        // sugere mas não aplica fila automaticamente — só mostra toast
                ...
                {!internalMode && window.VarMenu && (
                  <button className="om-vars-btn" onClick={() => setVarMenuOpen(true)} title="Inserir variável">
                    {"{}"}
                  </button>
                )}
