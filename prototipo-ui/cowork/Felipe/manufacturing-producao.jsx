...
// interpretar meia-noite UTC e exibir o dia anterior em fuso negativo (medido: 2026-08-01
// aparecia como 31/07/2026 em UTC-3).
...
function MfgProducaoForm({ recipes, producoes, settings, perms, editing, onSave, onCancel }) {
  const { Button, Input, Select, Textarea, Checkbox, DatePicker, Alert } = ds();
  ...
  return (
    <div className="mfg-ed">
      {/* [B-07] trilha local — a volta é por estado, não por rota */}
      ...
      <div className="mfg-ed-cols">
        <div className="mfg-ed-main">
          ...
          {falta.length > 0 && (
            <Alert tone="danger" title={"Estoque insuficiente em " + falta.length + " insumo" + (falta.length > 1 ? "s" : "")}
              action={<Button size="sm" onClick={() => window.__go && window.__go("compras")}>Abrir Compras</Button>}>
              Finalizar vai deixar saldo negativo.
            </Alert>
          )}
          <div className="mfg-form">
            <Textarea label="Observação" rows={2} value={op.obs} onChange={(e) => set({ obs: e.target.value })} placeholder="lote, equipamento, quem executou…" />
          </div>
        </div>
        ...
        <aside className="mfg-ed-side">
          ...
          <div className="mfg-form">
            <Input label="Referência" help={"prefixo " + settings.prefix + " definido em Configurações"}
              value={op.ref} onChange={(e) => set({ ref: e.target.value })} />
            <div className="mfg-row2">
              <DatePicker label="Data" value={fromISO(op.data)} onChange={(d) => set({ data: toISO(d) })} />
              <Select label="Local" value={op.local} onChange={(e) => set({ local: e.target.value })}>
          ...
          <div className="mfg-switch-host">
            <Checkbox checked={op.final} onChange={(v) => set({ final: v })} label="Finalizar — dá entrada no produto e baixa os ingredientes" />
          </div>
          {op.final && settings.permitirPreco && <Alert tone="info">Ao finalizar, o preço de custo do produto será atualizado ({fmt(c.unit)}) — comportamento ligado em Configurações.</Alert>}
        </aside>
      </div>
...
function MfgProducaoDrawer({ op, recipes, onClose, onEdit }) {
  ...
  return (
    <Drawer
      ...
      onClose={onClose}
      title={op.ref}
      subtitle={fmtDate(op.data) + " · " + op.local + " · " + (c.r ? c.r.name : "—") + " · " + num(op.qtd, 2) + " " + (c.r ? c.r.un : "") + " · por " + op.por}
      width={680}
      footer={<>
        <Button onClick={onClose}>Fechar</Button>
        <Button variant="primary" onClick={onEdit}><I.pencil size={13} /> Editar ordem</Button>
      </>}>
      <DrawerSection title="Ingredientes consumidos">
        <div className="mfg-grp">
          {c.linhas.map((l) => (
  ...
  return (
    <>
      ...
      <div className="mfg-tablewrap">
        ...
        {linhas.length === 0 && (
          <EmptyState variant="no-results" icon={<I.chart size={18} />}
            title="Sem produção no período"
            description="Ajuste as datas ou inclua os rascunhos."
            action={<Button size="sm" onClick={() => setSoFinal(false)}>Incluir rascunhos</Button>} />
        )}
  ...
  return (
    <div className="mfg-cfg">
      <div className="mfg-card">
        ...
        <div className="mfg-switch-host">
          <Switch checked={s.travarQtd} onChange={(v) => setS({ ...s, travarQtd: v })}
            label="Bloquear edição da quantidade de ingrediente"
            sublabel="quando ligado, a receita manda: nem editor nem ordem de produção permitem ajustar consumo." />
          <Switch checked={s.permitirPreco} onChange={(v) => setS({ ...s, permitirPreco: v })}
            label="Atualizar preço do produto ao finalizar produção"
