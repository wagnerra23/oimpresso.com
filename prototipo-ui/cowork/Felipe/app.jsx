...
function App() {
  ...
  if (route === "prod-consulta-v2") {
    // CONTORNO DECLARADO (importação 2026-08-28) — não é padrão deste shell.
    // A Consulta de Produtos V2 é um Design Component (`../Consulta de Produtos.dc.html`),
    // roda em runtime próprio e não pode ser montada como componente React aqui.
    // O comentário do topo deste arquivo diz "não usamos mais iframes" — este é o único,
    // e existe só para comparar V2 × cowork sem criar uma SEGUNDA fonte da mesma tela
    // (portar o DC pra jsx duplicaria a tela e as duas iriam divergir).
    // Substituir quando a V2 for traduzida pra página nativa do shell.
    content =
    <iframe
      src="Consulta%20de%20Produtos.dc.html?embed=1"
      title="Consulta de Produtos V2 (protótipo oficial)"
      style={{ display: "block", width: "100%", height: "100%", minHeight: "calc(100vh - 56px)", border: 0, background: "var(--bg)" }} />;
