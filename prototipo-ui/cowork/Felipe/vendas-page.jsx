...
function VdStepper({ fsm, vertical }) {
  ...
  return (
    <span className="vd-stp" title={`${set.steps[fsm]} — etapa ${fsm + 1} de 5`}>
      {set.steps.map((_, i) => {
        let cls = "vd-stp-dot";
        if (i < fsm) cls += " done";else
        if (i === fsm) cls += " current";
...
// Source pill — Integração Vendas × Oficina (2026-05-25)
// 3 valores: balcao · oficina (com link pra OS) · online
function VdSource({ v, onPickOs }) {
  const meta = (window.VENDAS_DATA.VENDAS_SOURCE_META || {})[v.source] || null;
  if (!meta) return null;
  const isOficina = meta.id === "oficina";
  return (
    <span className={`vd-src vd-src-${meta.id}`}
    title={isOficina && v.osRef ? `Vinda do módulo Oficina · #${v.osRef}` : `Venda ${meta.label}`}>
      <span className="vd-src-dot" />
      <span className="vd-src-lbl">{meta.label}</span>
      {isOficina && v.osRef &&
      <a className="vd-src-os"
      href="#"
      onClick={(e) => {e.preventDefault();e.stopPropagation();onPickOs?.(v.osRef);console.log("→ abrir", v.osRef);}}>
          ↗ #{v.osRef}
        </a>
      }
    </span>);
...
function VendasListPage() {
  ...
  // Integração Vendas × Oficina (2026-05-25) — listener cross-módulo
  // Drawer da OS dispara `oimpresso:open-venda` com {id} pra abrir a venda derivada
  useEffectV(() => {
    const onOpen = (e) => {
      const id = e.detail?.id;
      if (!id) return;
      // garante que estamos na sub-rota "lista" e que a venda existe
      const exists = VENDAS_LIST.find((v) => v.id === id);
      if (exists) setOpenId(id);
    };
    window.addEventListener("oimpresso:open-venda", onOpen);
    return () => window.removeEventListener("oimpresso:open-venda", onOpen);
  }, [VENDAS_LIST]);
  ...
  useEffectV(() => {
    const onKey = (e) => {
      ...
      if (e.key === "Escape") {
        if (cheatOpen) setCheatOpen(false);else
        if (palOpen) setPalOpen(false);else
        if (viewsOpen) setViewsOpen(false);else
        if (openId) setOpenId(null);else
        if (createOpen) setCreateOpen(false);else
      ...
      if (e.key === "j" || e.key === "J" || e.key === "ArrowDown") {
        ...
        e.preventDefault();window.__vendasSubSetter ? window.__vendasSubSetter("create") : setCreateOpen(true);
      } else if (e.key === "?") {
  ...
  const filtered = useMemoV(() => {
    ...
    if (subView) {
      if (topView === "pendentes") {
        ...
        if (subView === "b2b") out = out.filter((v) => v.client !== "Consumidor Final");else
        if (subView === "b2c") out = out.filter((v) => v.client === "Consumidor Final");
      } else if (topView === "origem") {
        // Integração Vendas × Oficina — filhos: balcao · oficina · online
        out = out.filter((v) => v.source === subView);
      }
    }
    if (query.trim()) {
  ...
  // Integração Vendas × Oficina (2026-05-25) — breakdown por source
  const kpi_bySource = today.reduce((acc, v) => {
    acc[v.source] = (acc[v.source] || 0) + v.totalNum;
    return acc;
  }, { balcao: 0, oficina: 0, online: 0 });
  ...
  const slaCounts = arItems.reduce((acc, v) => {
    ...
    if (k === "overdue") acc.o++;else
    if (k === "warning") acc.w++;else
    if (k === "fresh") acc.f++;
    return acc;
  }, { f: 0, w: 0, o: 0 });
  ...
  const palAll = useMemoV(() => {
    ...
    const shortcuts = [
    { grp: "Ações", ico: <I.plus size={14} />, title: "Nova venda", sub: "Página completa de cadastro", kbd: "N", action: () => {window.__vendasSubSetter ? window.__vendasSubSetter("create") : setCreateOpen(true);setPalOpen(false);} },
    ...
    <div className={`os-page vendas-page vendas-aplus`} data-vista={vista}>
      ...
      <header className="os-head vd-head-clean">
        ...
        <div className="os-head-r">
          <button className="os-btn primary" onClick={() => window.__vendasSubSetter ? window.__vendasSubSetter("create") : setCreateOpen(true)}>
            <I.plus size={11} />Nova venda <kbd className="kbd-hint">N</kbd>
          </button>
        </div>
      </header>

      {/* SUB-NAV em página — abaixo do page header ([W] 2026-06-11) */}
      {window.VdModNav && <window.VdModNav here="lista" />}
      ...
      <div className="os-kpis vd-kpis">
        ...
        <div className="os-kpi vd-kpi-hero">
          <span className="os-kpi-label">
            Faturado hoje
            {vista === "faturamento" && <small className="vd-kpi-tag"> · todas origens</small>}
          </span>
          ...
          {vista === "faturamento" && (kpi_bySource.oficina > 0 || kpi_bySource.online > 0) &&
          <div className="vd-kpi-breakdown">
              {kpi_bySource.balcao > 0 &&
            <div className="vd-kpi-b balcao">
                  <small>● Balcão</small>
                  <b>{vdFmtShort(kpi_bySource.balcao)}</b>
                </div>
            }
              {kpi_bySource.oficina > 0 &&
            <div className="vd-kpi-b oficina">
                  <small>● Oficina</small>
                  <b>{vdFmtShort(kpi_bySource.oficina)}</b>
                </div>
            }
              {kpi_bySource.online > 0 &&
            <div className="vd-kpi-b online">
                  <small>● Online</small>
                  <b>{vdFmtShort(kpi_bySource.online)}</b>
                </div>
            }
            </div>
          }
      ...
      {/* TOOLBAR — filtros, ACIMA da lista ([W] 2026-06-11 "essa linha vai ficar acima da lista") */}
      <div className="vd-toolbar vd-filterbar">
          ...
          <div className="vd-views" ref={viewsRef}>
            ...
            <div className="vd-views-menu vd-tree">
              ...
              <React.Fragment>
                ...
                const expandable = sv.id === "pendentes" || sv.id === "faturadas" || sv.id === "origem";
                ...
                if (sv.id === "pendentes") {
                  ...
                  filter((c) => c.ct > 0);
                } else if (sv.id === "origem") {
                  // Integração Vendas × Oficina — conta por v.source
                  const meta = VENDAS_SOURCE_META || {};
                  const counts = items.reduce((a, v) => (a[v.source] = (a[v.source] || 0) + 1, a), {});
                  children = ["balcao", "oficina", "online"].
                  filter((k) => counts[k]).
                  map((k) => ({ id: k, label: `● ${meta[k]?.label || k}`, ct: counts[k] }));
                }
                return (
        ...
        <div className="vd-search" role="search">
          <I.search size={13} />
          <input value={query} onChange={(e) => setQuery(e.target.value)}
            placeholder="Buscar venda, cliente ou chave…" aria-label="Buscar vendas" />
          {query && <button className="vd-search-x" onClick={() => setQuery("")} title="Limpar" aria-label="Limpar busca"><I.close size={12} /></button>}
        </div>
      </div>
        ...
        <table className="os-table vendas-table vd-aplus-table">
          <thead>
            <tr>
              ...
              <th style={{ width: 138 }} className="vd-col-source">Origem</th>
          ...
          <tbody>
            ...
            {!loading && filtered.map((v, ri) => {
              ...
              return (
                ...
                className={"os-row" + (v.urgent ? " urgent" : "") + (sel ? " selected" : "") + (isFocused ? " row-focused" : "") + (v.source === "oficina" ? " vd-row-oficina" : "")}
            ...
            <tr><td colSpan={12} className="os-empty">
      ...
      {/* TOTALIZADORES — soma do filtro atual ([W] 2026-06-12 "preciso dos totalizadores") */}
      {!loading && filtered.length > 0 && (() => {
        const tot = filtered.reduce((s, v) => s + v.totalNum, 0);
        const com = filtered.reduce((s, v) => { const ss = VENDEDORES_MAP[v.sellerId] || {}; return s + v.totalNum * (ss.commissionPct || 0); }, 0);
        const pagas = filtered.filter((v) => v.status === "paga").length;
        const pend = filtered.filter((v) => v.fsm < 4).length;
        return (
          <div className="vd-totalbar">
            <div className="vd-totalbar-l">
              <b>{filtered.length}</b> {filtered.length === 1 ? "venda" : "vendas"}
              <span className="vd-totalbar-sub">{pagas} paga{pagas === 1 ? "" : "s"} · {pend} a receber</span>
            </div>
            <div className="vd-totalbar-r">
              <div className="vd-totalbar-m">
                <span className="k">Comissão</span>
                <span className="v com">{vdFmt(com)}</span>
              </div>
              <div className="vd-totalbar-m">
                <span className="k">Total do filtro</span>
                <span className="v grand">{vdFmt(tot)}</span>
              </div>
            </div>
          </div>
        );
