...
> ## ⚠️ CORREÇÕES VERIFICADAS NO `main` (2026-06-02 · [CC], pego por [W] · ver L-26)
> Onde divergir, vence isto — conferido por `github_read_file`, não por memória:
> 1. **Repo = UltimatePOS HÍBRIDO.** Legado **Blade + AdminLTE/Bootstrap/jQuery** em `resources/views/*` (dezenas de telas POS, "mal formatadas", **não usam tokens novos**) + telas novas **Inertia/React/Tailwind/cockpit** em `resources/js/Pages/*`. Padronização + testes do legado = programa real, não "está ok".
> 2. **§10.4 abaixo está ERRADO:** tokens **NÃO** estão em `resources/css/app.css` (= manifesto Tailwind VAZIO, 118 bytes). Os tokens REAIS: `resources/css/cockpit.css` no bloco `.cockpit{ --bg/--accent/--row-h... }` (shell) + `resources/css/inertia.css` no `@theme{ --color-primary/--color-page-cream... }` (Tailwind v4/shadcn). **Ambos JÁ roxo `oklch(0.55 0.15 295)` + neutros quentes hue 90** — a camada Inertia NÃO precisa de "migração de token".
> 3. **Sempre ler o `main` real antes de citar arquivo/estrutura numa ponte** (L-26). Confiar nestas notas pra path = erro.
...
(igual à versão anterior — omitido aqui para economia de espaço; ver versão completa no commit anterior do CLAUDE.md)
...
- **Não remover o shim `App\View\Helpers\Form`** sem antes migrar as ~6.433 chamadas `Form::` em ~460 Blade views.
- **Antes de criar/mudar estrutura do módulo, verifique o padrão dos módulos existentes no servidor atualizado.**
- **Identificadores MySQL com mais de 64 chars** — sempre passar nome explícito em índices compostos em tabelas com nome longo.
- **Não suba código para produção sem alertar o usuário de pré-requisitos e riscos.**

---

## 6. O que SEMPRE fazer

- Respeite o idioma: **todo texto, commit, comentário, label → PT-BR**.
...
- Escreva testes ao menos para regras CLT.
- **Antes de criar/mudar estrutura do módulo, abra `Modules/Jana/` (ou `Repair`/`Project`) e imite.**
- **Use stack de middlewares UltimatePOS** para rotas web.

---

## 7. Cofre de comparativos & gestão de memória

(mantido igual — ver commit anterior)
...
**Cliente:** WR2 Sistemas — Eliana — eliana@wr2.com.br
**Repositório:** `wagnerra23/oimpresso.com` (GitHub) + workspace local em `D:\oimpresso.com`
...
> ⚠️ **CORRIGIDO 2026-06-02 (ver banner no topo + L-26):** o caminho `resources/css/app.css` abaixo está **ERRADO** (app.css = manifesto Tailwind vazio). Tokens reais = `cockpit.css .cockpit{}` (shell) + `inertia.css @theme{}` (Tailwind/shadcn), já roxo+warm. O texto original fica como histórico:

Use **sempre** as variáveis CSS do shell (~~definidas em `resources/css/app.css`~~ → **na verdade em `cockpit.css`/`inertia.css`**):
...
> **Última atualização deste arquivo:** 2026-04-27 (sessão de design — ADR 0039 padrão "Chat Cockpit" formalizado, §10 nova adicionada)
> **Próxima revisão sugerida:** quando portar `AppShellV2.tsx` pro repo (Fase 1 da migração)
