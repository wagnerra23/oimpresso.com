...
## 🧠 Espinha & ciclo de memória — LER 1º, REGISTRAR no fim

> Memória manda o **git**; design cuida **[CC]**; decide **[W]**. A memória **não depende de humano** — só a **decisão** depende. O ciclo fecha pela ponte pro Code.
>
> **Minha constituição = `CARTA_DESIGN_CC.md`** (subordinada ao `PROTOCOL.md` + `CLAUDE_DESIGN_BRIEFING.md` do git). NÃO é lei suprema; descreve só como [CC] obedece.
>
> **Soberania (autorizada por [W] 2026-05-31):** a constituição (ADR 0094 · UI-0013 · PROTOCOL · BRIEFING) é **só de [W]**; mudá-la = **reindexar tudo**. [CC] **propõe, nunca edita/numera/versiona/deleta**. Regra: `memory/decisions/_PROPOSTA-constituicao-soberania-W.md` (Code numera+versiona sob OK de [W]).

**Início de todo chat (eu, [CC], faço sem pedir — ler ANTES de propor):**
1. `STATUS.md` (espinha local) + `MEMORY_INDEX.md` (índice temático T1–T9).
2. Lei do git via connector: `prototipo-ui/PROTOCOL.md`, `prototipo-ui/CLAUDE_DESIGN_BRIEFING.md`, ADRs relevantes (0114/0110/0107/0104).
3. Memória do git: `memory/INDEX.md` (índice mestre) + `memory/proibicoes.md` + `memory/LICOES_CC.md` (erros a não repetir).
4. **Não reinventar o decidido nem repetir erro registrado** — apontar o ADR/L-ID.

**Fim de todo chat (eu faço sem pedir):**
1. Decisão nova → **ADR** `memory/decisions/NNNN-*.md` (Nygard · monotônico · confirmar nº livre no git).
2. **Resumo de sessão** → `memory/sessions/AAAA-MM-DD-<slug>.md` (formato canônico: Pedido · O que foi feito · Decisões · Erros+correção · Residual · Refs · Próximo passo).
3. **Erro novo** → append em `memory/LICOES_CC.md` (Erro · Sintoma · Regra · Ref).
4. Estado de tela mudou → atualizar `STATUS.md` + `MEMORY_INDEX.md`.
5. **Ponte zero-toque pro Code** (patch espelho + URLs + 1 prompt) pra [CL] commitar tudo no git. Wagner cola 1 vez = transporte.

**Arquivos da espinha (local, subordinados ao git):** `CARTA_DESIGN_CC.md` · `STATUS.md` · `MEMORY_INDEX.md` · `memory/sessions/*` · `memory/LICOES_CC.md` · `Painel Cowork - Estado Atual.html`.
...
## 🔒 ARQUIVO PRINCIPAL ÚNICO — `oimpresso.com.html`

**Todas as telas/módulos do ERP vivem DENTRO de `oimpresso.com.html`** como rotas do shell Cockpit V2. Esse é o único HTML que o Wagner abre. **Proibido criar `.html` novo** pra módulo, tela ou variação.

Padrão pra adicionar/evoluir uma tela:

1. Criar/editar `<modulo>-page.jsx` exportando `window.<Modulo>Page`
2. (opcional) `<modulo>-page.css` com classes prefixadas
3. Registrar `<script type="text/babel" src="...">` em `oimpresso.com.html`
4. Adicionar rota em `app.jsx`: registry de módulos + handler no router + array de rotas conhecidas
5. Adicionar entrada em `data.jsx` (sidebar item + GROUP_META se grupo novo)

**Renomeação semântica de módulo** (ex: `whatsapp` → `inbox` / "Caixa unificada"): mexer nos 5 pontos acima de uma vez, deletar arquivos antigos no mesmo passo. Não deixar legacy quebrado.

**Variações/explorações** (A/B de visual, layout alternativo) = Tweaks dentro do mesmo componente via `useTweaks`, NÃO arquivo novo. NÃO `<Modulo>v2.html`, NÃO `<Modulo> Alt.html`.

Os `.html` legados na raiz (`Compras.html`, `Vendas A+.html`, `Auditoria UI.html` etc.) são **histórico de exploração** — não criar novos, e quando uma tela é migrada pro Chat principal, o `.html` antigo pode ser removido (depois de confirmar com Wagner).
