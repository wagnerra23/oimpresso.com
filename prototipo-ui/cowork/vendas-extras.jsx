...
function VendasModule() {
  ...
  if (sub === "relatorios") body = <VendasRelatoriosPage />;else
  body = <VendasListPage />; // fallback: sub desconhecido (ex: legado removido) cai na lista
...
function VMIcon({ name, size = 13 }) {
  ...
  switch (name) {
    ...
    case "crm":return <svg viewBox="0 0 16 16" {...s}><circle cx="6" cy="6" r="2.3" /><path d="M2.5 13c0-2 1.6-3.2 3.5-3.2S9.5 11 9.5 13" /><path d="M10.8 4.2a2 2 0 0 1 0 3.6" /><path d="M11.4 9.9c1.5.3 2.6 1.4 2.6 3.1" /></svg>;
    case "oficina":return <svg viewBox="0 0 16 16" {...s}><path d="M10.4 2.9a2.9 2.9 0 0 0 3.8 3.8l-5.6 5.6a1.6 1.6 0 0 1-2.3-2.3l5.6-5.6-1.5-1.5Z" /><circle cx="5" cy="11" r="0.6" fill="currentColor" stroke="none" /></svg>;
    default:return null;
  }
}

// ──────────────────────────────────────────────────────────────
// SUB-NAV EM PÁGINA — telas vinculadas à venda, ABAIXO do page header
// de cada tela ([W] 2026-06-11: "page header fica abaixo do header" →
// header primeiro, tabs depois). Substitui o VdSubBreadcrumb nas 4
// sub-páginas (a tab "Vendas" faz o papel de voltar pra lista).
// ──────────────────────────────────────────────────────────────
function VdModNav({ here }) {
  const SUBS = window.__vendasSubs || [];
  const go = (id) => window.__vendasSubSetter && window.__vendasSubSetter(id);
  const goRoute = (id) => window.__selectRoute && window.__selectRoute(id);
  return (
    <nav className="vd-modnav" aria-label="Comercial — Vendas e telas vinculadas">
      {/* irmãos do grupo COMERCIAL juntos no início — [W] 2026-06-12 "Oficina deve ficar ao lado do CRM" */}
      <button className="vd-modnav-tab sib" onClick={() => goRoute("crm")} title="Ir para o CRM">
        <VMIcon name="crm" size={13} />CRM
      </button>
      <button className="vd-modnav-tab sib" onClick={() => goRoute("oficinaauto")} title="Ir para a Oficina Auto">
        <VMIcon name="oficina" size={13} />Oficina
      </button>
      <span className="vd-modnav-div" aria-hidden="true" />
      {SUBS.map((s) =>
      <button key={s.id} className={"vd-modnav-tab" + (here === s.id ? " on" : "")} onClick={() => go(s.id)}>
          <VMIcon name={s.icon} size={13} />{s.label}
        </button>
      )}
      <button className="vd-modnav-tab pdv" onClick={() => window.__vendasPdvOpen && window.__vendasPdvOpen()} title="Abrir PDV balcão (tela cheia)">
        <VMIcon name="screen" size={13} />PDV balcão
      </button>
    </nav>);

}
window.VdModNav = VdModNav;

// ──────────────────────────────────────────────────────────────
// BREADCRUMB compartilhado pras 4 sub-páginas (Refino navegação 2026-05-17)
// "← Vendas · Caixa do dia" — clicar volta pra Lista
// (desde 2026-06-11 substituído pelo VdModNav; mantido por histórico)
// ──────────────────────────────────────────────────────────────
...
function VendasCaixaPage() {
  ...
  const { VENDAS_SOURCE_META } = window.VENDAS_DATA;
  const bySource = useMemoVE(() => {
    const meta = VENDAS_SOURCE_META || {};
    const map = { balcao: { count: 0, total: 0, items: [] }, oficina: { count: 0, total: 0, items: [] }, online: { count: 0, total: 0, items: [] } };
    dayVendas.forEach((v) => {
      const k = v.source || "balcao";
      if (!map[k]) return;
      map[k].count++;
      map[k].total += _parseBRL(v.total);
      map[k].items.push(v);
    ...
    filter(([k, x]) => x.count > 0).
    map(([k, x]) => ({ id: k, label: meta[k]?.label || k, ...x }));
  }, [dayVendas, VENDAS_SOURCE_META]);
    ...
    <div className="os-page vc-page vd-subpage">
      <header className="os-head">
        ...
        <div className="os-head-r">
          ...
          <button className="os-btn ghost"><VMIcon name="cash" size={11} />Imprimir caixa (Z)</button>
      ...
      <div className="vc-grid">
        ...
        {/* Integração Vendas × Oficina (2026-05-25) — breakdown por origem */}
        <section className="vc-card vc-card-source">
          ...
          {bySource.map((g) => {
            const pct = totalDay ? Math.round(g.total / totalDay * 100) : 0;
            return (
              <div key={g.id} className={`vc-src-row vc-src-${g.id}`}>
                <div className="vc-src-h">
                  <span className="vc-src-dot" />
                  <b>{g.label}</b>
                  <span className="vc-src-ct">{g.count} venda{g.count !== 1 ? "s" : ""}</span>
                  <span className="vc-src-tot">{_fmtBRL(g.total)}</span>
                </div>
                <div className="vc-src-bar"><div style={{ width: pct + "%" }} /></div>
                <div className="vc-src-meta">
                  <small>{pct}% do faturamento do dia</small>
                  {g.id === "oficina" && g.items.some((v) => v.osRef) &&
                  <small className="vc-src-refs">
                      {g.items.filter((v) => v.osRef).slice(0, 3).map((v, i) =>
                    <React.Fragment key={v.id}>
                          {i > 0 && " · "}
                          <a href="#" onClick={(e) => {e.preventDefault();console.log("→ abrir", v.osRef);}}>↗ #{v.osRef}</a>
                        </React.Fragment>
                    )}
                      {g.items.filter((v) => v.osRef).length > 3 && ` · +${g.items.filter((v) => v.osRef).length - 3}`}
                    </small>
                  }
                </div>
              </div>);
