...
function PaymentGatewaysPage() {
  // KB-9.75: persistência localStorage
  const ls = (k, d) => { try { const v = localStorage.getItem('oimpresso.settings.gateways.' + k); return v == null ? d : JSON.parse(v); } catch (e) { return d; } };
  const setLs = (k, v) => { try { localStorage.setItem('oimpresso.settings.gateways.' + k, JSON.stringify(v)); } catch (e) {} };
  ...
  const [filtroDriver, _setFiltroDriver] = useState(() => ls('driver', 'all'));
  const setFiltroDriver = (v) => { _setFiltroDriver(v); setLs('driver', v); };

  const gatewaysFiltered = useMemo(() => GATEWAYS.filter(g => filtroDriver === 'all' || g.driver === filtroDriver), [filtroDriver]);
  ...
  React.useEffect(() => {
    const onKey = (e) => {
      ...
      if (inField) return;
      if (drawer || novoOpen || confirmToggle || cheatOpen) return;
      if (e.key === '?') { e.preventDefault(); setCheatOpen(true); }
      else if (e.key === 'n') { e.preventDefault(); setNovoOpen(true); }
      ...
      else if (e.key === 'Enter' && focusIdx >= 0) { e.preventDefault(); const g = gatewaysFiltered[focusIdx]; if (g) setDrawer(g); }
    };
  ...
  return (
    <div className="h-full bg-stone-50 flex flex-col font-sans" data-screen-label="02 Settings/Gateways">
      ...
      <Header
        ...
        right={<>
          <Btn variant="ghost" onClick={() => window.PgGotoRoute && window.PgGotoRoute('cobranca')} title="Voltar pra Cobrança">{I.arrowL}Voltar</Btn>
      ...
      <div className="px-6 pt-5 grid grid-cols-3 gap-3">
        ...
        <button onClick={() => window.PgGotoRoute && window.PgGotoRoute('cobranca')} className="text-left appearance-none p-0 m-0 bg-transparent border-0" title="Abrir Financeiro · Cobrança">
          <KpiCard label="Cobranças hoje ↗" value={kpis.cobsHoje} sub="emitidas em todos drivers · clique pra ver" icon={I.receipt} />
        </button>
      </div>
                ...
                return (
                  <tr key={g.id} className="border-b border-stone-100 hover:bg-stone-50/60 cursor-pointer" onClick={() => setDrawer(g)}>
                    ...
                    <td className="px-2 py-2.5 text-stone-700">
                      {acct ? (
                        <button className="pg-acct-link hover:underline hover:underline-offset-2 text-left inline-flex items-center gap-1" onClick={e => e.stopPropagation()}>
                    ...
                    <td className="px-2 py-2.5">
                      <Toggle on={g.ativo} onConfirm={(newVal) => setConfirmToggle({ gateway: g, newValue: newVal })} />
                    </td>
                    <td className="pl-2 pr-5 py-2.5" onClick={e => e.stopPropagation()}>
                      <div className="flex items-center justify-end gap-1">
                        <button title="Rodar health check" className="pg-rowbtn w-7 h-7 inline-flex items-center justify-center leading-none rounded-md text-stone-400 hover:text-stone-900 hover:bg-stone-100 transition">{I.refresh}</button>
                        <button title="Configurar" className="pg-rowbtn w-7 h-7 inline-flex items-center justify-center leading-none rounded-md text-stone-400 hover:text-stone-900 hover:bg-stone-100 transition">{I.settings}</button>
                        <button title="Mais ações" className="pg-rowbtn w-7 h-7 inline-flex items-center justify-center leading-none rounded-md text-stone-400 hover:text-stone-900 hover:bg-stone-100 transition">{I.more}</button>
                      </div>
                    </td>
                  </tr>
                );
      ...
      {/* SEÇÃO: drivers DISPONÍVEIS (não configurados ainda) */}
        {(() => {
          const configured = new Set(GATEWAYS.map(g => g.driver));
          const available = Object.values(DRIVERS).filter(d => !configured.has(d.key));
          if (available.length === 0) return null;
          return (
            <div className="mt-6">
              <div className="flex items-center gap-2 mb-2">
                <div className="text-[10px] uppercase tracking-widest font-medium text-stone-500">Drivers disponíveis</div>
                <div className="text-[10px] text-stone-400">({available.length} não configurado{available.length === 1 ? '' : 's'})</div>
              </div>
              <div className="grid grid-cols-5 gap-3">
                {available.map(d => (
                  <button key={d.key} onClick={() => setNovoOpen(true)} className={cn(
                    ...
                    <div className="mt-2 text-[11px] font-medium text-stone-900 inline-flex items-center gap-1">{I.plus}Configurar</div>
                  </button>
                ))}
              </div>
            </div>
      ...
      {confirmToggle && <ConfirmToggleModal data={confirmToggle} onClose={() => setConfirmToggle(null)} />}
      {cheatOpen && <CheatSheetSettings onClose={() => setCheatOpen(false)} />}
    </div>
  );
}

function CheatSheetSettings({ onClose }) {
  React.useEffect(() => {
...
// Modal de confirmação pra toggle ativo/inativo (Trust L3 fail-fast)
function ConfirmToggleModal({ data, onClose }) {
  const { gateway, newValue } = data;
  const isDisabling = !newValue;
  // Conta cobranças em aberto que dependem desse gateway
  const cobrancasAfetadas = isDisabling
    ? COBRANCAS.filter(c => c.gateway === gateway.driver && c.status === 'emitida').length
    : 0;
      ...
      <div className="relative w-[460px] bg-white rounded-lg shadow-2xl border border-stone-200" onClick={e => e.stopPropagation()}>
        <div className="p-5">
          <div className="flex items-start gap-3">
            <span className={cn("w-9 h-9 rounded-md grid place-items-center shrink-0",
              isDisabling ? "bg-rose-100 text-rose-700" : "bg-emerald-100 text-emerald-700")}>
              {isDisabling ? I.alert : I.check}
            </span>
            <div className="flex-1">
              ...
              <p className="text-[12px] text-stone-600 mt-1">
                <strong>{gateway.nome}</strong> · driver {DRIVERS[gateway.driver]?.nome}
              </p>
            </div>
          </div>

          {isDisabling && cobrancasAfetadas > 0 && (
            <div className="mt-3 bg-rose-50 border border-rose-200 rounded p-3 text-[11.5px] text-rose-900">
              <strong>{cobrancasAfetadas} cobrança(s) em aberto</strong> dependem deste gateway.
          ...
          <div className="mt-4 text-[10.5px] text-stone-500">
            Esta ação é auditada (Trust L3) e fica registrada no log da credencial.
          </div>
        </div>
        <div className="px-5 py-3 border-t border-stone-200 bg-stone-50 flex gap-2 justify-end">
          <Btn variant="outline" onClick={onClose}>Cancelar</Btn>
          <Btn variant={isDisabling ? "primary" : "primary"} onClick={onClose} className={isDisabling ? "!bg-rose-600 hover:!bg-rose-700" : ""}>
            {I.check}{isDisabling ? 'Desativar' : 'Ativar'}
          </Btn>
        </div>
      </div>
    </div>
...
function Toggle({ on, onConfirm }) {
  const [confirming, setConfirming] = React.useState(false);
  const handle = (e) => {
    e.stopPropagation();
    if (onConfirm) {
      setConfirming(true);
      // confirm é acionado pelo modal externo — mantém visual consistente
      onConfirm(!on);
      setTimeout(() => setConfirming(false), 100);
    }
  };
  return (
    <button onClick={handle} title={on ? 'Desativar (exige confirmação)' : 'Ativar'}
  ...
  return (
    <Field label={label}>
      ...
      <div
        ...
        onDrop={onDrop}
        className={cn(
          "min-h-[32px] py-1.5 px-2 rounded flex items-center gap-2 text-[11.5px] cursor-pointer transition border border-dashed",
          file
      ...
      >
        {file ? (
          <>
            <span className="text-emerald-700">{I.check}</span>
            <div className="flex-1 min-w-0">
              <div className="font-mono text-[11px] text-stone-900 truncate">{file.name}</div>
              <div className="text-[10px] text-stone-500 tabular-nums">{fmtSize(file.size)} · pronto pra envio</div>
            </div>
            <button
              type="button"
              onClick={(e) => { e.stopPropagation(); setFile(null); if (inputRef.current) inputRef.current.value = ''; }}
              className="text-stone-500 hover:text-rose-600 w-5 h-5 inline-grid place-items-center rounded"
          ...
          <>
            ...
            <span className="flex-1">
              {dragOver ? 'Solte o arquivo aqui' : `Arraste ${accept ? accept : 'arquivo'} ou clique pra selecionar`}
            </span>
          </>
        )}
      </div>
