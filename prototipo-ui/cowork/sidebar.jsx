...
function SidebarMenu({ activeRoute, onSelectRoute }) {
  ...
  const [expanded, setExpanded] = useState(() => {
    ...
    const init = {};
    // Canon repo: grupos abertos por default; MAIS fechado (fora do caminho).
    MOCK.MENU.forEach((e) => {if (e.group) init[e.group] = e.group !== "MAIS";});
        ...
        return (
          <div key={entry.group} className={"sb-group" + (isOpen ? " open" : "")} style={{ ["--gh"]: hue }}>
            ...
            {isOpen && entry.items.map((item) => {
              ...
              role="button" tabIndex={0}
              onKeyDown={(e) => {if (e.key === "Enter" || e.key === " ") {e.preventDefault();onSelectRoute(item.id);}}}
              onClick={() => onSelectRoute(item.id)}
              style={isActive ? { borderLeftColor: groupDot } : null}>
                  <Icon className="ic" />
                  <span className="label">{item.label}</span>
                  {item.badge ? <span className="badge">{item.badge}</span> : null}
                  {ghosts.length ? <span className="sb-ghost-count" style={{ marginLeft: "auto", fontSize: "10px", color: "var(--text-mute)", opacity: 0.6 }}>{ghosts.length}</span> : null}
                </div>];

              // Ghosts: telas subordinadas ao hub — só aparecem quando o hub (ou um ghost dele) está ativo
              if (ghosts.length && isActive) {
                ghosts.forEach((g) => {
                  const GIcon = I[g.icon];
                  const gActive = activeRoute === g.id;
                  rows.push(
                    <div key={g.id}
                    className={`sb-item sb-sub sb-ghost ${gActive ? "active" : ""}`}
                    role="button" tabIndex={0}
                    onKeyDown={(e) => {if (e.key === "Enter" || e.key === " ") {e.preventDefault();onSelectRoute(g.id);}}}
                    onClick={() => onSelectRoute(g.id)}
                    style={{ paddingLeft: "38px", opacity: gActive ? 1 : 0.7, ...(gActive ? { borderLeftColor: groupDot } : {}) }}>
                      {GIcon && <GIcon className="ic" style={{ width: 13, height: 13 }} />}
                      <span className="label" style={{ fontSize: "12px" }} data-comment-anchor="6e73134b04-span-217-23">{g.label}</span>
                    </div>
                  );
                });
              }
  ...
  return (
    <div className="user-menu" onClick={(e) => e.stopPropagation()}>
      ...
      {(MOCK.USER_MENU || []).map((it) => {
        ...
        return (
          <div key={it.id} className="um-item" onClick={() => {window.__selectRoute?.(it.id);onClose?.();}}>
        ...
        return (
          ...
          style={{ top: flyoutPos.top, left: flyoutPos.left }}>
            ...
            {entry.items.map((item) => {
              ...
              className={`sb-item sb-sub ${isActive ? "active" : ""}`}
              role="button" tabIndex={0}
              onKeyDown={(e) => {if (e.key === "Enter" || e.key === " ") {e.preventDefault();onSelectRoute(item.id);setFlyout(null);}}}
              onClick={() => {onSelectRoute(item.id);setFlyout(null);}}
              style={isActive ? { borderLeftColor: groupDot } : null}>
                  <Icon className="ic" />
                  <span className="label">{item.label}</span>
                  {item.badge ? <span className="badge">{item.badge}</span> : null}
                  {ghosts.length ? <span style={{ marginLeft: "auto", fontSize: "10px", color: "var(--text-mute)", opacity: 0.6 }}>{ghosts.length}</span> : null}
                </div>];
              ...
              if (ghosts.length && isActive) {
                ghosts.forEach((g) => {
                  ...
                  rows.push(
                    ...
                    className={`sb-item sb-sub sb-ghost ${gActive ? "active" : ""}`}
                    role="button" tabIndex={0}
                    onKeyDown={(e) => {if (e.key === "Enter" || e.key === " ") {e.preventDefault();onSelectRoute(g.id);setFlyout(null);}}}
                    onClick={() => {onSelectRoute(g.id);setFlyout(null);}}
                    style={{ paddingLeft: "32px", opacity: gActive ? 1 : 0.7, ...(gActive ? { borderLeftColor: groupDot } : {}) }}>
                      {GIcon && <GIcon className="ic" style={{ width: 13, height: 13 }} />}
                      <span className="label" style={{ fontSize: "12px" }}>{g.label}</span>
                    </div>
                  );
                });
              }
