...
function minus(date, days) { const x = new Date(date); x.setDate(x.getDate() - days); return x; }
function monthStart(date) { return new Date(date.getFullYear(), date.getMonth(), 1); }
// Extrai vínculos estruturados dos tokens #V-/#OS-/#PC-/#BL- da descrição.
// Cross-link deixa de ser "regex no texto" e vira relação navegável no drawer.
function parseLinks(desc) {
  const links = {};
  let m;
  if ((m = desc.match(/#V-(\d+)/)))  links.venda = m[1];
  if ((m = desc.match(/#OS-(\d+)/))) links.os = m[1];
  if ((m = desc.match(/#PC-(\d+)/))) links.compra = m[1];
  if ((m = desc.match(/#BL-(\d+)/))) links.boleto = m[1];
  return links;
}
function cleanDesc(desc) { return desc.replace(/\s*#(?:V|OS|PC|BL)-\d+/g, "").trim(); }
...
const RAW = [
  ...
  // ── Histórico · anos anteriores (dão sentido ao filtro Ano + âncora no grid) ──
  { id: "R-2511", kind: "receivable", desc: "Painel LED outdoor — campanha Natal", party: "Shopping Vale Sul", category: "Fachada", amount: 6800.00, due: d("2025-12-18"), paid_at: d("2025-12-19"), channel: "Boleto", invoice: "NFe 7711" },
  { id: "R-2487", kind: "receivable", desc: "Adesivagem vitrine coleção inverno", party: "Loja Bella Moda", category: "Adesivo", amount: 1240.00, due: d("2025-08-26"), paid_at: d("2025-08-28"), channel: "PIX", invoice: "NFe 7402" },
  { id: "P-1788", kind: "payable", desc: "13º salário Larissa", party: "Folha — Larissa Souza", category: "Folha", amount: 2800.00, due: d("2025-12-20"), paid_at: d("2025-12-20"), channel: "Transferência", invoice: "Folha 13/25" },
  { id: "P-1745", kind: "payable", desc: "Compra plotter Roland usada", party: "TecPlot Assistência", category: "Serviço", amount: 9500.00, due: d("2025-09-10"), paid_at: d("2025-09-10"), channel: "Transferência", invoice: "NF 01880" },
  { id: "R-2390", kind: "receivable", desc: "Sinalização interna prédio comercial", party: "Construtora Vértice", category: "Placa", amount: 4120.00, due: d("2024-11-22"), paid_at: d("2024-11-25"), channel: "Boleto", invoice: "NFe 6620" },
  { id: "P-1640", kind: "payable", desc: "Reforma fachada da gráfica", party: "Construfix Obras", category: "Serviço", amount: 7300.00, due: d("2024-10-05"), paid_at: d("2024-10-08"), channel: "Transferência", invoice: "NF 00455" },
];

const ROWS = RAW.map((r) => {
  const emissao = r.emissao || minus(r.due, 14);
  const competencia = r.competencia || monthStart(emissao);
  const links = parseLinks(r.desc);
  return { ...r, emissao, competencia, links, descClean: cleanDesc(r.desc), status: statusFor(r) };
}).sort((a, b) => a.due - b.due);

window.FIN_TODAY = TODAY;
window.FIN_ROWS = ROWS;
window.FIN_DAYS_FROM_TODAY = daysFromToday;
window.FIN_STATUS_FOR = statusFor; // pipeline único de status — consumido pelo Novo lançamento (crash "aberto" 2026-06-10)

// ── COSTURA venda→caixa ([W] "roda o fluxo" 2026-06-10) ──────────────────
// Vendas emite oimpresso:venda-created; o caixa ingere o título A RECEBER
// pelo MESMO pipeline de enriquecimento das linhas mock (status/links/competência).
window.addEventListener("oimpresso:venda-created", (e) => {
  const det = e.detail || {};
  if (!det.totalCents) return;
  const raw = {
    id: "R-" + (2660 + (window.__finVendaSeq = (window.__finVendaSeq || 0) + 1)),
    kind: "receivable",
    desc: (det.firstItem || "Venda balcão") + " · #" + (det.vendaId || "V-?"),
    party: det.clientName || "Consumidor Final",
    category: "Venda",
    amount: det.totalCents / 100,
    due: new Date(TODAY.getTime() + 2 * 864e5),
    paid_at: null,
    channel: det.channel || "PIX",
    invoice: "",
  };
  const emissao = TODAY;
  ROWS.unshift({ ...raw, emissao, competencia: monthStart(emissao), links: { venda: det.vendaId }, descClean: cleanDesc(raw.desc), status: statusFor(raw) });
});
