...
{ id:"tec-front", grp:"tec", nav:"Front-end & tokens", title:"Front-end — Inertia, React 19 e os tokens",
  ...
  {k:"table",head:["Passo","Porta","Dono"],rows:[
    ...
    ["<b>ALVO</b> · medir","protótipo servido, tema dark, após <code>__oiLazyDone</code> e <b>duas leituras iguais</b> de <code>querySelectorAll('*').length</code>; <code>getComputedStyle</code>, nunca a classe declarada","[CC] · read-only"],
    ["<b>EXPORT</b> · pedido","playbook em <code>cowork-inbox/&lt;mod&gt;/playbook/</code> — 1 thread = 1 seção = 1 PR ≤300 linhas, com ancoragem dupla (layout medido aqui · implementação no arquivo real do <code>main</code>)","[CC] escreve · [CL] executa"],
    ["<b>Pacote</b>","<code>node scripts/design-sync/gerar-payload-partes.mjs --root &lt;dir&gt; --out sync/ --previous sync/bundle.manifest.json</code> + recibo <code>bundle regenerado</code> no <code>github.md</code> (ADR 0387)","[CL] — exige os arquivos em disco"],
    ["<b>Guardas no CI</b>","<code>cowork-ssot-guard.mjs</code> (R1 zero <code>.md</code> em <code>cowork/</code> · R2 sem bundle datado · R3 allowlist) + <code>cowork-mirror-freshness.mjs</code> (<code>--absent-local</code> · <code>--check-orfaos</code> em DELTA · <code>--check-refs</code>)","máquina, no <code>design-memory-gate.yml</code>"],
    ["<b>Fecho</b>","<code>design-diff --compare --check</code> nos dois renders, prod deployada (T7) — antes disso nada é “igual ao design”","[CL] + [W2]"]]},
  {k:"alert",tone:"warn",title:"ADR 0374 está ATIVA — retratação de 10/09/2026",body:"Meu protocolo declarava, em 4 linhas, que a <b>0374 tinha sido revogada</b> por [W] em 07/09. Medido no <code>main</code> por [CL] (<code>11eff17f13</code>): <code>status: aceito</code> · <code>lifecycle: ativo</code> · <code>superseded_by: []</code>, e nenhuma ADR 0375→0392 a supersede. A emenda real é a <b>0389</b>, e ela <b>toca uma linha</b>: escrever pelo contexto do agente só quando o arquivo volta <b>inline</b> e <b>não existe rota de máquina</b>, sob 4 condições. Onde a rota existe, transcrever segue <b>proibido</b>. As 4 linhas foram corrigidas no build."},
  {k:"alert",tone:"info",title:"Retratação — 10/09/2026",body:"Esta página afirmou que <code>ds-push.mjs</code> <b>não existia</b>. Errado: ele está em <code>ed4398d77437</code>, com teste — conferido por [CL] no mesmo commit. A conclusão saiu de uma listagem de diretório, e <b>listagem que não devolve o arquivo não prova ausência</b>. A regra que fica: ausência só se afirma com leitura direta do caminho e um controle positivo na mesma sonda."},
