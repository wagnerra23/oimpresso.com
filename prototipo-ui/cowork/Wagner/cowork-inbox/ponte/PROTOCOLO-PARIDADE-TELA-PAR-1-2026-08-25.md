...
```bash
php artisan ui:lint && npm run casos:check && npm run contrato:check
node scripts/qa/prototipo-readiness.mjs
node scripts/governance/cowork-ssot-guard.mjs
```
> Caminhos conferidos no `main` em 2026-09-23 (`ef330df5351e`): `ui:lint` é comando **artisan** (`ds-gate.yml:116`), não script npm · o guard mora em `scripts/governance/` (`design-memory-gate.yml:194`) · `cowork-paridade.mjs` **nunca desceu** (`design-memory-gate.yml:494`) — a paridade é do `cowork-mirror-freshness.mjs` no CI, não se roda à mão.
