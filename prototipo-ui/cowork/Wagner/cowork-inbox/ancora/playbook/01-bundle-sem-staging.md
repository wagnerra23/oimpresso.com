...
- **âncora (código):** `scripts/design/ancora.mjs` — **49.089 B**, sha de árvore `752041ac450d`. Blocos que importam: `const LUGAR_FIXO = 'prototipo-ui/cowork'`, `caminhoDaAncora` (formato 3), `resolveAncora` (`if (stagingDir) { … }`), `printResolve` (a linha `'  âncora:     ⚠️ charter sem related_prototype nem -page.jsx — registre o protótipo'`).
...
## D · COMO VALIDAR
1. `ancora.mjs <tela>` **sem** `--staging`, para uma das 14: aparece a perna de bundle resolvida em `prototipo-ui/cowork/Wagner/<arquivo>`, com `tipo` dizendo de qual campo veio (`bundle_source` ou `visual_source`) **e** que veio do lugar fixo, não de staging.
...
7. `node scripts/design/ancora.mjs --selftest` verde (é step required em `.github/workflows/design-memory-gate.yml:316`).
...
- **NÃO TOCAR:** nenhum charter, nenhum hook, nenhum workflow, nenhum arquivo de `prototipo-ui/cowork/Wagner/`.
...
## PROVA
`scripts/design/ancora.mjs` contém a resolução por `LUGAR_FIXO` na perna do bundle + `BITE bundle sem staging` + `CONTROLE` · `--selftest` verde · hook importando `caminhoDaAncora` intacto (guarda) · `_saida-01.md`.
