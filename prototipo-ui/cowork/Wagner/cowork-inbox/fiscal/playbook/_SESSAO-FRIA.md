...
> 1 thread = 1 sessão nova = 1 PR. Gerado pelo [CC] em 2026-09-23 a partir do bloco json do `00-INDICE.md` (sha 99530522729d). Se o índice mudar, **o índice manda**, não esta folha.

## Prompt de abertura — cole no chip novo, trocando só o NN

```
/onda fiscal --thread 01
```
> O diretório é **`fiscal`** (literal). Não use o nome do módulo (`Fiscal`): o `/onda` só passa o argumento pra minúsculo.
...
|---|---|---|---|---|---|
| **01** | Rede: 2 specs E2E (cockpit + NF-e) | CL | `01-rede-e2e.md` | — | — |
| **02** | Paginacao .fx-pager no Cockpit | CL | `02-paginacao.md` | — | — |
| **03** | Afericao read-only (FEITA) | CC | `03-afericao.md` | — | tem _saida |
