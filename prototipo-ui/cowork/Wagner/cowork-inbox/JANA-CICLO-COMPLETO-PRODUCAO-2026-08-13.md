...
- ~~`prototipo-ui/ds-v6/` vs `prototipo-ui/cowork/ds-v6/` — 3 HTMLs homônimos (`gabarito-vendas` · `receita` · `showcase`) nos dois lugares. **Parece** dupe, mas o `ds-v6` é snapshot congelado mantido de propósito como referência histórica. Consolidar é decisão [W] à parte — **fora deste PR**.~~ ❌ **REGRA REVOGADA por [W] em 2026-09-22** ("pode retirar essa regra"). O `ds-v6` deixou de ser intocável: o `tokens.css` dele foi **dobrado no `styles.css`** em 2026-07-10 ([W] "faça o fold") e o próprio `cockpit_domains.css` do DS vivo declara que ele "perde a razão de existir (literais redundantes) e pode ser deletado". No Cowork a pasta **já foi removida** em 2026-09-22 (2 arquivos: `gabarito-vendas.html` + `tokens.css`; zero refs no host, e o `cowork-pele-paralela.mjs` já a ignorava). No git a remoção de `prototipo-ui/ds-v6/` e `prototipo-ui/cowork/ds-v6/` é pedido pro [CL] — **eu não escrevo no git**, e não afirmo que foi feita lá.
