...
## UC-EXT-02 — Saldo sem sync mostra "—", nunca R$ 0,00
Status: 🧪 (`ExtratoControllerTest`)
Dado `saldo_atualizado_em` nulo · Então a tela mostra "—" e o aviso "Sem sync" (nunca zero, que engana).

## UC-EXT-03 — Filtro de período é manual e preserva a posição
Status: ⬜ (visual — `preserveScroll`/`preserveState`, sem auto-apply)
