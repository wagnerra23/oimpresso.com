...
prefixo: prototipo-ui/cowork/Wagner/contrato/governance.contract.json (CRIAR — pasta de ESTÁGIO)
nao_toca: governance/design/contracts/** (pasta VIGENTE — promoção é outro PR) · resources/js/Pages/governance/** · Modules/Governance/**
depende: — (vaga 1)
antes:  governance/design/contracts/ tem 31 arquivos e NENHUM de governança
...
`governance/design/contracts/` tem **31 `.contract.json`** — Fiscal (7), Compras (2), outros. É a pasta **vigente**, a que o `contrato-de-tela.yml` lê, e usa nome de **tela** (`fiscal-cockpit`, `purchase-create`).
`prototipo-ui/cowork/Wagner/contrato/` tem **3** — `patrimonio`, `configuracoes`, `venda-menu`. É a pasta de **estágio**, para contrato nascido no Cowork, com nome **minúsculo do módulo**.
...
## B · O que fazer
1. Copiar o contrato do Cowork para **`prototipo-ui/cowork/Wagner/contrato/governance.contract.json`** — pasta de **estágio**, nome minúsculo do módulo, como os 3 que já estão lá. **Não** ir direto pra `governance/design/contracts/`.
...
- **Não** promover à pasta vigente (`governance/design/contracts/`) nem tocar no `contrato-de-tela.yml` neste PR.
- **Não** criar contrato para as 4 telas não cobertas aqui — é a D-CONTRATO-9, e o custo já foi medido como RECUSA por teto.

## Execução
```
PASSO   1) gh pr list --state open × prototipo-ui/cowork/Wagner/contrato/
...
## Checklist de saída
1. arquivo em `design-docs/contrato-cowork/` (estágio) · 2. `cobertura_parcial` presente com as 4 não cobertas · 3. JSON parseia · 4. `governance/design/contracts/` e o workflow **intocados** · 5. caminho minúsculo preservado · 6. divergências notadas registradas · 7. placar no PR
