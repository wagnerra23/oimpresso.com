...
**O que quebrou:** o handoff (3) e o bundle v2 foram recusados carregando `(schema removido pela ADR 0397 — a integridade é validada no próprio placar-indice.mjs)` e `_scripts/placar-indice.mjs` — **máquina do repo**, em cópia anterior a #7063/#7071. Descê-las apagaria `constituicao`/`nota_caminho`/`custo`/`afeta` do schema e o `descobrirIndices` do placar. Controle positivo do diagnóstico: o playbook `patrimonio`, **válido no `main`**, reprovava contra o schema do ZIP — playbook bom reprovando denuncia o schema, não o playbook.
...
### Forma do playbook — `prototipo-ui/cowork/Wagner/cowork-inbox/<mod>/playbook/`
```
00-INDICE.md         FONTE = **primeiro bloco ```json embutido neste .md** (só `.md` roteia pelo DesignSync; `.json` solto não chega).
                     Schema: `prototipo-ui/cowork/Wagner/(schema removido pela ADR 0397 — a integridade é validada no próprio placar-indice.mjs)` **no `main`, lido no turno** —
...
- **Onde mora:** `prototipo-ui/cowork/Wagner/cowork-inbox/<mod>/playbook/` (guard R3 permite `.md` dentro do dono). O índice é **pedido** (lista de threads a executar, com sha), não inventário — é o mesmo estatuto da `ponte/01-LISTA-COMPLETA.md`.
- **Landing (corrigido 2026-09-22, lido no `main` a378c9bf):** a pasta `cowork-inbox/` desce inteira para `prototipo-ui/cowork/Wagner/cowork-inbox/` (guard R3: `.md` é permitido dentro de um dono). O import sincroniza com `/PURGE` — **o espelho vira cópia exata do Cowork**, então corrigir playbook é AQUI, nunca no `main`, e cópia local velha apaga trabalho do `main`. A fonte da máquina é o 1º bloco ```json do `00-INDICE.md` (nunca `playbook.json`). O pacote é gerado no CI (`.github/workflows/cowork-bundle.yml`, artifact a cada push em `cowork/Wagner/**`).
...
- **Abertura de thread** = `_SESSAO-FRIA.md` do módulo (se houver) ou `/onda <Mod> --thread NN` + o `NN-*.md`. Sem chat anterior — se a thread precisa perguntar algo, o arquivo está incompleto.
- **Thread de TELA é ponteiro, não pedido (validado 2026-09-22 no `main` fdc8a9c4).** O dono dos 4 blocos A–D + read-order é `scripts/design-sync/pedido.mjs` (`/onda <Mod/Tela> <secao>`); ele só sai com exit 0 se existir `governance/design/targets/<slug>.alvo.json` medido por `alvo.mjs` — senão **exit 2 NÃO MEDI**. Hoje existe **1** alvo no repo (`jana--index`). Logo: (1) toda thread que toca `.tsx` **começa** por uma thread irmã `ALVO <slug>` (dono [CL]: `npm run alvo:medir -- <espelho servido> --tela <slug> --secoes <slug>.secoes.json`, prova `arquivo governance/design/targets/<slug>.alvo.json`); (2) o `NN-*.md` da tela carrega só **prefixo · nao_toca · dependências · PARAR SE · o comando `/onda`** — nunca reescreve alvo, dados ou EARS em prosa (segundo dono do pedido, envelhece: foi o que aconteceu com os números de 04/09 do HRM/Ponto); (3) a prova de fidelidade da thread é `secao-check` exit 0 (slots) — `design-diff --compare --check` (T7) continua fora do CI, porque o frescor do espelho exige `DesignSync.get_file` com login.
...
**Arquivo único, cross-módulo, append-only:** `COLAR-NO-CODE-ACERTOS-E-LICOES.md` → pousa em `prototipo-ui/cowork/Wagner/` pelo import (guard R3); **o Code promove o que for canon para `memory/reference/prototipo-ui/`** (ADR 0397 D3: política, runbooks e referências vivem lá — `design-docs/` foi removida pela D5). **Um bloco por ciclo, mais novo em cima; bloco antigo nunca se reescreve** — o erro registrado é o valor. Não é por módulo: o acerto do Fiscal é o que evita o erro do Ponto.
...
- **`governance/design/component-registry.json`** = registro **bloco-de-protótipo → componente React**. É exatamente o dicionário de papéis.
