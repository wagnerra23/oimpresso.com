/* @ts-nocheck */
/* eslint-disable */
// cobranca-page.jsx — referência histórica F1 · Financeiro/Cobrança
...
function CobrancaPage() {
  // filtros (com URL-sync conceitual via localStorage)
  const [tabStatus, setTabStatus] = useState('all'); // all | emitida | paga | vencida | cancelada
  const [tipoFilter, setTipoFilter] = useState('all'); // all | boleto | pix | pix_recv | card
  const [gatewayFilter, setGatewayFilter] = useState('all');
  const [accountFilter, setAccountFilter] = useState('all');
  const [origemFilter, setOrigemFilter] = useState('all'); // all | sale | invoice | subscription_license
    ...
    <div className="h-full bg-stone-50 flex flex-col font-sans" data-screen-label="01 Cobrança">
      ...
      <div className="px-6 pt-4 pb-2 flex items-center gap-2 flex-wrap">
          ...
          {[
            ...
            <button key={t.id} onClick={() => setTabStatus(t.id)} className={cn(
              ...
              <span className={cn("text-[10px] tabular-nums px-1 rounded", tabStatus === t.id ? "bg-stone-200 text-stone-700" : "text-stone-400")}>
                {tabStatus === t.id ? totalCounts[t.id] : totalCounts[t.id]}
              </span>
            </button>
          ))}
        </div>
        ...
        <div className="ml-auto relative">
          ...
          <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="cliente, doc, nosso nº, origem…"
            className="h-7 w-[280px] pl-7 pr-2 bg-white border border-stone-300 rounded-md text-[12px] focus:outline-none focus:border-stone-500 focus-visible:ring-2 focus-visible:ring-stone-400" />
        </div>
        <Btn variant="ghost">{I.download}Exportar</Btn>
      </div>

      {/* CHIPS TIPO + DROPDOWNS GATEWAY/ACCOUNT/ORIGEM */}
          ...
          <table className="w-full text-[12.5px]" style={{fontVariantNumeric:'tabular-nums'}}>
            ...
            <tbody>
              ...
              {filtered.map(c => {
                ...
                return (
                  <tr key={c.id} className="border-b border-stone-100 hover:bg-stone-50/60 cursor-pointer" onClick={() => setDrawer(c)}>
                    ...
                    <td className="pl-2 pr-5 py-2.5 text-right" onClick={(e) => e.stopPropagation()}>
                      <button title="Copiar identificador" className="inline-flex items-center justify-center w-6 h-6 rounded hover:bg-stone-200 text-stone-500">{I.copy}</button>
                      {c.tipo === 'boleto' && <button title="Baixar PDF" className="inline-flex items-center justify-center w-6 h-6 rounded hover:bg-stone-200 text-stone-500 ml-0.5">{I.download}</button>}
                      {c.tipo?.startsWith('pix') && c.tipo !== 'pix_recv' && <button title="Copiar BR Code" className="inline-flex items-center justify-center w-6 h-6 rounded hover:bg-stone-200 text-stone-500 ml-0.5">{I.qrcode}</button>}
