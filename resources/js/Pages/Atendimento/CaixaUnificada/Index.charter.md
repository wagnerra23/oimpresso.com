...
| Data | Autor | Mudança |
|---|---|---|
| 2026-06-18 | Claude Code [CL] (handoff Cowork [CC]→[CL] · escolha [W]) | **Redesign visual do `ChannelHealthBanner` (Cowork).** O handoff `PROMPT_PARA_CODE_CHANNEL-HEALTH-BANNER` chegou com premissa stale ("a tela não avisa") — validei vs `main` (§10.4) e o banner US-WA-308 já estava live no topo (`Index.tsx:422`), e o agregado backend (`unhealthyChannels` + `last_health_check_at`) já existia (a "Onda 4" do prompt). [W] escolheu **trocar o visual** pelo design Cowork: tom graduado warn/err, dispensável, resumo multi-canal e CTA Reconectar. Adaptado aos estados REAIS do `whatsmeow:health-probe` — `disconnected`/`banned` (err) + `degraded` (warn); o prompt assumia um estado `down` que o backend **nunca emite** (seria dead-code). **Posicionado no topo da COLUNA de conversas** (fiel ao protótipo, direção [W] 2026-06-18): o `ChannelHealthBanner` passou a ser renderizado por `ConversationListV4` (logo após a busca), não mais full-width no `Index.tsx`; o prop eager `unhealthyChannels` desce `Index → ConversationListV4 → banner`. Layout por primitivos `<Stack>`/`<Inline>` (ADR 0253 — layout-primitives ratchet verde; centragem de ícone via idioma `grid place-items-center`). Cor 100% semântica (tokens `warning`/`destructive`, R1 + ADR 0281). `R-WA-CAIXA-UNIF-014` (payload) intacto. Charter v17. |
