...
const FISCAL_LABEL: Record<FiscalType, { label: string; classes: string }> = {
  ...
  none: { label: 'Não emite',  classes: 'bg-muted text-muted-foreground ring-border' },
};
      ...
      <ul className="mt-3 space-y-1.5">
        {totals.map((t) => (
          <li key={t.key} className="flex items-center gap-2 text-xs">
            <span className="w-20 text-foreground">{t.label}</span>
            ...
            <span className="w-6 text-right font-mono tabular-nums text-foreground">{t.count}</span>
          </li>
        ))}
      </ul>
    </div>
      ...
      <div className="min-h-screen bg-muted p-4 md:p-6">
          ...
          <div className="flex flex-wrap items-end justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <Link
                ...
                className="inline-flex items-center gap-1.5 rounded-lg bg-card px-3 py-2 text-sm font-medium text-foreground shadow-sm ring-1 ring-border hover:bg-muted"
              >
                Voltar
              </Link>
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-lg bg-card px-3 py-2 text-sm font-medium text-foreground shadow-sm ring-1 ring-border hover:bg-muted"
              >
                <kbd className="rounded bg-muted px-1 text-[10px] font-mono text-muted-foreground ring-1 ring-border">?</kbd>
              </button>
              <Link
        ...
        <section className="overflow-hidden rounded-lg bg-card shadow-sm ring-1 ring-border">
          ...
          <form
            ...
            className="flex items-center gap-2 border-b border-border p-2.5"
          >
                    ...
                    {rows.map((p) => (
                      ...
                      >
                        <td className="px-4 py-2.5">
                          ...
                          {p.descricao_curta && (
                            <div className="mt-0.5 text-[11px] text-muted-foreground">{p.descricao_curta}</div>
                          )}
                        </td>
                        <td className="px-4 py-2.5 text-foreground">
                          {CICLO_LABEL[p.ciclo] ?? p.ciclo_label}
                          {p.ciclo === 'custom' && p.ciclo_dias && (
                            <span className="ml-1 text-[11px] text-muted-foreground">({p.ciclo_dias}d)</span>
                          )}
                        </td>
                        <td className="px-4 py-2.5 text-right font-mono tabular-nums text-foreground">
                          {BRLshort(p.valor)}
                        </td>
                        <td className="px-4 py-2.5 text-center font-mono tabular-nums text-foreground">
...
function EmptyState() {
  return (
    <div className="p-12 text-center">
      ...
      <div className="text-lg font-medium text-foreground">Nenhum plano cadastrado.</div>
