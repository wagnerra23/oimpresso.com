...
**Recibos**, lidos do sumário JUnit por arquivo e não do console — **três** runs, porque a
allowlist da lane cresceu no meio (merge de #6884, #6886 e #6882):
...
run 34002406074 (allowlist 20) → JobSheetShow: 5 tests · 5 passed · 0 skipped · 29 assertions
                                 lane inteira: 146 passed · 13 skipped · 799 assertions
```

`skipped: 0` com assertions de dois dígitos é o que separa "passou" de "pulou" — é a leitura
que a consequência 1 abaixo exige.

⚠️ **Os três recibos PROVAM a ressalva do UC-JSS-04 em vez de só afirmá-la:** o MESMO
arquivo, com os MESMOS 5 testes passando e `skipped: 0` nos três, rendeu **29 · 38 · 29**
assertions. O ramo condicional do UC-JSS-04 — o que exige estágio em `sale_process_stages` —
executou em **1 de 3** runs.

E a causa **não** é o tamanho da allowlist: o run 3 tem MAIS arquivos que o run 2 e voltou
para 29. (A primeira redação desta nota atribuía a diferença à composição da lane; o run 3
refutou isso e a atribuição foi corrigida em vez de mantida.) O que sobra como explicação é
a **ordem aleatória** dentro do run sobre um banco compartilhado — se algum teste que semeia
estágio corre antes. Isso não foi medido, então fica dito como o que é: hipótese. O **fato**
medido é que o ramo não roda sempre.

Consequência prática: a cobertura desse ramo **não é propriedade deste arquivo**. Por isso o
Status abaixo promete só a metade legada, que roda nos três. Os outros 3 contratos do JobSheet (`Create`, `Edit`,
