...
> Zero JSX estrutural, zero handler, zero prop, zero copy alterada (verificado: `git diff origin/main...HEAD -- resources/js/Pages/Repair/ProducaoOficina/Index.tsx`
...
|---|---|---|---|---|---|
| UC-RPOE-01 | O kanban monta com as 5 colunas na ordem do charter | must | charter §Mission + §Goals | `ProducaoOficinaIndexTest` | 🧪 |
| UC-RPOE-02 | Cabe em 1280px sem scroll horizontal | must | charter §UX Targets | `ProducaoOficinaIndexTest` | 🧪 |
| UC-RPOE-03 | Filtro de `slot`/`area` liga o contador e o "Limpar filtros" | should | charter §Goals (US-REPAIR-PROD-3) | `ProducaoOficinaIndexTest` | 🧪 |
| UC-RPOE-04 | O drawer é o único container — zero modal, zero `<form>` | must | charter §UX Anti-patterns | `ProducaoOficinaIndexTest` | 🧪 |
| UC-RPOE-05 | Zero violação axe CRITICAL | should | ratchet level 0 (`A11yAxeBrowserTest`) | `ProducaoOficinaIndexTest` | 🧪 |
| UC-RPOE-06 | O card do kanban não é operável por teclado (**defeito medido**) | must `[A11Y]` | WCAG 2.1.1 + 2.5.7 | `ProducaoOficinaIndexTest` | 🧪 |

---

## UC-RPOE-01 · O kanban monta com as 5 colunas na ordem do charter `[must]`
...
- **Teste:** `ProducaoOficinaIndexTest` ("UC-RPOE-01 · render — o kanban monta com as 5 colunas na ordem do charter").
...
## UC-RPOE-02 · Cabe em 1280px sem scroll horizontal `[must]`
...
- **Teste:** `ProducaoOficinaIndexTest` ("UC-RPOE-02 · render — cabe em 1280 sem scroll horizontal").
...
## UC-RPOE-03 · Filtro de `slot`/`area` liga o contador e o "Limpar filtros" `[should]`
...
- **Teste:** `ProducaoOficinaIndexTest` ("UC-RPOE-03 · render — o filtro liga o contador comparativo e o Limpar filtros").
...
## UC-RPOE-04 · O drawer é o único container — zero modal, zero `<form>` `[must]`
...
- **Teste:** `ProducaoOficinaIndexTest` ("UC-RPOE-04 · Non-Goal — drawer é o único container, sem modal e sem form").
...
## UC-RPOE-05 · Zero violação axe CRITICAL `[should]`
...
- **Teste:** `ProducaoOficinaIndexTest` ("UC-RPOE-05 · a11y — zero violação axe CRITICAL").
- **Ratchet:** mesmo piso do `A11yAxeBrowserTest` (level 0). Subir pra level 1 (critical + serious) é PR follow-up — não se sobe às cegas.
- **Status: 🧪**

## UC-RPOE-06 · O card do kanban não é operável por teclado — defeito medido, capturado `[must]` `[A11Y]`
...
- **Teste:** `ProducaoOficinaIndexTest` ("UC-RPOE-06 · a11y — o card do kanban não é alcançável nem operável por teclado (defeito capturado)").
...
- **Por que não é o axe que pega:** o axe não tem regra pra "handler de clique em elemento
  ...
  **aditiva** e não duplica o UC-RPOE-05.
