...
- **Âncora:** `CU-FISC-01` do SDD §6 (o cockpit entrega a leitura consolidada do mês) + o alvo `prototipo-ui/cowork/Wagner/fiscal-page.jsx:125-137` (`FxAlerts`).
...
- **Âncora:** `fiscal-page.jsx` §`FxNotasPage` do protótipo Cowork, baixado do **vivo** por `DesignSync` em 2026-09-03 (`truncated: false`) — não do espelho `prototipo-ui/cowork/Wagner/`, que mediu **1 de 258** arquivos e cuja própria máquina declara que qualquer comparação contra ele é INCONCLUSIVA. De lá vêm o default **8**, as opções 8/25/50, a copy e o contador `{pagina} / {paginas}`.
...
## UC-FCKP-13 — A tela diz, por superfície, o que é leitura real e o que é demonstração `[must]`
...
- **Decisão [W] que o destravou:** em 2026-09-04, [W] autorizou **construir** a saída (a) do CU-FISC-16 (marcar a procedência) e descartou a **(c)** declarar Non-Goal. A **(b)** (esconder atrás de flag) foi recusada **pelo agente**, não por [W]: esconder remove a informação em vez de qualificá-la, e o CU pede que a contadora *consiga dizer* o que é o quê. Registrar essa recusa como decisão de [W] seria inventar canon — a autoria fica declarada aqui e no charter. Foi o contrato que faltava para o `[BACKLOG]` virar UC — ele estava sem id de propósito justamente porque UC órfão bloqueia o merge de quem for atendê-lo.
...
- **Teste:** `Modules/Fiscal/Tests/Feature/ProcedenciaCockpitTest.php` — `it('UC-FCKP-13 · CU-FISC-16 · todo método mock* do controller tem superfície declarada como demonstração')`, `it('UC-FCKP-13 · CU-FISC-16 · nenhuma superfície declarada demonstração sobrevive ao método mock* sumir')`, `it('UC-FCKP-13 · CU-FISC-16 · toda chave que a tela sela existe na declaração do controller')`, `it('UC-FCKP-13 · CU-FISC-16 · toda superfície declara origem do vocabulário fechado e uma explicação')`. **Estático de propósito** (sem DB, sem HTTP): assim ele roda também na lane advisory em SQLite, onde `CockpitControllerTest` e `CockpitCacheTest` pulam — e teste que pula sai com `0 failed` sem ter medido nada (LC-13).
...
- **Veredito visual DECODIFICADO (2026-09-04, run `33870086835`):** o `visual-regression` acusou `Fiscal/Cockpit` na zona cinza. Decodifiquei os dois lados com `scripts/tests/snap-diff.mjs` em vez de explicar por dedução (§5 2026-08-29): **10.773 px** alterados, `Δmax=234` (assinatura de CONTEÚDO, não de rasterização). Decomposto por faixa e conferido a olho nos recortes pareados:
  - **cabeçalho `y0–90` — 8.175 px (75,9%), `Δmax=234`:** o botão `Procedência` entra entre `Buscar` e `Eventos` e empurra o `fx-env` para a esquerda. **É este PR, e é intencional.**
  - **ribbon `y150–240` — 2.486 px (23,1%), `Δmax=227`:** o sub-rótulo do KPI `Certif. A1` mudou de `vigente` para `sem certificado`. **NÃO é este PR** — é o [#6722](https://github.com/wagnerra23/oimpresso.com/pull/6722), provado por `git log -S "sem certificado"` (um único commit, `f089dc5a93`) e por `git diff origin/main...HEAD` do `.tsx`, que não tem nenhuma das duas strings.
  - **sub-nav e resto — 112 px (1,0%), `Δmax=1`:** rasterização, ruído.
- **Por que ESTE PR disparou e os cinco anteriores não — medido na métrica do próprio gate:** os limiares são `τ_baixo=0,1%` (auto-aprova) e `τ_alto=2%` (auto-falha). Reproduzi a métrica do gate por bisseção da tolerância por-pixel (tol≈108 → **0,1163%**, contra os 0,1200% que ele reportou) e decompus nela: o botão deste PR vale **0,0762%** e a dívida do #6722 vale **0,0400%** — **cada um sozinho fica ABAIXO do piso e não dispararia nada**. Juntos somam 0,1163% e cruzam. O predicado do gate é **absoluto** (diff contra baseline), não delta do PR, então a dívida acumula invisível enquanto cada parcela couber sob o piso, e quem passa por último paga a conta inteira. É a família de [proibicoes §5](../../../../memory/proibicoes.md) 2026-08-24 (*gate visual bloqueando por tela fora do raio do PR*) e 2026-08-10 (*catraca cujo universo vem do lado mutável*).
- **Dívida de baseline, herdada e declarada:** a baseline do Cockpit é de 2026-09-02 ([#6579](https://github.com/wagnerra23/oimpresso.com/pull/6579)) e **seis** PRs tocaram a tela desde então (#6705, #6707, #6711, #6712, #6713, #6722) sem regravá-la — nenhum deles carrega o label de aprovação da zona cinza. Logo 23% do vermelho que este PR recebe é dívida de terceiros, e ela **reaparece** no próximo PR que tocar a tela. Regravar a baseline aqui absorveria em silêncio a mudança do #6722, que nunca teve aprovação visual — por isso não foi feito: é decisão [W] (gate F1.5), não conserto de quem passou por último.
...
- **Âncora:** `CU-FISC-16` do [SDD §6.5](../../../../memory/requisitos/Fiscal/SDD-cockpit-fiscal-v1.0.md) + decisão [W] de 2026-09-04 + o alvo `prototipo-ui/cowork/Wagner/fiscal-page.jsx:60` (botão) e `fiscal-actions.jsx:93-99` (selo).
...
- **Âncora:** `CU-FISC-01` do SDD §6 (o cockpit entrega a leitura consolidada do mês) + o alvo `FxSpark` de `prototipo-ui/cowork/Wagner/fiscal-page.jsx:80-84`, lido nos **dois** donos do inventário de design em 2026-09-04 — do vivo por `DesignSync` (`truncated: false`) e do espelho `prototipo-ui/cowork/Wagner/` —, que **concordam neste trecho**. De lá vêm o viewBox 56×15, a base em y=14, a amplitude 12, `strokeWidth` 1.2, `currentColor` e `aria-hidden`.
- **Status:** 🧪 veredito da lane pendente — a lane nasce neste PR e o run real vem do CI; o que existe hoje é o run local acima.
