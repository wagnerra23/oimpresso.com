...
> **v20 (2026-07-13) · contrato de intenção vivo:** o charter aponta para `financeiro-unificado.intent.json`, que transforma fluxos críticos em evidências verificáveis. O empty state e Cmd+K de "Novo lançamento" passam a abrir o mesmo `TituloCreateSheet` de "Novo recebimento" — nunca a rota legada `/financeiro/unificado/novo`.
> **v20.1 (2026-07-13) · estados visuais:** `default`, `empty`, `loading`, `dark` e `error` entram no manifesto L2. O runner usa tenant vazio, defer congelado, tema em memória e flash de erro — estados reais sem contaminar produção.
