...
| UC-PEDIT-08 | Desligar as 3 flags continua possível pela Blade (`hidden 0`) | must `[V0]` | par obrigatório do 05/07 + `AR-PROD-051/056` | `ProdutoEditPayloadContratoTest` (Pest) | 🧪 nasce com o PR #4943 (2 casos: render da Blade + `0` explícito) |
...
## UC-PEDIT-08 · Desligar as 3 flags continua possível pela Blade · `must` `[V0]`
- **Persona:** a operadora abre um produto que controla estoque, **desmarca** "Gerenciar estoque?" e salva. Tem que desligar de verdade. Se voltar marcado, ela perdeu o controle sobre o próprio cadastro — e o sistema estaria "corrigido" contra ela.
- **Aceite (dois casos):** (a) o GET de `/products/{id}/edit` renderiza `<input type="hidden" name="X" value="0">` para `enable_stock`, `not_for_selling` e `enable_sr_no`; (b) um `PUT` com essas chaves em `'0'` **desliga** as três no banco.
- **Teste:** [`ProdutoEditPayloadContratoTest`](../../../../tests/Feature/Produto/ProdutoEditPayloadContratoTest.php) — 2 casos citando `UC-PEDIT-08`.
- **Contrato:** é o **par obrigatório** do `UC-PEDIT-05`/`UC-PEDIT-07`. Aqueles exigem que a AUSÊNCIA preserve; sozinhos, tornariam impossível desligar pela tela que roda em produção, porque `spatie/laravel-html::checkbox()` **não emite hidden** (ao contrário do Laravel Collective) — desmarcado = chave ausente. A correção só é completa com os dois lados: writer preserva ausência **+** tela declara o `0`.
- **Regressão que defende:** que uma correção futura remova o `hidden` da Blade "porque é redundante" e reintroduza, em silêncio, a impossibilidade de desmarcar.
- **O que este UC NÃO cobre:** a camada JS do navegador — o plugin `input-icheck` embrulha o checkbox. Pest renderiza HTML, não executa o plugin. Esse elo só fecha com **browser real** (smoke biz=1 pós-merge ou staging).
- **Status: 🧪 nasce com o PR #4943.**
