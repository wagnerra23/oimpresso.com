...
| Data | Autor | Mudança |
|---|---|---|
| 2026-08-24 | [M+C] | **Pacote V6 + patch de cor — fecha o §15.1 e resolve a ADR 0401 pelo lado do sistema.** O V6 é o V5 com UMA diferença (o resto do zip é byte-idêntico): a §15 reescrita, separando as **6 divergências abertas** das fechadas. Conferidas uma a uma **contra a tela em produção antes de tocar código**, e **duas não confirmaram** — o §15 mediu o deploy anterior ao #6171: a faixa **já tinha 4 KPIs** com a coluna Margem ativa (#12) e a placa do ícone **já era tintada**, não branca (#2). **Aplicadas: (6) `MoreHorizontal` no ⋯ do cabeçalho** — o da linha já era horizontal, e dois gatilhos de mesmo papel com desenhos diferentes fazem procurar duas vezes. **(11) grupo "Outras visões" removido** — o menu volta a ser lista FECHADA de apresentação + dados. As 4 sub-telas seguem servidas pelo mesmo controller e alcançáveis por `?tela=`, mas **perdem entrada na tela**; decisão tomada ciente de que a sidebar cobre só Categorias e Grupo de preços de venda (telas legadas) e **não** cobre Insumos · BOM nem Histórico de uso — RUNBOOK §Antes/Agora atualizado. **NÃO aplicado: (8) `+N reservado`** continua barrado pela falta de sempre — natureza do local (`venda`/`bloqueado`) não existe no cadastro do UltimatePOS, e sem ela o número seria adivinhado; o `N locais` fica **onde está** (2ª linha do selo), porque o handoff se contradiz — §3.2 manda pra coluna Produto, §4.3 pra coluna Disponível, e §4.3 é o que o código já implementa com racional escrito. **(5)** facetas Unidade e Margem ratificadas. **Patch de cor (§1-§6), que chegou depois e vence onde repete:** a tela passa a ter UMA receita — **fundo do tom a 6%, borda a 22%, texto/glyph no 600** — na placa do KPI (que ganha borda e perde os `oklch` inline) e no selo da coluna Disponível, que vira `rounded-md` como o DS desenha selo (`--radius-md`); "Não estocável" vai a fundo transparente e perde o ponto, por ser ausência de saldo por natureza e não estado de problema. Card do KPI fica neutro e o selecionado marca com **anel `primary`**, não com tinta de fundo. Tons por **token do DS** (`warning` · `destructive` · `success` · `primary`), não pela paleta Tailwind que o patch nomeia — é o que o §6 manda quando o valor "parece faltar": procurar com o outro nome, nunca criar. **Isso desfaz duas paridades DE PROPÓSITO, ambas com aval humano nesta sessão:** a catraca com a `KpiStripClickable` da Consulta de Clientes (outra tela, resolve-se depois) e a **ADR 0401** — o ciano `rgb(231,248,253)` da aba ativa e o badge escuro de contagem eram cópia crua da tela em produção *a pedido do produto*, e viram `--accent-soft` a 50% e `--bg-2`. Trocados em `cockpit.css`, **num lugar só**, exatamente como a nota de lá previa; o bloco `[data-theme="dark"]` desses seis valores **morreu, e a ausência é a correção** — todo token de origem já tem par de tema. |
