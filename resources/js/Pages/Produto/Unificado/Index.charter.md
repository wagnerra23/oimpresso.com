...
related_prototype: prototipo-ui/cowork/prototipos/produto/produto-unificado-v2.dc.html
owner: wagner
status: draft
...
| Data | Autor | Mudança |
|---|---|---|
| 2026-08-14 | [M+C] | `related_prototype` sai de `n/a` → **`prototipo-ui/cowork/prototipos/produto/produto-unificado-v2.dc.html`**. **Não reabre a decisão de 2026-08-13** (linha abaixo): aquela baniu o `produto-app.jsx`, cujo drawer 480px e BulkBar foram descartados por [W] — e ele **segue banido**. Este é **outro artefato**, e o critério de âncora é **estrutural** (§5 2026-08-11, emenda que revogou a regra `biz=NNN`): o v2 desenha **esta** tela — as mesmas 5 sub-telas (`produtos\|categorias\|insumos\|tabelas\|historico`), os mesmos 5 recortes de KPI (`ativos\|populares\|semgiro\|margem\|demanda`, chaves idênticas às do `aplicarRecorte()`), drawer **760** (o canon de entidade cadastral, ADR 0185/0179) e rodapé de paginação no formato `/contacts`. Ou seja: ele **concorda** com a referência `/contacts` que [W] fixou, não a contradiz. O arquivo era **LIVE-ONLY** (servido em `127.0.0.1:8792`, nunca no git) — espelhado por `cp` do `curl`, nunca transcrito (§5 2026-08-11). Comparativo em [`produto-unificado-visual-comparison.md`](../../../../../memory/requisitos/Produto/_telas/produto-unificado-visual-comparison.md), com a medição que mostra **0 divergências** entre os 38 tokens `.cockpit` do protótipo e `tokens/_generated-cockpit-light.css`. Pedido de origem: [M] 2026-08-14 — *"Precisa ser exatamente a cópia."* |
