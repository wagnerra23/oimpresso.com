...
owner: wagner
last_run: "2026-08-28"
last_run_ci: "2026-08-28 (PR #6431) — REVALIDAÇÃO ANALÍTICA, suíte NÃO executada: o CT 100 está fora do ar (tailscale ssh devolveu 502 Bad Gateway + timeout na porta 22), e rodar Pest local é proibido. O que FOI feito: a mudança deste PR é puramente tipográfica (font-size de 2 CardTitle passando de text-base/16px para o token --fs-3/12,5px, medido no protótipo com a mesma sonda). Revisei os 8 UC um a um: nenhum afirma tipografia — os hits de escala no arquivo são escala de TURNO, outro sentido — e os 2 elementos tocados (Últimos 7 dias, Fila de aprovações) não são citados por UC nenhum. Nenhum critério de aceite muda. Isto NÃO substitui execução: o veredito de Status continua vindo do manifesto G-7, e a próxima sessão com CT 100 no ar deve rodar. ---- REGISTRO ANTERIOR (2026-08-23/24), preservado: NÃO EXECUTADO NA LANE. Os 6 UC rodaram no CT 100 (container oimpresso-staging, MySQL oimpresso_staging) em 2026-08-23: 6 passed, 76 assertions, 0 skipped — assertions>0 prova execução, não `0 failed` (LC-13). Mas a lane `PHP / Pest (Ponto · MySQL)` roda uma ALLOWLIST explícita e este arquivo não está nela: teste fora da allowlist é verde impossível — existe e nunca roda por PR. Adicioná-lo exige editar .github/workflows/ponto-pest.yml, fora do prefixo deste PR. Veredito oficial vem do manifesto G-7, não desta linha nem da run manual."
