...
export default function NfseIndex({ notas, filters, flash }: Props) {
  ...
  const columns: ColumnDef<NfseRow, any>[] = [
    ...
    },
    {
      accessorKey: 'numero',
      header: 'Nº Nota',
      cell: ({ row }) => row.original.numero ?? <span className="text-muted-foreground text-xs">pendente</span>,
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-right tabular-nums">
          ...
          {row.original.valor_iss != null && (
            <div className="text-xs text-muted-foreground">ISS: {brl(row.original.valor_iss)}</div>
          )}
        </div>
      ),
    },
    ...
    <AppShellV2 title="NFSe">
      ...
      <div className="p-6 space-y-5 max-w-6xl mx-auto">
        ...
        >
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs text-muted-foreground">Competência de</label>
            <Input type="month" value={de} onChange={(e) => setDe(e.target.value)} className="h-9" />
          </div>

          <div className="flex flex-col gap-1">
            <label className="text-xs text-muted-foreground">Competência até</label>
            <Input type="month" value={ate} onChange={(e) => setAte(e.target.value)} className="h-9" />
          </div>

          <div className="flex flex-col gap-1">
            <label className="text-xs text-muted-foreground">Tomador / Nº</label>
            <Input
              placeholder="Buscar..."
              value={q}
