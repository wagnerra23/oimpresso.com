    ...
    <div className="mx-auto max-w-7xl p-6 space-y-4">
      ...
      <KpiGrid cols={4}>
        <KpiCard
          ...
          description={`${num(kpis.last30d)} nos últimos 30 dias`}
        />
        <KpiCard
          icon="zap"
          tone="success"
          label="Taxa de autonomia"
          value={pct(kpis.taxa_autonomia)}
      ...
      <Card>
        ...
        <CardContent>
          <StackedBar d={distribuicao} total={kpis.total} />
        </CardContent>
      </Card>
      ...
      <KpiGrid cols={4}>
        ...
        <KpiCard
          ...
          tone="default"
          label="Custo Brain B"
          value={usd(kpis.custo_total_usd)}
          description={`${num(kpis.tokens_total)} tokens`}
        />
      </KpiGrid>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <TopList title="Top 10 — Domínios mais ativos" rows={por_dominio} keyField="domain" />
        <TopList title="Top 10 — Tipos de evento mais frequentes" rows={por_event_type} keyField="event_type" />
      </div>
    </div>
  )
}
