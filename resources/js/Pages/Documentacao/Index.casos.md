...
- **Teste:** `tests/Feature/DocumentacaoRouteTest.php` — `UC-INDEX-01 · a capa entrega o Guia convertido NO SERVIDOR…` (lane `ci-sqlite-pest`; chama o controller direto, roda sem banco).
...
- **Teste:** `tests/Feature/DocumentacaoRouteTest.php` — `UC-INDEX-02 · Guia ausente no deploy…` (lane `ci-sqlite-pest`; chama o controller direto, roda sem banco).
...
- **Teste:** `tests/Feature/DocumentacaoRouteTest.php` — `UC-INDEX-03 · o rail e derivado do frontmatter…` (lane `ci-sqlite-pest`; chama o controller direto, roda sem banco).
...
## Trilha do tempo
- 2026-09-24 · [C] US-DOC-001 onda 2b: os 3 UCs passam a ser defendidos por casos Pest que chamam o controller (rodam em qualquer lane) em vez dos stubs e2e `fixme` de 2026-08-06, que só provavam presença. Status segue ⬜ até o CI registrar a execução.
