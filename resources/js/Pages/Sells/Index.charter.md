...
- v9 (2026-07-24 · este) — **errata factual na Mission** (nenhuma mudança de comportamento, código ou design): a linha de integrações citava `Modules/Copiloto`, que **não existe na árvore** — o módulo da Jana sempre foi `Modules/Jana`, e a integração de Insights **saiu desta tela**. Recibos medidos 2026-07-24: `ls -d Modules/Copiloto` → não existe (`Modules/Jana` existe); `Index.tsx` linhas 53-55 **no sha d4afe95** (citacao HISTORICA — a redacao do comentario mudou depois; hoje o mesmo bloco vive em ~53-58 e diz "Insights Jana SAIU desta tela") dizia textualmente *"Insights Jana movido pra /ia/dashboard (Jana V2 canon). Antes era tab embutida aqui via `viewMode === 'insights'` + SellsInsightsView"*. Menção removida da Mission com nota inline (L-07/L-22 — corrigir com trilha, nunca apagar em silêncio). **Não** mexi nas outras 4 integrações citadas (NfeBrasil · Whatsapp · Repair · PaymentGateway): `NfeBrasil`/`Whatsapp`/`PaymentGateway` têm 0 menções no `.tsx`, mas isso **não prova** ausência de integração (pode ser via Controller) — sem prova, não toco. Achado por varredura `fact-anchor` (doc afirma `Modules/<X>` inexistente); ver `memory/LICOES_CODE.md` LC-08 + PR #4774.
- v8 (2026-07-03 · este) — restaura o **indicador "venda com devolução"** (badge de retorno na coluna `#invoice`): backend `inertiaList` passa a selecionar `return_exists` (subquery `sell_return` via `return_parent_id + type='sell_return'`) e devolve `has_return` no payload; `SellsTabelaUnificada` desenha um badge (`vd-return-flag`, ícone `Undo2`, tooltip "Venda com devolução") ao lado do número. A "setinha" existia no Blade legado (`SellController@index` → `return_exists`/`fa-undo`) e **sumiu no rewrite Cowork #1032** (o payload React nunca carregava o flag) — venda devolvida aparecia como normal. Reportado por Guilherme @ biz=4 ROTA LIVRE. Defendido por **UC-S12** (`Index.casos.md` + `SellsIndexCoworkPayloadTest.php`, ADR 0264 G-2). Estilo DS (não o selinho vermelho legado) — decisão [W] 2026-07-03.
