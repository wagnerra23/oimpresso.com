...
export default function MemCofreChat({ session_id, history, recent, modules, ai_enabled }: Props) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const res = await fetch('/memcofre/chat/ask', {
        method: 'POST',
        headers: {
  ...
  const newSession = async () => {
    const res = await fetch('/memcofre/chat/new', {
      method: 'POST',
      headers: {
        'X-CSRF-TOKEN': (document.querySelector('meta[name="csrf-token"]') as HTMLMetaElement)?.content || '',
        'Accept': 'application/json',
  ...
  return (
    <>
      <Head title="Cofre de Memórias — Chat" />
        ...
        <header className="flex items-start justify-between gap-3 mb-4">
          <div className="min-w-0">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Converse com o conhecimento consolidado no Cofre de Memórias — specs, arquiteturas, ADRs e changelogs.
            </p>
          </div>
